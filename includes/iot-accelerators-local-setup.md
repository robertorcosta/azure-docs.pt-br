...
1. Execute o script **start.cmd**. O script solicitará as seguintes informações:
   ...
   * O local do datacenter do Azure para usar.
     ...
     > [!TIP]
     > Quando o script é concluído, ele também salva as variáveis de ambiente em um arquivo chamado ** \<sua pasta\>\\base.\\\<PCs nome\>da solução. env**. Você pode usá-las para implantações futuras de acelerador de solução. Observe que quaisquer variáveis de ambiente definidas em seu computador local substituem os valores no arquivo **serviços\\scripts\\local\\.env** ao executar **docker-compose**.
...
Os valores de configuração serão lidos deste Azure Key Vault recurso. Essas variáveis de ambiente podem ser salvas ** \<no arquivo.\>\\env da\\\<solução\>Home Folder. PCs** da implantação. Observe que quaisquer variáveis de ambiente definidas em seu computador local substituem os valores no arquivo **serviços\\scripts\\local\\.env** ao executar **docker-compose**.
