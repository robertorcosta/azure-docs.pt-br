...
| Corpo da notificação | Referenciar qualquer propriedade no `data` campo de uma notificação | `$body.<property>` | Consulte [*como: entender os dados do evento*](https://docs.microsoft.com/azure/digital-twins/how-to-interpret-event-data) para obter exemplos de notificações. Qualquer propriedade no `data` campo pode ser referenciada usando `$body`
