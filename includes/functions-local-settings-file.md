...
O local.settings.jsem arquivos armazena configurações do aplicativo, cadeias de conexão e configurações usadas pelas ferramentas de desenvolvimento local. As configurações na local.settings.jsno arquivo são usadas somente quando você está executando projetos localmente. O arquivo de configurações local tem esta estrutura:
