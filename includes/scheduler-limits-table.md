...
| -------- | ----------------- |
| **Tamanho do trabalho** | O tamanho máximo do trabalho é de 16.000. Se uma operação PUT ou uma PATCH resultar em um tamanho de trabalho maior do que esse limite, um código de status 400 Solicitação incorreta será retornado. | 
| **Coleções de empregos** | O número máximo de coleções de trabalho por assinatura do Azure é de 200.000. | 
...
| **Freqüência** | Por padrão, a cota máxima de frequência é de uma hora em uma coleção de trabalhos gratuita e de um minuto em uma coleção de trabalhos padrão. <p>É possível realizar a frequência máxima em uma coleção de trabalhos menor do que o máximo. Todos os trabalhos na coleção estão limitados ao valor definido na coleção de trabalhos. Se você tentar criar um trabalho com uma frequência maior do que a máxima na coleção de trabalhos, a solicitação falhará com um código de status 409 Conflito. | 
| **Tamanho do corpo** | O tamanho máximo do corpo para uma solicitação é de 8.192 chars. |
| **Tamanho da solicitação de URL** | O tamanho máximo para uma URL de solicitação é de 2.048 chars. |
| **Contagem de cabeçalho** | A contagem máxima de cabeçalhos é de 50 cabeçalhos. | 
| **Tamanho do cabeçalho de agregação** | O tamanho máximo do cabeçalho agregado é de 4.096 chars. |
| **Timeout** | O tempo limite de solicitação é estático, ou seja, não é configurável. e são 60 segundos para ações HTTP. Para operações mais demoradas, siga os protocolos assíncronos do HTTP. Por exemplo, retornar um 202 imediatamente, mas continuar funcionando em segundo plano. | 
| **Histórico de trabalho** | O corpo de resposta máxima armazenado no histórico de trabalho é de 2.048 bytes. |
| **Retenção de histórico de trabalho** | O histórico de trabalho é mantido por até dois meses ou até as últimas 1.000 execuções. | 
