...
> [!IMPORTANT]
> Tenha cuidado se você modificar ou adicionar regras de entrada ou saída nos NSGs configurados em Lote. Se a comunicação com os nós de computação na sub-rede especificada for negada por um NSG, o serviço Lote definirá o estado dos nós de computação como **inutilizável**. Além disso, nenhum bloqueio de recurso deve ser aplicado a recursos criados pelo Lote; caso contrário, isso pode impedir a limpeza de recursos como resultado de ações iniciadas pelo usuário, como a exclusão de um pool.
...
Não é obrigatório especificar NSGs no nível da sub-rede da rede virtual porque o Lote configura os próprios NSGs (veja acima). Se tiver um NSG associado à sub-rede em que os nós de computação do Lote estão implantados ou quiser aplicar regras de NSG personalizadas para substituir os padrões aplicados, você precisará configurar esse NSG com pelo menos as regras de segurança de entrada e saída, conforme mostrado nas seguinte tabelas.
...
Configure o tráfego de entrada na porta 3389 para o Windows se precisar permitir o acesso RDP aos nós do pool. Não é necessário para os nós do pool serem usados.
