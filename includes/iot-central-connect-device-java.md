...
* Um computador de desenvolvimento com o Java SE Development Kit 8 ou mais recente. Você pode baixar o JDK do Java 8 (LTS) para várias plataformas em [Baixar Builds do Zulu do OpenJDK](https://www.azul.com/downloads/zulu-community/).
...
Na cópia do SDK do Microsoft Azure IoT para Java baixado anteriormente, abra o arquivo *azure-iot-sdk-java/device/iot-device-samples/pnp-device-sample/temperature-controller-device-sample/src/main/java/samples/com/microsoft/azure/sdk/iot/device/TemperatureController.java* em um editor de texto.
...
A classe `TargetTemperatureUpdateCallback` contém o método `TwinPropertyCallBack` para gerenciar as atualizações da propriedade gravável de um componente por meio do IoT Central. Esse método usa a classe `PnpConvention` para criar a resposta:
...
A classe `MethodCallback` contém o método `call` para gerenciar os comandos do componente chamados por meio do IoT Central:
...
Para executar o aplicativo de exemplo, abra um ambiente de linha de comando e acesse a pasta *azure-iot-sdk-java/device/iot-device-samples/pnp-device-sample/temperature-controller-device-sample* que contém a pasta *src* com o arquivo de exemplo *TemperatureController.java*.
