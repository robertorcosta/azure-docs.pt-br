...
|trackingStoreConnectionStringName||O nome de uma cadeia de conexão a ser usada para as tabelas de histórico e instâncias. Se não for especificado, a `azureStorageConnectionStringName` conexão será usada.|
|trackingStoreNamePrefix||O prefixo a ser usado para as tabelas de histórico e instâncias quando `trackingStoreConnectionStringName` é especificado. Se não for definido, o valor de prefixo padrão será `DurableTask` . Se `trackingStoreConnectionStringName` não for especificado, as tabelas de histórico e instâncias usarão o `hubName` valor como seu prefixo e qualquer configuração para `trackingStoreNamePrefix` será ignorada.|
