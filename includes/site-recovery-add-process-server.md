...
2. Antes **de começar,** **selecione Adicionar servidores de processo adicionais para dimensionar a implantação**.
...
4. Em **Configurações da Internet,** especifique como o Provedor em execução no Servidor de Configuração se conecta à Recuperação do Site do Azure pela Internet.
   ...
   * Se o proxy existente exigir autenticação ou se você quiser usar um proxy personalizado para a conexão Provedor, **selecione Conectar com configurações de proxy personalizadas**.
...
5. Em **Verificação de pré-requisitos,** a configuração executa uma verificação para garantir que a instalação possa ser executada. Se aparecer um aviso sobre a **verificação global de sincronização de tempo,** verifique se a hora no relógio do sistema (configurações**de data e hora)** é a mesma do fuso horário.
