...
1. No prompt, forneça a senha que você usou quando criou a VM.
...
1. Se você usou um endereço IP público durante a criação da VM, poderá usar esse IP para se conectar à VM. Para obter o IP público, execute o seguinte comando: 

   ```powershell
   $publicIp = Get-AzureRmPublicIpAddress -Name <Public IP> -ResourceGroupName <Resource group name>
   ```

   Nessa instância, o IP público é o mesmo que o IP privado passado durante a criação da interface de rede virtual.
