...
* Primeiro, faça um instantâneo do blob de base usando a operação de [Snapshot Blob](/rest/api/storageservices/Snapshot-Blob) .
* Em seguida, copie o instantâneo para uma conta de armazenamento de destino usando [Copy Blob](/rest/api/storageservices/Copy-Blob).
...
* O blob não foi substituído por [PutPage](/rest/api/storageservices/Put-Page) ou [Copiar Blob](/rest/api/storageservices/Copy-Blob) entre dois instantâneos.
...
* Faça um instantâneo do blob de base usando [Snapshot Blob](/rest/api/storageservices/Snapshot-Blob).
* Copie o instantâneo para a conta de armazenamento de backup de destino na mesma ou em qualquer outra região do Azure usando [Copiar Blob](/rest/api/storageservices/Copy-Blob). Esse é o blob de páginas de backup. Tire um instantâneo do blob de páginas de backup e armazene-o na conta de backup.
* Faça outro instantâneo do blob de base usando Blob de Instantâneo.
* Obtenha a diferença entre o primeiro e o segundo instantâneo do blob de base usando [GetPageRanges](/rest/api/storageservices/Get-Page-Ranges). Use o novo parâmetro **prevsnapshot** para especificar o valor de DateTime do instantâneo do qual você deseja obter a diferença. Quando esse parâmetro estiver presente, a resposta REST inclui apenas as páginas que foram alteradas entre o instantâneo de destino e o instantâneo anterior, incluindo páginas em branco.
* Use [PutPage](/rest/api/storageservices/Put-Page) para aplicar essas alterações ao blob de páginas de backup.
...
Para saber mais sobre como criar uma conta de armazenamento, consulte [Criar uma conta de armazenamento](../articles/storage/common/storage-account-create.md).
...
3. Faça um instantâneo de *mybackupstdpageblob* chamado *mybackupstdpageblob_ss1* usando [Blob de Instantâneo](/rest/api/storageservices/Snapshot-Blob) e armazene-o em *mybackupstdaccount*.
