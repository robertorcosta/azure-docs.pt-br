...
As seguintes operações de plano de controle em Discos Gerenciados podem envolver a movimentação do Disco de um local de armazenamento para outro. Isso é orquestrado através de uma cópia de fundo de dados que pode levar várias horas para ser concluído, normalmente menos de 24 horas, dependendo da quantidade de dados nos discos. Durante esse tempo, seu aplicativo pode experimentar latência de leitura maior do que o habitual, pois algumas leituras podem ser redirecionadas para o local original e podem levar mais tempo para serem concluídas. Não há nenhum impacto na latência de gravação durante esse período.

- Atualize o tipo de armazenamento.
- Desconecte e conecte um disco de uma VM para outra.
...
| **Max. Memória** |Quantidade de memória exigida para execução perfeita do aplicativo |% de Bytes Confirmados em Uso |Usar o vmstat |
| **Max. Cpu** |Quantidade de CPU exigida para a execução perfeita do aplicativo |% do Tempo do Processador |%util |
...
Os principais fatores que influenciam o desempenho de um aplicativo em execução no Armazenamento Premium são a natureza das solicitações de IO, tamanho da VM, tamanho do disco, número de discos, cache de disco, multithreading e profundidade de fila. Você pode controlar alguns desses fatores com botões fornecidos pelo sistema. A maioria dos aplicativos talvez não apresente uma opção para alterar o tamanho de E/S e a profundidade da fila diretamente. Por exemplo, se você estiver usando o SQL Server, não será possível escolher a profundidade de fila e o tamanho de E/S. O SQL Server escolhe os valores ideais do tamanho de E/S e da profundidade da fila para obter o melhor desempenho. É importante compreender os efeitos de ambos os tipos de fator no desempenho do aplicativo para que você possa provisionar recursos apropriados que atendam às necessidades de desempenho.

Ao longo desta seção, consulte a lista de verificação de requisitos de aplicativo que você criou para identificar quanto você precisa para otimizar o desempenho do aplicativo. Com base nisso, você poderá determinar quais fatores dessa seção será preciso ajustar. Para ver os efeitos de cada fator no desempenho do aplicativo, execute os parâmetros de comparação na configuração do aplicativo. Consulte o artigo benchmarking, vinculado no final, para obter etapas para executar ferramentas comuns de benchmarking em VMs Windows e Linux.

### <a name="optimize-iops-throughput-and-latency-at-a-glance"></a>Otimizar IOPS, throughput e latência em um relance

A tabela abaixo resume os fatores de desempenho e as etapas necessárias para otimizar o IOPS, o throughput e a latência. As seções a seguir deste resumo descreverão cada fator mais detalhadamente.
...
| **Tamanho vM** |Use um tamanho de VM que ofereça IOPS maior que o requisito do seu aplicativo. |Use um tamanho de VM com limite de Taxa de Transferência maior que o requisito do seu aplicativo. |Use um tamanho de VM que ofereça limites de escala maiores que o requisito do seu aplicativo. |
...
| **Distribuição de disco** |Use vários discos e distribua-os em conjunto para obter um limite combinado mais alto de IOPS e Taxa de Transferência. O limite combinado por VM deve ser superior aos limites combinados dos discos premium conectados. | &nbsp; | &nbsp; |
| **Tamanho da distribuição** |Tamanho menor de distribuição para padrão pequeno de E/S aleatório visto em aplicativos OLTP. Por exemplo, use o tamanho da faixa de 64 KB para o aplicativo SQL Server OLTP. |Tamanho maior de distribuição para padrão grande de E/S sequencial visto em aplicativos de data warehouse. Por exemplo, use o tamanho da faixa de 256 KB para o aplicativo do armazém de dados do SQL Server. | &nbsp; |
...
Uma solicitação de E/S é uma unidade da operação de entrada/saída que seu aplicativo executará. Identificar a natureza das solicitações de E/S, aleatória ou sequencial, leitura ou gravação, grande ou pequena, ajudará você a determinar os requisitos de desempenho do aplicativo. É importante entender a natureza das solicitações de IO, para tomar as decisões certas ao projetar sua infra-estrutura de aplicativos. Os IOs devem ser distribuídos uniformemente para alcançar o melhor desempenho possível.

O tamanho de E/S é um dos fatores mais importantes. O tamanho de E/S é o tamanho da solicitação de operação de entrada/saída gerada pelo aplicativo. O tamanho de E/S tem um impacto significativo no desempenho, especificamente na IOPS e na largura de banda que o aplicativo é capaz de atingir. A fórmula a seguir mostra a relação entre IOPS, tamanho iO e largura de banda/throughput.  
...
* Um aplicativo OLTP gera milhões de solicitações de E/S pequenas e aleatórias. Para lidar com esses tipos de solicitações de IO, você deve projetar sua infra-estrutura de aplicativos para obter IOPS mais alto.  
* Um aplicativo de data warehouse gera solicitações de E/S grandes e sequenciais. Para lidar com esses tipos de solicitações de IO, você deve projetar sua infra-estrutura de aplicativos para obter maior largura de banda ou throughput.
...
Por exemplo, considere um aplicativo que exija 16.000 IOPS. Para obter esse desempenho, você\_precisará de um VM Padrão D14 Azure IaaS, que pode dar um IOPS máximo de 16.000 usando 32 discos padrão de armazenamento de 1 TB. Cada disco de armazenamento padrão de 1 TB pode atingir um máximo de 500 IOPS. O custo estimado dessa VM por mês será de US$ 1.570. O custo mensal de 32 discos de armazenamento padrão será de US$ 1.638. O custo mensal total estimado será de US$ 3.208.
...
| **Custo de discos por mês** |$1.638,40 (discos de 32 x 1 TB) |US$ 544,34 (4 discos x P30) |
...
O Azure Premium Storage oferece uma variedade de tamanhos para que você possa escolher um que melhor se adapte às suas necessidades. Cada tamanho de disco tem um limite de escala diferente para IOPS, largura de banda e armazenamento. Escolha o tamanho certo do disco do Armazenamento Premium de acordo com os requisitos do aplicativo e o tamanho da VM de alta escala. A tabela abaixo mostra os tamanhos dos discos e suas capacidades. Atualmente, os tamanhos de disco 4, P6, P15, P60, P70, e P80 têm suporte apenas para o Managed Disks.
...
> [!WARNING]
> O cache de disco não é suportado para discos 4 TiB e maiores. Se vários discos estiverem conectados à VM, cada disco menor que 4 TiB suportará o cache.
>
...
| **Configuração do cache de disco** | **recomendação sobre quando usar esta configuração** |
| --- | --- |
...
*Nenhum*  
Atualmente, **Nenhum** é suportado apenas em discos de dados. Não é suportado em discos do Sistema Operacional. Se você definir **Nenhum** em um disco do SISTEMA OPERACIONAL, ele irá substituir isso internamente e defini-lo como **ReadOnly**.
...
As seguintes distribuições Linux foram validadas para SSDs Premium. Para melhor desempenho e estabilidade com SSDs premium, recomendamos que você atualize suas VMs para uma dessas versões ou mais recentes. 
...
| --- | --- | --- | --- |
| Ubuntu | 12.04 ou mais novo| 3.2.0-75.110+ | &nbsp; |
| Ubuntu | 14.04 ou mais novo| 3.13.0-44.73+  | &nbsp; |
| Debian | 7.x, 8.x ou mais novo| 3.16.7-ckt4-1+ | &nbsp; |
| SUSE | SLES 12 ou mais novo| 3.12.36-38.1+ | &nbsp; |
| SUSE | SLES 11 SP4 ou mais novo| 3.0.101-0.63.1+ | &nbsp; |
| CoreOS | 584.0.0+ ou mais novo| 3.18.4+ | &nbsp; |
| CentOS | 6.5, 6.6, 6.7, 7.0 ou mais novas| &nbsp; | [LIS4 necessário](https://www.microsoft.com/download/details.aspx?id=55106) <br> *Confira a observação na próxima seção* |
| CentOS | 7.1+ ou mais novo| 3.10.0-229.1.2.el7+ | [LIS4 recomendado](https://www.microsoft.com/download/details.aspx?id=55106) <br> *Confira a observação na próxima seção* |
| Red Hat Enterprise Linux (RHEL) | 6.8+, 7.2+, ou mais novo | &nbsp; | &nbsp; |
| Oracle | 6.0+, 7.2+, ou mais novo | &nbsp; | UEK4 ou RHCK |
| Oracle | 7.0-7.1 ou mais novo | &nbsp; | UEK4 ou RHCK c/[LIS4](https://www.microsoft.com/download/details.aspx?id=55106) |
| Oracle | 6.4-6.7 ou mais novo | &nbsp; | UEK4 ou RHCK c/[LIS4](https://www.microsoft.com/download/details.aspx?id=55106) |
...
Em alguns casos, o comando acima também atualizará o kernel. Se uma atualização do kernel for necessária, talvez seja necessário executar os comandos acima novamente após a reinicialização para instalar totalmente o pacote microsoft-hyper-v.
...
De acordo com o tipo de carga de trabalho que o aplicativo está executando, escolha um tamanho de distribuição apropriado. Para solicitações pequenas e aleatórias de E/S, use um tamanho de distribuição menor. Considerando que para pedidos de IO seqüenciais de grande porte, use um tamanho de listra maior. Descubra as recomendações de tamanho de distribuição para o aplicativo que será executado no Armazenamento Premium. Para o SQL Server, configure o tamanho da faixa de 64 KB para cargas de trabalho OLTP e 256 KB para cargas de trabalho de armazenamento de dados. Confira [Melhores práticas de desempenho para o SQL Server em VMs do Azure](../articles/virtual-machines/windows/sql/virtual-machines-windows-sql-performance.md#disks-guidance) para saber mais.
...
A profundidade da fila ou o comprimento da fila ou o tamanho da fila é o número de solicitações de IO pendentes no sistema. O valor da profundidade da fila determina quantas operações de IO seu aplicativo pode alinhar, que os discos de armazenamento serão processados. Afeta todos os três indicadores de desempenho de aplicativos que discutimos neste artigo viz., IOPS, throughput e latência.
