...
| Número de conexões simultâneas em um namespace |Namespace |Solicitações subsequentes de conexões adicionais são rejeitadas e uma exceção é recebida pelo código de chamada. As operações REST não contam para conexões TCP simultâneas. |NET Messaging: 1.000.<br /><br />AMQP: 5.000. |
...
| Tamanho da propriedade da mensagem para uma fila, um tópico ou uma entidade de assinatura |Entidade | A exceção `SerializationException` é gerada. |O tamanho máximo da propriedade de mensagem para cada propriedade é 32.000. O tamanho cumulativo de todas as propriedades não pode exceder 64.000. Esse limite se aplica ao cabeçalho inteiro da [mensagem orientada](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage), que tem propriedades de usuário e propriedades do sistema, como [número de sequência](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.sequencenumber), [rótulo](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.label)e [ID da mensagem](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.messageid). |
