---
title: 'Tutorial: Seu primeiro modelo de ML com o R'
titleSuffix: Azure Machine Learning
description: Neste tutorial, você conhecerá os padrões básicos de design no Azure Machine Learning e treinará um modelo de regressão logística usando pacotes R azuremlsdk e caret para prever a probabilidade de uma fatalidade em um acidente de automóvel.
...
Neste tutorial, você conhecerá os padrões básicos de design no Azure Machine Learning.  Você treinará e implantará um modelo **caret** para prever a probabilidade de uma fatalidade em um acidente de automóvel. Depois de concluir este tutorial, você terá o conhecimento prático do SDK do R para aumentar o desenvolvimento de experimentos e fluxos de trabalho mais complexos.

Neste tutorial, você aprende as seguintes tarefas:

> [!div class="checklist"]
> * Conectar seu workspace
> * Carregar dados e preparar para o treinamento
> * Carregar dados para o armazenamento de dados para que esteja disponível para treinamento remoto
...
2. Crie um workspace do Azure Machine Learning e baixe seu arquivo de configuração usando as etapas abaixo.


### <a name="create-a-workspace"></a>Criar um workspace

Um Workspace do Azure Machine Learning é o recurso fundamental na nuvem que você usa para experimentar, treinar e implantar modelos de machine learning. Ele vincula sua assinatura do Azure e o grupo de recursos a um objeto facilmente consumido no SDK. Caso você já tenha um workspace do Azure Machine Learning, [passe para a próxima seção](#config). Caso contrário, crie um agora.

[!INCLUDE [aml-create-portal](../../../includes/aml-create-in-portal.md)]

### <a name="config"></a> Baixar arquivo config.json

1. Na parte superior da página do workspace, selecione **Baixar config.json**.

    ![Baixar arquivo config.json](media/tutorial-1st-r-experiment/download-config.png)  

1. Adicione esse arquivo ao seu diretório de **tutoriais**.

Agora você já pode executar o tutorial.

Recomendamos usar o RStudio para executar este tutorial. No RStudio, escolha Arquivo > Novo Projeto > Diretório Existente e selecione o diretório de **tutoriais** criado acima.

> [!Important]
> O restante deste artigo contém o mesmo conteúdo que você vê no arquivo **train-and-deploy-to-aci.Rmd**.  
> Se você tiver experiência com o RMarkdown, fique à vontade para usar o código desse arquivo.  Ou você pode copiar/colar os code snippets de lá ou deste artigo para um script R ou para a linha de comando.
...
Este tutorial usa dados do pacote **DAAG**. Esse conjunto de dados inclui dados de mais de 25 mil batidas de carro nos EUA, com variáveis que podem ser usadas para prever a probabilidade de uma fatalidade. Primeiro, importe os dados para o R e transforme-os em um novo dataframe `accidents` para análise e exporte-os para um arquivo `Rdata`.
...
Carregue dados para a nuvem para que eles possam ser acessados por seu ambiente de treinamento remoto. Cada workspace do Azure ML é fornecido com um armazenamento de dados padrão que armazena informações de conexão no contêiner de blobs do Azure que é provisionado na conta de armazenamento anexada ao workspace. O código a seguir carregará os dados de acidentes que você criou em cima desse armazenamento de dados.
