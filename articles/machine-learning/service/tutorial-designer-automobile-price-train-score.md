...
Na [parte dois](tutorial-designer-automobile-price-deploy.md) do tutorial, você aprende a implantar seu modelo preditivo como um ponto de extremidade de inferência em tempo real para prever o preço de qualquer carro com base nas especificações técnicas enviadas. 

> [!Note]
>Uma versão concluída deste tutorial está disponível como um pipeline de exemplo.
>
>Para encontrá-lo, vá para o **designer em seu workspace**. Na seção **Novo pipeline**, selecione **Amostra 1 – Regressão: Previsão de Preços de Automóveis (Básica)** .
...
1. Selecione o nome do pipeline padrão – **"Pipeline criado em..."** , na parte superior da tela – e renomeie para algo significativo. Por exemplo, **"Previsão de preços de automóveis"** . O nome não precisa ser exclusivo.
...
Os conjuntos de dados normalmente exigem algum pré-processamento antes da análise. Talvez você tenha observado alguns valores ausentes ao inspecionar o conjunto de dados. Os valores ausentes precisam ser limpos para que o modelo possa analisar os dados corretamente.
...
1. Na caixa de **Comentários** do painel de Propriedades, insira "Remover linhas com valores ausentes".  
...
Como você deseja prever o preço, que é um número, é possível usar um algoritmo de regressão. Para este exemplo, você usará um modelo de regressão linear.
...
1. Insira **dividir dados** na caixa de pesquisa para encontrar o módulo **Dividir Dados** e conectá-lo à porta esquerda do módulo **Limpar Dados Ausentes**.
...
1. No painel Propriedades, defina a **Fração de linhas no primeiro conjunto de dados de saída** como 0,7.
    ...
    Isso divide 70% dos dados para treinar o modelo e 30% para testá-lo.

1. Na caixa **Comentários** de Propriedades, insira "Dividir o conjunto de dados em conjunto de treinamento (0,7) e conjunto de teste (0,3)".
...
1. Expanda **Algoritmos de Machine Learning**.
    ...
    Isso exibe várias categorias de módulos que podem ser usados para inicializar os algoritmos de aprendizado.
...
1. Na caixa de diálogo **Coluna de Rótulos**, expanda o menu suspenso e selecione **Nomes de coluna**. 
...
Após treinar seu modelo usando 70% dos dados, você poderá usá-lo para pontuar os outros 30% e ver se o modelo funciona corretamente.

1. Insira **pontuar modelo** na caixa de pesquisa para encontrar o módulo **Pontuar Modelo** e arraste-o para a tela do pipeline. 
...
1. Insira **avaliar** na caixa de pesquisa para encontrar o módulo **Avaliar Modelo** e arraste-o para a tela do pipeline. 
