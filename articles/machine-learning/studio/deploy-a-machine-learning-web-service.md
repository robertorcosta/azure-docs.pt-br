...
description: Como converter um experimento de treinamento em um experimento preditivo, prepará-lo para implantação e implantá-lo como um serviço web (clássico) do Azure Machine Learning Studio( clássico).
...
---
# <a name="deploy-an-azure-machine-learning-studio-classic-web-service"></a>Implantar um serviço web do Azure Machine Learning Studio (clássico)

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

O Azure Machine Learning Studio (clássico) permite que você construa e teste uma solução analítica preditiva. Em seguida, é possível implantar a solução como um serviço Web.

Os serviços web do Machine Learning Studio (clássico) fornecem uma interface entre um aplicativo e um modelo de pontuação de fluxo de trabalho (clássico) do Machine Learning Studio. Um aplicativo externo pode se comunicar com um modelo de pontuação de fluxo de trabalho (clássico) do Machine Learning Studio (clássico) em tempo real. Uma chamada para um serviço web do Machine Learning Studio (clássico) retorna os resultados da previsão para um aplicativo externo. Para fazer uma chamada a um serviço Web, você passa uma chave de API que foi criada quando você implantou o serviço Web. Um serviço web do Machine Learning Studio (clássico) é baseado no REST, uma escolha de arquitetura popular para projetos de programação web.

O Azure Machine Learning Studio (clássico) tem dois tipos de serviços web:

* Serviço de Solicitação-Resposta (RRS): Um serviço de baixa latência e altamente escalável que marca um único registro de dados.
...
* **[Crie um experimento]** de treinamento - No Studio (clássico), você pode treinar e testar um modelo de análise preditiva usando dados de treinamento que você fornece, usando um grande conjunto de algoritmos de aprendizado de máquina incorporados.
...
Para treinar um modelo de análise preditiva, você usa o Azure Machine Learning Studio (clássico) para criar um experimento de treinamento onde você inclui vários módulos para carregar dados de treinamento, preparar os dados conforme necessário, aplicar algoritmos de aprendizado de máquina e avaliar o Resultados. Você pode iterar em um teste e testar algoritmos de aprendizado de máquina diferentes para comparar e avaliar os resultados.
...
* [Crie um experimento simples no Azure Machine Learning Studio (clássico)](create-experiment.md)
* [Desenvolva uma solução preditiva com o Azure Machine Learning Studio (clássico)](tutorial-part1-credit-risk.md)
* [Importe seus dados de treinamento para o Azure Machine Learning Studio (clássico)](import-data.md)
* [Gerenciar iterações de experimentos no Azure Machine Learning Studio (clássico)](manage-experiment-iterations.md)
...
Para obter mais informações sobre como realizar essa conversão, consulte [Como preparar seu modelo para implantação no Azure Machine Learning Studio (clássico)](convert-training-experiment-to-scoring-experiment.md).
...
Para implantar o teste preditivo, clique em **Executar** na parte inferior da tela do teste. Após a conclusão do teste, clique em **Implantar Serviço Web** e selecione **Implantar Serviço Web [novo]**.  A página de implantação do portal Machine Learning Studio (clássico) Web Service é aberta.
...
### <a name="web-service-portal-deploy-experiment-page"></a>Página de experimentos do portal do Serviço Web
...
Para testar seu novo serviço web, clique em **Testar o serviço web** em tarefas comuns. Na página de teste, você poderá testar o serviço Web como um Serviço de solicitação-resposta (RRS) ou Serviço de execução em lote (BES).
...
![Digite valores apropriados para testar seu serviço web](./media/publish-a-machine-learning-web-service/figure-5-test-request-response.png)

Para testar o BES, clique em **Lote**. Na página de teste em lote, clique em Pesquisar na sua entrada e selecione um arquivo CSV que contém os valores de exemplo apropriados. Se você não tem um arquivo CSV e criou seu experimento preditivo usando o Machine Learning Studio (clássico), você pode baixar o conjunto de dados para o seu experimento preditivo e usá-lo.
...
![Baixe seu conjunto de dados da tela do Studio (clássico)](./media/publish-a-machine-learning-web-service/figure-7-mls-download.png)

Clique em **Testar**. O status de seu trabalho de execução de lote é exibido à direita em **Trabalhos em lote de teste**.

![Teste seu trabalho de execução em lote com o portal de serviços web](./media/publish-a-machine-learning-web-service/figure-6-test-batch-execution.png)
...
![Configure seu serviço web](./media/publish-a-machine-learning-web-service/figure-8-arm-configure.png)

### <a name="access-your-new-web-service"></a>Acessar seu Novo serviço Web

Depois de implantar seu serviço web no Machine Learning Studio (clássico), você pode enviar dados para o serviço e receber respostas programáticamente.
...
Para obter mais informações sobre como acessar um serviço web do Machine Learning Studio (clássico), consulte [Como consumir um serviço web do Azure Machine Learning Studio (clássico).](consume-web-services.md)

### <a name="manage-your-new-web-service"></a>Gerenciar seu Novo serviço Web

Você pode gerenciar seus novos serviços web usando o portal Desportos Web do Machine Learning Studio (clássico). Na página principal do [portal,](https://services.azureml.net/)clique **em Web Services**. Na página de serviços Web, você pode excluir ou copiar um serviço. Para monitorar um serviço específico, clique no serviço e, em seguida, clique em **Painel**. Para monitorar trabalhos em lotes associados ao serviço Web, clique em **Log de solicitações em lote**.
...
![Implante seu serviço web do Studio (clássico)](./media/publish-a-machine-learning-web-service/figure-2.png)

### <a name="test-your-classic-web-service"></a>Testar seu serviço Web Clássico

Você pode testar o serviço web no portal Machine Learning Studio (clássico) Web Services ou no Machine Learning Studio (clássico).
...
Você pode clicar no link de visualização de **teste** para testar seu serviço no portal Azure Machine Learning Studio (clássico) Web Services, como mostrado anteriormente na seção Novo serviço web.
...
Você pode habilitar o log para diagnosticar quaisquer falhas que esteja vendo quando o serviço Web for acessado. Para obter mais informações, consulte [Ativar o registro para serviços web (clássicos) do Machine Learning Studio](web-services-logging.md).

![Habilite o login no portal de serviços web](./media/publish-a-machine-learning-web-service/figure-4.png)
...
Depois de implantar seu serviço web no Azure Machine Learning Studio (clássico), você pode enviar dados para o serviço e receber respostas programáticas.
...
Para obter mais informações, consulte [Gerenciar um espaço de trabalho (clássico) do Azure Machine Learning Studio](manage-workspace.md) e gerenciar um serviço web usando o portal [Azure Machine Learning Studio (clássico) Web Services](manage-new-webservice.md).
...
Uma opção para atualizar seu serviço Web é treinar novamente o modelo de forma programática. Para obter mais informações, consulte [Retrain Machine Learning Studio (clássico) modelos programáticamente](/azure/machine-learning/studio/retrain-machine-learning-model).

## <a name="next-steps"></a>Próximas etapas

* Para obter mais detalhes técnicos sobre como a implantação funciona, consulte [Como um modelo de Machine Learning Studio (clássico) progride de um experimento para um serviço web operacionalizado.](model-progression-experiment-to-web-service.md)

* Para obter detalhes sobre como deixar seu modelo pronto para ser implantado, consulte [Como preparar seu modelo para implantação no Azure Machine Learning Studio (clássico)](convert-training-experiment-to-scoring-experiment.md).

* Há várias maneiras para consumir a API REST e acessar o serviço Web. Veja [como consumir um serviço web do Azure Machine Learning Studio (clássico).](consume-web-services.md)

<!-- internal links -->
[Crie um experimento de treinamento]: #create-a-training-experiment
[Converta-o em um experimento preditivo]: #convert-the-training-experiment-to-a-predictive-experiment
