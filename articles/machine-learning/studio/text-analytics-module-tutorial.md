...
Neste tutorial, você aprende essas etapas à medida que percorremos um modelo de análise de sentimentos usando o conjunto de registros do Amazon Book Reviews (Confira este artigo de pesquisa "biografias, Bollywood, Mark-boxes and BLENDERS: adaptação de domínio para classificação de sentimentos", de John Blitzer, Gamba Dredze e Fernando Pereira; Associação de linguísticos computacionais (ACL), 2007.) Esse conjunto de informações consiste em pontuações de revisão (1-2 ou 4-5) e um texto de forma livre. O objetivo é prever a pontuação da crítica: baixa (1-2) ou alta (4-5).
