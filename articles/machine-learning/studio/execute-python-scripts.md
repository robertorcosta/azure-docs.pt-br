...
O ponto de entrada do Python para retornar somente uma estrutura de dados como saída. No momento, não é possível retornar objetos Python arbitrários, como modelos treinados diretamente para o tempo de execução do estúdio (clássico). Como [executar o script R][execute-r-script], que tem a mesma limitação, é possível, em muitos casos, pickle objetos em uma matriz de bytes e, em seguida, retorná-los dentro de um quadro de dados.
