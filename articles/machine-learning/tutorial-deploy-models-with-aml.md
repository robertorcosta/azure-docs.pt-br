...
Em seguida, crie um arquivo de ambiente, chamado **myenv.yml**, que especifica todas as dependências de pacote do script. Este arquivo é usado para assegurar que todas essas dependências sejam instaladas na imagem do Docker. Este modelo precisa de `scikit-learn` e `azureml-sdk`. Todos os arquivos de ambiente personalizados precisam listar os azureml-defaults com a versão >= 1.0.45 como uma dependência pip. Esse pacote contém a funcionalidade necessária para hospedar o modelo como um serviço Web.
...
Observe que, se você estiver definindo seu próprio arquivo de ambiente, deverá listar azureml-defaults com a versão >= 1.0.45 como uma dependência pip. Esse pacote contém a funcionalidade necessária para hospedar o modelo como um serviço Web.
