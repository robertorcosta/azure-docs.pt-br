...
* Para depurar localmente, você deve ter uma instalação de Docker em funcionamento no sistema local.
    ...
    Para verificar a instalação do Docker, use o `docker run hello-world` comando de um terminal ou de um prompt de comando. Para obter informações sobre como instalar o Docker ou solucionar erros do Docker, consulte a [documentação do Docker](https://docs.docker.com/).
...
Para implantar localmente, modifique seu código para usar `LocalWebservice.deploy_configuration()` para criar uma configuração de implantação. Em seguida `Model.deploy()` , use para implantar o serviço. O exemplo a seguir implanta um modelo (contido na variável de modelo) como um serviço Web local:
...
Um código de status 502 indica que o serviço gerou uma exceção ou falhou no `run()` método do arquivo score.py. Use as informações neste artigo para depurar o arquivo.
...
1. Para facilitar o trabalho com a imagem, use o comando a seguir para adicionar uma marca. Substituir `myimagepath` pelo valor de local da etapa anterior.
...
1. Para localizar os arquivos usados pelo serviço, use o seguinte comando do shell bash no contêiner se o diretório padrão for diferente de `/var/azureml-app`:
