...
Chamar a `from_config()` função emitirá o prompt.
...
A `from_config()` função procura um arquivo JSON que contém as informações de conexão do espaço de trabalho. Você também pode especificar os detalhes da conexão explicitamente usando o `Workspace` Construtor, que também solicitará a autenticação interativa. Ambas as chamadas são equivalentes.
...
A saída será um JSON semelhante ao seguinte. Anote os `clientId`campos, `clientSecret`e `tenantId` , pois você precisará deles para outras etapas neste artigo.
...
Em seguida, execute o seguinte comando para obter os detalhes sobre a entidade de serviço que você acabou de `clientId` criar, usando o valor acima como a `--id` entrada para o parâmetro.

```azurecli-interactive
az ad sp show --id your-client-id
```

Veja a seguir um exemplo simplificado da saída JSON do comando. Anote o `objectId` campo, pois você precisará de seu valor para a próxima etapa.
...
Em seguida, use o comando a seguir para atribuir o acesso de sua entidade de serviço ao seu espaço de trabalho do Machine Learning. Você precisará do nome do seu espaço de trabalho e do nome do `-w` grupo `-g` de recursos para os parâmetros e, respectivamente. Para o `--user` parâmetro, use o `objectId` valor da etapa anterior. O `--role` parâmetro permite que você defina a função de acesso para a entidade de serviço e, em geral, você usará o **proprietário** ou o **colaborador**. Ambos têm acesso de gravação a recursos existentes, como clusters de computação e repositórios de armazenamento, mas somente o **proprietário** pode provisionar esses recursos.
...
Agora que você tem a autenticação de entidade de serviço habilitada, é possível autenticar o seu espaço de trabalho no SDK sem fazer logon fisicamente como um usuário. Use o `ServicePrincipalAuthentication` Construtor de classe e use os valores obtidos das etapas anteriores como os parâmetros. O `tenant_id` parâmetro é mapeado `tenantId` para de acima `service_principal_id` , mapeia `clientId`para e `service_principal_password` mapeia para `clientSecret`.
...
A `sp` variável agora mantém um objeto de autenticação que você usa diretamente no SDK. Em geral, é uma boa ideia armazenar as IDs/segredos usados acima em variáveis de ambiente, conforme mostrado no código a seguir.
...
Use as etapas a seguir para gerar um token de autenticação usando o Node. js. Em seu ambiente, execute `npm install adal-node`. Em seguida, use `tenantId`o `clientId`, o `clientSecret` e a entidade de serviço que você criou nas etapas acima como valores para as variáveis correspondentes no script a seguir.
...
Use a `accessToken` propriedade para buscar o token de autenticação. Consulte a [documentação da API REST](https://github.com/microsoft/MLOps/tree/master/examples/AzureML-REST-API) para obter exemplos de como usar o token para fazer chamadas à API.

### <a name="python"></a>Python

Use as etapas a seguir para gerar um token de autenticação usando o Python. Em seu ambiente, execute `pip install adal`. Em seguida, use `tenantId`o `clientId`, o `clientSecret` e a entidade de serviço que você criou nas etapas acima como valores para as variáveis apropriadas no script a seguir.
...
Os serviços Web implantados no serviço kubernetes do Azure têm a autenticação baseada em chave *habilitada* por padrão. Os serviços implantados das instâncias de contêiner do Azure têm a autenticação baseada em chave *desabilitada* por padrão `auth_enabled=True`, mas você pode habilitá-la definindo ao criar o serviço Web ACI. Veja a seguir um exemplo de como criar uma configuração de implantação ACI com a autenticação baseada em chave habilitada.
...
Em seguida, você pode usar a configuração ACI personalizada na implantação `Model` usando a classe.
...
Para buscar as chaves de autenticação, `aci_service.get_keys()`use. Para regenerar uma chave, use a `regen_key()` função e passe **primária** ou **secundária**.
...
> [!IMPORTANT]
> Você precisará solicitar um novo token após a hora do `refresh_by` token. Se você precisar atualizar os tokens fora do SDK do Python, uma opção é usar a API REST com a autenticação de entidade de serviço para fazer `service.get_token()` a chamada periodicamente, conforme discutido anteriormente.
>
