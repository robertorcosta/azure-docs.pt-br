...
Adicione um PythonScriptStep ao seu pipeline do Azure ML. Configure seu [RunConfiguration](https://docs.microsoft.com/python/api/azureml-core/azureml.core.runconfiguration?view=azure-ml-py) com a dependência em opencensus-ext-Azure. Configure a variável de ambiente `APPLICATIONINSIGHTS_CONNECTION_STRING`.
...
A variável de ambiente `APPLICATIONINSIGHTS_CONNECTION_STRING` é necessária para a biblioteca OpenCensus. É recomendável definir essa variável de ambiente em vez de passá-la como um parâmetro de pipeline para evitar passar pelas cadeias de conexão de texto não criptografado.
