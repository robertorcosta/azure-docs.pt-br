...
1. Defina o caminho no repositório de dados onde estão os dados. O caminho é um caminho relativo. Veja `data/testoutput` como um exemplo, o que significa que os dados de entrada dos **dados de exportação** serão exportados para `data/testoutput` o no datastore definido nas **configurações de saída** do módulo.
