...
ms.locfileid: "104604229"
---
# <a name="azure-security-baseline-for-azure-sentinel"></a>Linha de base de segurança do Azure para Azure Sentinel

Essa linha de base de segurança aplica diretrizes do [benchmark de segurança do Azure versão 1,0](../security/benchmarks/overview-v1.md) para o Azure Sentinel. O Azure Security Benchmark fornece recomendações sobre como você pode proteger suas soluções de nuvem no Azure.
O conteúdo é agrupado pelos **controles de segurança** definidos pelo benchmark de segurança do Azure e pelas diretrizes relacionadas aplicáveis ao Azure Sentinel. Os **controles** não aplicáveis ao Azure Sentinel foram excluídos.

 
Para ver como o Azure Sentinel é completamente mapeado para o benchmark de segurança do Azure, consulte o [arquivo de mapeamento de linha de base de segurança do Azure Sentinel completo](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Diretrizes**: Mantenha um inventário das contas de usuário que têm acesso administrativo ao plano de controle (como portal do Azure) do seu espaço de trabalho do Azure Sentinel. 

Você pode usar o painel controle de acesso e identidade na portal do Azure para sua assinatura para configurar o controle de acesso baseado em função do Azure (RBAC do Azure). As funções são aplicadas a usuários, grupos, entidades de serviço e identidades gerenciadas no Azure Active Directory (Azure AD). O Azure Sentinel também usa o RBAC do Azure para fornecer funções administrativas internas, como o colaborador do Azure Sentinel, que pode ser atribuído a usuários, grupos e serviços no Azure. 
...
**Orientação**: controle o acesso ao plano para seu espaço de trabalho do Azure Sentinel (por exemplo, portal do Azure) está disponível por meio da API REST e dá suporte a SSO. Para autenticar, defina o cabeçalho de autorização para suas solicitações para um token Web JSON obtido do Azure Active Directory (AD do Azure).
...
**Diretrizes**: Use Azure Active Directory (AD do Azure) como o sistema de autenticação e autorização central para seus espaços de trabalho do Azure Sentinel. O Azure AD protege os dados usando criptografia forte para dados em repouso e em trânsito. O Azure Active Directory também inclui sais, hashes e armazena com segurança as credenciais do usuário.

Você tem acesso à atividade de entrada do Azure AD, às fontes de log de eventos de auditoria e de risco, que permitem a integração com o Azure Sentinel ou uma solução de gerenciamento de eventos e informações do sistema de terceiros.
...
**Diretrizes**: implemente assinaturas separadas para incluir, opcionalmente, grupos de gerenciamento para espaços de trabalho de desenvolvimento, teste e produção Sentinel.
...
- [Visão geral de Azure Monitor chave gerenciada pelo cliente](https://docs.microsoft.com/azure/azure-monitor/logs/customer-managed-keys#customer-managed-key-overview) 
