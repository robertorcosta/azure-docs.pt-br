---
title: Loja de Consulta - Banco de Dados Azure para MariaDB
description: Saiba mais sobre o recurso Query Store no Banco de Dados Do Azure para MariaDB para ajudá-lo a acompanhar o desempenho ao longo do tempo.
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
---
# <a name="monitor-azure-database-for-mariadb-performance-with-query-store"></a>Monitore o banco de dados do Azure para o desempenho do MariaDB com a Query Store

**Aplica-se a:** Banco de dados Azure para MariaDB 10.2

O recurso Query Store no Azure Database for Mariadb fornece uma maneira de acompanhar o desempenho da consulta ao longo do tempo. O Repositório de Consultas simplifica a solução de problemas ajudando você a rapidamente localizar as consultas de execução mais longa e que consomem mais recursos. O Repositório de Consultas captura automaticamente um histórico das estatísticas de runtime e consultas e o retém para sua análise. Ele separa os dados por janelas de tempo para que você possa ver padrões de uso do banco de dados. Os dados para todos os usuários, bancos de dados e consultas são armazenados no banco de dados do esquema **mysql** no Banco de Dados Do Azure para a instância MariaDB.
...
1. Faça login no portal Azure e selecione seu banco de dados Azure para o servidor MariaDB.
...
- As estatísticas em tempo de execução armazenam para persistir as informações das estatísticas de execução de consulta.
- As estatísticas de espera armazenam informações de estatísticas de espera persistentes.
...
- Se um servidor MariaDB `default_transaction_read_only` estiver ligado, o Query Store não poderá capturar dados.
...
- As estatísticas de espera usam a captura de uma fração de eventos. A freqüência pode ser `query_store_wait_sampling_frequency`modificada usando o parâmetro .
