---
title: Avaliar instâncias de VM da GCP quanto à migração para o Azure com as Migrações para Azure
description: Descreve como avaliar instâncias de VM da GCP quanto à migração para o Azure usando as Migrações para Azure.
author: rashi-ms
ms.author: rajosh
ms.manager: abhemraj
ms.topic: tutorial
...
Este artigo mostra como avaliar as instâncias de VM da GCP (Google Cloud Platform) quanto à migração para o Azure usando a ferramenta Migrações para Azure: descoberta e avaliação.
...
Decida se deseja executar uma avaliação usando critérios de dimensionamento com base nos dados/metadados de configuração do servidor que são coletados no estado em que se encontram localmente ou nos dados de desempenho.
