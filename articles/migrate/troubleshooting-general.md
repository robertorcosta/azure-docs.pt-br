...
1. Verify that the Azure Migrate appliance OVA file is downloaded correctly by checking its hash value. [Saiba mais](https://docs.microsoft.com/azure/migrate/tutorial-assessment-vmware). If the hash value doesn't match, download the OVA file again and retry the deployment.
...
Error 60025: "An Azure AD operation failed. The error occurred while creating or updating the Azure AD application" occurs when the Azure user account used to initiate the discovery is different from the account used to register the appliance. Faça uma das opções a seguir:
...
**Problema** | **Correção**
--- | ---
Tipo de inicialização sem suporte | Azure doesn't support VMs with an EFI boot type. We recommend that you convert the boot type to BIOS before you run a migration. <br/><br/>You can use Azure Migrate Server Migration to handle the migration of such VMs. It will convert the boot type of the VM to BIOS during the migration.
...
SO Linux condicionalmente endossado | Azure endorses only [selected Linux OS versions](../virtual-machines/linux/endorsed-distros.md). Consider upgrading the machine before you migrate to Azure.
SO Linux não endossado | The machine might start in Azure, but Azure provides no operating system support. Consider upgrading to an [endorsed Linux version](../virtual-machines/linux/endorsed-distros.md) before you migrate to Azure.
Sistema operacional desconhecido | The operating system of the VM was specified as "Other" in vCenter Server. This behavior blocks Azure Migrate from verifying the Azure readiness of the VM. Make sure that the operating system is [supported](https://aka.ms/azureoslist) by Azure before you migrate the machine.
...
VM not found for the required storage performance | The storage performance (input/output operations per second [IOPS] and throughput) required for the machine exceeds Azure VM support. Reduza os requisitos de armazenamento para a máquina antes da migração.
VM not found for the required network performance | O desempenho de rede (entrada/saída) necessário para a máquina excede o suporte de VM do Azure. Reduza os requisitos de rede para a máquina.
VM not found in the specified location | Use um local de destino diferente antes da migração.
...
Could not determine disk suitability because of an internal error | Tente criar uma nova avaliação para o grupo.
VM com núcleos necessários e memória não encontrada | Azure couldn't find a suitable VM type. Reduza a memória e o número de núcleos da máquina local antes de migrar.
Could not determine VM suitability because of an internal error | Tente criar uma nova avaliação para o grupo.
Could not determine suitability for one or more disks because of an internal error | Tente criar uma nova avaliação para o grupo.
Could not determine suitability for one or more network adapters because of an internal error | Tente criar uma nova avaliação para o grupo.
...
As an example, if you have an on-premises disk with 32 GB of memory, but the aggregated read and write IOPS for the disk is 800 IOPS, Server Assessment recommends a premium disk (because of the higher IOPS requirements), and then recommends a disk SKU that can support the required IOPS and size. A correspondência mais próxima neste exemplo seria P15 (256 GB, IOPS de 1100). Even though the size required by the on-premises disk was 32 GB, Server Assessment recommends a larger disk because of the high IOPS requirement of the on-premises disk.
...
A Avaliação de Servidor coleta continuamente os dados de desempenho dos computadores locais e o utiliza para recomendar a SKU da VM e a SKU de disco no Azure. This performance data is collected by Server Assessment as follows:
...
- After migration to Azure, on-premises machines are turned off and equivalent VMs are spun up in Azure. Essas máquinas adquirem um endereço MAC diferente.
...
3. Select the **Network** tab, and start capturing network traffic:
   - No Chrome, selecione **Preservar log**. A gravação deve ser iniciada automaticamente. A red circle indicates that traffic is being captured. If the red circle doesn't appear, select the black circle to start.
...
6. Select the **Console** tab to check for any warnings or errors. Para salvar o log do console:
   - No Chrome, clique com o botão direito em qualquer lugar no log do console. Select **Save as**, to export, and zip the log.
