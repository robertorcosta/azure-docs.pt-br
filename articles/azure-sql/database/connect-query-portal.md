...
A conclusão deste início rápido requer o banco de dados de exemplo AdventureWorksLT. Se você não tem uma cópia funcional do banco de dados de exemplo AdventureWorksLT no Banco de Dados SQL, o seguinte guia de início rápido cria um rapidamente:
...
Configurar um administrador do Azure AD (Azure Active Directory) permite que você use uma identidade exclusiva para entrar no portal do Azure e no banco de dados. Para se conectar ao banco de dados usando o Azure AD, siga as etapas abaixo a fim de configurar um administrador do Azure AD para sua Instância do SQL Server.
...
#### <a name="set-an-active-directory-admin-for-the-server"></a>Definir um administrador do Active Directory para o servidor

1. No portal do Azure, selecione sua Instância do SQL Server.

2. No menu **Servidor SQL**, selecione **Administrador do Active Directory**.

3. Na barra de ferramentas da página do SQL Server **Administrador do Active Directory**, selecione **Definir administrador** e escolha o usuário ou o grupo como administrador do Azure AD.
...
6. No menu **SQL Server**, primeiro selecione **bancos de dados SQL** e depois o seu.

7. No menu **Banco de dados SQL**, selecione **Editor de consultas (versão prévia)** . Na página **Logon**, no rótulo **autenticação do Active Directory**, uma mensagem será exibida informando que você foi conectado se você for um administrador do Azure AD. Selecione o botão **Continuar como** *\<your user or group ID>* . Se a página indicar que você não fez logon com êxito, talvez seja necessário atualizá-la.
...
* Se você tiver uma conexão do Link Privado, o editor de consultas funcionará sem a necessidade de adicionar o endereço IP do cliente ao firewall do Banco de Dados SQL.
