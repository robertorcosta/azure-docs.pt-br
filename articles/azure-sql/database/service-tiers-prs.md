...
description: A camada de serviço Premium RS para o banco de dados SQL do Azure está sendo desativada e o suporte para ele está terminando-consulte Opções de migração.
...
- Camadas de serviço vCore
  ...
  As camadas de serviço **Uso Geral** e **Comercialmente Crítico** no [modelo de compra com base em vCore](service-tiers-vcore.md). Essas duas camadas de serviço estão na disponibilidade geral. O modelo de compra baseado em vCore também oferece a camada de serviço de **hiperescala** que se adapta sob demanda às necessidades da carga de trabalho com o dimensionamento automático de até 100 TB por banco de dados. A camada de serviço Hiperescala fornece desempenho de E/S comparável à camada de serviço Premium no [Modelo de compra baseado em DTU](service-tiers-dtu.md) a um preço mais próximo da camada de serviço Premium RS.
...
Nossas camadas de serviço no modelo de compra baseado em vCore são elegíveis para ofertas especiais que podem economizar até 80% em comparação aos preços incluídos na licença. Use suas licenças do SQL Server Standard ou Enterprise Edition com o Software Assurance ativo para economizar até 55% versus preços incluídos na licença com o [benefício híbrido do Azure](https://azure.microsoft.com/pricing/hybrid-benefit/) para SQL Server. Você pode combinar o benefício híbrido com o preço [de capacidade reserva do Banco de Dados SQL do Azure](reserved-capacity-overview.md) e economizar até 80% ao comprometer-se antecipadamente com um prazo de um ou três anos.  Ative hoje mesmo os dois benefícios no portal do Azure.
...
- Para obter detalhes sobre os tamanhos da computação específicos e as opções de tamanho de armazenamento disponíveis para bancos de dados individuais, confira [Limites de recursos baseados em vCore do Banco de Dados SQL para bancos de dados individuais](resource-limits-vcore-single-databases.md)
- Para obter detalhes sobre tamanhos da computação específicos e opções de tamanho de armazenamento disponíveis para pools elásticos, veja [Limites de recursos baseados em vCore do Banco de Dados SQL para pools elásticos](resource-limits-vcore-elastic-pools.md).
