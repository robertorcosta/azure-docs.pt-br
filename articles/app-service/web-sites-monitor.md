...
description: Saiba como monitorar aplicativos no Serviço de Aplicativo do Azure usando o portal do Azure. Entenda as cotas e métricas relatadas.
author: btardif
...
ms.locfileid: "79500428"
---
# <a name="monitor-apps-in-azure-app-service"></a>Monitorar aplicativos no Serviço de Aplicativo do Azure
[O Azure App Service](https://go.microsoft.com/fwlink/?LinkId=529714) oferece funcionalidade de monitoramento incorporada para aplicativos web, aplicativos móveis e API no [portal Azure](https://portal.azure.com).

No portal do Azure, você pode rever *cotas* e *métricas* para um plano de aplicativo e Serviço de Aplicativo, e configurar alertas e *métricas* baseadas em regras *de autodimensionamento.*
...
| **Banda** | A quantidade total de largura de banda de saída permitida para esse aplicativo em um dia. Essa cota é definida novamente a cada 24 horas, à meia-noite UTC. |
| **Filesystem** | A quantidade total de armazenamento permitida. |

A única cota aplicável aos aplicativos hospedados no *Basic*, *Standard*e *Premium* é filesystem.
...
| Métrica | Descrição |
| --- | --- |
| **Tempo de resposta** | O tempo demorado para o aplicativo atender solicitações, em segundos. |
| **Tempo médio de resposta (preterido)** | O tempo médio demorado para o aplicativo atender solicitações, em segundos. |
...
| **Tempo de CPU** | A quantidade de CPU consumida pelo aplicativo em segundos. Para obter mais informações sobre essa métrica, consulte [o tempo da CPU versus a porcentagem da CPU](#cpu-time-vs-cpu-percentage). |
...
| **Uso do sistema de arquivos** | Percentual da cota do sistema de arquivos consumida pelo aplicativo. |
...
| **E/S de Outros Bytes por Segundo** | A taxa na qual o processo do aplicativo está emitindo bytes para operações de I/O que não envolvem dados, como operações de controle.|
| **E/S de Outras Operações por Segundo** | A taxa na qual o processo do aplicativo está emitindo operações de I/O que não são operações de leitura ou gravação.|
...
| **Bytes Particulares** | Private Bytes é o tamanho atual, em bytes, de memória que o processo do aplicativo alocou que não pode ser compartilhado com outros processos.|
...
| **Tamanho da fila do disco** | O número médio de solicitações de leitura e gravação enfileiradas no armazenamento. Um alto comprimento de fila de disco é uma indicação de um aplicativo que pode estar desacelerando por causa do excesso de I/O do disco. |
...
**Porcentagem da CPU**: Útil para aplicativos hospedados em planos Básico, Padrão e Premium, porque eles podem ser dimensionados. A porcentagem da CPU é uma boa indicação do uso geral em todas as instâncias.
...
* **Métricas de** granularidade minuciosa são mantidas por 30 horas.
* **As** métricas de granularidade da hora são mantidas por 30 dias.
* **As** métricas de granularidade diária são mantidas por 30 dias.
...
Para encontrar cotas, selecione **Cotas de Configurações** > **Quotas**. No gráfico, você pode ver: 
1. O nome da cota.
1. O intervalo de redefinição.
1. O limite atual.
1. O valor atual.

![Gráfico métrico no portal][metrics] Azure Você pode acessar métricas diretamente da página visão **geral** do recurso. Aqui você verá gráficos representando algumas das métricas dos aplicativos.

Clicar em qualquer um desses gráficos irá levá-lo à exibição de métricas onde você pode criar gráficos personalizados, consultar diferentes métricas e muito mais. 
