...
Execute o comando [az group create](/cli/azure/group#az-group-create) para criar um grupo de recursos:
...
1. Execute o comando [`az acr create`](/cli/azure/acr#az-acr-create) para criar um Registro de Contêiner do Azure:
...
1. Execute o comando [`az acr show`](/cli/azure/acr#az-acr-show) para recuperar as credenciais do registro:
...
1. Crie um plano do Serviço de Aplicativo usando o comando [`az appservice plan create`](/cli/azure/appservice/plan#az-appservice-plan-create):
...
1. Crie o aplicativo Web com o comando [`az webpp create`](/cli/azure/webapp#az-webapp-create):
...
1. Use [`az webapp config appsettings set`](/cli/azure/webapp/config/appsettings#az-webapp-config-appsettings-set) para definir a variável de ambiente `WEBSITES_PORT` conforme esperado pelo código do aplicativo: 
...
1. Habilite a [identidade gerenciada](./overview-managed-identity.md) para o aplicativo Web usando o comando [`az webapp identity assign`](/cli/azure/webapp/identity#az-webapp-identity-assign):
...
1. Recupere a sua ID de assinatura com o comando [`az account show`](/cli/azure/account#az-account-show), que você precisará na próxima etapa:
...
1. Use o comando [`az webapp config container set`](/cli/azure/webapp/config/container#az-webapp-config-container-set) para especificar o registro de contêiner e a imagem a ser implantada para o aplicativo Web:
