...
Para completar as etapas deste artigo, [crie um aplicativo app service](/azure/app-service/)ou use um aplicativo que você criou para outro tutorial.
...
O ponto final acima não funciona para o Linux App Services no momento. Considere usar ftp ou a [API zip implantar](https://docs.microsoft.com/azure/app-service/containers/app-service-linux-faq#continuous-integration-and-deployment) em vez disso.
...
Por padrão, o mecanismo de implantação assume que um arquivo ZIP está pronto para funcionar como está e não executa nenhuma automação de compilação. Para habilitar a mesma automação de compilação `SCM_DO_BUILD_DURING_DEPLOYMENT` que em uma implantação do [Git,](deploy-local-git.md)defina a configuração do aplicativo executando o seguinte comando no Cloud [Shell](https://shell.azure.com):
...
Para implantar um arquivo WAR no Serviço `https://<app-name>.scm.azurewebsites.net/api/wardeploy`de Aplicativo, envie uma solicitação POST para . A solicitação POST deve conter o arquivo .war no corpo da mensagem. As credenciais de implantação para seu aplicativo são fornecidas na solicitação usando a autenticação BÁSICA HTTP.

Use `/api/wardeploy` sempre ao implantar arquivos WAR. Esta API expandirá seu arquivo WAR e o colocará na unidade de arquivo compartilhado. o uso de outras APIs de implantação pode resultar em comportamento inconsistente. 
...
O exemplo a seguir usa [Publish-AzWebapp](/powershell/module/az.websites/publish-azwebapp) upload do arquivo .war. Substitua os espaços reservados `<group-name>`, `<app-name>` e `<war-file-path>`.
