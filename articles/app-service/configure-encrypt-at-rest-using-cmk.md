...
Depois de carregar o arquivo no armazenamento de BLOBs e ter uma URL SAS para o arquivo, defina a configuração `WEBSITE_RUN_FROM_PACKAGE` aplicativo como a URL SAS. O exemplo a seguir faz isso usando CLI do Azure:
...
Agora você pode substituir o valor da configuração do aplicativo `WEBSITE_RUN_FROM_PACKAGE` por uma referência de Key Vault para a URL codificada por SAS. Isso mantém a URL SAS criptografada no Key Vault, que fornece uma camada extra de segurança.
...
1. Use o seguinte comando [`az keyvault secret set`](/cli/azure/keyvault/secret#az-keyvault-secret-set) para adicionar a URL externa como um segredo em seu cofre de chaves:   

    ```azurecli    
    az keyvault secret set --vault-name "Contoso-Vault" --name "external-url" --value "<SAS-URL>"    
    ```    

1.  Use o comando [`az webapp config appsettings set`](/cli/azure/webapp/config/appsettings#az-webapp-config-appsettings-set) a seguir para criar a configuração de aplicativo `WEBSITE_RUN_FROM_PACKAGE` com o valor como uma referência Key Vault para a URL externa:
...
- Executar o aplicativo do pacote de implantação torna `wwwroot/` somente leitura. Seu aplicativo recebe um erro ao tentar gravar nesse diretório.
