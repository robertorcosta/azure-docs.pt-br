...
|`No refs in common and none specified; doing nothing. Perhaps you should specify a branch such as 'master'.`|Você não especificou uma ramificação durante `git push`ou não definiu `push.default` o valor `.gitconfig`em.|Execute `git push` novamente, especificando o Branch mestre: `git push azure master`.|
