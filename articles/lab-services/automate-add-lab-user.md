...
A primeira coisa a ser observada é que o tipo do recurso `Microsoft.Authorization/roleAssignments` não é como seria para um grupo de recursos.  Em vez disso, o tipo de recurso `{provider-namespace}/{resource-type}/providers/roleAssignments`segue o padrão. Nesse caso, o tipo de recurso será `Microsoft.DevTestLab/labs/providers/roleAssignments`.
...
A ID da assinatura é obtida usando `subscription().subscriptionId` a função de modelo.  
...
A ID da função é definida na seção de variáveis e `devTestLabUserRoleId`nomeada. No modelo, a ID da função é definida como: 111111111-0000-0000-11111111111111111. 
...
Para especificar o recurso ao qual as permissões estão sendo concedidas podem ser especificadas por uma `ResourceName`combinação `ResourceType`de `ResourceGroup` , ou pelo `scope` parâmetro. Qualquer combinação de parâmetros é usada, forneça informações suficientes ao cmdlet para identificar exclusivamente o objeto de Active Directory (usuário, grupo ou entidade de serviço), escopo (grupo de recursos ou recurso) e definição de função.

## <a name="use-azure-command-line-interface-cli"></a>Usar a CLI (interface de linha de comando) do Azure
No CLI do Azure, a adição de um usuário de laboratórios a um laboratório é feita `az role assignment create` usando o comando. Para obter mais informações sobre CLI do Azure cmdlets, consulte [gerenciar o acesso aos recursos do Azure usando o RBAC e o CLI do Azure](../role-based-access-control/role-assignments-cli.md).

O objeto que está sendo concedido acesso pode ser especificado pelos parâmetros `objectId`, `signInName`, `spn` . O laboratório ao qual o objeto está sendo concedido acesso pode `scope` ser identificado pela URL ou uma combinação dos parâmetros `resource-name`, `resource-type`e. `resource-group`
