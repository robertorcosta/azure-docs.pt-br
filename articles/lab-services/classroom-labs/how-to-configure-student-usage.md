...
1. Na janela **Enviar convite por email** , insira uma mensagem opcional e, em seguida, selecione **Enviar**. 
    ...
    A coluna **convite** da lista **usuários** exibe o status do convite para cada usuário adicionado. O status deve ser alterado para **enviando** e **enviado na \<data>**. 
...
1. No painel **usuários** , selecione **cota por usuário: \<número> hora (s)** na barra de ferramentas. 
1. Na janela **cota por usuário** , especifique o número de horas que você deseja dar a cada aluno fora da hora da classe agendada e, em seguida, selecione **salvar**.
    ...
    Os valores alterados agora são exibidos no botão **cota por usuário: \<número de horas>** na barra de ferramentas e na lista de usuários, conforme mostrado aqui:
...
1. Na **>de endereço de \<email ajustar cota de usuários selecionados **, insira o número de horas de laboratório adicionais que você deseja conceder ao aluno ou aos alunos selecionados e, em seguida, selecione **aplicar**. 
...
> [!IMPORTANT]
> Quando os alunos entram em um laboratório de sala de aula, eles não recebem a opção de criar um conta Microsoft. Por esse motivo, recomendamos que você inclua esse link de inscrição, http://signup.live.comno email de registro de laboratório da sala de aula enviado aos alunos que estão usando contas que não são da Microsoft.
