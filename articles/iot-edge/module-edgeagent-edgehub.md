---
title: Propriedades dos gêmeos do módulo agente e hub - Azure IoT Edge
...
O agente do IoT Edge e hub do IoT Edge são dois módulos que compõem o runtime do IoT Edge. Para obter mais informações sobre as responsabilidades de cada módulo de tempo de [execução, consulte Entenda o tempo de execução do Azure IoT Edge e sua arquitetura](iot-edge-runtime.md).
...
* **Propriedades relatadas**. O módulo pode definir propriedades relatadas, e o backend da solução pode lê-las e consulta-las. As propriedades relatadas são usadas juntamente com as propriedades desejadas para sincronizar a configuração ou as condições do módulo.
...
| runtime.settings.registryCredentials<br>.{registryId}.username | O nome de usuário do registro de contêiner. Para registro de contêiner do Azure, o nome de usuário geralmente é o nome do registro.<br><br> As credenciais de registro são necessárias para quaisquer imagens de módulos privados. | Não |
...
| systemModules.edgeAgent.settings.image | O URI da imagem do agente do IoT Edge. Atualmente, o agente IoT Edge não é capaz de se atualizar sozinho. | Sim |
...
| modules.{moduleId}.restartPolicy | {"nunca" \| "on-failure" \| "on-doenso" \| "always"} | Sim |
| Módulos. {moduleId}.imagePullPolicy | {"on-create" \| "nunca"} | Não |
| Módulos. {moduleId}.env | Uma lista de variáveis de ambiente para passar para o módulo. Toma o formato`"<name>": {"value": "<value>"}` | Não |
...
A cópia das propriedades atuais desejadas é útil para dizer se o dispositivo aplicou a implantação mais recente ou ainda está executando um manifesto de implantação anterior.
...
| lastDesiredStatus.code | Este código de status refere-se às últimas propriedades desejadas vistas pelo agente IoT Edge. Valores permitidos: `200` Êxito, `400` Configuração inválida, `412` Versão do esquema inválido, `417` As propriedades desejadas estão vazias, `500` Falha |
...
| systemModules.edgeHub.exitCode | O código de saída relatado pelo contêiner do hub IoT Edge se o contêiner sair |
...
| -------- | ----------- | -------- |
| schemaVersion | Tem que ser "1.0" | Sim |
| routes.{routeName} | Uma cadeia de caracteres que representa uma rota do hub do IoT Edge. Para obter mais informações, consulte [Declare routes](module-composition.md#declare-routes). | O elemento `routes` pode estar presente mas vazio. |
| storeAndForwardConfiguration.timeToLiveSecs | O tempo em segundos que o hub IoT Edge mantém as mensagens se desconectadas dos pontos finais de roteamento, seja ioT Hub ou um módulo local. O valor pode ser qualquer inteiro positivo. | Sim |
...
| clients.{device or moduleId}.lastConnectTime | Da última vez, o dispositivo ou módulo se conectou. |
| clients.{device or moduleId}.lastDisconnectTime | Da última vez, o dispositivo ou módulo se desligou. |
