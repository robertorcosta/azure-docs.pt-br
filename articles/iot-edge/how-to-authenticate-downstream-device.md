...
Para obter um exemplo de um programa do C Autenticando no Hub IoT com certificados X. 509, consulte o exemplo de [iotedge_downstream_device_sample](https://github.com/Azure/azure-iot-sdk-c/tree/x509_edge_bugbash/iothub_client/samples/iotedge_downstream_device_sample) do SDK do IOT do c. Algumas das principais linhas desse exemplo estão incluídas aqui para demonstrar o processo de autenticação.
...
Para obter um exemplo de um programa node. js que se autentica no Hub IoT com certificados X. 509, consulte o exemplo [simple_sample_device_x509. js](https://github.com/Azure/azure-iot-sdk-node/blob/master/device/samples/simple_sample_device_x509.js) do SDK do IOT do node. js. Algumas das principais linhas desse exemplo estão incluídas aqui para demonstrar o processo de autenticação.
...
O SDK do Python atualmente dá suporte apenas ao uso de certificados X509 e chaves de arquivos, não aqueles que são definidos em linha. No exemplo a seguir, os caminhos de filerelevantes são armazenados em variáveis de ambiente.

Ao definir o nome do host para seu dispositivo downstream, use o nome do host do dispositivo IoT Edge gateway para o parâmetro **hostname** . O nome do host pode ser encontrado no arquivo config. YAML do dispositivo de gateway. 
