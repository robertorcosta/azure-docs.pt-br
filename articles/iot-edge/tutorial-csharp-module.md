---
title: Tutorial – Desenvolver um módulo C# para o Linux usando o Azure IoT Edge
...
Este tutorial demonstra como desenvolver um módulo em **C#** usando o **Visual Studio Code** e implantá-lo em um dispositivo IoT Edge. Se estiver desenvolvendo módulos usando contêineres Windows, acesse [Desenvolver módulos do IoT Edge em C# usando contêineres Windows](tutorial-csharp-module-windows.md).
...
Antes de iniciar este tutorial, você deve ter percorrido o tutorial anterior para configurar seu ambiente de desenvolvimento, [Desenvolver um módulo do IoT Edge usando contêineres Linux](tutorial-develop-for-linux.md). Após concluir esse tutorial, você já deverá ter os seguintes pré-requisitos:
...
O arquivo do ambiente armazena as credenciais para o registro de contêiner e as compartilha com o runtime do IoT Edge. O runtime precisa dessas credenciais para efetuar pull de imagens privadas para o dispositivo IoT Edge. Use as credenciais da seção **Chaves de acesso** do Registro de Contêiner do Azure.
...
Na seção anterior, você criou uma solução do IoT Edge e adicionou o código ao CSharpModule. O novo código filtra as mensagens em que a temperatura relatada do computador está dentro dos limites aceitáveis. Agora você precisa compilar a solução como uma imagem de contêiner e enviá-la por push para seu registro de contêiner.
...
1. No gerenciador do VS Code, clique com o botão direito do mouse no arquivo **deployment.template.json** e selecione **Criar e Efetuar Push da Solução IoT Edge**.
...
4. No dispositivo, expanda **Módulos** para ver uma lista de módulos implantados e em execução. Clique no botão Atualizar. Você deve ver o novo **CSharpModule** sendo executado junto com o módulo **SimulatedTemperatureSensor** e **$edgeAgent** e **$edgeHub**.
