...
As seguintes propriedades são suportadas `location` para HDFS em configurações no conjunto de dados baseado em formato:

| Propriedade   | Descrição                                                  | Obrigatório |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | A propriedade `location` de tipo em conjunto de dados deve ser definida como **HdfsLocation**. | Sim      |
...
As seguintes propriedades são suportadas `storeSettings` para HDFS em configurações na fonte de cópia baseada em formato:

| Propriedade                 | Descrição                                                  | Obrigatório                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| type                     | A propriedade `storeSettings` do tipo abaixo deve ser definida como **HdfsReadSettings**. | Sim                                           |
...
| resourceManagerEndpoint | O ponto final do Gerenciador de Recursos de Fio | Sim se usando DistCp |
...
Suporte ao Copy Activity usando O DistCp para copiar arquivos como está no Azure Blob (incluindo [cópia em estágio)](copy-activity-performance.md)ou no Azure Data Lake Store, nesse caso, ele pode aproveitar totalmente o poder do seu cluster em vez de ser executado no Runtime de integração auto-hospedado. Ele proporcionará melhor taxa de transferência de cópia, especialmente se o cluster for muito avançado. Com base em sua configuração no Azure Data Factory, a atividade de cópia criará automaticamente um comando distcp, enviará ao cluster Hadoop e monitorará o status da cópia.
...
Consulte configurações e exemplos relacionados ao DistCp no [HDFS como](#hdfs-as-source) seção de origem.
...
### <a name="option-1-join-self-hosted-integration-runtime-machine-in-kerberos-realm"></a><a name="kerberos-join-realm"></a>Opção 1: Adere à máquina de execução de integração auto-hospedada no reino kerberos
...
**Exemplo: Fonte HDFS em atividade de cópia usando DistCp**
