---
title: Usar instância gerenciada do Banco de Dados SQL do Azure com Azure-SSIS (SQL Server Integration Services) no Azure Data Factory
description: Saiba como usar uma instância gerenciada do Banco de Dados SQL do Azure com SSIS (SQL Server Integration Services) no Azure Data Factory.
...
ms.locfileid: "83663534"
---
# <a name="use-azure-sql-database-managed-instance-with-sql-server-integration-services-ssis-in-azure-data-factory"></a>Usar instância gerenciada do Banco de Dados SQL com SSIS (SQL Server Integration Services) no Azure Data Factory

[!INCLUDE[appliesto-adf-asa-md](includes/appliesto-adf-xxx-md.md)]

Agora você pode mover suas cargas de trabalho, pacotes e projetos do SSIS (SQL Server Integration Services) para a nuvem do Microsoft Azure. Implante, execute e gerencie projetos do SSIS e pacotes no Banco de Dados SQL do Azure ou na Instância Gerenciada do Banco de Dados SQL com ferramentas conhecidas, como o SSMS (SQL Server Management Studio). Este artigo destaca as seguintes áreas específicas ao usar a Instância Gerenciada do Banco de Dados SQL do Azure com o Azure-SSIS IR (Integration Runtime):

- [Provisionar um Azure-SSIS IR com o catálogo do SSIS (SSISDB) hospedado pela instância gerenciada do Banco de Dados SQL do Azure](#provision-azure-ssis-ir-with-ssisdb-hosted-by-azure-sql-database-managed-instance)
...
- [Failover do Azure-SSIS IR com uma Instância Gerenciada do Banco de Dados SQL do Azure](configure-bcdr-azure-ssis-integration-runtime.md#azure-ssis-ir-failover-with-a-sql-database-managed-instance)
- [Migrar cargas de trabalho SSIS locais para SSIS no ADF com a instância gerenciada do Banco de Dados SQL do Azure como destino da carga de trabalho do banco de dados](scenario-ssis-migration-overview.md#azure-sql-database-managed-instance-as-database-workload-destination)

## <a name="provision-azure-ssis-ir-with-ssisdb-hosted-by-azure-sql-database-managed-instance"></a>Provisionar um Azure-SSIS IR com SSISDB hospedado pela instância gerenciada do Banco de Dados SQL do Azure

### <a name="prerequisites"></a>Pré-requisitos

1. [Habilite o Azure AD (Azure Active Directory) na instância gerenciada do Banco de Dados SQL do Azure](enable-aad-authentication-azure-ssis-ir.md#configure-azure-ad-authentication-for-azure-sql-database-managed-instance), ao escolher a autenticação do Azure Active Directory.

1. Escolha como conectar a instância gerenciada do SQL, no ponto de extremidade privado ou no ponto de extremidade público:

    - Ponto de extremidade privado (preferencial)

        1. Escolha a rede virtual para o Azure-SSIS IR ingressar:
            - Dentro da mesma rede virtual que a instância gerenciada do SQL, com **sub-rede diferente**.
            - Dentro de uma rede virtual diferente da instância gerenciada pelo SQL, via emparelhamento de rede virtual (que é limitado à mesma região devido a restrições de emparelhamento da VNet Global) ou uma conexão da rede virtual à rede virtual.

            Para saber mais sobre conectividade da instância gerenciada do SQL, confira [Conectar seu aplicativo à Instância Gerenciada do Banco de Dados SQL do Azure](https://review.docs.microsoft.com/azure/sql-database/sql-database-managed-instance-connect-app).

        1. [Configure a rede virtual](#configure-virtual-network).

    - Ponto de extremidade público

        As instâncias gerenciadas do Banco de Dados SQL do Azure podem fornecer conectividade em [pontos de extremidade públicos](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-public-endpoint-configure). Os requisitos de entrada e saída precisam ser atendidos para permitir o tráfego entre a instância gerenciada do SQL e o Azure-SSIS IR:

        - quando Azure-SSIS IR não está dentro de uma rede virtual (preferencial)

            **Requisito de entrada da instância gerenciada do SQL**, para permitir o tráfego de entrada de Azure-SSIS IR.
            ...
            Para saber mais, confira [Permitir tráfego de ponto de extremidade público no grupo de segurança de rede](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-public-endpoint-configure#allow-public-endpoint-traffic-on-the-network-security-group).

        - quando Azure-SSIS IR está dentro de uma rede virtual

            Há um cenário especial quando a instância gerenciada do SQL está em uma região em que o Azure-SSIS IR não oferece suporte, o Azure-SSIS IR está dentro de uma rede virtual sem o emparelhamento da VNet devido à limitação de emparelhamento global da VNet. Neste cenário, o **Azure-SSIS IR dentro de uma rede virtual** conecta a instância gerenciada SQL ao **ponto de extremidade público**. Use as regras do NSG (grupo de segurança de rede) mostradas abaixo para permitir o tráfego entre a instância gerenciada SQL e o Azure-SSIS IR:

            1. **Requisito de entrada da instância gerenciada do SQL**, para permitir o tráfego de entrada de Azure-SSIS IR.
             ...
             1. **Requisito de saída do Azure-SSIS IR**, para permitir o tráfego de saída para a instância gerenciada do SQL.

                | Protocolo de transporte | Fonte | Intervalo de portas de origem | Destino |Intervalo de portas de destino |
                |---|---|---|---|---|
                |TCP|VirtualNetwork|*|[Endereço IP do ponto de extremidade público da instância gerenciada do SQL](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-find-management-endpoint-ip-address)|3342|

### <a name="configure-virtual-network"></a>Configurar rede virtual

1. **Permissão de usuário**. O usuário que cria o Azure-SSIS IR do Azure deve ter a [atribuição de função](https://docs.microsoft.com/azure/role-based-access-control/role-assignments-list-portal#list-role-assignments-for-a-user-at-a-scope) pelo menos no recurso do Azure Data Factory com uma das opções abaixo:
...
1. **Rede virtual**.
    ...
    1. Certifique-se de que você não tem nenhum [bloqueio de recurso](https://docs.microsoft.com/azure/azure-resource-manager/management/lock-resources) no grupo de recursos/assinatura à qual pertence a rede virtual. Se você configurar um bloqueio somente leitura/exclusão, o início e a parada do Azure-SSIS IR falharão ou pararão de responder.
    ...
    1. Permita tráfego na regra do NSG (grupo de segurança de rede), para permitir o tráfego entre a instância gerenciada do SQL e o Azure-SSIS IR e o tráfego necessário para o Azure-SSIS IR.
        1. **Requisito de entrada da instância gerenciada do SQL**, para permitir o tráfego de entrada de Azure-SSIS IR.
        ...
        1. **Requisito de saída do Azure-SSIS IR**, para permitir o tráfego de saída para a instância gerenciada do SQL, e outro tráfego necessário ao Azure-SSIS IR.

        | Protocolo de transporte | Fonte | Intervalo de portas de origem | Destino | Intervalo de portas de destino | Comentários |
        |---|---|---|---|---|---|
        | TCP | VirtualNetwork | * | VirtualNetwork | 1433, 11000-11999 |Permita tráfego de saída para a instância gerenciada do SQL. Se a política de conexão estiver definida como **Proxy** em vez de **Redirecionar**, apenas a porta 1433 será necessária. |
...
1. Selecione o ponto de extremidade privado ou público da instância gerenciada SQL.

    Ao [provisionar o Azure-SSIS IR](create-azure-ssis-integration-runtime.md#provision-an-azure-ssis-integration-runtime) no portal do Azure/aplicativo do ADF, na página Configurações do SQL, use o **ponto de extremidade privado** ou **público** da instância gerenciada do SQL ao criar o catálogo SSIS (SSISDB).
...
1. Selecione autenticação do Azure AD quando aplicável.
    ...
    Para saber mais sobre como habilitar a autenticação do Azure AD, confira [Habilitar o Azure AD na instância gerenciada do Banco de Dados SQL do Azure](enable-aad-authentication-azure-ssis-ir.md#configure-azure-ad-authentication-for-azure-sql-database-managed-instance).
...
1. Associe o Azure-SSIS IR à rede virtual quando aplicável.
    ...
    Quando estiver na mesma rede virtual que a instância gerenciada do SQL, escolha uma **sub-rede diferente** da instância gerenciada do SQL. 
...
A política de retenção de logs do SSISDB é definida pelas propriedades abaixo nas [catalog.catalog_properties](https://docs.microsoft.com/sql/integration-services/system-views/catalog-catalog-properties-ssisdb-database?view=sql-server-ver15):
