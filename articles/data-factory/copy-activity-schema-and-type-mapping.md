...
As propriedades a seguir têm suporte `translator`  ->  `mappings` no objeto-> `source` com `sink`e:
...
A atividade de cópia dá suporte aos seguintes tipos de dados provisórios: 
