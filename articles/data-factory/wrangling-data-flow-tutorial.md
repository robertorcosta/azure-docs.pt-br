...
Para executar uma execução de depuração de pipeline de um fluxo de dados de disputa, clique em **Depurar** na tela do pipeline. Uma vez que você publica seu fluxo de dados, **o Trigger agora** executa uma execução demanda do último pipeline publicado. A disputa de fluxos de dados pode ser agendada com todos os gatilhos existentes da Fábrica de Dados Do Azure.
