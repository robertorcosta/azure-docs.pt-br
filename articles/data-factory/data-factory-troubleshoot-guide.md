...
- **Recomendação**: Atualize a função do Azure para retornar uma carga JSON válida. Por exemplo, uma C# função pode retornar `(ActionResult)new<OkObjectResult("{`\"ID\":\"123\"`}");`.
...
- **Recomendação**: Verifique o AzureMLService para obter mais logs de erros e corrija o pipeline ml
...
- **Causa**: o tipo de conteúdo é incompatível com o corpo da atividade da Web.
...
- **Recomendação**: Use Fiddler ou postmaster para verificar o ponto de extremidade.
