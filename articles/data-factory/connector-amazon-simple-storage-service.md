...
>[!TIP]
>Para o cenário de migração de dados do Amazon S3 para o Armazenamento do Azure, saiba mais em [Usar o Azure Data Factory para migrar dados do Amazon S3 para o Armazenamento do Azure](data-migration-guidance-s3-azure-storage.md).
...
Especificamente, este conector do Amazon S3 dá suporte à cópia de arquivos no estado em que se encontram ou à análise de arquivos com os [formatos de arquivo e codecs de compactação com suporte](supported-file-formats-and-compression-codecs.md). Você também pode optar por [Preservar os metadados do arquivo durante a cópia](#preserve-metadata-during-copy). O conector usa a [Assinatura da AWS versão 4](https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html) para autenticar solicitações para S3.
...
As seguintes propriedades são compatíveis com o Amazon S3 em `location` configurações de conjunto de dados baseado no formato:

| Propriedade   | Descrição                                                  | Obrigatório |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | A propriedade de tipo em `location` no conjunto de dados deve ser configurada com **AmazonS3Location**. | Sim      |
| bucketName | O nome do bucket S3.                                          | Sim      |
| folderPath | O caminho para a pasta sob o bucket fornecido. Se quiser usar um caractere curinga para filtrar a pasta, ignore essa configuração e especifique nas configurações de origem da atividade. | Não       |
| fileName   | O nome do arquivo sob o bucket + folderPath fornecido. Se quiser usar um caractere curinga para filtrar os arquivos, ignore essa configuração e especifique nas configurações de origem da atividade. | Não       |
| version | A versão do objeto S3 se o controle de versão do S3 está habilitado. Se não for especificada, a versão mais recente será buscada. |Não |
...
As seguintes propriedades são compatíveis com o Amazon S3 em `storeSettings` configurações na fonte de cópia baseada em formato:

| Propriedade                 | Descrição                                                  | Obrigatório                                                    |
| ------------------------ | ------------------------------------------------------------ | ----------------------------------------------------------- |
| type                     | A propriedade de tipo em `storeSettings` deve ser configurada com **AmazonS3ReadSettings**. | Sim                                                         |
...
| OPÇÃO DOIS: Prefixo S3<br>- prefixo | Prefixo para o nome da chave S3 sob o bucket especificado configurado no conjunto de dados para filtrar os arquivos S3 de origem. As chaves S3 cujo nome começa com `bucket_in_dataset/this_prefix` são selecionadas. Utiliza o filtro do lado do serviço do S3, que fornece melhor desempenho do que o filtro curinga. | Não |
