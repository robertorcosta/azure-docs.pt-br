---
title: Mover dados de um servidor FTP usando o Azure Data Factory
...
| authenticationType |Especifique o tipo de autenticação. |Yes |Básica, Anônima |
| Nome de Usuário |Especifique o usuário que tem acesso ao servidor FTP. |Não |&nbsp; |
| password |Especifique a senha para o usuário (nome de usuário). |No |&nbsp; |
| encryptedCredential |Especifique a credencial criptografada para acessar o servidor FTP. |No |&nbsp; |
| gatewayName |Especifique o nome do gateway no Gateway de Gerenciamento de Dados para se conectar a um servidor FTP local. |No |&nbsp; |
| porta |Especifique a porta ouvida pelo servidor FTP. |No |21 |
| enableSsl |Especifique se o canal FTP sobre SSL/TLS deve ser usado. |No |true |
| enableServerCertificateValidation |Especifique se deseja habilitar a validação do certificado TLS/SSL do servidor ao usar o canal FTP sobre SSL/TLS. |No |true |
...
| useBinaryTransfer |Especifique se o modo de transferência binário deve ser usado. Os valores são true para o modo binário (esse é o valor padrão) e false para ASCII. Essa propriedade só pode ser usada quando o tipo de serviço vinculado associado for do tipo: FtpServer. |No |
...
| --- | --- | --- | --- |
| recursiva |Indica se os dados são lidos recursivamente das subpastas ou somente da pasta especificada. |True, False (padrão) |No |
