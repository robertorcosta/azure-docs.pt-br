---
title: Empurrar dados para o índice de pesquisa usando a Fábrica de Dados
description: Saiba como empurrar dados para o Azure Cognitive Search Index usando o Azure Data Factory.
...
---
# <a name="push-data-to-an-azure-cognitive-search-index-by-using-azure-data-factory"></a>Empurre os dados para um índice de pesquisa cognitiva do Azure usando a Fábrica de Dados do Azure
> [!div class="op_single_selector" title1="Selecione a versão do serviço Data Factory que você está usando:"]
> * [Versão 1](data-factory-azure-search-connector.md)
> * [Versão 2 (versão atual)](../connector-azure-search.md)

> [!NOTE]
> Este artigo aplica-se à versão 1 do Data Factory. Se você estiver usando a versão atual do serviço Data Factory, consulte [o conector Azure Cognitive Search em V2](../connector-azure-search.md).

Este artigo descreve como usar a Atividade de Cópia para empurrar dados de um armazenamento de dados de origem suportado para um índice de pesquisa cognitiva do Azure. Armazenamentos de dados de origem com suporte listados na coluna de origem a [fontes e coletores com suporte](data-factory-data-movement-activities.md#supported-data-stores-and-formats) tabela. Este artigo baseia-se no artigo [atividades de movimentação](data-factory-data-movement-activities.md) de dados, que apresenta uma visão geral da movimentação de dados com a Atividade de Cópia e combinações de armazenamento de dados suportadas.
...
Você pode criar um pipeline com uma atividade de cópia que empurra dados de um armazenamento de dados de origem para um índice de pesquisa usando diferentes ferramentas/APIs.
...
Ao usar o assistente, as definições de JSON para essas entidades do Data Factory (serviços vinculados, conjuntos de dados e o pipeline) são automaticamente criadas para você. Ao usar ferramentas/APIs (exceto a API .NET), você define essas entidades do Data Factory usando o formato JSON.  Para obter uma amostra com definições JSON para entidades de fábrica de dados que são usadas para copiar dados para o índice de pesquisa, consulte [o exemplo jSON: Copie dados do SQL Server no local para uma](#json-example-copy-data-from-on-premises-sql-server-to-azure-cognitive-search-index) seção de índice de pesquisa cognitiva do Azure deste artigo.

As seções a seguir fornecem detalhes sobre as propriedades JSON que são usadas para definir entidades de fábrica de dados específicas para um índice de pesquisa:

## <a name="linked-service-properties"></a>Propriedades do serviço vinculado

A tabela a seguir fornece descrições para elementos JSON específicos do serviço vinculado à Pesquisa Cognitiva do Azure.

| Propriedade | Descrição | Obrigatório |
| -------- | ----------- | -------- |
| type | A propriedade do tipo deve ser definida **como: AzureSearch**. | Sim |
...
| WriteBatchSize | Faz upload de dados no índice de pesquisa quando o tamanho do buffer atinge writeBatchSize. Veja a [propriedade WriteBatchSize](#writebatchsize-property) para obter detalhes. | 1 a 1.000. O valor padrão é 1000. | Não |
...
## <a name="json-example-copy-data-from-on-premises-sql-server-to-azure-cognitive-search-index"></a>Exemplo JSON: Copie dados do SQL Server no local para o índice de pesquisa cognitiva do Azure
...
A amostra copia dados da série de tempo de um banco de dados sql server no local para pesquisar o índice de hora em hora. As propriedades JSON usadas nesses exemplos são descritas nas seções após os exemplos.
...
**Serviço vinculado ao Azure Cognitive Search:**
...
**Copiar atividade em um pipeline com fonte SQL e afundamento do Índice de Pesquisa Cognitiva do Azure:**
...
Se você estiver copiando dados de um armazenamento de `executionLocation` dados em nuvem no Azure Cognitive Search, a propriedade é necessária. O snippet JSON a seguir mostra a alteração necessária na Atividade de Cópia `typeProperties` como um exemplo. Verifique a seção [Copiar dados entre armazenamentos de dados de nuvem](data-factory-data-movement-activities.md#global) para obter mais detalhes e valores com suporte.
