...
integrationRuntime | O ambiente de computação no qual o fluxo de dados é executado. Se não for especificado, a solução de tempo de execução de integração do Azure será usada automaticamente. Há suporte apenas para os tempos de execução de integração de resolução automática de região. | IntegrationRuntimeReference | Não
...
Por exemplo, para chegar ao número de linhas gravadas em um coletor chamado ' sink1 ' em uma atividade denominada ' dataflowActivity ' `@activity('dataflowActivity').output.runStatus.metrics.sink1.rowsWritten`, use.

Para obter o número de linhas lidas de uma fonte denominada ' origem1 ' que foi usada nesse coletor, `@activity('dataflowActivity').output.runStatus.metrics.sink1.sources.source1.rowsRead`use.

> [!NOTE]
> Se um coletor tiver zero linhas gravadas, ele não aparecerá em métricas. A existência pode ser verificada `contains` usando a função. Por exemplo, `contains(activity('dataflowActivity').output.runStatus.metrics, 'sink1')` verificará se alguma linha foi gravada em sink1.
