...
>[!TIP]
>Para o cenário de migração do data Lake ou data warehouse, saiba mais sobre [como usar Azure data Factory para migrar dados de seu data Lake ou data warehouse para o Azure](data-migration-guidance-overview.md).
...
As propriedades a seguir têm suporte para o blob `location` do Azure em configurações em conjunto de base de formato:
...
As propriedades a seguir têm suporte para o blob `storeSettings` do Azure em configurações em fonte de cópia baseada em formato:
...
| prefixo                   | Prefixo do nome do blob no contêiner fornecido configurado no conjunto de dados para filtrar os blobs de origem. Os BLOBs cujo nome começa com esse prefixo são selecionados. <br>Aplica-se `wildcardFolderPath` somente `wildcardFileName` quando as propriedades e não são especificadas. | Não                                                          |
...
As propriedades a seguir têm suporte para o blob `storeSettings` do Azure em configurações no coletor de cópia com base em formato:
...
**Opção de nome de arquivo:** Determina como os arquivos de destino são nomeados na pasta de destino. As opções de nome de arquivo são:
   ...
   * **Como dados na coluna**: defina o arquivo de saída para o valor de uma coluna. O caminho é relativo ao contêiner de conjunto de um, não à pasta de destino. Se você tiver um caminho de pasta em seu conjunto de seus, ele será substituído.
