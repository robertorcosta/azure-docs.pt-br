---
title: Proteção de zonas e registros de DNS - Azure DNS
description: Neste caminho de aprendizado, comece a proteger zonas de DNS e conjuntos de registros no Microsoft Azure DNS.
services: dns
author: asudbring
ms.service: dns
ms.topic: article
...
ms.locfileid: "79371470"
---
# <a name="how-to-protect-dns-zones-and-records"></a>Proteger registros e zonas DNS

[!INCLUDE [updated-for-az](../../includes/updated-for-az.md)]

Registros e zonas DNS são recursos críticos. Excluir uma região DeDNS ou um único registro de DNS pode resultar em uma paralisação do serviço. É importante que as zonas e registros de DNS estejam protegidos contra alterações não autorizadas ou acidentais.
...
A função de Contribuinte de Zona de DNS é uma função incorporada para gerenciar recursos privados de DNS. Essa função aplicada a um usuário ou grupo permite que ele gerencie recursos DeDNS.

O grupo de recursos *myResourceGroup* contém cinco zonas para Contoso Corporation. A concessão das permissões de Colaborador de Zona DNS do administrador de DNS a esse grupo de recursos permite controle total sobre essas zonas DNS. Evita conceder permissões desnecessárias. O administrador de DNS não pode criar ou parar máquinas virtuais.
...
Controle **de acesso aberto (IAM)** para o grupo de recursos e selecione **Adicionar**e selecione a função Contribuinte de zona **de DNS.** Selecione os usuários ou grupos necessários para conceder permissões.
...
Por exemplo, o grupo de recursos *myResourceGroup* contém o *contoso.com* de região e um *customers.contoso.com*de subzona . Os registros CNAME são criados para cada conta do cliente. A conta de administrador usada para gerenciar registros CNAME é atribuída permissões para criar registros na *região customers.contoso.com.* A conta só pode gerenciar *customers.contoso.com.*

É possível conceder permissões de nível da zona RBAC por meio do Portal do Azure.  Controle **de acesso aberto (IAM)** para a região, selecione **Adicionar,** em seguida, selecione a função **Contribuinte da zona DNS** e selecione os usuários ou grupos necessários para conceder permissões.
...
A função Colaborador de Zona de DNS interna permite controle total sobre um recurso DNS. É possível construir suas próprias funções personalizadas do Azure para fornecer um controle mais fino.
