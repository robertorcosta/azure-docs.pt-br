---
title: Serviço de metadados de instância do Azure para Windows
description: Saiba mais sobre o serviço de metadados de instância do Azure e como ele fornece informações sobre as instâncias de máquina virtual em execução no momento no Windows.
...
```
> [!NOTE]
> O `-NoProxy` sinalizador só está disponível no PowerShell 6 ou posterior. Você pode omitir o sinalizador se não tiver uma configuração de proxy.

**Resposta**

> [!NOTE]
> A resposta é uma cadeia de caracteres JSON. Redirecione sua consulta REST por meio do `ConvertTo-Json` cmdlet para uma impressão bonita.
...
publicador | O editor da imagem da VM. | 2017-04-02
...
Os metadados de armazenamento fazem parte da API da instância, sob o `instance/compute/storageProfile` ponto de extremidade.
...
> [!NOTE]
> O certificado intermediário para o Azure China 21Vianet é da CA raiz DigiCert global, em vez de Baltimore.
...
Se houver um elemento de dados não encontrado ou uma solicitação malformada, IMDS retornará os erros HTTP padrão. Por exemplo:
