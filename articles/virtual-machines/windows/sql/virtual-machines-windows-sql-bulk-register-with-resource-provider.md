...
Este artigo descreve como registrar em massa seu SQL Server VM (máquina virtual) no Azure com o provedor de recursos de VM do `Register-SqlVMs` SQL usando o cmdlet do PowerShell.
...
Abra um terminal do PowerShell administrativo e navegue até onde você salvou `RegisterSqlVMs.psm1` o arquivo. Em seguida, execute o seguinte cmdlet do PowerShell para importar o script como um módulo: 
...
Tanto um relatório quanto um arquivo de log são gerados toda `Register-SqlVMs` vez que o cmdlet é usado. 

### <a name="report"></a>Relatório

O relatório é gerado como um `.txt` arquivo chamado `RegisterSqlVMScriptReport<Timestamp>.txt` onde o carimbo de data/hora é o horário em que o cmdlet foi iniciado. O relatório lista os seguintes detalhes:
...
Copie o script completo e salve-o `RegisterSqLVMs.psm1`como.
