...
1. <span id="ports"> </span> Abra as portas do firewall.  
   ...
   | Finalidade | Porta TCP | Anotações
   | ------ | ------ | ------
...
1. Vá para o compartilhamento usando o explorador de arquivos ou a caixa de diálogo **executar** (tecla com o logotipo do Windows + r). Use o caminho `\\storageaccountname.file.core.windows.net\filesharename`de rede. Por exemplo, `\\sqlvmstorageaccount.file.core.windows.net\sqlpremiumfileshare`
...
1. Na folha **Adicionar investigação de integridade** , <span id="probe"> </span> defina os parâmetros de investigação de integridade a seguir.
   ...
   - **Protocolo**: TCP.
   - **Porta**: a porta que você criou no firewall para a investigação de integridade nesta [etapa](#ports). Neste artigo, o exemplo usa a porta `59999`TCP.
...
A lista a seguir descreve os valores que você precisa atualizar:
   ...
   - `<Cluster Network Name>`: O nome do cluster de failover do Windows Server para a rede. Em **Failover Cluster Manager** > **redes**Gerenciador de cluster de failover, clique com o botão direito do mouse na rede e selecione **Propriedades**. O valor correto está em **Nome**, na guia **Geral**.
...
1. Abra **Gerenciador de cluster de failover**. Selecione **funções**. Observe qual nó possui a função FCI do SQL Server.
