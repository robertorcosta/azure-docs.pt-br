...
* O **serviço de mobilidade** é um componente que é implantado em todas as VMs padrão que você deseja replicar. Ele captura gravações de dados na VM padrão e as encaminha ao servidor de processo. Leia sobre os [pré-requisitos de máquina replicados](../../site-recovery/vmware-walkthrough-overview.md).
...
2. Selecione **criar um recurso** > **Gerenciamento** > de**backup** e **site Recovery (OMS)**. Como > alternativa, você pode selecionar **procurar** > **cofre dos serviços de recuperação****Adicionar**. 
...
2.  > Vá para os **cofres dos serviços de recuperação****configurações** > **site Recovery** > **etapa 1: preparar a infraestrutura** > **meta de proteção**.
...
Selecione **preparar infraestrutura** > **destino**e especifique o modelo de implantação que você deseja usar para VMs após o failover. Você pode escolher **Clássico** ou **Gerenciador de recursos**, dependendo do cenário.
...
2. Habilite a replicação da seguinte maneira:
   1. Selecione **replicar** > **origem**do aplicativo. Depois de habilitar a replicação pela primeira vez, selecione **+Replicar** no cofre para habilitar a replicação para máquinas virtuais adicionais.
...
Você pode ver o status do failover de teste em **configurações** > **trabalhos** > *YOUR_FAILOVER_PLAN_NAME*. No painel, você pode ver uma divisão das etapas e os resultados de êxito/falha. Se o failover de teste falhar em alguma etapa, selecione-a para verificar a mensagem de erro. 
