...
Este tutorial usará a CLI (interface de linha de comando) do Azure para implantar recursos no Azure.
...
Use o comando a seguir para criar um Grupo de Recursos `<resourceGroupName>` em uma região. Substitua `<resourceGroupName>` por um nome de sua escolha. Estamos usando `East US 2` para este tutorial. Para obter mais informações, confira o [Início Rápido](../quick-create-cli.md) a seguir.

```azurecli-interactive
az group create --name <resourceGroupName> --location eastus2
```

## <a name="create-an-availability-set"></a>Crie um Conjunto de Disponibilidade
...
1. Obtenha uma lista de imagens de VM (Máquina Virtual) que oferecem o RHEL com HA:
    ...
    Para este tutorial, estamos escolhendo a imagem `RedHat:RHEL-HA:7.6:7.6.2019062019`.
...
### <a name="register-a-new-application-in-azure-active-directory"></a>Registrar um novo aplicativo no Azure Active Directory
 ...
 6. Selecione um Tipo de Aplicativo **Web**, insira uma URL de logon (por exemplo http://localhost) e clique em Adicionar. A URL de logon não é usada e pode ser qualquer URL válida
 7. Selecione **Certificados e segredos** e clique em **Novo segredo do cliente**
...
Siga o tutorial para [Criar uma função personalizada para recursos do Azure usando a CLI do Azure](../../../role-based-access-control/tutorial-custom-role-cli.md#create-a-custom-role).
...
Use a seção abaixo para instalar o SQL Server e o mssql-tools nas VMs. Execute cada uma dessas ações em todos os nós. Para obter mais informações, confira [instalar o SQL Server em uma VM do Red Hat](/sql/linux/quickstart-install-connect-red-hat).
...
## <a name="configure-sql-server-always-on-availability-group"></a>Configurar o Grupo de Disponibilidade Always On do SQL Server

Use as etapas a seguir para configurar o Grupos de disponibilidade Always On do SQL Server para suas VMs. Para obter mais informações, confira [configurar o Grupos de disponibilidade Always On do SQL Server para alta disponibilidade no Linux](/sql/linux/sql-server-linux-availability-group-configure-ha)
...
Habilite grupos de disponibilidade AlwaysOn em cada nó que hospeda uma instância do SQL Server. Em seguida, reinicie o mssql-server. Execute o seguinte script:
...
1. Conecte-se a **todos os nós** usando o SSMS (SQL Server Management Studio) ou o SQL CMD. Execute os seguintes comandos para habilitar a sessão AlwaysOn_health e criar uma chave mestra:
...
Execute o seguinte script em todas as instâncias do SQL usando o SQL CMD ou o SSMS:
...
Em todos os SQL Servers, crie um logon do SQL para o Pacemaker. O Transact-SQL a seguir cria um logon.
...
Em todos os SQL Servers, salve as credenciais usadas para o logon do SQL Server. 
...
1. Confira seu recurso e verifique se eles estão online antes de continuar usando o seguinte comando:
...
1. Defina a propriedade **stonith-enabled** como false

    ```bash
    sudo pcs property set stonith-enabled=false
    ```

1. Crie o recurso de IP virtual usando o seguinte comando:
...
1. Crie uma restrição de ordenação para garantir que o recurso AG esteja em funcionamento antes do endereço IP. Enquanto a restrição de colocalização implica uma restrição de ordenação, isso a impõe.
...
1. Se você verificar suas restrições novamente, verá que outra restrição foi adicionada devido ao failover manual:
...
Para utilizar um Ouvinte do Grupo de Disponibilidade para seus SQL Servers criados no Azure, primeiro você precisará criar e configurar um balanceador de carga.

> [!div class="nextstepaction"]
> [Criar e configurar o balanceador de carga no portal do Azure](../../../virtual-machines/windows/sql/virtual-machines-windows-portal-sql-alwayson-int-listener.md#create-and-configure-the-load-balancer-in-the-azure-portal)