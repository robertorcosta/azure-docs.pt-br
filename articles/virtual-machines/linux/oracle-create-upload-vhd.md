...
* O Hyper-V e o Azure suportam o Oracle Linux com o Unbreakable Enterprise Kernel (UEK) ou o Red Hat Compatible Kernel.
...
* Certifique-se de que o repositório `Addons` está habilitado. Edite `/etc/yum.repos.d/public-yum-ol6.repo`o arquivo (Oracle `/etc/yum.repos.d/public-yum-ol7.repo`Linux 6) ou (Oracle Linux 7) e altere a `enabled=0` linha `enabled=1` para abaixo de **[ol6_addons]** ou **[ol7_addons]** neste arquivo.
...
9. Modifique a linha de inicialização do kernel em sua configuração de grub para incluir parâmetros adicionais de kernel para o Azure. Para fazer isso, abra "/boot/grub/menu.lst" em um editor de texto e certifique-se de que o kernel inclua os seguintes parâmetros:
...
* O Azure suporta o Oracle Linux com o Unbreakable Enterprise Kernel (UEK) ou o Red Hat Compatible Kernel. Recomenda-se o Oracle Linux com UEK.
...
9. Modifique a linha de inicialização do kernel em sua configuração de grub para incluir parâmetros adicionais de kernel para o Azure. Para fazer isso, abra "/etc/default/grub" em um editor de texto e edite o parâmetro `GRUB_CMDLINE_LINUX`. Por exemplo:
   ...
   Isso também garantirá que todas as mensagens do console sejam enviadas para a primeira porta serial, que pode auxiliar o suporte do Azure com problemas de depuração. Ele também desliga as convenções de nomeação para NICs no Oracle Linux 7 com o Unbreakable Enterprise Kernel. Além disso, recomendamos que você *remova* os seguintes parâmetros:
...
14. Execute os comandos a seguir para desprovisionar a máquina virtual e prepará-la para provisionamento no Azure:
        ...
        # export HISTSIZE=0
        # logout
15. Clique em **Action -> Shut Down** no Hyper-V Manager. Agora, seu VHD Linux está pronto para ser carregado no Azure.
