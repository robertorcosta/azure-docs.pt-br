...
1. Em **Conta do Administrador,** se a VM de origem foi generalizada, digite seu **nome de usuário** e chave **pública SSH**. Se a VM de origem foi especializada, essas opções serão acinzentadas porque as informações da VM de origem são usadas.
1. Se você quiser permitir o acesso remoto à VM, em **portas de entrada públicas,** escolha **Permitir portas selecionadas** e, em seguida, selecione **SSH (22)** a partir da queda. Se você não quiser permitir o acesso remoto à VM, deixe **nenhum** selecionado para **portas de entrada públicas**.
