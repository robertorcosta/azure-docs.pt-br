---
title: Capture uma imagem de uma VM Linux usando o Azure CLI
...
Você pode usar o serviço **Azure VM Image Builder (Public Preview)** para construir sua imagem personalizada, sem necessidade de aprender ferramentas ou criar pipelines de compilação, simplesmente fornecendo uma configuração de imagem, e o Construtor de Imagens criará a Imagem. Para obter mais informações, consulte [Getting Started with Azure VM Image Builder](https://docs.microsoft.com/azure/virtual-machines/linux/image-builder-overview).
...
Para uma versão simplificada deste artigo e para testar, avaliando ou aprendendo sobre VMs no Azure, consulte [Criar uma imagem personalizada de uma VM do Azure usando a CLI](tutorial-custom-images.md).  Caso contrário, continue lendo aqui para obter a imagem completa.


## <a name="step-1-deprovision-the-vm"></a>Etapa 1: desprovisionar a VM
Primeiro, você vai desprovisionar a VM usando o agente de VM do Azure para excluir dados e arquivos específicos do computador. Use o comando `waagent` com o `-deprovision+user` parâmetro em sua VM do Linux de origem. Para obter mais informações, consulte o [guia de usuário do Azure Linux Agent](../extensions/agent-linux.md).
...
4. Depois de concluir o comando, insira **sair** para fechar o cliente SSH.  A VM ainda estará funcionando neste momento.
...
1. Desaloque a VM desprovisionada com [az vm deallocate](/cli/azure/vm). O exemplo a seguir desaloca o VM chamado *myVM* no grupo de recursos chamado *myResourceGroup*.  
    ...
    Aguarde que a VM se desloque completamente antes de seguir em frente. Isso pode demorar alguns minutos para ser concluído.  O VM é desligado durante a desalocação.

2. Marque a VM como generalizada com [az vm generalize](/cli/azure/vm). O exemplo a seguir marca a VM chamada *myVM* no grupo de recursos chamado *myResourceGroup* como generalizado.
...
Este comando retorna JSON que descreve a imagem VM. Salve esta saída para referência posterior.
