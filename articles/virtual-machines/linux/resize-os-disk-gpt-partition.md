...
1. Use o `lsblk` comando para descobrir qual volume lógico (LV) está montado na raiz do sistema de arquivos ("/"). Nesse caso, vemos que **_rootvg-rootlv_*_ está montado em _* /**.  Se outro sistema de arquivos for desejado, substitua o LV e o ponto de montagem por este documento.
...
1. Verifique se há espaço livre no grupo de volumes LVM que contém a partição raiz.  Se houver espaço livre, pule para a etapa **12**
   ...
   Neste exemplo, o **PE/tamanho** da linha informa que há 38.02 GB livres no grupo de volumes.  Nenhum redimensionamento de disco é necessário antes de adicionar espaço ao grupo de volumes
...
1. Quando a VM reiniciar, execute as etapas a seguir:

   1. Instale o pacote **Cloud-utils-growpart** para fornecer o comando **growpart** , que é necessário para aumentar o tamanho do disco do sistema operacional.
...
1. Determine qual disco e partição contém os volumes físicos do LVM (VP) no grupo de volumes (VG) denominado rootvg com o comando **pvscan** .  Anote o tamanho e o espaço livre listados entre colchetes **[]**.
...
1. Verifique o tamanho da partição com **lsblk**.  Examine o 
...
1. Expanda a partição que contém este PV usando **growpart** , o nome do dispositivo e o número da partição.  Isso expandirá a partição especificada para usar todo o espaço contíguo livre no dispositivo.
...
1. Verifique se a partição foi redimensionada para o tamanho esperado com o comando **lsblk** novamente.  Observe que, no exemplo sda4 foi alterado de 63G para 95G.
...
1. Verifique se o novo tamanho do VP é o tamanho esperado, comparando-os aos valores originais **[tamanho/livre]** .
...
1. Expanda o volume lógico desejado (LV) pelo valor desejado, que não precisa ser todo o espaço livre no grupo de volumes.  No exemplo a seguir, **/dev/mapper/rootvg-rootlv** será redimensionado de 2 GB para 12 GB (um aumento de 10 GB) por meio do comando a seguir. Este comando também redimensionará o sistema de arquivos.
...
1. O comando lvresize chama automaticamente o comando de redimensionamento apropriado para o sistema de arquivos no LV. Verifique se **/dev/mapper/rootvg-rootlv** , que está montado em **/** tem um tamanho de sistema de arquivos maior usando o seguinte comando:
