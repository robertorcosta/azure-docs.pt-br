...
> [!WARNING]
> Use esse processo apenas como último recurso. Sempre tente redefinir uma senha usando o [portal do Azure ou Azure PowerShell](reset-rdp.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json) primeiro.
...
2. Na sessão remota para a nova VM, remova os seguintes arquivos para limpar o ambiente:
    ...
    * De `%windir%\System32\GroupPolicy\Machine\Scripts`
      * exclu`scripts.ini`
    * De `%windir%\System32\GroupPolicy`
      * Remova `gpt.ini` (se `gpt.ini` existia antes, e você o renomeou `gpt.ini.bak`como, renomeie `.bak` o `gpt.ini`arquivo de volta para)
