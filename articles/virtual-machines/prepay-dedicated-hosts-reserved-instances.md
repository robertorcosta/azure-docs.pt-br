...
-  de **Cota insuficiente** -uma reserva com escopo para uma única assinatura deve ter a cota vCPU disponível na assinatura para a nova instância reservada. Por exemplo, se a assinatura de destino tiver um limite de cota de 10 vCPUs para a série DSv3, você não poderá comprar uma reserva de hosts dedicados que dão suporte a essa série. A verificação de cota para reservas inclui as VMs e os hosts dedicados já implantados na assinatura. Você pode [criar](https://docs.microsoft.com/azure/azure-supportability/resource-manager-core-quotas-request) de solicitação de aumento de cota para resolver esse problema.

- As **restrições de capacidade** -em raras circunstâncias, o Azure limita a compra de novas reservas para o subconjunto de SKUs de host dedicado, devido à baixa capacidade em uma região.
...
2. Selecione **todos os serviços**  **reservas**de \>.
...
Se você tiver um contrato EA, poderá usar a **opção Adicionar mais** para adicionar rapidamente instâncias adicionais. A opção não está disponível para outros tipos de assinatura.
...
-  de **escopo do grupo de recursos único** — aplica o desconto de reserva aos recursos correspondentes somente no grupo de recursos selecionado.

-  de **escopo de assinatura única** — aplica o desconto de reserva aos recursos de correspondência na assinatura selecionada.

-  de **escopo compartilhado** — aplica o desconto de reserva aos recursos correspondentes em assinaturas qualificadas que estão no contexto de cobrança. Para clientes do EA, o contexto de cobrança é o registro. Para assinaturas individuais com tarifas pagas conforme o uso, o escopo do orçamento são todas as assinaturas qualificadas criadas pelo administrador da conta.
