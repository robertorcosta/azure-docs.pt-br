---
title: SAP HANA escalar horizontalmente HSR-Pacemaker com o SLES em solução de problemas de VMs do Azure | Microsoft Docs
...
Como configurar um dispositivo SBD em uma VM do Azure é descrito em [ SBD fence ](./high-availability-guide-suse-pacemaker.md#sbd-fencing).
...
Verifique se as entradas em **/etc/sysconfig/sbd** correspondem à descrição em [Configurando o Pacemaker no SUSE Linux Enterprise Server no Azure](./high-availability-guide-suse-pacemaker.md#sbd-fencing). Verifique se a configuração de inicialização em **/etc/iscsi/iscsid.conf** está definida como automática.
...
Você pode monitorar o status de integridade do SAP HANA Landscape como o **\<HANA SID\> ADM** do usuário chamando o script do SAP Python da seguinte maneira. Talvez seja necessário adaptar o caminho:
...
Para usar a interface do navegador, substitua **\<node\>** por um nó de SAP Hana real na URL a seguir. Em seguida, insira as credenciais do cluster (cluster **do usuário**):
...
Você também pode fazer upload da saída **hb_report** no Hawk, abaixo do **Histórico**, mostrado a seguir. Ver hb_report para coletar arquivos de log: 
