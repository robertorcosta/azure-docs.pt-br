...
- [Desconcentre uma instância SAP ASCS/SCS em um cluster de failover do Windows usando um disco compartilhado de cluster](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-high-availability-guide-wsfc-shared-disk)
...
- Você não pode implantar conjuntos de disponibilidade do Azure dentro de uma Zona de Disponibilidade do Azure, a menos que você use [o Azure Proximity Placement Group](https://docs.microsoft.com/azure/virtual-machines/linux/co-location). A forma como você pode implantar a camada SAP DBMS e os serviços centrais entre as regiões e, ao mesmo tempo, implantar a camada de aplicativo SAP usando conjuntos de disponibilidade e ainda alcançar proximidade das VMs está documentada no artigo [Azure Proximity Placement Groups para latência de rede ideal com aplicativos SAP](sap-proximity-placement-scenarios.md). Se você não está aproveitando grupos de colocação de proximidade do Azure, você precisa escolher um ou outro como uma estrutura de implantação para máquinas virtuais.
- Não é possível usar um [Azure Basic Load Balancer](https://docs.microsoft.com/azure/load-balancer/load-balancer-overview) para criar soluções de cluster de failover baseadas no Cluster de Failover do Windows Server ou no Linux Pacemaker. Em vez disso, você precisa usar o [Azure Standard Load Balancer SKU](https://docs.microsoft.com/azure/load-balancer/load-balancer-standard-availability-zones).
...
Você não precisa realizar esses testes manualmente. Você pode encontrar um teste de [latência da zona de disponibilidade](https://github.com/Azure/SAP-on-Azure-Scripts-and-Utilities/tree/master/AvZone-Latency-Test) do procedimento PowerShell que automatiza os testes de latência descritos. 
...
Essa arquitetura de implantação é chamada de ativa/ativa porque você implanta seus servidores de aplicativos SAP ativos em duas ou três zonas. A instância do SAP Central Services que usa a replicação de enfileiramento é implantada entre duas zonas. O mesmo vale para a camada de DBMS, que é implantada nas mesmas zonas que o SAP Central Service.
...
- Não usando [o Azure Proximity Placement Group,](https://docs.microsoft.com/azure/virtual-machines/linux/co-location)você trata as Zonas de Disponibilidade do Azure como falhas e atualiza domínios para todas as VMs porque os conjuntos de disponibilidade não podem ser implantados nas Zonas de Disponibilidade do Azure.
- Se você quiser combinar implantações zonais para a camada DBMS e serviços centrais, mas quiser usar os conjuntos de disponibilidade do Azure para a camada de aplicativos, você precisa usar grupos de proximidade do Azure, conforme descrito no artigo [Azure Proximity Placement Groups, para obter a latência de rede ideal com aplicativos SAP](sap-proximity-placement-scenarios.md).
...
- Para todas as máquinas virtuais implantadas, você precisa usar [discos gerenciados do Azure](https://azure.microsoft.com/services/managed-disks/). Não há suporte para discos não gerenciados em implantações de zona.
...
- A terceira região é usada para hospedar o dispositivo SBD no caso de você criar um [cluster SUSE Linux Pacemaker](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-pacemaker#create-azure-fence-agent-stonith-device) ou instâncias adicionais de aplicativos.
- Para obter consistência de tempo de execução para processos de negócios críticos, você pode tentar direcionar certos trabalhos em lote e usuários para instâncias de aplicativos que estão em zona com a instância DBMS ativa usando grupos de servidores em lote SAP, grupos de logon SAP ou grupos RFC. No entanto, no caso de um failover da zona, seria preciso mover manualmente esses grupos para as instâncias em execução nas VMs da zona com a VM do DB ativo.  
...
> [!IMPORTANT]
> Neste cenário ativo/ativo, as taxas adicionais para largura de banda são anunciadas pela Microsoft a partir de 01/04/2020. Verifique os [detalhes dos preços da largura de banda do documento](https://azure.microsoft.com/pricing/details/bandwidth/). A transferência de dados entre a camada de aplicativo SAP e a camada SAP DBMS é bastante intensa. Portanto, o cenário ativo/ativo pode contribuir bastante para os custos. Continue verificando este artigo para obter os custos exatos 
...
- Não é possível implantar conjuntos de disponibilidade em Zonas de Disponibilidade do Azure. Para compensar isso, você pode usar grupos de colocação de proximidade do Azure, conforme documentado no artigo [Azure Proximity Placement Groups para uma latência de rede ideal com aplicativos SAP](sap-proximity-placement-scenarios.md).
...
- Você está pressupondo que há uma distância significativa entre as instalações que hospedam uma zona de disponibilidade ou é forçado a permanecer em uma determinada região do Azure. Não é possível implantar conjuntos de disponibilidade em Zonas de Disponibilidade do Azure. Para compensar isso, você pode usar grupos de colocação de proximidade do Azure, conforme documentado no artigo [Azure Proximity Placement Groups para uma latência de rede ideal com aplicativos SAP](sap-proximity-placement-scenarios.md).
- Ao usar essa arquitetura, monitore o status de perto e tente manter as instâncias de DBMS e SAP Central Services ativas na mesma zona da sua camada de aplicativo implantada. Caso ocorra um failover do SAP Central Service ou da instância do DBMS, certifique-se de que você possa realizar o failover manual de volta para a zona com a camada de aplicativo SAP implantada o mais rápido possível.
