...
[dbms-guide-5.8]:dbms-guide.md#9053f720-6f3b-4483-904d-15dc54141e30 (Servidor SQL geral para SAP no resumo do Azure)
...
[deployment-guide-3.3]:deployment-guide.md#54a1fc6d-24fd-4feb-9c57-ac588a55dff2 (Cenário 2: Implantação de uma VM com uma imagem personalizada para SAP)
[deployment-guide-3.4]:deployment-guide.md#a9a60133-a763-4de8-8986-ac0fa33aa8c1 (Cenário 3: Mover uma VM do local usando um Azure VHD não generalizado com SAP)
...
[deployment-guide-5.1]:deployment-guide.md#bb61ce92-8c5c-461f-8c53-39f5e5ed91f2 (Verifique a disponibilidade para extensão do Azure para SAP)
[deployment-guide-5.2]:deployment-guide.md#e2d592ff-b4ea-4a53-a91a-e5521edb6cd1 (Verificação de saúde para a extensão do Azure para configuração SAP)
[deployment-guide-5.3]:deployment-guide.md#fe25a7da-4e4e-4388-8907-8abc2d33cfd8 (Solução de problemas extensão do Azure para SAP)
...
[deployment-guide-troubleshooting-chapter]:deployment-guide.md#564adb4f-5c95-4041-9616-6635e83a810b (Verifica e solução de problemas para coleta de dados de ponta a ponta para o Agente host SAP)
...
[planning-guide-11.5]:planning-guide.md#4e165b58-74ca-474f-a7f4-5e695a93204f (Usando o Autostart para instâncias SAP)
...
[planning-guide-3.1]:planning-guide.md#be80d1b9-a463-4845-bd35-f4cebdb5424a (Regiões azure)
[planning-guide-3.2.1]:planning-guide.md#df49dc09-141b-4f34-a4a2-990913b30358 (Domínios de falhas)
...
[planning-guide-3.3.2]:planning-guide.md#ff5ad0f9-f7f4-4022-9102-af07aef3bc92 (Armazenamento Premium Azure)
[planning-guide-5.1.1]:planning-guide.md#4d175f1b-7353-4137-9d2f-817683c26e53 (Movendo uma VM de on-premises para Azure com um disco não generalizado)
...
[planning-guide-5.2.1]:planning-guide.md#1b287330-944b-495d-9ea7-94b83aff73ef (Preparação para mover uma VM de on-premises para Azure com um disco não generalizado)
...
[planning-guide-5.2]:planning-guide.md#6ffb9f41-a292-40bf-9e70-8204448559e7 (Preparando VMs com SAP para Azure)
...
[planning-guide-5.3.2]:planning-guide.md#a43e40e6-1acc-4633-9816-8f095d5a7b6a (Carregando um VHD de on-premises para OZure)
...
[planning-guide-7.1]:planning-guide.md#3e9c3690-da67-421a-bc3f-12c520d99a30 (Cenário de treinamento/demonstração de VM individual com o SAP NetWeaver)
[planning-guide-7]:planning-guide.md#96a77628-a05e-475d-9df3-fb82217e8f14 (Conceitos de implantação somente em nuvem de instâncias SAP)
[planning-guide-9.1]:planning-guide.md#6f0a47f3-a289-4090-a053-2521618a28c3 (Solução de monitoramento do Azure para o SAP)
[planning-guide-azure-premium-storage]:planning-guide.md#ff5ad0f9-f7f4-4022-9102-af07aef3bc92 (Armazenamento Premium Azure)
[planning-guide-managed-disks]:planning-guide.md#c55b2c6e-3ca1-4476-be16-16c81927550f (Discos gerenciados)
...
[virtual-machines-linux-cli-deploy-templates]:../../linux/cli-deploy-templates.md (Implantar e gerenciar máquinas virtuais usando modelos do Azure Resource Manager e da CLI do Azure)
...
No Azure Resource Manager, você pode usar grupos de recursos para gerenciar todos os recursos do aplicativo em sua assinatura do Azure. Para obter mais informações, consulte [a visão geral do Azure Resource Manager][resource-group-overview].
...
1. **Noções básicas:**
   ...
   * **Tipo de disco da VM**: selecione o tipo de disco para o disco do SO. Se você quiser usar o Armazenamento Premium para os discos de dados, é recomendável usar o Armazenamento Premium para o disco do SO também.
...
1. **Configurações:**
   * **Armazenamento**
     * **Tipo de disco**: selecione o tipo de disco como disco do SO. Se você quiser usar o Armazenamento Premium para os discos de dados, é recomendável usar o Armazenamento Premium para o disco do SO também.
     * **Usar Managed Disks**: se você desejar usar Managed Disks, selecione Sim. Para obter mais informações sobre Managed Disks, consulte o capítulo [Managed Disks][planning-guide-managed-disks] no guia de planejamento.
     * **Conta de armazenamento**: selecione uma conta de armazenamento existente ou crie uma nova. Nem todos os tipos de armazenamento funcionam para a execução de aplicativos SAP. Para obter mais informações sobre os tipos de armazenamento, consulte [Estrutura de armazenamento de uma VM para implantações de RDBMS](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/dbms_guide_general#65fa79d6-a85f-47ee-890b-22e794f51a64).
   * **Rede**
   ...
   * **Extensões**: você pode instalar extensões de máquina virtual, adicionando-as à implantação. Você não precisa adicionar extensões nesta etapa. As extensões necessárias para o suporte da SAP são instaladas posteriormente. Ver capítulo [Configure a extensão azure para SAP][deployment-guide-4.5] neste guia.
...
1. **Configurações:**
   ...
   * **Tipo de armazenamento** (modelo de duas camadas somente): o tipo de armazenamento a ser usado.
      ...
      * [Estrutura de armazenamento de uma VM para implantações RDBMS](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/dbms_guide_general#65fa79d6-a85f-47ee-890b-22e794f51a64)
   ...
   * **Sub-rede nova ou existente**: Determina se uma nova rede virtual e sub-rede são criadas ou uma sub-rede existente é usada. Se você já tiver uma rede virtual conectada à sua rede local, selecione **Existindo**.
   * **ID de sub-rede**: Se você quiser implantar a VM em um VNet existente onde você tem uma sub-rede definida para a qual a VM deve ser atribuída, nomeie o ID dessa sub-rede específica. A ID geralmente tem esta aparência: /subscriptions/&lt;id da assinatura>/resourceGroups/&lt;nome do grupo de recursos>/providers/Microsoft.Network/virtualNetworks/&lt;nome de rede virtual> /subnets/&lt;nome da sub-rede>
...
Dependendo de como a rede local estiver configurada, talvez você precise configurar o proxy na VM. Se a sua VM estiver conectada à sua rede local via VPN ou ExpressRoute, a VM pode não ser capaz de acessar a Internet e não poderá baixar as extensões vm necessárias ou coletar informações de infra-estrutura do Azure para o agente Host SAP através da extensão SAP para a Zure. Para obter mais informações, confira [Configurar o proxy][deployment-guide-configure-proxy].
...
Para ter certeza de que o SAP suporta seu ambiente, configure a extensão Azure para SAP conforme descrito em [Configurar a extensão Azure para SAP][deployment-guide-4.5]. Verifique os pré-requisitos para SAP e as versões mínimas necessárias do SAP Kernel e do SAP Host Agent, nos recursos listados nos [recursos SAP][deployment-guide-2.2].

#### <a name="vm-extension-for-sap-check"></a>Extensão vm para verificação SAP

Verifique se a extensão VM para SAP está funcionando, conforme descrito em [Verificações e solução de problemas para coleta de dados de ponta a ponta para o SAP Host Agent][deployment-guide-troubleshooting-chapter].
...
1. **Configurações:**
   ...
   * **Rede**
     ...
     * **Endereço IP público**: selecione o endereço IP público que você deseja usar ou insira os parâmetros para criar um novo endereço IP público. Você pode usar um endereço IP público para acessar a máquina virtual pela Internet. Verifique se você também pode criar um grupo de segurança de rede para ajudar a proteger o acesso à máquina virtual.
     * **Grupo de segurança de rede**: para obter mais informações, confira [Controlar o fluxo de tráfego de rede com grupos de segurança de rede][virtual-networks-nsg].
   * **Extensões**: você pode instalar extensões de máquina virtual, adicionando-as à implantação. Você não precisa adicionar uma extensão nesta etapa. As extensões necessárias para o suporte da SAP são instaladas posteriormente. Ver capítulo [Configure a extensão azure para SAP][deployment-guide-4.5] neste guia.
   * **Alta disponibilidade**: selecione um conjunto de disponibilidade ou insira os parâmetros para criar um novo conjunto de disponibilidade. Para obter mais informações, confira [Conjuntos de disponibilidade do Azure][planning-guide-3.2.3].
   * <bpt i="1000001" x="1000001" type="formatting">{b&gt;</bpt>Monitoramento<ept i="1000001">&lt;b}</ept>
     * **Diagnósticos de inicialização**: você pode selecionar **Desabilitar** para o diagnóstico de inicialização.
     * **Diagnóstico do SO convidado**: você pode selecionar **Desabilitar** para diagnóstico de monitoramento.
...
1. **Configurações:**
   ...
   * **Tipo de armazenamento** (modelo de duas camadas somente): o tipo de armazenamento a ser usado.
      ...
      * [Introdução ao Armazenamento do Microsoft Azure][storage-introduction]
   * **URI do VHD de imagem de usuário** (somente modelo de imagem do disco não gerenciado): o URI do VHD de imagem do SO privado, por exemplo, https://&lt;accountname>.blob.core.windows.net/vhds/userimage.vhd.
   * **Conta de armazenamento de imagem do usuário** (somente modelo de imagem de disco não gerenciado): o nome da conta de armazenamento em que a imagem de sistema operacional privada é armazenada, por exemplo, &lt;accountname> em https://&lt;accountname>.blob.core.windows.net/vhds/userimage.vhd.
   * **userImageId** (somente modelo de imagem de disco gerenciado): ID da imagem de Disco Gerenciado que você deseja usar
   * **Nome de usuário do administrador** e **Senha do administrador**: nome de usuário e senha.

     Um novo usuário é criado para entrar na máquina virtual.
   * **Sub-rede nova ou existente**: determina se uma nova rede virtual e uma sub-rede são criadas ou uma sub-rede existente é usada. Se você já tiver uma rede virtual conectada à sua rede local, selecione **Existindo**.
   * **ID de sub-rede**: Se você quiser implantar a VM em um VNet existente onde você tem uma sub-rede definida para a qual a VM deve ser atribuída, nomeie o ID dessa sub-rede específica. A ID geralmente tem esta aparência: /subscriptions/&lt;id da assinatura>/resourceGroups/&lt;nome do grupo de recursos>/providers/Microsoft.Network/virtualNetworks/&lt;nome de rede virtual> /subnets/&lt;nome da sub-rede>
...
Dependendo de como a rede local estiver configurada, talvez você precise configurar o proxy na VM. Se a Sua VM estiver conectada à sua rede local via VPN ou ExpressRoute, a VM pode não ser capaz de acessar a Internet e não poderá baixar as extensões vm necessárias ou coletar informações de infra-estrutura do Azure para o agente Host SAP através da extensão SAP para Azure, consulte [Configurar o proxy][deployment-guide-configure-proxy].

#### <a name="configure-azure-vm-extension-for-sap"></a>Configure a extensão Azure VM para SAP
...
### <a name="scenario-3-moving-an-on-premises-vm-by-using-a-non-generalized-azure-vhd-with-sap"></a><a name="a9a60133-a763-4de8-8986-ac0fa33aa8c1"></a>Cenário 3: mover uma VM local usando um VHD do Azure não generalizado com o SAP
...
Nesse cenário, o Agente de VM **não** é instalado automaticamente durante a implantação. Como o VM Agent e o Azure Extension for SAP são necessários para executar o SAP NetWeaver no Azure, você precisa baixar, instalar e habilitar ambos os componentes manualmente após criar a máquina virtual.
...
1. **Configurações:**
   ...
   * **Tipo de armazenamento** (modelo de duas camadas somente): o tipo de armazenamento a ser usado.
      ...
      * [Armazenamento premium: armazenamento de alto desempenho para cargas de trabalho da máquina virtual do Azure][storage-premium-storage-preview-portal]
      * [Introdução ao Armazenamento do Microsoft Azure][storage-introduction]
   * **URI do VHD de disco do SO** (somente modelo de disco não gerenciado): o URI do disco do SO particular, por exemplo, https://&lt;accountname>.blob.core.windows.net/vhds/osdisk.vhd.
   * **Id do Disco Gerenciados do disco do SO** (apenas modelo de disco gerenciado): a Id do disco de SO do Disco Gerenciado /subscriptions/92d102f7-81a5-4df7-9877-54987ba97dd9/resourceGroups/group/providers/Microsoft.Compute/disks/WIN
   * **Sub-rede nova ou existente**: determina se uma nova rede virtual e uma sub-rede são criadas ou uma sub-rede existente é usada. Se você já tiver uma rede virtual conectada à sua rede local, selecione **Existindo**.
   * **ID de sub-rede**: Se você quiser implantar a VM em um VNet existente onde você tem uma sub-rede definida para a qual a VM deve ser atribuída, nomeie o ID dessa sub-rede específica. A ID geralmente tem esta aparência: /subscriptions/&lt;id da assinatura>/resourceGroups/&lt;nome do grupo de recursos>/providers/Microsoft.Network/virtualNetworks/&lt;nome de rede virtual> /subnets/&lt;nome da sub-rede>
...
## <a name="update-the-configuration-of-azure-extension-for-sap"></a>Atualize a configuração do Azure Extension para SAP

Atualize a configuração do Azure Extension para SAP em qualquer um dos seguintes cenários:
* A equipe conjunta Microsoft/SAP amplia os recursos da extensão vm e solicita mais ou menos contadores.
* A Microsoft introduz uma nova versão da infra-estrutura subjacente do Azure que fornece os dados, e a Extensão Azure para SAP precisa ser adaptada a essas alterações.
* Você monta discos de dados adicionais para sua VM do Azure ou remove um disco de dados. Nesse cenário, atualize a coleta de dados relacionados ao armazenamento. Alterar sua configuração adicionando ou excluindo pontos finais ou atribuindo endereços IP a uma VM não afeta a configuração de extensão.
...
Para atualizar as configurações, atualize a configuração do Azure Extension para SAP seguindo as etapas em [Configure a extensão Azure para SAP][deployment-guide-4.5].
...
1. Em **Ferramentas de linha de comando**, em **PowerShell**, selecione **Instalar Windows**.
...
1. Para executar o Microsoft Web Platform Installer (Microsoft Web PI), selecione **Sim**.
1. É exibida uma página como esta:

   ![Página de instalação para cmdlets do Azure PowerShell][deployment-guide-figure-500]<a name="figure-5"></a>

1. Selecione **Instalar**e aceite os Termos de Licença de Software da Microsoft.
1. O PowerShell é instalado. Selecione **Concluir** para fechar o assistente de instalação.
...
1. Selecione **Instalar**e aceite os Termos de Licença de Software da Microsoft.
1. A CLI do Azure está instalada. Selecione **Concluir** para fechar o assistente de instalação.
...
1. Para VMs que fazem parte de domínios locais, verifique se as configurações de proxy da Internet eventuais também se aplicam à conta do Sistema Local do Windows (S-1-5-18) na VM, conforme descrito em [Configurar o proxy][deployment-guide-configure-proxy]. O Agente de VM é executado nesse contexto e precisa ser capaz de se conectar ao Azure.
...
1. Selecione**modelos administrativos** > de **configuração** > de computador Do Windows**Components** > **Internet Explorer**. Verifique se a configuração **Definir as configurações de proxy por computador (não por usuário)** está desabilitada ou não definida.
...
Quando você tiver preparado a VM conforme descrito em [Cenários de implantação de VMs para SAP no Azure][deployment-guide-3], o agente estará instalado na máquina virtual. O próximo passo é implantar a extensão Azure para SAP, que está disponível no Azure Extension Repository nos data centers globais do Azure. Para obter mais informações, confira [Planejamento e implementação de Máquinas Virtuais do Azure para SAP NetWeaver][planning-guide-9.1].

Você pode usar o PowerShell ou o Azure CLI para instalar e configurar a extensão Azure para SAP. Para instalar a extensão em uma VM Windows ou Linux usando uma máquina Windows, confira [Azure PowerShell][deployment-guide-4.5.1]. Para instalar a extensão em uma VM Linux usando uma área de trabalho do Linux, confira [CLI do Azure][deployment-guide-4.5.2].
...
Para instalar a extensão Azure para SAP usando o PowerShell:
...
1. Execute o seguinte cmdlet do PowerShell.
    Para obter uma lista dos ambientes disponíveis, execute `commandlet Get-AzEnvironment`. Se quiser usar o Azure global, o ambiente será **AzureCloud**. Para o Azure na China, selecione **AzureChinaCloud**.
...
Para obter `Set-AzVMAEMExtension`mais informações sobre , consulte [Set-AzVMAEMExtension][msdn-set-Azvmaemextension].

![Execução bem-sucedida do Cmdlet Set-AzVMAEMExtension específico do SAP][deployment-guide-figure-900]

A `Set-AzVMAEMExtension` configuração faz todas as etapas para configurar a coleta de dados host para SAP.

A saída do script inclui as seguintes informações:

* A confirmação de que a coleta de dados para o disco do SISTEMA OPERACIONAL e todos os discos de dados adicionais foi configurada.
...
* Uma linha de saída fornece o status da atualização real da extensão VM para configuração SAP.
* Outra linha de saída confirma que a configuração foi implantada ou atualizada.
* A última linha da saída é informativa. Ele mostra suas opções para testar a extensão VM para configuração SAP.
* Para verificar se todas as etapas da extensão Azure VM para configuração SAP foram executadas com sucesso, e se a Infra-estrutura do Azure fornece os dados necessários, proceda com a verificação de prontidão para a extensão Do Azure para SAP, conforme descrito na [verificação de prontidão para extensão Azure para SAP][deployment-guide-5.1].
...
Para instalar a extensão Azure para SAP usando o Azure CLI:
   ...
   1. Habilite a extensão do Azure para SAP:
...
1. Verifique se a extensão do Azure para SAP está ativa no Azure Linux VM. Verifique se o arquivo \\var\\lib\\AzureEnhancedMonitor\\PerfCounters existe. Se existir, em um prompt de comando, execute este comando para exibir as informações coletadas pela Extensão Do Azure para SAP:
...
## <a name="checks-and-troubleshooting-for-end-to-end-data-collection-for-sap-host-agent"></a><a name="564adb4f-5c95-4041-9616-6635e83a810b"></a>Verifica e solução de problemas para coleta de dados de ponta a ponta para o Agente host SAP

Depois de ter implantado sua VM do Azure e configurar a extensão Azure relevante para SAP, verifique se todos os componentes da extensão estão funcionando conforme o esperado.

Execute a verificação de prontidão para a extensão Azure para SAP conforme descrito na [verificação de prontidão para a extensão Azure para SAP][deployment-guide-5.1]. Se todos os resultados da verificação de prontidão forem positivos e todos os contadores de desempenho relevantes aparecerem OK, o Azure Extension for SAP foi configurado com sucesso. Você pode prosseguir com a instalação do Agente de Host SAP descrita nas Notas SAP em [Recursos SAP][deployment-guide-2.2]. Se a verificação de prontidão indicar que faltam contadores, execute a verificação de saúde da extensão Azure para SAP, conforme descrito na [verificação de saúde para a configuração SAP do Azure][deployment-guide-5.2]. Para obter mais opções de solução de problemas, consulte [Troublesazure Extension for SAP][deployment-guide-5.3].

### <a name="readiness-check-for-the-azure-extension-for-sap"></a><a name="bb61ce92-8c5c-461f-8c53-39f5e5ed91f2"></a>Verifique a extensão do Azure para SAP

Esta verificação garante que todas as métricas de desempenho que aparecem dentro do aplicativo SAP sejam fornecidas pela extensão Azure subjacente para SAP.
...
1. No prompt de comando, altere o diretório para a pasta de instalação\\da\\extensão\\Azure para SAP: C: Pacotes Plugins\\&lt;Microsoft.AzureCAT.AzureEnhancedMonitoring.AzureCATExtensionHandler versão>queda de \\>

   A *versão* no caminho para a extensão pode variar. Se você ver pastas para várias versões da extensão na pasta de instalação, verifique a configuração do serviço Windows AzureEnhancedMonitoring e, em seguida, mude para a pasta indicada como *Path para executável*.

   ![Propriedades do serviço executando a extensão do Azure para SAP][deployment-guide-figure-1000]
...
Se a extensão do Azure para SAP não estiver instalada ou o serviço AzureEnhancedMonitoring não estiver sendo executado, a extensão não foi configurada corretamente. Para obter informações detalhadas sobre como implantar a extensão, consulte [Solução de problemas da extensão Do Azure para SAP][deployment-guide-5.3].

> [!NOTE]
> O Azperflib.exe é um componente que não pode ser usado para finalidades próprias. É um componente que fornece exclusivamente dados de infra-estrutura do Azure relacionados à VM para o Agente host SAP.
> 

##### <a name="check-the-output-of-azperflibexe"></a>Verifique a saída de azperflib.exe

A saída de azperflib.exe mostra que todos os contadores de desempenho do Azure para SAP populados. Na parte inferior da lista de contadores coletados, um resumo e um indicador de saúde mostram o status da Extensão Azure para SAP.
...
| Valores resultantes de Azperflib.exe | Extensão do Azure para o estado de saúde SAP |
| --- | --- |
...
| **Estado de saúde** |Só OK se o status de retorno aparecer **OK**. |
| **Diagnostics** |Informações detalhadas sobre o status da integridade. |

Se o valor **do estado de Saúde** não estiver **OK,** siga as instruções em [Health check for Azure Extension for SAP configuration][deployment-guide-5.2].
...
1. Certifique-se de que a extensão Azure para SAP esteja instalada e em execução.

   a.  Execute `sudo sh -c 'ls -al /var/lib/waagent/Microsoft.OSTCExtensions.AzureEnhancedMonitorForLinux-*/'`

   **Resultado esperado**: Lista o conteúdo da extensão Azure para diretório SAP.
...
Se alguma dessas verificações falhar e para obter informações detalhadas sobre como reimplantar a extensão, consulte [Solução de problemas da extensão Do Azure para SAP][deployment-guide-5.3].

### <a name="health-check-for-the-azure-extension-for-sap-configuration"></a><a name="e2d592ff-b4ea-4a53-a91a-e5521edb6cd1"></a>Verificação de saúde para a extensão do Azure para configuração SAP

Se alguns dos dados de infra-estrutura não forem entregues corretamente conforme indicado pelo teste descrito `Test-AzVMAEMExtension` no [teste Readiness check for Azure Extension for SAP][deployment-guide-5.1], execute o cmdlet para verificar se a infra-estrutura do Azure e a extensão Do Azure para SAP estão configuradas corretamente.
...
1. Execute o seguinte cmdlet do PowerShell. Para obter a lista de ambientes disponíveis, execute o cmdlet `Get-AzEnvironment`. Para usar o Azure global, selecione o ambiente **AzureCloud**. Para o Azure na China, selecione **AzureChinaCloud**.
...
1. O script testa a configuração da máquina virtual que você seleciona.
   ...
   ![Saída do teste bem sucedido da extensão Azure para SAP][deployment-guide-figure-1300]

Verifique se cada resultado da verificação de integridade é **OK**. Se algumas verificações não exibirem **OK,** execute o cmdlet de atualização conforme descrito em [Configure a extensão Azure para SAP][deployment-guide-4.5]. Aguarde 15 minutos e repita as verificações descritas na [verificação de prontidão para a extensão Do Azure para verificação SAP][deployment-guide-5.1] e [Health para a configuração Azure Extension for SAP][deployment-guide-5.2]. Se as verificações ainda indicarem um problema com alguns ou todos os contadores, consulte [Solução de problemas da extensão do Azure para SAP][deployment-guide-5.3].

> [!Note]
> Você pode encontrar alguns avisos em casos em que usar Discos do Azure Standard Gerenciados. Os avisos serão exibidos em vez dos testes retornando "OK". Isso é normal e intencional no caso desse tipo de disco. Veja também [a solução de problemas da extensão do Azure para SAP][deployment-guide-5.3]
> 

### <a name="troubleshooting-azure-extension-for-sap"></a><a name="fe25a7da-4e4e-4388-8907-8abc2d33cfd8"></a>Solução de problemas extensão do Azure para SAP
...
##### <a name="the-installation-directory-of-the-azure-extension-for-sap-is-empty"></a>O diretório de instalação do Azure Extension for SAP está vazio
...
##### <a name="service-for-azure-extension-for-sap-does-not-exist"></a>Serviço para extensão Do Azure para SAP não existe
...
![Execução de azperflib.exe indica que o serviço da Extensão Azure para SAP não está sendo executado][deployment-guide-figure-1400]
<a name="figure-14"></a>

###### <a name="solution"></a>Solução

Se o serviço não existir, a extensão Azure para SAP não foi instalada corretamente. Reimplante a extensão usando as etapas descritas para seu cenário de implantação em [Cenários de implantação de VMs para SAP no Azure][deployment-guide-3].
...
##### <a name="service-for-azure-extension-for-sap-exists-but-fails-to-start"></a>Serviço para extensão Azure para SAP existe, mas não consegue iniciar
...
A configuração está incorreta. Reinicie a extensão Azure para SAP no VM, conforme descrito em [Configurar a extensão Azure para SAP][deployment-guide-4.5].
...
##### <a name="the-execution-of-set-azvmaemextension-and-test-azvmaemextension-show-warning-messages-stating-that-standard-managed-disks-are-not-supported"></a>A execução do Set-AzVMAEMExtension e do Test-AzVMAEMExtension mostram mensagens de aviso informando que os discos gerenciados padrão não são suportados

###### <a name="issue"></a>Problema

Ao executar as mensagens Set-AzVMAEMExtension ou Test-AzVMAEMExtensão como estas são mostradas:
...
As mensagens são causadas pelo fato de que os Discos Gerenciados Padrão não estão fornecendo as APIs usadas pela Extensão SAP para SAP para verificar estatísticas das Contas de Armazenamento Padrão do Azure. Isso não é motivo de preocupação. A razão para introduzir os dados de coleta de contas de armazenamento de disco padrão foi o estrangulamento de I/Os que ocorria com freqüência. Os discos gerenciados evitarão essa limitação limitando o número de discos em uma conta de armazenamento. Portanto, não ter esse tipo de dados não é crítico.
...
Para obter uma lista completa e atualizada de problemas conhecidos, consulte SAP Note [1999351], que tem informações adicionais de solução de problemas para a Extensão Azure para SAP.

Se a solução de problemas usando o SAP Note [1999351] não resolver o problema, execute o `Set-AzVMAEMExtension` script de configuração conforme descrito em [Configure a extensão do Azure para SAP][deployment-guide-4.5]. Talvez você precise esperar por uma hora porque os contadores de diagnóstico ou análise de armazenamento podem não ser criados imediatamente depois de terem sido habilitados. Se o problema persistir, abra uma mensagem de suporte ao cliente SAP no componente BC-OP-NT-AZR para Windows ou BC-OP-LNX-AZR para uma máquina virtual Linux.
