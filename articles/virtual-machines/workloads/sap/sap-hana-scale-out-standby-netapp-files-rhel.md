---
title: Escala SAP HANA com standby com arquivos Do Azure NetApp no RHEL| Microsoft Docs
description: Guia de alta disponibilidade para SAP NetWeaver no Red Hat Enterprise Linux com arquivos Azure NetApp para aplicativos SAP
...
ms.locfileid: "80754157"
---
# <a name="deploy-a-sap-hana-scale-out-system-with-standby-node-on-azure-vms-by-using-azure-netapp-files-on-red-hat-enterprise-linux"></a>Implantar um sistema de scale-out SAP HANA com nó de espera em VMs azure usando arquivos Azure NetApp no Red Hat Enterprise Linux 
...
Este artigo descreve como implantar um sistema SAP HANA altamente disponível em uma configuração de escala com espera em máquinas virtuais (VMs) do Azure Red Hat Enterprise, usando [o Azure NetApp Files](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-introduction/) para os volumes de armazenamento compartilhados.  

Na configuração de exemplo, comandos de instalação, e assim por diante, a ocorrência HANA é **03** e o ID do sistema HANA é **HN1**. Os exemplos são baseados no HANA 2.0 SP4 e no Red Hat Enterprise Linux para SAP 7.6. 
...
* SAP Note [2002167] recomendou configurações do SO para Red Hat Enterprise Linux
...
* Documentação geral do RHEL
  ...
  * [Guia de Rede Linux Enterprise Red Hat](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/networking_guide)
...
Para esta configuração de exemplo, as sub-redes são:  
  ...
  - `storage`10.9.3.0/26  
  - `hana`10.9.2.0/26  
  - `anf`10.9.0.0/26 (sub-rede delegada para arquivos do Azure NetApp)
...
5. Implantar volumes de arquivos do Azure NetApp seguindo as instruções em [Criar um volume NFS para arquivos do Azure NetApp](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-create-volumes).  
   ...
   À medida que você estiver implantando os volumes, certifique-se de selecionar a versão **NFSv4.1.** Implante os volumes na [sub-rede](https://docs.microsoft.com/rest/api/virtualnetwork/subnets)azure NetApp Files . 
   ...
   * volume **HN1**-log-mnt0002 (nfs://10.9.0.4/**HN1**-log-mnt00002)
   * volume **HN1**compartilhado (nfs://10.9.0.4/**HN1**-compartilhado)
   
   Neste exemplo, usamos um volume separado de Arquivos Do Azure NetApp para cada dados HANA e volume de log. Para uma configuração mais otimizada de custo em sistemas menores ou não produtivos, é possível colocar todas as montagens de dados em um único volume e todos os logs montados em um volume único diferente.  

### <a name="important-considerations"></a>Considerações importantes

À medida que você está criando seus Arquivos Azure NetApp para escala SAP HANA com cenário de nó stand by, esteja ciente das seguintes considerações importantes:
...
- Ler-escrever em /hana/log de 250 megabytes por segundo (MB/s) com tamanhos de I/O de 1 MB.  
- Leia a atividade de pelo menos 400 MB/s para tamanhos de /hana/data para tamanhos de I/O de 16 MB e 64 MB.  
- Atividade de gravação de pelo menos 250 MB/s para tamanhos de I/O de 16 MB e 64 MB. 
...
| /hana/shared | 1xRAM por 4 nódulos de trabalhadores | 1xRAM por 4 nódulos de trabalhadores | v3 ou v4.1 |
...
2. Crie três máquinas virtuais **(hanadb1,** **hanadb2,** **hanadb3)** fazendo as seguintes etapas:  
   ...
   a. Use uma imagem do Red Hat Enterprise Linux na galeria Azure suportada pelo SAP HANA. Usamos uma imagem RHEL-SAP-HA 7.6 neste exemplo.  
...
3. **[A]** Adicione uma rota de rede, de modo que a comunicação aos Arquivos Do Azure NetApp vá através da interface da rede de armazenamento.  

   Neste exemplo será `Networkmanager` usado para configurar a rota de rede adicional. As instruções a seguir assumem que a interface da rede de armazenamento é `eth1`.  
   Primeiro, determine o nome `eth1`da conexão para dispositivo . Neste exemplo, o nome `eth1` `Wired connection 1`de conexão para dispositivo é .  
   ...
   Em seguida, configure a rota adicional para a `eth1`rede delegada Azure NetApp Files via .  
...
3. **[A] Instale** o pacote cliente NFS.  

    <pre><code>
    yum install nfs-utils
    </code></pre>

3. **[A]** Prepare o SISTEMA OPERACIONAL para executar o SAP HANA no Azure NetApp com NFS, conforme descrito nos [aplicativos SAP do NetApp no Microsoft Azure usando arquivos do Azure NetApp][anf-sap-applications-azure]. Crie arquivo de configuração */etc/sysctl.d/netapp-hana.conf* para as configurações do NetApp.  
...
4. **[A]** Criar arquivo de configuração */etc/sysctl.d/ms-az.conf* com configurações adicionais de otimização.  
...
5. **[A]** Ajuste as configurações do sunrpc, conforme recomendado nos aplicativos SAP do [NetApp no Microsoft Azure usando arquivos Do Azure NetApp][anf-sap-applications-azure].  
...
6. **[A]** Chapéu Vermelho para configuração HANA.
    ...
    > [!NOTE]
    > Se instalar o HANA 2.0 SP04, `compat-sap-c++-7` será necessário instalar o pacote conforme descrito na nota SAP [2593824], antes de poder instalar o SAP HANA. 
...
5. **[A]** Desative o firewall temporariamente, para que ele não interfira na instalação do HANA. Você pode reatiminá-lo, após a instalação do HANA ser concluída. 
   <pre><code>
    # Execute as root
...
5. **[A]** Rehabilite o firewall.  
   ...
   - Abra as portas de firewall necessárias
       ...
       > [!IMPORTANT]
       > Crie regras de firewall para permitir a comunicação entre nó hana e tráfego de clientes. As portas necessárias estão listadas em [Portas TCP / IP de todos os produtos SAP](https://help.sap.com/viewer/ports). Os seguintes comandos são apenas um exemplo. Neste cenário com sistema utilizado número 03.
