---
title: Implantar bocal de análise de log do Azure para monitoramento de fundição em nuvem
...
O Bocal de Análise de Log (o Bocal) é um componente CF (Cloud Foundry, fundição em nuvem), que encaminha métricas da mangueira de incêndio do [agregador de fundição de nuvem](https://docs.cloudfoundry.org/loggregator/architecture.html) para logs do Monitor do Azure. Com o Bocal, é possível coletar, exibir e analisar a integridade do sistema e as métricas de desempenho do CF em várias implantações.

Neste documento, você aprende como implantar o Bocal no seu ambiente CF e, em seguida, acessar os dados do console de logs do Monitor do Azure.
...
### <a name="1-deploy-a-cf-or-pivotal-cloud-foundry-environment-in-azure"></a>1. Implante um ambiente cf ou de fundição de nuvem pivô no Azure
...
Antes de configurar o cliente uaa command-line, certifique-se de que o RubyGems esteja instalado.

### <a name="3-create-a-log-analytics-workspace-in-azure"></a>3. Crie um espaço de trabalho do Log Analytics no Azure

É possível criar o espaço de trabalho do Log Analytics manualmente ou usando um modelo. O modelo implantará uma configuração de visualizações e alertas de KPI pré-configurados para o console de logs do Monitor Do Azure. 

#### <a name="to-create-the-workspace-manually"></a>Para criar o workspace manualmente:

1. No portal Azure, pesquise a lista de serviços no Azure Marketplace e selecione os espaços de trabalho do Log Analytics.
...
Para obter mais informações, consulte [Comece com os registros do Monitor Do Azure](https://docs.microsoft.com/azure/log-analytics/log-analytics-get-started).
...
Se você tiver implantado a solução de monitoramento por meio do modelo do Marketplace, acesse o Portal do Azure e encontre a solução. Você pode encontrar a solução no grupo de recursos especificado no modelo. Clique na solução, navegue até o "console de análise de log", as visualizações pré-configuradas são listadas, com KPIs do sistema de fundição em nuvem, dados de aplicativos, alertas e métricas de saúde vm. 
...
### <a name="1-import-the-oms-view"></a>1. Importe a visão OMS

No portal OMS, navegue para exibir a**navegação****de importação** >  **de designers** > e selecione um dos arquivos omsview. Por exemplo, selecione *Cloud Foundry.omsview* e salve a exibição. Agora um bloco é exibido na página **Visão Geral** do OMS. Selecione-a para ver as métricas visualizadas.
...
### <a name="2-create-alert-rules"></a>2. Crie regras de alerta
...
| Type=CF_ValueMetric_CL Name_s=slowConsumerAlert                               | Número de resultados > 0   | Quando o Bocante recebe um alerta lento do loggregator, ele envia o **lentoConsumerAlert** ValueMetric para logs do Monitor Do Azure. |
...
2. Localize o bocal de **análise do Log Do Microsoft Azure para** ladrilho PCF.
...
Se você remover o Bocal, os dados no Portal do OMS não serão removidos automaticamente. Ele expira com base na configuração de retenção de logs do Monitor Do Azure.
...
A partir do PCF2.0, as métricas de desempenho da VM são transferidas para o bocal do Azure Log Analytics pelo System Metrics Forwarder e integradas ao espaço de trabalho do Log Analytics. Você não precisa mais do agente do Log Analytics para as métricas de desempenho da VM. No entanto ainda pode usar o agente do Log Analytics para coletar informações de Syslog. O agente do Log Analytics é instalado como um complemento Bosh nas VMs do seu CF. 
