...
> [!NOTE]
> O aplicativo que envia as solicitações iniciais deve saber sobre a ID da sessão e `SessionClient.AcceptMessageSession(SessionID)` usar para bloquear a sessão na qual ela está esperando a resposta. É uma boa ideia usar um GUID que identifique exclusivamente a instância do aplicativo como uma ID de sessão. Não deve haver nenhum manipulador de sessão `AcceptMessageSession(timeout)` ou na fila para garantir que as respostas estejam disponíveis para serem bloqueadas e processadas por destinatários específicos.
