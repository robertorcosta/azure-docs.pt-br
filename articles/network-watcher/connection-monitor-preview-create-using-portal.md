...
description: Este artigo descreve como criar um monitor na visualização do monitor de conexão usando o portal do Azure.
...
ms.locfileid: "91948554"
---
# <a name="create-a-monitor-in-connection-monitor-preview-by-using-the-azure-portal"></a>Criar um monitor na visualização do monitor de conexão usando o portal do Azure

Saiba como usar a visualização do monitor de conexão para monitorar a comunicação entre seus recursos. Este artigo descreve como criar um monitor usando o portal do Azure. O monitor de conexão dá suporte a implantações de nuvem híbridas e do Azure.

## <a name="before-you-begin"></a>Antes de começar 

Nos monitores de conexão que você cria usando a visualização do monitor de conexão, você pode adicionar máquinas locais e VMs do Azure como fontes. Esses monitores de conexão também podem monitorar a conectividade com pontos de extremidade. Os pontos de extremidade podem estar no Azure ou em qualquer outra URL ou IP.
...
Para criar um monitor na visualização do monitor de conexão usando o portal do Azure:

1. Na home page portal do Azure, vá para **observador de rede**.
1. No painel esquerdo, na seção **monitoramento** , selecione **Monitor de conexão (versão prévia)**.

   Você verá todos os monitores de conexão que foram criados na visualização do monitor de conexão. Para ver os monitores de conexão que foram criados no monitor de conexão clássica, vá para a guia **Monitor de conexão** .

   :::image type="content" source="./media/connection-monitor-2-preview/cm-resource-view.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
...
1. Na guia **noções básicas** , insira informações para o monitor de conexão: 
   ...
   * **Configuração do espaço de trabalho**: escolha um espaço de trabalho personalizado ou o espaço de trabalho padrão. Seu espaço de trabalho mantém os dados de monitoramento.
   ...
   :::image type="content" source="./media/connection-monitor-2-preview/create-cm-basics.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
...
1. Adicione fontes, destinos e configurações de teste em seus grupos de teste. Para saber mais sobre como configurar seus grupos de teste, confira [criar grupos de teste no monitor de conexão](#create-test-groups-in-a-connection-monitor). 
   ...
   :::image type="content" source="./media/connection-monitor-2-preview/create-tg.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
...
1. Na parte inferior da guia, selecione **Avançar: criar alertas**. Para saber mais sobre a criação de alertas, consulte [criar alertas no monitor de conexão](#create-alerts-in-connection-monitor).
   ...
   :::image type="content" source="./media/connection-monitor-2-preview/create-alert.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
...
1. Na guia **revisar + criar** , examine as informações básicas e os grupos de teste antes de criar o monitor de conexão. Se você precisar editar o monitor de conexão, poderá fazer isso voltando para as respectivas guias. 
   :::image type="content" source="./media/connection-monitor-2-preview/review-create-cm.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
   > [!NOTE] 
   > A guia **revisar + criar** mostra o custo por mês durante o estágio de visualização do monitor de conexão. Atualmente, a coluna **custo/mês atual** não mostra nenhum encargo. Quando o monitor de conexão se tornar disponível de forma geral, essa coluna mostrará um encargo mensal. 
   > 
   > Mesmo durante o estágio de visualização do monitor de conexão, Log Analytics se aplicam encargos de ingestão.

1. Quando você estiver pronto para criar o monitor de conexão, na parte inferior da guia **revisar + criar** , selecione **criar**.

A visualização do monitor de conexão cria o recurso de monitor de conexão em segundo plano.
...
* **Fontes**: você pode especificar VMs do Azure e máquinas locais como fontes se os agentes estiverem instalados neles. Para saber mais sobre como instalar um agente para sua fonte, consulte [instalar agentes de monitoramento](https://docs.microsoft.com/azure/network-watcher/connection-monitor-preview#install-monitoring-agents).
   * Para escolher os agentes do Azure, selecione a guia **pontos de extremidade do Azure** . Aqui você vê somente as VMs que estão associadas à região que você especificou quando criou o monitor de conexão. Por padrão, as VMs são agrupadas na assinatura à qual pertencem. Esses grupos são recolhidos. 
      ...
      **Assinatura**  >  do **Grupo**  >  de recursos **VNET**  >  **Sub-rede**  >  **VMs com agentes**
      ...
      :::image type="content" source="./media/connection-monitor-2-preview/add-azure-sources.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
   ...
   * Para escolher os agentes locais, selecione a guia **não-pontos de extremidade do Azure** . Por padrão, os agentes são agrupados em espaços de trabalho por região. Todos esses espaços de trabalho têm o Monitor de Desempenho de Rede configurado. 
      ...
      :::image type="content" source="./media/connection-monitor-2-preview/add-non-azure-sources.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
...
* **Destinos**: você pode monitorar a conectividade com uma VM do Azure, um computador local ou qualquer ponto de extremidade (um IP público, uma URL ou um FQDN) especificando-o como um destino. Em um único grupo de teste, você pode adicionar VMs do Azure, máquinas locais, URLs do Office 365, URLs do Dynamics 365 e pontos de extremidade personalizados.
    ...
    * Para escolher VMs do Azure como destinos, selecione a guia **pontos de extremidade do Azure** . Por padrão, as VMs do Azure são agrupadas em uma hierarquia de assinatura que está na região que você selecionou em **criar monitor de conexão** na guia **noções básicas** . Você pode alterar a região e escolher VMs do Azure na nova região. Em seguida, você pode fazer uma busca detalhada do nível de **assinatura** para outros níveis na hierarquia, assim como é possível quando você define os pontos de extremidade do Azure de origem.
      ...
      :::image type="content" source="./media/connection-monitor-2-preview/add-azure-dests1.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::

      :::image type="content" source="./media/connection-monitor-2-preview/add-azure-dests2.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
    ...
    * Para escolher os agentes não Azure como destinos, selecione a guia **pontos de extremidade não Azure** . Por padrão, os agentes são agrupados em espaços de trabalho por região. Todos esses espaços de trabalho têm Monitor de Desempenho de Rede configurados. 
      ...
      :::image type="content" source="./media/connection-monitor-2-preview/add-non-azure-dest.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
    ...
    * Para escolher pontos de extremidade públicos como destinos, selecione a guia **endereços externos** . A lista de pontos de extremidade inclui URLs de teste do Office 365 e URLs de teste do Dynamics 365, agrupadas por nome. Você também pode escolher os pontos de extremidade que foram criados em outros grupos de teste no mesmo monitor de conexão. 
       ...
       :::image type="content" source="./media/connection-monitor-2-preview/add-endpoints.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
...
* **Configurações de teste**: você pode adicionar uma ou mais configurações de teste a um grupo de teste. Crie uma nova configuração de teste usando a **nova** guia de configuração. Ou adicione uma configuração de teste de outro grupo de teste no mesmo monitor de conexão da guia **escolher existente** .
   ...
   :::image type="content" source="./media/connection-monitor-2-preview/add-test-config.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
...
:::image type="content" source="./media/connection-monitor-2-preview/create-alert-filled.png" alt-text="Diagrama que mostra um monitor de conexão e define a relação entre os grupos de teste e os testes.":::
