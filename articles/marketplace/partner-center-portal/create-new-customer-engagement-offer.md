---
title: Crie uma nova Dinâmica 365 para engajamento do cliente & oferta de PowerApps no Mercado Comercial
description: Como criar um novo Dynamics 365 para engajamento do cliente & powerApps oferecem para listagem ou venda no Azure Marketplace, AppSource ou através do programa Cloud Solution Provider (CSP) usando o portal Commercial Marketplace no Microsoft Partner Center.
author: dsindona
ms.author: dsindona
...
ms.locfileid: "81869818"
---
# <a name="create-a-new-dynamics-365-for-customer-engagement--powerapps-offer"></a>Crie uma nova dinâmica 365 para o engajamento do cliente & oferta de PowerApps

Este tópico explica como criar uma nova dinâmica 365 para o engajamento do cliente & oferta de PowerApps. Todos os aplicativos para Dynamics 365 para Engajamento do Cliente (PowerApps, Vendas, Serviço, Serviço de Projeto e Serviço de Campo) devem passar por nosso processo de certificação e suportar uma experiência de teste. O processo de certificação verifica sua solução quanto aos requisitos padrão, compatibilidade e práticas adequadas. A experiência de avaliação permite aos usuários implantar sua solução em um ambiente ativo do Dynamics 365.

Para começar a criar o Dynamics 365 para engajamento do cliente & ofertas de PowerApps, certifique-se de criar primeiro [uma conta do Partner Center](./create-account.md) e abrir o painel do Mercado [Comercial,](https://partner.microsoft.com/dashboard/commercial-marketplace/offers)com a guia **Visão Geral** selecionada.
...
Selecione um mínimo de uma e no máximo três categorias. A **Categoria** será usada para colocar sua oferta nas áreas de pesquisa de mercado apropriadas. Certifique-se de chamar como sua oferta suporta essas categorias na descrição da oferta.
...
Selecione todos os produtos Dynamics 365 aos os que esta oferta se aplica.

### <a name="app-version"></a>Versão do aplicativo

Digite o número da versão da sua oferta. Os clientes verão esta versão listada na página de detalhes da oferta. Se você estiver apenas atualizando o número da versão devido a alterações de marketing/descritivos, verifique a caixa **de alteração somente marketing.** Esta opção permite que a oferta contorne as etapas de certificação e provisionamento.
...
Você deve fornecer pelo menos um (e até três) documentos de marketing relacionados aqui, como white papers, folhetos, checklists ou apresentações. Esses documentos devem estar no formato .pdf.
...
Você pode fornecer o logotipo da sua oferta em três tamanhos: **Pequeno (48 x 48)**, **Grande (216 x 216)** e **Largo (255 x 115)**. Os tamanhos Pequeno e Grande são necessários.
...
> [!IMPORTANT]
> É sua responsabilidade atender a quaisquer requisitos legais locais, mesmo que esses requisitos não estejam listados aqui ou no Partner Center. Mesmo que você selecione todos os mercados, leis locais e restrições ou outros fatores podem impedir que certas ofertas sejam listadas em alguns países e regiões.
...
O modelo de licença base determina como os clientes são designados para o seu aplicativo no CRM Admin Center. Selecione **Recurso** para licenciamento baseado em exemplo ou **Usuário** se as licenças forem atribuídas a um por inquilino.

### <a name="requires-s2s-outbound-and-crm-secure-store-access"></a>Requer acesso de saída s2S e crm secure store

Verifique esta caixa para habilitar a configuração do acesso de saída do CRM Secure Store ou do Server-to-Server (S2S). Este recurso exige uma consideração especializada da equipe do Dynamics 365 durante a fase de certificação. Microsoft entrará em contato com você para concluir as etapas adicionais para dar suporte ao recurso.

### <a name="application-configuration-url"></a>URL de configuração de aplicativo

Forneça a URL para a página web de configuração que permite ao cliente configurar seu aplicativo.

### <a name="crm-package"></a>Pacote de CRM

Na URL do campo **de localização do pacote,** digite a URL de uma conta do Azure Storage que contém o arquivo .zip do pacote DE CRM carregado. Esta URL deve incluir uma chave SAS somente leitura para permitir que a Microsoft pegue seu pacote para verificação.

Verifique a caixa rotulada **Há mais de um pacote de CRM no meu arquivo de pacote,** se aplicável. Nesse caso, certifique-se de incluir todos os pacotes em seu arquivo .zip.

### <a name="crm-package-availability"></a>Disponibilidade do pacote de CRM

Nesta seção, selecione **+ Adicionar região** para especificar as regiões geográficas em que seu pacote de CRM estará disponível para os clientes. A implantação nas seguintes regiões soberanas requer permissão e validação especiais durante o processo de certificação: [Alemanha](https://docs.microsoft.com/azure/germany/), [Us Government Cloud](https://docs.microsoft.com/azure/azure-government/documentation-government-welcome)e TIP.

Por padrão, o **URL de configuração do aplicativo** que você inseriu acima será usado para cada região. Se preferir, você pode inserir uma URL de configuração de aplicativo separada para uma ou mais regiões específicas. 
...
- **Nome da função** (obrigatório): Forneça o nome da função de segurança que você definiu em seu test drive dynamics 365 personalizado. Esse nome da função de segurança será atribuído ao usuário durante o test drive (por exemplo, test-drive-role).
...
Para implantar o Test Drive em seu nome, crie e forneça uma assinatura Azure separada e exclusiva (não necessária para unidades de teste power bi).
...
É necessário carregar um arquivo .pdf que lista os principais cenários de uso da sua oferta listados em um documento (formato .pdf). Todos os cenários listados aqui podem ser verificados por nossa equipe de validação antes de aprovarmos sua oferta para o mercado.
...
- Envie a oferta para publicação selecionando **Enviar**. Enviaremos um e-mail para que você saiba quando uma versão de pré-visualização da oferta estiver disponível para você revisar e aprovar. Retorne ao Partner Center e selecione **Go-live** para a oferta de publicação ao público (ou se uma oferta privada, para o público privado).
