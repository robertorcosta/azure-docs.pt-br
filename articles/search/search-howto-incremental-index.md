...
Usando um cliente de API, construa uma [solicitação obter indexador](https://docs.microsoft.com/rest/api/searchservice/get-indexer) para obter a configuração atual do indexador. Quando você usa a versão da API de visualização para obter o indexador, `cache` uma propriedade definida como NULL é adicionada às definições.
...
Por padrão, `cache` a propriedade é nula. Use um cliente de API para definir a configuração de cache (o portal não oferece suporte a essa atualização Particulate). 
...
+ A `enableReprocessing` Propriedade booliana é opcional`true` (por padrão) e indica que o enriquecimento incremental está habilitado. Quando necessário, você pode defini-lo `false` como para suspender o processamento incremental enquanto outras operações com uso intensivo de recursos, como a indexação de novos documentos, estão em `true` andamento e, em seguida, invertendo-os novamente para mais tarde.
...
Como alternativa, você pode usar REST para [executar o indexador](https://docs.microsoft.com/rest/api/searchservice/run-indexer):
...
Para modificar um qualificable, você pode usar o portal ou a API. Por exemplo, se você estiver usando a conversão de texto, uma simples alteração `en` embutida de para `es` ou outra linguagem será suficiente para testes de prova de conceito de enriquecimento incremental.
...
Para configurar o enriquecimento incremental para um novo indexador, tudo o que você precisa fazer é incluir a `cache` Propriedade na carga de definição do indexador ao chamar [Create indexer (2019-05-06-Preview)](https://docs.microsoft.com/rest/api/searchservice/2019-05-06-preview/create-indexer). 
...
|---------------|------------------|
| [Criar indexador (2019-05-06-Preview)](https://docs.microsoft.com/rest/api/searchservice/2019-05-06-preview/create-indexer) | Cria e executa um indexador no primeiro uso, incluindo a criação de um cache se a definição do indexador o especificar. |
