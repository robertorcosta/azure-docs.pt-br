...
description: Aumente as pontuações de classificação de pesquisa para os resultados da Pesquisa Cognitiva do Azure adicionando perfis de pontuação.
manager: nitinme
author: Brjohnstmsft
ms.author: brjohnst
...
# <a name="add-scoring-profiles-to-an-azure-cognitive-search-index"></a>Adicionar perfis de pontuação a um índice da Azure Cognitive Search
 ...
 O Azure Cognitive Search usa a pontuação padrão para calcular uma pontuação inicial, mas você pode personalizar o cálculo através de um *perfil de pontuação*. Os perfis de pontuação oferecem maior controle sobre a classificação de itens nos resultados da pesquisa. Por exemplo, convém aumentar itens com base em seu potencial de receita, promover itens mais recentes ou talvez aumentar itens que estejam no inventário há muito tempo.  
...
```  
 ...
 Essa consulta pesquisa o termo 'estalagem' e passa o local atual. Observe que esta consulta inclui outros `scoringParameter`parâmetros, tais como . Os parâmetros de consulta são descritos nos [documentos de pesquisa &#40;a Azure Cognitive Search Rest API&#41;](https://docs.microsoft.com/rest/api/searchservice/Search-Documents).  
...
A pontuação de pesquisa é calculada com base nas propriedades estatísticas dos dados e da consulta. A Pesquisa Cognitiva do Azure encontra documentos que incluem os termos de `searchMode`pesquisa na seqüência de consultas (alguns ou todos, dependendo), favorecendo documentos que contêm muitas instâncias do termo de pesquisa. O score de pesquisa sobe ainda mais se o termo for raro em todo o índice de dados, mas comum dentro do documento. A base para essa abordagem da relevância da computação é conhecida como [TF-IDF](https://en.wikipedia.org/wiki/Tf%E2%80%93idf) ou freqüência de documento inversa de freqüência de termo.  
...
|`weights`|Opcional. Contém pares de valor de nome que cada um especifica um nome de campo e peso relativo. O peso relativo deve ser um inteiro positivo ou o número de ponto flutuante.<br /><br /> Os pesos são usados para indicar a importância de um campo pesquisável em relação ao outro.|  
...
|`magnitude`|A função de pontuação magnitude é usada para alterar a classificação com base no intervalo de valores de um campo numérico. Alguns dos exemplos de uso mais comuns disso são:<br /><br /> -   **Classificações de estrelas:** Altere a pontuação com base no valor dentro do campo "Star Rating". Quando dois itens forem relevantes, o item com a classificação mais alta será exibido primeiro.<br />-   **Margem:** Quando dois documentos são relevantes, um varejista pode querer impulsionar documentos com margens mais altas primeiro.<br />-   **Clique na contagem de contagens:** Para aplicativos que rastreiam clique através de ações para produtos ou páginas, você pode usar a magnitude para impulsionar itens que tendem a obter mais tráfego.<br />-   **Download conta:** Para aplicativos que rastreiam downloads, a função magnitude permite que você impulsione itens que têm mais downloads.|  
...
|`functionAggregation`|Opcional. Aplicável apenas quando funções são especificadas. Os valores válidos incluem: soma (padrão), média, mínimo, máximo e firstMatching. Uma pontuação de pesquisa é um valor único calculado por meio de diversas variáveis, incluindo várias funções. Esse atributo indica como os aumentos de todas as funções são combinados em um único aumento agregado que é aplicado à pontuação do documento base. A pontuação base é baseada no valor [tf-idf](http://www.tfidf.com/) calculado a partir do documento e da consulta de pesquisa.|  
|`defaultScoringProfile`|Ao executar uma solicitação de pesquisa, se nenhum perfil de pontuação for especificado, então a pontuação padrão será usada (somente[tf-idf).](http://www.tfidf.com/)<br /><br /> Um nome de perfil de pontuação padrão pode ser definido aqui, fazendo com que o Azure Cognitive Search use esse perfil quando nenhum perfil específico é dado na solicitação de pesquisa.|  
...
##  <a name="set-boostingduration"></a><a name="bkmk_boostdur"></a>Definir boostingDuração  
