...
Para [Filtrar](query-odata-filter-orderby-syntax.md) os campos de coleção no Azure pesquisa cognitiva, você pode usar [ `any` os `all` operadores e](search-query-odata-collection-operators.md) junto com **expressões lambda**. Uma expressão lambda é um subfiltro que é aplicado a cada elemento de uma coleção.
...
> [!NOTE]
> `lt` / O `le` / Azure `gt` não dá suporte aos operadores para cadeias de caracteres, dentro ou fora de uma expressão lambda. / `ge`
...
Os valores do `Edm.GeographyPoint` tipo em uma coleção não podem ser comparados diretamente entre si. Em vez disso, eles devem ser usados como parâmetros `geo.distance` para `geo.intersects` as funções e. A `geo.distance` função, por sua vez, deve ser comparada a um valor de distância `lt`usando `le`um `gt`dos operadores `ge`de comparação,, ou. Essas regras também se aplicam aos campos de não coleção EDM. GeographyPoint.
...
- No corpo de uma `any`, as expressões geoespaciais geográficas podem `or`ser combinadas usando. No corpo de um `all`, essas expressões podem ser combinadas usando `and`.
...
- Regras para `any`:
  ...
  - Expressões de comparação simples `eq`envolvendo `lt`, `le`, `gt`, ou `ge` podem ser combinadas `and` / `or`com. Por exemplo:
    - `ratings/any(r: r gt 2 and r le 5)`
    - `ratings/any(r: r le 5 or r gt 7)`
  - Expressões de comparação combinadas com (conconjuntos) podem ser combinadas com `and` o `or`. Esse formulário é conhecido em lógica booleana como "[disjunctive normal](https://en.wikipedia.org/wiki/Disjunctive_normal_form)" (DNF). Por exemplo:
...
- Regras para `all`:
  ...
  - Expressões de comparação simples `ne`envolvendo `lt`, `le`, `gt`, ou `ge` podem ser combinadas `and` / `or`com. Por exemplo:
