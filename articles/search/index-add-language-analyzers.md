...
+ O analisador em inglês do Lucene estende o analisador padrão. Ele remove possessivos (apóstrofos à direita) de palavras, aplica a lematização conforme o algoritmo de lematização de Porter e remove as palavras irrelevantes do inglês.  
...
## <a name="how-to-specify-a-language-analyzer"></a>Como especificar um analisador de idioma

Defina um analisador de linguagem em campos "pesquisáveis" do tipo EDM. String durante a definição de campo.

Embora as definições de campo tenham várias propriedades relacionadas ao analisador, somente a propriedade "Analyzer" pode ser usada para analisadores de idioma. O valor de "Analyzer" deve ser um dos analisadores de idioma da lista de analisadores de suporte.
...
Para obter mais informações sobre como criar um índice e definir propriedades de campo, consulte [criar índice (REST)](/rest/api/searchservice/create-index). Para obter mais informações sobre a análise de texto, consulte [analisadores no Azure pesquisa cognitiva](search-analyzers.md).

<a name="language-analyzer-list"></a>

## <a name="supported-language-analyzers"></a>Analisadores de idioma com suporte

 Abaixo está a lista de idiomas com suporte, com o Lucene e os nomes do Microsoft Analyzer.  
...
+ [Crie um índice](search-what-is-an-index.md)
+ [Criar um índice de vários idiomas](search-language-support.md)
+ [Criar índice (API REST)](/rest/api/searchservice/create-index)  
+ [Classe LexicalAnalyzerName (SDK do Azure para .NET)](/dotnet/api/azure.search.documents.indexes.models.lexicalanalyzername)