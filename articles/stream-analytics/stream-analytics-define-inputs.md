...
O Stream Analytics oferece suporte à compactação em todas as fontes de entrada de fluxo de dados. Os tipos de compressão suportados são: Nenhum, GZip e Deflate compressão. O suporte para a compactação não está disponível para os dados de referência. Se o formato de entrada forem dados Avro compactados, eles serão manipulados de forma transparente. Você não precisa especificar o tipo de compactação com a serialização Avro. 

## <a name="create-edit-or-test-inputs"></a>Criar, editar ou testar entradas

Você pode usar o [portal Azure,](stream-analytics-quick-create-portal.md) [Visual Studio](stream-analytics-quick-create-vs.md)e Visual [Studio Code](quick-create-vs-code.md) para adicionar e visualizar ou editar entradas existentes em seu trabalho de streaming. Você também pode testar conexões de entrada e [testar consultas](stream-analytics-manage-job.md#test-your-query) a partir de dados de amostra do portal Azure, [Visual Studio](stream-analytics-vs-tools-local-run.md)e Visual [Studio Code](visual-studio-code-local-run.md). Quando você escreve uma consulta, você lista a entrada na cláusula FROM. Você pode obter a lista de entradas disponíveis na página **Consulta** no portal. Se você quiser usar várias entradas, poderá usar `JOIN` para associá-las ou codificar várias consultas `SELECT`.


## <a name="stream-data-from-event-hubs"></a>Transmitir dados dos Hubs de Eventos

Os Hubs de Eventos do Azure fornecem ingestores de eventos altamente escalonável de publicação/assinatura. Um hub de eventos pode coletar milhões de eventos por segundo para que você possa processar e analisar as enormes quantidades de dados produzidos por seus dispositivos e aplicativos conectados. Juntos, os Hubs de Eventos e o Stream Analytics fornecem uma solução de ponta a ponta para análise em tempo real. Os Hubs de Eventos permitem que você envie eventos para o Azure em tempo real, e os trabalhos do Stream Analytics podem processá-los em tempo real. Por exemplo, você pode enviar cliques da Web, leituras do sensor ou eventos de log online para Hubs de Eventos. Em seguida, você pode criar trabalhos do Stream Analytics para usar Hubs de Eventos como os fluxos de dados de entrada para filtragem, agregação e correlação em tempo real.
...
Cada entrada do trabalho do Hub de Eventos do Stream Analytics deve ser configurada para ter seu próprio grupo de consumidores. Quando um trabalho contém uma autojunção ou tem várias entradas, algumas entradas podem ser lidas por mais de um leitor downstream. Essa situação afeta o número de leitores em um único grupo de consumidores. Para evitar exceder o limite de Hub de Eventos dos cinco leitores por grupo de consumidores por partição, é uma melhor prática designar um grupo de consumidores para cada trabalho do Stream Analytics. Há também um limite de 20 grupos de consumidores para um hub de eventos de nível Standard. Para obter mais informações, consulte [entradas Solucionar problemas do Azure Stream Analytics](stream-analytics-troubleshoot-input.md).

### <a name="create-an-input-from-event-hubs"></a>Crie uma entrada no Event Hubs

A tabela a seguir explica cada propriedade na página **de entrada New** no portal Azure para transmitir a entrada de dados de um hub de eventos:
...
| **Espaço de nome do Event Hub** | Um namespace Hub de Eventos é um contêiner para um conjunto de entidades de mensagens. Ao criar um novo hub de eventos, você também cria o namespace. |
...
| **Chave de partição** | Se sua entrada for particionada por uma propriedade, você pode adicionar o nome desta propriedade. As teclas de partição são opcionais e são usadas para melhorar o desempenho da sua consulta se ela incluir uma cláusula PARTITION BY ou GROUP BY nesta propriedade. |
...
O Azure IoT Hub é um evento de assinatura de publicações altamente escalável otimizado para cenários de IoT.
...
### <a name="iot-hub-consumer-groups"></a>Grupos de consumidores do Iot Hub
...
O carimbo de data/hora padrão de eventos de Armazenamento de Blobs no Stream Analytics é o carimbo de data/hora que o blob foi modificado pela última vez, que é `BlobLastModifiedUtcTime`. Se uma bolha for carregada em uma conta de armazenamento às 13:00, e o trabalho do Azure Stream Analytics for iniciado usando a opção *Agora* às 13:01, a bolha não será captada à medida que seu tempo modificado cair fora do período de execução do trabalho.

Se uma bolha for carregada em um contêiner de conta de armazenamento às 13:00, e o trabalho do Azure Stream Analytics for iniciado usando *o Custom Time* às 13:00 ou antes, a bolha será recolhida à medida que seu tempo modificado cair dentro do período de execução do trabalho.

Se um trabalho do Azure Stream Analytics for iniciado usando *o Now* às 13:00, e uma bolha for carregada no contêiner da conta de armazenamento às 13:01, o Azure Stream Analytics pegará a bolha.
...
O upload de um número muito grande de bolhas ao mesmo tempo pode fazer com que o Stream Analytics pule a leitura de poucas bolhas em casos raros. Recomenda-se carregar bolhas com pelo menos 2 segundos de diferença para o armazenamento Blob. Se essa opção não for viável, você pode usar o Event Hubs para transmitir grandes volumes de eventos. 
...
| **Recipiente** | O contêiner para o blob de entrada. Os contêineres fornecem um agrupamento lógico de blobs armazenados no serviço Blob do Microsoft Azure. Quando você carrega um blob no serviço de Armazenamento de Blobs do Azure, você deve especificar um contêiner para aquele blob. Você pode escolher **Usar contêiner existente** ou **Criar novo** para ter um novo contêiner criado.|
| **Padrão do caminho** (opcional) | O caminho do arquivo usado para localizar os blobs no contêiner especificado. Se você quiser ler bolhas da raiz do recipiente, não defina um padrão de caminho. No caminho, você pode optar por especificar uma ou mais instâncias das três variáveis a seguir: `{date}`, `{time}` ou `{partition}`<br/><br/>Exemplo 1: `cluster1/logs/{date}/{time}/{partition}`<br/><br/>Exemplo 2: `cluster1/logs/{date}`<br/><br/>O caractere `*` não é um valor permitido para o prefixo de caminho. Apenas <a HREF="https://msdn.microsoft.com/library/azure/dd135715.aspx">caracteres de blobs do Azure</a> válidos são permitidos. Não inclua nomes de contêineres ou nomes de arquivos. |
| **Formato da data** (opcional) | Se você usar a variável de data no caminho, o formato de data no qual os arquivos são organizados. Exemplo: `YYYY/MM/DD` |
