...
O portal Azure surface as principais métricas de desempenho que podem ser usadas para monitorar e solucionar problemas de sua consulta e desempenho do trabalho. Para ver essas métricas, procure o trabalho do Stream Analytics em cujas métricas você está interessado em ver e exiba a seção **Monitoramento** na página de visão geral.  
...
| Fontes de Entrada Recebidas       | Número de mensagens recebidas pelo trabalho. Para o Event Hub, uma mensagem é um único EventData. Para Blob, uma mensagem é uma única bolha. Observe que as fontes de entrada são contadas antes da desserialização. Se houver erros de desserialização, as fontes de entrada podem ser maiores do que eventos de entrada. Caso contrário, pode ser menor ou igual a eventos de entrada, uma vez que cada mensagem pode conter vários eventos. |
...
| Erros de runtime         | Número total de erros relacionados ao processamento de consultas (excluindo erros encontrados ao ingerir eventos ou saída de resultados) |
