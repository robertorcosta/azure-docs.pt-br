---
title: Implantar aplicativos ASP.NET Core no serviço kubernetes do Azure com o iniciador DevOps do Azure
description: O iniciador DevOps do Azure facilita a introdução ao Azure. Com o DevOps Starter, você pode implantar seu aplicativo ASP.NET Core com o AKS (serviço kubernetes do Azure) em algumas etapas rápidas.
ms.author: mlearned
ms.manager: gwallace
ms.prod: devops
...
ms.locfileid: "82233322"
---
# <a name="deploy-aspnet-core-apps-to-azure-kubernetes-service-with-azure-devops-starter"></a>Implantar aplicativos ASP.NET Core no serviço kubernetes do Azure com o iniciador DevOps do Azure
...
> [!div class="checklist"]
> * Usar o iniciador DevOps para implantar um aplicativo de ASP.NET Core no AKS
...
## <a name="use-devops-starter-to-deploy-an-aspnet-core-app-to-aks"></a>Usar o iniciador DevOps para implantar um aplicativo de ASP.NET Core no AKS

O iniciador DevOps cria um pipeline de CI/CD no Azure Pipelines. É possível criar uma nova organização do Azure DevOps ou usar uma existente. O iniciador DevOps também cria recursos do Azure, como um cluster AKS, na assinatura do Azure de sua escolha.
...
1. Saia da área de configuração do Azure e selecione **Concluído**. Depois de alguns minutos, o processo é concluído. Um exemplo de aplicativo do ASP.NET Core é configurado em um repositório Git na sua organização do Azure DevOps, um cluster do AKS é criado, um pipeline de CI/CD é executado, e seu aplicativo é implantado no Azure. 
    ...
    Depois que tudo isso for concluído, o painel inicial do Azure DevOps será exibido na portal do Azure. Você também pode ir para o painel inicial do DevOps diretamente de **todos os recursos** no portal do Azure. 
...
O DevOps Starter configura automaticamente um cluster AKS, que você pode explorar e personalizar. Para se familiarizar com o cluster do AKS, faça o seguinte:
...
1. No menu DevOps do Azure, selecione **Code** > **arquivos**de código e, em seguida, vá para o repositório.
...
Caso esteja fazendo testes, é possível evitar o acúmulo de encargos de cobrança limpando seus recursos. Quando eles não forem mais necessários, é possível excluir o cluster do AKS e os recursos relacionados criados neste tutorial. Para fazer isso, use a funcionalidade **excluir** no painel inicial do DevOps.
...
> [Usar o painel do Kubernetes](https://docs.microsoft.com/azure/aks/kubernetes-dashboard)
