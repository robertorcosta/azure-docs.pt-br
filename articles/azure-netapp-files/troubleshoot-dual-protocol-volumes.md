...
| A criação de volume de protocolo duplo falha com o erro `Failed to validate LDAP configuration, try again after correcting LDAP configuration` .    |  Considere as seguintes resoluções:   <ul><li>Certifique-se de que o certificado raiz necessário seja adicionado quando você ingressar Active Directory (AD) à conta do NetApp. Consulte [carregar Active Directory certificado raiz público da autoridade de certificação](create-volumes-dual-protocol.md#upload-active-directory-certificate-authority-public-root-certificate).   </li><li>O registro de ponteiro (PTR) do computador host do AD pode estar ausente no servidor DNS. Você precisa criar uma zona de pesquisa inversa no servidor DNS e, em seguida, adicionar um registro PTR da máquina host do AD nessa zona de pesquisa inversa. <br> Por exemplo, suponha que o endereço IP do computador do AD seja `1.1.1.1` , o nome do host do computador do AD (como encontrado usando o `hostname` comando) é `AD1` , e que é `myDomain.com` .  O registro PTR adicionado à zona de pesquisa inversa deve ser `1.1.1.1`  ->  `AD1.myDomain.com` . </li></ul>  |
