...
* Versão de API do `2020-01-01` ou superior
...
* Durante a visualização, `outboundType` só pode ser definida no momento da criação do cluster e não pode ser atualizada posteriormente.
* Durante a visualização, `outboundType` clusters AKS devem usar o CNI do Azure. Kubenet é configurável, o uso requer associações manuais da tabela de rotas para a sub-rede AKS.
* A configuração `outboundType` requer clusters AKS com uma `vm-set-type` de `VirtualMachineScaleSets` e `load-balancer-sku` de `Standard`.
* A definição de `outboundType` como um valor de `UDR` requer uma rota definida pelo usuário com conectividade de saída válida para o cluster.
* A definição de `outboundType` como um valor de `UDR` implica que o IP de origem de entrada roteado para o balanceador de carga pode **não corresponder** ao endereço de destino de egresso de saída do cluster.

## <a name="overview-of-outbound-types-in-aks"></a>Visão geral dos tipos de saída no AKS

Um cluster AKS pode ser personalizado com um `outboundType` exclusivo do balanceador de carga de tipo ou o roteamento definido pelo usuário.
...
Abaixo está uma topologia de rede implantada em clusters AKS por padrão, que usam uma `outboundType` de `loadBalancer`.
...
Por fim, o cluster AKS pode ser implantado na sub-rede existente que dedicamos ao cluster. A sub-rede de destino a ser implantada é definida com a variável de ambiente, `$SUBNETID`.
...
Implante o aplicativo de aplicativo de votação do Azure copiando o YAML abaixo para um arquivo chamado `example.yaml`.
