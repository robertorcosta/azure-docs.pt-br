...
O binário de cliente do `linkerd` é executado no computador cliente e permite que você interaja com a malha do serviço Linkerd. Use os comandos a seguir para instalar o binário do Linkerd `linkerd` Client em um shell baseado em bash no [subsistema Linux ou Windows para Linux][install-wsl]. Esses comandos copiam o `linkerd` binário de cliente para o local do programa do usuário padrão no `PATH`.
