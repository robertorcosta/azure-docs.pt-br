---
title: Usar um balanceador de carga SKU padrão
...
Por exemplo, fazer implantações azuis/verdes para migrar clusters é uma prática comum `load-balancer-sku` , dado que o tipo de um cluster só pode ser definido no momento da criação do cluster. No entanto, os balanceadores de carga de *SKU básicos* usam endereços IP de *SKU básicos* que não são compatíveis com os balanceadores de carga *SKU padrão* , pois exigem endereços IP de *SKU padrão* . Ao migrar clusters para atualizar Load Balancer SKUs, um novo endereço IP com um SKU de endereço IP compatível será necessário.
