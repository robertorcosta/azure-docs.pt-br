...
Endereços IP para os pods e os nós do cluster são atribuídos a partir da sub-rede especificada na rede virtual. Cada nó é configurado com um endereço IP primário. Por padrão, 30 endereços IP adicionais são pré-configurados pelo CNI do Azure e atribuídos a pods programados no nó. Quando você escalar horizontalmente o cluster, cada nó está configurado da mesma forma com endereços IP da sub-rede. Você também pode ver o [máximo de pods por nó](#maximum-pods-per-node).
...
* **CLI do Azure**: especifique o argumento `--max-pods` ao implantar um cluster com o comando [AZ AKs Create][az-aks-create] . O valor máximo é 250.
...
**Intervalo de endereços do serviço kubernetes**: esse é o conjunto de IPS virtuais que o kubernetes atribui a [Serviços][services] internos em seu cluster. Você pode usar qualquer intervalo de endereço particular que atenda aos seguintes requisitos:
...
Use o comando [AZ AKs Create][az-aks-create] com o argumento `--network-plugin azure` para criar um cluster com a rede avançada. Atualize o valor `--vnet-subnet-id` com a ID de sub-rede obtida na etapa anterior:
