...
> [!Note]
> Os clusters AKS existentes com a integração do AAD (Azure Active Directory) não são afetados pela nova experiência do AAD gerenciada pelo AKS.

A integração do Azure AD com o AAD gerenciado pelo AKS foi projetada para simplificar a experiência de integração do Azure AD, na qual os usuários eram solicitados anteriormente a criar um aplicativo cliente, um aplicativo de servidor e exigia que o locatário do Azure AD concedesse permissões de leitura de diretório. Na nova versão, o provedor de recursos AKS gerencia os aplicativos de cliente e servidor para você.
...
- O CLI do Azure, versão 2.5.1 ou posterior
...
* Saiba mais sobre o [controle de acesso baseado em função do AD do Azure][azure-ad-rbac].
