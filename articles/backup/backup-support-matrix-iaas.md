...
Backup de discos gerenciados depois de habilitar o bloqueio de grupo de recursos | {1&gt;Sem suporte.&lt;1}<br/><br/> O backup do Azure não pode excluir os pontos de restauração mais antigos, e os backups começarão a falhar quando o limite máximo de pontos de restauração for atingido.
...
Backup da VM para uma região ou uma assinatura diferentes |{1&gt;Sem suporte.&lt;1}
...
Ajuste automático de relógio | {1&gt;Sem suporte.&lt;1}<br/><br/> O backup do Azure não é ajustado automaticamente para alterações de horário de verão ao fazer backup de uma VM.<br/><br/>  Modifique a política manualmente, se necessário.
...
Fazer backup da VM cuja hora da máquina foi alterada | {1&gt;Sem suporte.&lt;1}<br/><br/> Se a hora da máquina for alterada para uma data/hora futura depois de habilitar o backup para essa VM; No entanto, mesmo se a alteração de tempo for revertida, o backup bem-sucedido não será garantido.  
...
Backup de VMs do Azure no Linux com o agente MARS | {1&gt;Sem suporte.&lt;1}<br/><br/> O agente do MARS só pode ser instalado em computadores Windows.
Backup de VMs do Azure no Linux com DPM/MABS | {1&gt;Sem suporte.&lt;1}
...
Restaurar arquivos em VMs clássicas | {1&gt;Sem suporte.&lt;1}
Restaurar arquivos de VMs criptografadas | {1&gt;Sem suporte.&lt;1}
Restaurar arquivos de contas de armazenamento restritas à rede | {1&gt;Sem suporte.&lt;1}
...
**Restaurar** | **Com suporte**
--- | ---
Restaure em região/assinatura/zona. | {1&gt;Sem suporte.&lt;1}
Restaurar uma VM existente | Use a opção para substituir disco.
Restaurar o disco com a conta de armazenamento habilitada com a Criptografia do Serviço de Armazenamento do Azure (SSE) | {1&gt;Sem suporte.&lt;1}<br/><br/> Restaure em uma conta que não esteja habilitado com SSE.
Restaurar as contas de armazenamento mistas |{1&gt;Sem suporte.&lt;1}<br/><br/> Com base no tipo de conta de armazenamento, todos os discos restaurados serão premium ou standard e não mistos.
...
Fazer backup de VMs implantadas em um [conjunto de dimensionamento](https://docs.microsoft.com/azure/virtual-machine-scale-sets/overview) |{1&gt;Sem suporte.&lt;1}
...
Discos com o Acelerador de Gravação habilitado | {1&gt;Sem suporte.&lt;1}<br/><br/> O backup do Azure exclui automaticamente os discos com Acelerador de Gravação habilitado durante o backup. Como não é feito backup, você não poderá restaurar esses discos de pontos de recuperação da VM.
...
Endereço IP estático | {1&gt;Sem suporte.&lt;1}<br/><br/> Uma nova VM que é criada a partir de um ponto de restauração recebe um endereço IP dinâmico.<br/><br/> Para VMs clássicas, não é possível fazer backup de uma VM com um endereço IP reservado e nenhum ponto de extremidade definido.
