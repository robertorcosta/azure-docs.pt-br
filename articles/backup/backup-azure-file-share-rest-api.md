...
O URI de POSTAgem tem `{subscriptionId}`, `{vaultName}`, `{vaultresourceGroupName}`e parâmetros de `{fabricName}`. Em nosso exemplo, o valor para os diferentes parâmetros seria o seguinte:
