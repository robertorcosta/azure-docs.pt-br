---
title: Script do PowerShell – Restaurar um compartilhamento de arquivo excluído
...
Esse script ajuda você a restaurar um compartilhamento de arquivo, caso você o tenha excluído acidentalmente. O recurso de segurança de exclusão temporária de compartilhamentos de arquivo fornece a opção de restaurar um compartilhamento de arquivo dentro do período de retenção de 14 dias, permitindo a recuperação de todo o conteúdo do compartilhamento de arquivo, instantâneos e pontos de recuperação. Para saber mais sobre a exclusão temporária, acesse este [link](../soft-delete-azure-file-share.md).
...
1. Instale os últimos módulos do Az Azure PowerShell por meio [deste link](https://docs.microsoft.com//powershell/azure/install-az-ps?view=azps-3.3.0) antes de executar o script.
2. Mantenha os detalhes a seguir em um lugar de fácil acesso, pois você precisará passá-los como valores para diferentes parâmetros do script:
    ...
    * **-ResourceGroupName**: grupo de recursos da conta de armazenamento que hospeda o compartilhamento de arquivo.
    * **-StorageAccountName**: nome da conta de armazenamento que hospeda o compartilhamento de arquivo.
    * **-FileShareName**: nome do compartilhamento de arquivo a ser restaurado

### <a name="execution-steps"></a>Etapas de execução

1. Salve o script acima em seu computador com um nome de sua escolha. Neste exemplo, nós o salvamos como *Undelete.ps1*
2. Execute o script de acordo com o cenário que atenda às suas necessidades.

#### <a name="scenario-1"></a>Cenário 1

Não há várias versões excluídas com o mesmo nome do compartilhamento de arquivo que você está tentando restaurar.

O exemplo a seguir restaura o compartilhamento de arquivo *share1* presente na conta de armazenamento *afsshare*.
...
A saída deverá mostrar a mensagem `Completed:Restore File Share`

#### <a name="scenario-2"></a>Cenário 2

Há várias versões excluídas com o mesmo nome do compartilhamento de arquivo que você está tentando restaurar.

O exemplo a seguir restaura uma versão do compartilhamento de arquivo *share1*

##### <a name="step-1"></a>Etapa 1

Execute o script, conforme mostrado a seguir, fornecendo o nome do compartilhamento de arquivo.
...
Escolha a versão da saída da etapa 1 que deseja restaurar e passe-a como um valor para o parâmetro **-DeletedShareVersion**.

O exemplo a seguir restaura a versão *01D5D7F77ACC7864* do compartilhamento de arquivo *share1*.
