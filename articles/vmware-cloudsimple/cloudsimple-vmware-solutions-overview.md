---
title: Soluções VMware do Azure (AVS)-visão geral
description: Saiba mais sobre os recursos, cenários e benefícios da solução VMware no Azure pelo serviço AVS.
author: sharaths-cs
ms.author: dikamath
ms.date: 08/20/2019
ms.topic: article
...
---
# <a name="what-is-azure-vmware-solutions-avs"></a>O que é o Azure VMware Solutions (AVS)

A **solução VMware do Azure (AVS)** é um serviço totalmente gerenciado que permite que você execute a plataforma VMware no Azure. Essa solução inclui vSphere, vCenter, vSAN, NSX-T e ferramentas correspondentes. Seu ambiente VMware é executado nativamente na infraestrutura bare-metal do Azure em locais de nuvem do Azure. O serviço inclui todos os recursos necessários para consumir as plataformas VMware de forma eficiente e segura.

![Solução VMware no Azure por visão geral da AVS](media/azure-vmware-solution-by-cloudsimple.png)
...
* **Continuidade operacional**. A AVS oferece acesso nativo às plataformas VMware. A arquitetura AVS é compatível com o seu existente:
    * Aplicativos
    * Operations
    * Segurança
    * Backup
...
* **Agilidade da infraestrutura**. Você não precisa mais prever todas as suas necessidades de capacidade e, em seguida, acabar com insuficiências de capacidade ou infraestrutura desperdiçadas. A AVS é entregue como um serviço de nuvem e você pode adicionar ou reduzir a capacidade a qualquer momento
* **Segurança**. O acesso ao ambiente da AVS por meio do Azure fornece proteção interna contra DDoS e monitoramento de segurança.
* **Custo mais baixo**. A plataforma AVS é altamente projetada e fornece altos níveis de automação, eficiência operacional e economia de escala. Além disso, a AVS publica arquiteturas de solução que aproveitam a presença do VMware em uma nuvem pública para reduzir os custos. Os exemplos incluem o Azure AD, backup no armazenamento do Azure, gateway de aplicativo, balanceador de carga e outros.
* **Uma nova plataforma híbrida**. O serviço permite acesso de alta velocidade e baixa latência ao restante do Azure. Além disso, o gerenciamento de AVS permite o gerenciamento unificado de máquinas virtuais VMware e o restante do Azure usando a mesma interface do usuário e API. Suas equipes de desenvolvimento podem aproveitar as plataformas públicas e privadas de maneira integrada e consistente.
* **Monitoramento de infraestrutura, solução de problemas e suporte**. A AVS opera em sua infraestrutura subjacente como um serviço. O hardware de falha é substituído automaticamente. Você pode se concentrar no consumo enquanto a AVS garante que o ambiente seja executado sem problemas.
...
* [Criar serviço AVS](quickstart-create-cloudsimple-service.md)
* [Criar uma nuvem privada de AVS](quickstart-create-private-cloud.md)
