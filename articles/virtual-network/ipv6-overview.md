---
title: Visão geral do IPv6 para a rede virtual do Azure
...
ms.locfileid: "80984003"
---
# <a name="what-is-ipv6-for-azure-virtual-network"></a>O que é IPv6 para a rede virtual do Azure?
...
- Suporte de [Load Balancer público do IPv6 padrão](virtual-network-ipv4-ipv6-dual-stack-standard-load-balancer-powershell.md) para criar aplicativos resilientes e escalonáveis, que incluem:
    ...
    - Observação: o balanceamento de carga não executa nenhuma conversão de protocolo (nenhum NAT64). 
...
- O IPv6 para a rede virtual do Azure está disponível em todas as regiões comerciais globais do Azure usando todos os métodos de implantação.  A implantação na nuvem do governo dos EUA é temporariamente limitada ao modelo ARM (JSON), à CLI (interface de linha de comando) e ao PowerShell.  O suporte a IPv6 no portal de nuvem do governo dos EUA estará disponível em breve.  
- Gateways de ExpressRoute podem ser usados para tráfego somente IPv4 em uma VNET com IPv6 habilitado.  O suporte para tráfego IPv6 está em nosso roteiro.   
