...
Para assinaturas Pagamento Conforme o Uso ou Microsoft Partner Network criadas após 15 de novembro de 2017, haverá restrições técnicas que bloqueiam o email que é enviado diretamente de VMs dentro dessas assinaturas. Se você quiser a capacidade de enviar email de máquinas virtuais do Azure diretamente para provedores de email externos (não usando uma retransmissão SMTP autenticada), você pode fazer uma solicitação para remover a restrição. As solicitações serão examinadas e aprovadas conforme os critérios da Microsoft e serão concedidas somente após a realização de verificações antifraude. Para fazer uma solicitação, abra um caso de suporte usando o seguinte tipo de problema:**Conectividade** > **de rede** > virtual **técnica** > **não pode enviar e-mail (SMTP/Porta 25)**. Certifique-se de adicionar detalhes sobre por que sua implantação tem que enviar emails diretamente aos provedores de email em vez de usar uma retransmissão autenticada.

Depois que uma assinatura do Pay-As-You-Go ou da Microsoft Partner Network for isenta e as VMs tiverem sido 'paradas' & 'Started' do portal Azure, todas as VMs dentro dessa assinatura serão isentas daqui para frente. A isenção só é aplicável à assinatura solicitada e só se aplica ao tráfego da Máquina Virtual encaminhado diretamente para a internet. O tráfego da porta de roteamento 25 através de serviços do Azure PaaS, como [o Azure Firewall,](https://azure.microsoft.com/services/azure-firewall/) não tem suporte.
...
Se você estiver usando esses tipos de assinatura, você é encorajado a usar serviços de relé SMTP, como descrito anteriormente neste artigo ou alterar seu tipo de assinatura.

## <a name="cloud-service-provider-csp"></a>Provedor de Serviços de Nuvem (CSP)

Se você estiver usando os recursos do Azure através do CSP, você pode solicitar ao CSP que crie uma solicitação de isenção de desbloqueio com a Microsoft em seu nome se um relé SMTP seguro não puder ser usado.

## <a name="need-help-contact-support"></a>Precisa de ajuda? Contate o suporte

Se você ainda precisar de ajuda, [entre em contato com o suporte](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade) para resolver seu problema rapidamente usando o seguinte tipo de problema: Tipo de problema de gerenciamento de **assinaturas:** **Solicitação para ativar o fluxo de e-mail do Port 25**.
