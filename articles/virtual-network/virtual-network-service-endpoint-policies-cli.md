...
Salve o URI de recurso para a conta de armazenamento permitida em uma variável. Antes de executar o comando a seguir, substitua *\<sua ID de assinatura >* com o valor real da sua ID de assinatura.
...
Monte o compartilhamento de arquivos do Azure para o diretório que você criou. Antes de executar o comando a seguir, substitua *\<> de chave de conta de armazenamento* pelo valor de *AccountKey* de **$saConnectionString 1**.
...
Antes de executar o comando a seguir, substitua *\<> de chave de conta de armazenamento* pelo valor de *AccountKey* do **$saConnectionString 2**.

```bash
sudo mount --types cifs //notallowedstorageacc.file.core.windows.net/my-file-share /mnt/MyAzureFileShare2 --options vers=3.0,username=notallowedstorageacc,password=<storage-account-key>,dir_mode=0777,file_mode=0777,serverino
```

O acesso é negado e você recebe um erro de `mount error(13): Permission denied`, pois essa conta de armazenamento não está na lista de permissões da política de ponto de extremidade de serviço que aplicamos à sub-rede. 
