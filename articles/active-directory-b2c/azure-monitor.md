...
Para carregar seu locatário do Azure AD (o **cliente**), crie um [modelo de Azure Resource Manager](../lighthouse/how-to/onboard-customer.md) para sua oferta com as informações a seguir. Os valores de `mspOfferName` e `mspOfferDescription` são visíveis quando você exibe detalhes da oferta na [página provedores de serviço](../lighthouse/how-to/view-manage-service-providers.md) do portal do Azure.
...
Entre no diretório que contém sua assinatura usando [Connect-AzAccount](/powershell/azure/authenticate-azureps). Use o sinalizador `-tenant` para forçar a autenticação para o diretório correto.
...
Por fim, implante o modelo de Azure Resource Manager e os arquivos de parâmetro que você baixou e atualizou anteriormente. Substitua os valores `Location`, `TemplateFile`e `TemplateParameterFile` de acordo.
...
1. Selecione o diretório que contém sua assinatura.
    ...
    ![Alterar diretório](./media/azure-monitor/azure-monitor-portal-03-select-subscription.png)
