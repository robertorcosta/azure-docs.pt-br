...
|RefreshTokenUserJourneyId| Não | O identificador de um percurso do usuário que deve ser executado durante a solicitação [atualizar uma](authorization-code-flow.md#4-refresh-the-token) postagem de token `/token` de acesso para o ponto de extremidade. |
