---
title: Defina um perfil técnico para um emissor JWT em uma política personalizada
titleSuffix: Azure AD B2C
description: Defina um perfil técnico para um emissor de token web JSON (JWT) em uma política personalizada no Azure Active Directory B2C.
...
O atributo **Nome** do elemento **Protocolo** precisa ser definido como `None`. Defina o elemento **OutputTokenFormat** como `JWT`.
...
|RefreshTokenUserJourneyId| Não | O identificador de uma jornada de usuário que deve ser executada `/token` durante a atualização de uma solicitação [de token](authorization-code-flow.md#4-refresh-the-token) de acesso POST para o ponto final. |
