...
Esse provedor é usado para suprimir a tela "escolher provedor de identidade". Normalmente, ele é referenciado em um perfil técnico configurado para um provedor de identidade externo, como o Facebook. O seguinte `SM-SocialLogin` perfil técnico está incluído no [pacote de início de política personalizada](custom-policy-get-started.md#custom-policy-starter-pack).
...
O perfil `SM-Saml-idp` técnico a seguir é usado pelo [perfil técnico do emissor SAML](saml-issuer-technical-profile.md)
