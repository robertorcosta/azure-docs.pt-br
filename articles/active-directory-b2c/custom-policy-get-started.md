...
1. Em **URI de redirecionamento**, selecione **Web**e `https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com`, em `your-tenant-name` seguida, insira, em que é o nome de domínio do seu Azure ad B2C locatário.
...
1. Para **URI de redirecionamento**, insira `myapp://auth`.
1. Selecione **Criar**. Após a criação, copie a ID do aplicativo e salve-a para uso posterior.
...
1. `SocialAndLocalAccounts/` **`TrustFrameworkExtensions.xml`** No arquivo, substitua o valor de `client_id` pela ID do aplicativo do Facebook:
