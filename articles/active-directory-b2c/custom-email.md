...
1. Insira um nome de modelo exclusivo `Verification email` como e, em seguida, selecione **salvar**.
1. Para começar a editar seu novo modelo, selecione **Adicionar versão**.
1. Selecione **Editor de código** e **continue**.
1. No editor de HTML, Cole o modelo HTML a seguir ou use o seu próprio. Os `{{otp}}` parâmetros `{{email}}` e serão substituídos dinamicamente pelo valor de senha de uso único e pelo endereço de email do usuário.
...
1. Expanda **configurações** à esquerda e, para **assunto de email**, `{{subject}}`digite.
...
Adicione a transformação de declarações a seguir `<ClaimsTransformations>` ao elemento `<BuildingBlocks>`no. Faça as seguintes atualizações para o XML de transformação de declarações:
...
* Atualize o valor do parâmetro `personalizations.0.dynamic_template_data.subject` de entrada da linha de assunto com uma linha de assunto apropriada para sua organização.
...
Abaixo das transformações de declarações `<BuildingBlocks>`no, adicione o seguinte [ContentDefinition](contentdefinitions.md) para fazer referência ao URI de dados da versão 2.0.0:
...
1. Retorne `email` de volta para o perfil técnico autodeclarado que tem uma referência a esse controle de exibição.
...
Em definições de conteúdo, ainda `<BuildingBlocks>`em, adicione o seguinte [DisplayControl](display-controls.md) do tipo [VerificationControl](display-control-verification.md) à sua política.
...
Assim como acontece com os perfis técnicos de OTP, adicione os seguintes perfis `<ClaimsProviders>` técnicos ao elemento.
...
Na etapa final, adicione uma referência para o DisplayControl que você criou. Substitua seu perfil `LocalAccountSignUpWithLogonEmail` técnico autodeclarado existente pelo seguinte se você tiver usado uma versão anterior da política de Azure ad B2C. Este perfil técnico usa `DisplayClaims` uma referência para o DisplayControl.
...
Para localizar o email, você deve enviar cadeias de caracteres localizadas para SendGrid ou seu provedor de email. Por exemplo, para localizar o assunto do email, o corpo, a mensagem de código ou a assinatura do email. Para fazer isso, você pode usar a transformação declarações [GetLocalizedStringsTransformation](string-transformations.md) para copiar cadeias de caracteres localizadas em tipos de declaração. Na transformação `GenerateSendGridRequestBody` declarações, que gera a carga JSON, o usa declarações de entrada que contêm as cadeias de caracteres localizadas.
...
1. Altere a `GenerateSendGridRequestBody` transformação de declarações para usar declarações de entrada com o trecho de código XML a seguir.
