...
O fluxo de migração contínuo ainda requer a migração em massa de contas de usuário, mas usa uma [política personalizada](restful-technical-profile.md) para consultar uma [API REST](rest-api-claims-exchange-dotnet.md) (que você cria) para definir a senha de cada usuário na primeira entrada.
...
1. Verifique se a conta está sinalizada para migração avaliando um atributo de extensão booliano.
    ...
    - Se o atributo de extensão booliana retornar `False`, continue o processo de entrada normalmente.
...
![diagrama de fluxograma da abordagem de migração direta para a migração de usuário](./media/user-migration/diagram-01-seamless-migration.png)<br />*Diagrama: fluxo de migração contínuo*
