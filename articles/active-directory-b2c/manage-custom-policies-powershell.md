...
Execute o comando a seguir, substituindo `{b2c-tenant-name}` pelo nome do seu locatário Azure AD B2C. Entre com uma conta que tenha atribuído a função de [administrador de política IEF do B2C](../active-directory/users-groups-roles/directory-assign-admin-roles.md#b2c-ief-policy-administrator) no diretório.
...
Se você emitir o comando `Set-AzureADMSTrustFrameworkPolicy` com a ID de uma política que já existe em seu locatário do Azure AD B2C, o conteúdo dessa política será substituído.
