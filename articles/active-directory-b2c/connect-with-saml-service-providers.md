...
1. Abra **gerenciar certificados** > de usuário**usuário** > atual**certificados** > **pessoais** > *yourappname.yourtenant.onmicrosoft.com*
1. Selecione o certificado > **ação** > **todas as tarefas** > **Exportar**
1. Selecione **Sim** > **Avançar** > **Sim, exportar a chave** > privada**próxima**
...
Abra `SocialAndLocalAccounts\` **`TrustFrameworkExtensions.xml`** o pacote de início de política personalizada.

Localize a `<ClaimsProviders>` seção e adicione o trecho de código XML a seguir.

Você pode alterar o valor dos `IssuerUri` metadados. Esse é o URI do emissor que é retornado na resposta SAML de Azure AD B2C. Seu aplicativo de terceira parte confiável deve ser configurado para aceitar um URI de emissor durante a validação de Asserção SAML.
...
1. Adicione o seguinte trecho de código XML `<RelyingParty>` logo antes do elemento. Esse XML substitui o número 7 da etapa de orquestração da jornada do usuário _SignUpOrSignIn_ . Se você iniciou a partir de uma pasta diferente no pacote de início, ou personalizado o percurso do usuário adicionando ou removendo etapas de orquestração, verifique se o número `order` (no elemento) está alinhado com aquele especificado no percurso do usuário para a etapa do emissor do token (por exemplo, em outras pastas do pacote de início, é o `LocalAccounts`número 4 de `SocialAccounts` , 6 para `SocialAndLocalAccountsWithMfa`e 9).
...
1. Substitua todo `<TechnicalProfile>` o elemento no `<RelyingParty>` elemento pelo seguinte XML de perfil técnico.
...
1. Em **URI de redirecionamento**, selecione **Web**e `https://localhost`, em seguida, insira. Você modificará esse valor posteriormente no manifesto do registro do aplicativo.
...
O `identifierUris` é uma coleção de cadeia de caracteres que contém URI (s) definidos pelo usuário que identificam exclusivamente um aplicativo Web dentro de seu locatário de Azure ad B2C. Seu provedor de serviços deve definir esse valor no `Issuer` elemento de uma solicitação SAML.
...
Para este tutorial, que usa o aplicativo de teste do SAML, use o seguinte `samlMetadataUrl`valor para:
...
Se você não fornecer um URI de metadados, poderá especificar explicitamente a URL de resposta. Essa propriedade opcional representa a `AssertionConsumerServiceUrl` (`SingleSignOnService` URL nos metadados do provedor de serviço) e `BindingType` presume-se que `HTTP POST`seja.
...
Para este tutorial, no qual você usa o aplicativo de teste do SAML, `url` defina a `replyUrlsWithType` propriedade de como o valor mostrado no trecho de JSON a seguir.
...
Essa propriedade opcional representa a `Logout` URL (`SingleLogoutService` URL nos metadados de terceira parte confiável) e o `BindingType` para isso é considerado. `Http-Redirect`

Para este tutorial, que usa o aplicativo de teste do SAML `logoutUrl` , deixe `https://samltestapp2.azurewebsites.net/logout`definido como:
...
* **Emissor**: Use a EntityId no arquivo de metadados
...
* **Certificado**: é *B2C_1A_SamlIdpCert*, mas sem a chave privada. Para obter a chave pública do certificado:
    ...
    1. Copie o valor no `<X509Certificate>` elemento.
...
Selecione **logon** e você deverá receber uma tela de entrada do usuário. Após a entrada, uma Asserção SAML é emitida de volta para o aplicativo de exemplo.
...
- Obtenha o aplicativo Web de teste do SAML de [Azure ad B2C repositório de comunidade do GitHub](https://github.com/azure-ad-b2c/saml-sp-tester).
