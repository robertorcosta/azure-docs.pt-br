---
title: Configure o login e faça login com uma conta no Twitter
...
Para usar o Twitter como provedor de identidade no Azure AD B2C, você precisará criar um aplicativo do Twitter. Se você ainda não tem uma conta no [https://twitter.com/signup](https://twitter.com/signup)Twitter, você pode se inscrever em .
...
1. Insira `https://your-tenant.b2clogin.com/your-tenant.onmicrosoft.com/your-user-flow-Id/oauth1/authresp` como o valor da **URL de Retorno de Chamada**. Substitua `your-tenant` pelo nome do locatário e `your-user-flow-Id` pelo identificador do fluxo de usuário. Por exemplo, `b2c_1A_signup_signin_twitter`. Você precisa usar todas as letras minúsculas ao inserir seu nome de inquilino e id de fluxo de usuário, mesmo que sejam definidas com letras maiúsculas no Azure AD B2C.
...
1. Selecione **provedores de identidade**e selecione **Twitter**.
1. Digite um **nome**. Por exemplo, *Twitter*.
1. Para o **ID do cliente,** digite a chave de API do consumidor do aplicativo do Twitter que você criou anteriormente.
1. Para o **segredo do Cliente,** digite a chave secreta da API do consumidor que você gravou.
1. Selecione **Salvar**.
