...
Primeiro, ligue o registro. Para obter instruções sobre como fazê-lo, consulte [o registro do Azure Key Vault](logging.md)).
...
Em geral, as solicitações para o Key Vault são limitadas a 2000 solicitações/10 segundos. Exceções são operações-chave, como documentado nos [limites de serviço do Key Vault](service-limits.md)
...
- Quando ocorrer o estrangulamento do Key Vault, adapte o código solicitante para usar um backoff exponencial para tentar novamente. O algoritmo é explicado aqui: [Como estrangular seu aplicativo](overview-throttling.md#how-to-throttle-your-app-in-response-to-service-limits)
...
Orientação detalhada, incluindo solicitação para aumentar limites, pode ser encontrado aqui: [Orientação de estrangulamento do Key Vault](overview-throttling.md)
