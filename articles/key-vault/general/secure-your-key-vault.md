...
No plano de gerenciamento, você usa RBAC (controle de acesso baseado em função do Azure (RBAC do Azure)) para autorizar as operações que um chamador pode executar. No modelo de RBAC, cada assinatura do Azure tem uma instância do Azure Active Directory. Você pode conceder acesso a usuários, grupos e aplicativos desse diretório. O acesso é concedido para gerenciar recursos na assinatura do Azure que usa o modelo de implantação do Azure Resource Manager. Para conceder acesso, use o [Portal do Azure](https://portal.azure.com/), a [CLI do Azure](/cli/azure/install-azure-cli?view=azure-cli-latest), o [Azure PowerShell](/powershell/azure/) ou as [APIs REST do Azure Resource Manager](https://msdn.microsoft.com/library/azure/dn906885.aspx).
...
- **Equipe de segurança**: a equipe de TI do escritório do Diretor de segurança ou colaboradores equivalentes. A equipe de segurança é responsável por guardar adequadamente os segredos. Os segredos podem incluir certificados TLS/SSL, chaves RSA usadas para assinatura, cadeias de conexão e chaves da conta de armazenamento.
...
- Obter referências da equipe de segurança para a inicialização e certificados TLS/SSL (impressões digitais), a chave de armazenamento (URI do segredo) e a chave RSA (URI da chave) para a assinatura.
...
- Veja os [exemplos de cliente do Azure Key Vault](https://docs.microsoft.com/samples/browse/?term=Key%20Vault). Este conteúdo ilustra como usar um certificado de inicialização para autenticação no Microsoft Azure AD a fim de acessar um cofre de chaves.
...
Para ver um tutorial de introdução para um administrador, consulte [O que é o Azure Key Vault?](overview.md).

Para saber mais sobre o log de uso do Key Vault, confira [Log do Azure Key Vault](logging.md).
...
Se tiver dúvidas sobre o Key Vault, visite a [página de Perguntas e Respostas da Microsoft](https://docs.microsoft.com/answers/topics/azure-key-vault.html).
