...
As atividades na tabela a seguir são usadas para acessar variáveis em runbooks e configurações DSC. A diferença entre `Get-AzAutomationVariable` e `Get-AutomationVariable` é explicada para as variáveis criptografadas no início deste artigo.
...
> [!NOTE]
> Evite usar variáveis no parâmetro `Name` de `Get-AutomationVariable` em um runbook ou configuração DSC. O uso desse parâmetro pode complicar a descoberta de dependências entre runbooks ou configurações DSC e variáveis de automação em tempo de design.
...
O script usa o cmdlet `New-AzAutomationVariable` para criar uma nova variável e definir seu valor inicial. Em seguida, ele pode recuperar o valor usando `Get-AzAutomationVariable`. Se o valor for um tipo simples, o mesmo tipo será recuperado. Se for um tipo complexo, um tipo de `PSCustomObject` será recuperado.
...
Use a atividade `Set-AutomationVariable` para definir o valor de uma variável de automação em um runbook do PowerShell ou configuração DSC e o `Get-AutomationVariable` para recuperá-la. Você não deve usar os cmdlets `Set-AzAutomationVariable` e `Get-AzAutomationVariable` ou seus equivalentes de módulo AzureRM em um runbook ou configuração DSC, pois eles são menos eficientes do que as atividades de fluxo de trabalho. 

Observe que você não pode recuperar o valor de uma variável segura com `Get-AzAutomationVariable` ou o seu módulo AzureRM equivalente. 
...
Os comandos de exemplo a seguir mostram como definir e recuperar uma variável em um runbook textual. Este exemplo pressupõe a criação de variáveis de inteiro chamadas `NumberOfIterations` e `NumberOfRunnings` e uma variável de cadeia de caracteres denominada `SampleMessage`.
...
Em um runbook gráfico, você pode adicionar a atividade de `Get-AutomationVariable` ou `Set-AutomationVariable`. Basta clicar com o botão direito do mouse na variável no painel Biblioteca do editor gráfico e selecionar a atividade desejada.
...
A imagem a seguir mostra as atividades de exemplo para atualizar uma variável com um valor simples em um runbook gráfico. Neste exemplo, `Get-AzVM` recupera uma única máquina virtual do Azure e salva o nome do computador em uma variável de cadeia de caracteres de automação existente. Não importa se o [link é um pipeline ou uma sequência](../automation-graphical-authoring-intro.md#links-and-workflow) , pois o código só espera um único objeto na saída.
