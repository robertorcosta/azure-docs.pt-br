...
* Cria um ativo de certificado de automação chamado `AzureRunAsCertificate` na conta de automação especificada. O ativo de certificado contém a chave privada do certificado que o aplicativo do Azure AD usa.
...
A conta Executar como clássica do Azure executa as seguintes tarefas.
  ...
  * Cria um ativo de conexão de automação chamado `AzureClassicRunAsConnection` na conta de automação especificada. O ativo de conexão contém o nome da assinatura, a ID da assinatura e o nome do ativo do certificado.

## <a name="permissions"></a>Permissões da conta Executar como
...
|---|---------|---------|---|
|Criar aplicativo do Azure AD|[New-AzADApplication](/https://docs.microsoft.com/powershell/module/az.resources/new-azadapplication?view=azps-3.5.0)     | Função de desenvolvedor de aplicativo<sup>1</sup>        |[Azure AD](../active-directory/develop/howto-create-service-principal-portal.md#required-permissions)</br>> Do Azure AD > registros do aplicativo |
|Adicione uma credencial ao aplicativo.|[New-AzADAppCredential](https://docs.microsoft.com/powershell/module/az.resources/new-azadappcredential?view=azps-3.5.0)     | Administrador de aplicativos ou administrador global<sup>1</sup>         |[Azure AD](../active-directory/develop/howto-create-service-principal-portal.md#required-permissions)</br>> Do Azure AD > registros do aplicativo|
|Criar e obter uma entidade de serviço do Azure AD|[New-AzADServicePrincipal](https://docs.microsoft.com/powershell/module/az.resources/new-azadserviceprincipal?view=azps-3.5.0)</br>[Get-AzADServicePrincipal](https://docs.microsoft.com/powershell/module/az.resources/get-azadserviceprincipal?view=azps-3.5.0)     | Administrador de aplicativos ou administrador global<sup>1</sup>        |[Azure AD](../active-directory/develop/howto-create-service-principal-portal.md#required-permissions)</br>> Do Azure AD > registros do aplicativo|
...
Se você não for um membro da instância de Active Directory da assinatura antes de ser adicionado à função de administrador global da assinatura, você será adicionado como convidado. Nessa situação, você receberá uma `You do not have permissions to create…` aviso na página Adicionar conta de automação. 

Se você for um membro da instância de Active Directory da assinatura quando a função de administrador global for atribuída, você também poderá receber um aviso de `You do not have permissions to create…` na página Adicionar conta de automação. Nesse caso, você pode solicitar a remoção da instância de Active Directory da assinatura e solicitar que ela seja adicionada novamente, para que você se torne um usuário completo no Active Directory.
...
1. Observe os valores de **nome**, **ID da assinatura**e **grupo de recursos** na página Propriedades. Esses valores correspondem aos valores para os parâmetros de script `AutomationAccountName`, `SubscriptionId`e `ResourceGroupName` PowerShell, respectivamente.
...
Se você tiver criado uma conta Executar como clássica com um certificado público autoassinado (arquivo **. cer** ), o script o criará e o salvará na pasta arquivos temporários em seu computador. Ele pode ser encontrado no perfil do usuário `%USERPROFILE%\AppData\Local\Temp`, que você usou para executar a sessão do PowerShell.
...
3. Na página de propriedades contas Executar como, selecione a conta Executar como ou a conta Executar como clássica que você deseja excluir. 
...
6. Depois que a conta tiver sido excluída, você poderá recriá-la na página de propriedades contas Executar como selecionando a opção criar **conta Executar como do Azure**.

   ![Recriar a conta de Automação Executar como](media/manage-runas-account/automation-account-create-runas.png)

## <a name="cert-renewal"></a>Renovando um certificado autoassinado
...
## <a name="auto-cert-renewal"></a>Configurando a renovação automática de certificado com um runbook de automação
...
Se você precisar restringir, além do que a entidade de serviço executar como pode fazer, poderá adicionar outros tipos de recursos ao elemento `NotActions` da definição de função personalizada. O exemplo a seguir restringe o acesso a `Microsoft.Compute/*`. Se você adicionar esse tipo de recurso para `NotActions` para a definição de função, a função não poderá acessar nenhum recurso de computação. Para saber mais sobre definições de função, confira [noções básicas sobre definições de função para recursos do Azure](../role-based-access-control/role-definitions.md).
...
Para essas instâncias de configuração incorreta, a conta de automação detecta as alterações e exibe um status de `Incomplete` no painel de propriedades contas Executar como para a conta.
