...
Uma solução é ter um link de erro no runbook apontando para uma atividade que lida com a etapa um. Por exemplo, o runbook pode conectar o `Write-Warning` cmdlet a uma atividade para a etapa dois, como o cmdlet [Start-AzAutomationRunbook](https://docs.microsoft.com/powershell/module/az.automation/start-azautomationrunbook?view=azps-3.5.0) .
...
A `Get-AutomationVariable` atividade e o cmdlet [Start-AzVM](https://docs.microsoft.com/powershell/module/Az.Compute/Start-AzVM?view=azps-3.5.0) são configurados para converter exceções em erros. Se houver problemas ao obter a variável ou iniciar a VM, o código gerará erros.<br><br> ![Configurações](media/automation-runbook-graphical-error-handling/activity-blade-convertexception-option.png)de atividade de tratamento de erro do runbook de automação.

Links de erro fluem dessas atividades para uma única `error management` atividade de código. Essa atividade é configurada com uma expressão simples do PowerShell `throw` que usa a palavra-chave para `$Error.Exception.Message` parar o processamento, junto com para obter a mensagem que descreve a exceção atual.<br><br> ![Exemplo de código de tratamento de erro de runbook de automação](media/automation-runbook-graphical-error-handling/runbook-example-error-handling-code.png)
