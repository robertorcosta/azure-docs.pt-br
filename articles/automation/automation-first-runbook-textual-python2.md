...
1. Selecione **Python 2** para **o tipo Runbook**.
...
Agora você adiciona um comando `Hello World`simples para imprimir o texto .
...
1. Um [trabalho de runbook](automation-runbook-execution.md) é criado e seu status é exibido.
   O status do trabalho começa como enfileirado, indicando que está esperando um trabalhador de runbook na nuvem para vir disponível. E ele muda para Iniciando quando um trabalhador reivindica o trabalho e para Executando quando o runbook realmente começa a ser executado.
1. Quando o trabalho do runbook é concluído, sua saída é exibida. Neste caso, você `Hello World`deve ver.
...
Quando você publica um runbook, você sobreescreve a versão publicada existente com a versão de rascunho.
...
1. Se você rolar à esquerda para ver o runbook na página **Runbooks,** você deve ver um Status de **Autoria** **publicado**.
1. Role para a direita para exibir o painel do **MyFirstRunbook-Python**.
   As opções em toda a parte superior permitem que você inicie o manual, veja o manual ou agende-o para começar em algum momento no futuro.
2. Clique **em Iniciar** e clique em **OK** quando a lâmina Iniciar runbook for aberta.
3. Um painel de emprego é aberto para o trabalho de runbook que você criou. Você pode fechar este painel, mas vamos deixá-lo aberto para que você possa assistir o progresso do trabalho.
1. O status do trabalho é mostrado em **Resumo do trabalho** e corresponde aos status que você viu quando testou o runbook.
2. Uma vez que o status do runbook será concluído, clique **em Saída**. O painel Saída é aberto, `Hello World`onde você pode ver .
3. Feche o painel Saída.
4. Clique em **Todos os Logs** para abrir o painel Fluxos do trabalho do runbook. Você só `Hello World` deve ver no fluxo de saída. No entanto, este painel pode mostrar outros fluxos para um trabalho de runbook, como Verbose e Error, se o runbook escrever para eles.
5. Feche os painéis Fluxos e Trabalho para retornar ao painel MyFirstRunbook-Python.
6. Clique **em Empregos** para abrir a página Empregos para este manual. Esta página lista todos os empregos criados por este runbook. Você deve ver apenas um trabalho listado, já que executou o trabalho apenas uma vez.
7. Você pode clicar nesse trabalho para abrir o mesmo painel do Trabalho exibido ao iniciar o runbook. Este painel permite que você volte no tempo e veja os detalhes de qualquer trabalho que foi criado para um runbook específico.
...
Para fazer isso, o script precisa autenticar usando as credenciais da sua conta de Automação. Para ajudá-lo, você pode usar o [pacote do utilitário de automação do Azure](https://github.com/azureautomation/azure_automation_utility) para facilitar a autenticação e a interação com os recursos do Azure.

> [!NOTE]
> A conta de Automação deve ter sido criada com o recurso principal do serviço para que haja um certificado Run As.
> Se sua conta de Automação não foi criada com o diretor de serviço, você pode autenticar conforme descrito no [Authenticate com as Bibliotecas de Gerenciamento do Azure para Python](/azure/python/python-sdk-azure-authenticate).
...
Para trabalhar com VMs do Azure, crie uma instância do [Cliente da Computação do Azure para Python](https://docs.microsoft.com/python/api/azure-mgmt-compute/azure.mgmt.compute.computemanagementclient).
...
Onde `MyResourceGroup` está o nome do grupo de recursos `TestVM` que contém a VM, e é o nome da VM que você deseja iniciar.

Teste e execute o runbook novamente para ver se ele inicia a VM.

## <a name="use-input-parameters"></a>Usar parâmetros de entrada

Atualmente, o runbook usa valores codificados para os nomes do grupo de recursos e da VM.
...
Ao iniciar um runbook Python (no painel de teste ou como um runbook publicado), você pode inserir os valores para parâmetros na página Iniciar runbook em **Parâmetros**.

Depois de começar a inserir um valor na primeira caixa, um segundo aparece, e assim por diante, para que você possa inserir quantos valores de parâmetro saem necessário.

Os valores estão disponíveis `sys.argv` para o script na matriz como no código que você acabou de adicionar.
...
Você também pode usar as seguintes convenções para recuperar vários fluxos de seus runbooks Python, incluindo fluxos DE AVISO, ERRO e DEBUG.
...
O exemplo a seguir mostra `try...except` esta convenção usada em um bloco.
...
> [!NOTE]
> O Azure Automation `sys.stderr`não suporta .
...
- Para saber mais sobre os tipos de runbook, suas vantagens e limitações, consulte [os tipos de runbook da Azure Automation](automation-runbook-types.md).
