...
Para receber dados do cliente, o runbook dá suporte a um único parâmetro `WebhookData`chamado. Esse parâmetro define um objeto que contém dados que o cliente inclui em uma solicitação POST.
...
Não há nenhuma configuração do webhook necessária para dar suporte ao `WebhookData` parâmetro e o runbook não é necessário para aceitá-lo. Se o runbook não definir o parâmetro, todos os detalhes da solicitação enviada do cliente serão ignorados.
...
Se você especificar um valor para `WebhookData` a criação do webhook, ele será substituído quando o webhook iniciar o runbook com os dados da solicitação post do cliente. Isso acontece mesmo que o aplicativo não inclua dados no corpo da solicitação. 

Se você iniciar um runbook que define `WebhookData` o uso de um mecanismo diferente de um webhook, você pode fornecer um `WebhookData` valor para o qual o runbook reconhece. Esse valor deve ser um objeto com as mesmas [Propriedades](#webhook-properties) que o `WebhookData` parâmetro para que o runbook possa trabalhar com ele da mesma forma que funciona com `WebhookData` objetos reais passados por um webhook.
...
Agora, passamos o seguinte objeto JSON na interface do usuário `WebhookData` para o parâmetro. Este exemplo, com retornos de carro e caracteres de nova linha, corresponde ao formato passado de um webhook.
...
Você pode incluir lógica em um runbook para determinar se ele é chamado por um webhook. Faça com que o runbook `WebhookName` Verifique a propriedade `WebhookData` do parâmetro. O runbook pode executar uma validação adicional procurando informações específicas nas propriedades `RequestHeader` e. `RequestBody`
...
O cliente recebe um dos seguintes códigos de retorno da `POST` solicitação.
...
O exemplo a seguir mostra o corpo da solicitação que está disponível para o runbook na `RequestBody` propriedade de. `WebhookData` Esse valor é formatado em JSON para ser compatível com o formato incluído no corpo da solicitação.
