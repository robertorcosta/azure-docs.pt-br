...
## <a name="supported-vm-series-and-sizes"></a>Séries e tamanhos de VM suportados

### <a name="pools-in-virtual-machine-configuration"></a>Pools na configuração de Máquina Virtual

Os pools em lote na configuração da Máquina Virtual suportam quase todos os tamanhos de VM[(Linux](../virtual-machines/linux/sizes.md), [Windows).](../virtual-machines/windows/sizes.md) Veja a tabela a seguir para saber mais sobre tamanhos e restrições suportadas.
...
<sup>1</sup> Esses tamanhos de VM podem ser alocados em pools de lotes na configuração da Máquina Virtual, mas você deve criar uma nova conta de lote e solicitar um aumento de [cota](batch-quota-limit.md#increase-a-quota)específico . Essa limitação será removida assim que a cota de vCPU por série VM for totalmente suportada para contas em lote.
...
* **Disponibilidade da região** - Uma série ou tamanho VM pode não estar disponível nas regiões onde você cria suas contas em lote. Para verificar se um tamanho está disponível, consulte [Produtos disponíveis por região](https://azure.microsoft.com/regions/services/).
