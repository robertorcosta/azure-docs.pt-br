...
Se você ainda não tiver um certificado, a maneira mais fácil de obter um é gerar um certificado autoassinado usando a ferramenta de `makecert` linha de comando.

Normalmente, você pode `makecert` encontrar neste caminho: `C:\Program Files (x86)\Windows Kits\10\bin\<arch>`. Abra um prompt de comando como administrador e navegue até `makecert` usando o exemplo a seguir.

```console
cd C:\Program Files (x86)\Windows Kits\10\bin\x64
```

Em seguida, use `makecert` a ferramenta para criar arquivos de certificado `batchcertificate.cer` autoassinados `batchcertificate.pvk`chamados e. O CN (nome comum) usado não é importante para esse aplicativo, mas é útil fazê-lo algo que informe para que o certificado é usado.

```console
makecert -sv batchcertificate.pvk -n "cn=batch.cert.mydomain.org" batchcertificate.cer -b 09/23/2019 -e 09/23/2019 -r -pe -a sha256 -len 2048
```

O lote requer `.pfx` um arquivo. Use a ferramenta [pvk2pfx](https://docs.microsoft.com/windows-hardware/drivers/devtest/pvk2pfx) para converter os `.cer` arquivos `.pvk` e criados por `makecert` em um único `.pfx` arquivo.
