...
Depois de configurar um painel, você pode publicá-lo e compartilhá-lo com outros usuários na sua organização. Você permite que outras pessoas exibam seu painel usando o [controle de acesso baseado em função do Azure (RBAC do Azure)](../role-based-access-control/role-assignments-portal.md). Atribuir um único usuário ou um grupo de usuários a uma função. Essa função define se esses usuários podem exibir ou modificar o painel publicado.

Todos os painéis publicados são implementados como recursos do Azure. Elas existem como itens gerenciáveis em sua assinatura e estão contidas em um grupo de recursos. De uma perspectiva de controle de acesso, os painéis não são diferentes de outros recursos, como uma máquina virtual ou uma conta de armazenamento. Blocos individuais no painel impõem seus próprios requisitos de controle de acesso com base nos recursos que exibem. Você pode compartilhar um Dashboard em larga escala ao proteger os dados em blocos individuais.
...
Você pode atribuir permissões ao grupo de recursos que contém vários dashboards ou a um painel individual. Por exemplo, você pode decidir que um grupo de usuários deve ter permissões limitadas na assinatura, mas um maior acesso a um painel específico. Atribua esses usuários a uma função para esse painel.

## <a name="publish-a-dashboard"></a>Publicar um painel
...
1. Depois de publicar o painel, selecione **gerenciar compartilhamento**.

1. Em **controle de acesso** , selecione **atribuições de função** para ver os usuários existentes que já atribuiram uma função para esse painel.
...
1. Selecione a função que representa as permissões a serem concedidas, como **colaborador**.
