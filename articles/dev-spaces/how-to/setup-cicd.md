...
> [!TIP]
> Se a sua versão falhar com uma mensagem de erro como *FALHA NO UPGRADE: expirou aguardando a condição*, tente inspecionar os pods em seu cluster [usando o painel do Kubernetes](../../aks/kubernetes-dashboard.md). Se você ver que os pods estão falhando em iniciar com mensagens de erro como *Falha em puxar a imagem "azdsexample.azurecr.io/mywebapi:122": erro rpc: código = Desc desconhecido = Resposta de erro do daemon: Obter https:\//azdsexample.azurecr.io/v2/mywebapi/manifests/122: não autorizado: autenticação necessária*, pode ser porque seu cluster não foi autorizado a puxar do seu Registro de Contêiner Azure. Certifique-se de ter concluído o pré-requisito [Autorizar o cluster do AKS a receber do seu Registro de Contêiner do Azure](../../aks/cluster-container-registry-integration.md).
