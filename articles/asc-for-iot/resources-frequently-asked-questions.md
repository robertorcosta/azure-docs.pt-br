...
Embora outras soluções forneçam um conjunto de recursos que permitem que os clientes criem suas próprias soluções, o Azure Security Center for IoT fornece uma solução única de segurança de IoT de ponta a ponta que fornece uma visão ampla sobre a segurança de todos os seus recursos azure relacionados. O Azure permite uma implantação rápida e integração total com os gêmeos do módulo IoT Hub para facilitar a integração com as ferramentas de gerenciamento de dispositivos existentes.
...
1. Crie seu próprio agente e implemente o Microsoft Azure Security Center para esquema de mensagem de segurança IoT. Essa opção permite o uso do Microsoft Azure Security Center para ferramentas de análise de IoT em cima do agente de segurança do dispositivo.

1. Não há instalação de agentes de segurança em seus dispositivos IoT. Essa opção permite o monitoramento de comunicação do IoT Hub, com recursos reduzidos de monitoramento e gerenciamento de segurança.
...
1. Verifique se o [serviço foi habilitado com sucesso](quickstart-onboard-iot-hub.md) para a **segurança** em seu Hub IoT.

1. Verifique se o dispositivo está [configurado no IoT Hub com o módulo Azure Security Center for IoT](quickstart-create-security-twin.md).
