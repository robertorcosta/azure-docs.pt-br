...
---
# <a name="administer-dns-and-create-conditional-forwarders-in-an-azure-ad-domain-services-managed-domain"></a>Administrar o DNS e criar encaminhadores condicionais em um Azure AD Domain Services domínio gerenciado
...
Em um ambiente híbrido, as zonas e os registros DNS configurados em outros namespaces DNS, como um ambiente de AD DS local, não são sincronizados com o Azure AD DS. Para resolver os recursos nomeados em outros namespaces DNS, crie e use encaminhadores condicionais que apontam para servidores DNS existentes em seu ambiente.
...
1. Na caixa de diálogo **conectar ao servidor DNS** , selecione **o seguinte computador**e, em seguida, insira o nome de domínio DNS do domínio gerenciado, como *aaddscontoso.com*:
    ...
    ![Conectar-se ao domínio gerenciado AD DS do Azure no console DNS](./media/manage-dns/connect-dns-server.png)
...
Uma zona DNS AD DS do Azure deve conter apenas a zona e os registros para o próprio domínio gerenciado. Não crie zonas adicionais no Azure AD DS para resolver recursos nomeados em outros namespaces DNS. Em vez disso, use encaminhadores condicionais no domínio gerenciado AD DS do Azure para informar ao servidor DNS onde ir para resolver os endereços desses recursos.

Um encaminhador condicional é uma opção de configuração em um servidor DNS que permite definir um domínio DNS, como *contoso.com*, para encaminhar consultas para o. Em vez do servidor DNS local que está tentando resolver consultas para registros nesse domínio, as consultas DNS são encaminhadas para o DNS configurado para esse domínio. Essa configuração garante que os registros DNS corretos sejam retornados, pois você não cria uma zona DNS local com registros duplicados no domínio gerenciado AD DS do Azure para refletir esses recursos.

Para criar um encaminhador condicional em seu domínio gerenciado AD DS do Azure, conclua as seguintes etapas:

1. Selecione sua zona DNS do Azure AD DS, como *aaddscontoso.com*. vb
...
A resolução de nomes dos recursos em outros namespaces de VMs conectadas ao domínio gerenciado do Azure AD DS agora deve ser resolvida corretamente. As consultas para o domínio DNS configurado no encaminhador condicional são passadas para os servidores DNS relevantes.
