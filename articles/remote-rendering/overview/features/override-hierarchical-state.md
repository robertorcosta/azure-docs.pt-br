...
* **See-through**: a geometria é renderizada de forma semitransparente, por exemplo, para revelar as partes internas de um objeto. A seguinte imagem mostra todo o carro sendo renderizado no modo de transparência, exceto a pinça de freio vermelha:
...
* **Selected**: a geometria é renderizada com um [contorno de seleção](outlines.md).

  ![Contorno de seleção](./media/selection-outline.png)

* **DisableCollision**: a geometria é isenta de [consultas espaciais](spatial-queries.md). O sinalizador **Hidden** não desativa as colisões, de modo que esses dois sinalizadores geralmente são definidos juntos.
