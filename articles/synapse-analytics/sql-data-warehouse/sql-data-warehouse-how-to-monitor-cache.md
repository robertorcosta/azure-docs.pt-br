...
Você pode usar o Azure Monitor para exibir métricas do cache Gen2 para solucionar problemas de desempenho de consulta. Primeiro vá para o portal Azure e clique em **Monitor,** **Métricas** e **+ Selecione um escopo**:

![Azure Monitor](./media/sql-data-warehouse-how-to-monitor-cache/cache-0.png)

Use as barras de pesquisa e drop down para localizar seu data warehouse. Em seguida, selecione aplicar.

![Azure Monitor](./media/sql-data-warehouse-how-to-monitor-cache/cache-1.png)

As principais métricas para solucionar problemas do cache Gen2 são **Percentual de ocorrência no cache** e **Percentual de uso do cache**. Selecione **A porcentagem de impacto cache** e use o botão adicionar **métrica** para adicionar porcentagem usada **em cache**. 
...
**Cenário 1:** você está usando o cache de forma ideal. [Soluciona problemas de](sql-data-warehouse-manage-monitor.md) outras áreas que podem estar atrasando suas consultas.
...
**Cenário 3:** é provável que sua consulta esteja sendo executada lentamente por motivos não relacionados ao cache. [Soluciona problemas de](sql-data-warehouse-manage-monitor.md) outras áreas que podem estar atrasando suas consultas. Você também pode considerar [reduzir verticalmente sua instância](sql-data-warehouse-manage-monitor.md) para reduzir o tamanho do cache e economizar custos. 
...
> [!IMPORTANT]
> Se a porcentagem de acerto de cache ou porcentagem usada em cache não estiver atualizada após a reexecução da carga de trabalho, o conjunto de trabalho já pode estar residindo na memória. Apenas as tabelas de columnstore agrupadas são armazenadas em cache.
