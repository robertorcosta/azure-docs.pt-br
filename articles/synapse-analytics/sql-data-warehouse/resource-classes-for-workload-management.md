...
description: Orientação para o uso de classes de recursos para gerenciar recursos de concorrência e computação para consultas no Azure Synapse Analytics.
...
- Classes de recursos dinâmicos, que são adequadas para conjuntos de dados que estão crescendo em tamanho e precisam de maior desempenho à medida que o nível de serviço é aumentado.
...
- Para exibir a utilização de recursos para as classes de recursos, consulte [Limites de memória e simultaneidade](memory-concurrency-limits.md).
...
- TABELA DE ALTER ... PARTIÇÃO DE SWITCH, SPLIT OU MERGE
- ALTER INDEX DISABLE
- DROP INDEX
...
Apenas consultas governadas por recurso consomem slots de simultaneidade. Consultas de sistema e algumas consultas triviais não consomem nenhum slots. O número exato de slots de simultaneidade consumidos é determinado pela classe de recurso da consulta.
...
Classes de recursos são implementadas atribuindo usuários a funções de banco de dados. Quando um usuário executa uma consulta, a consulta é executada com a classe de recurso do usuário. Por exemplo, se um usuário é um membro da função de banco de dados staticrc10, suas consultas são executadas com pequenas quantidades de memória. Se um usuário de banco de dados for um membro das funções de banco de dados xlargerc ou staticrc80, suas consultas serão executadas com grandes quantidades de memória.

Para aumentar a classe de recursos de um usuário, use [sp_addrolemember](/sql/relational-databases/system-stored-procedures/sp-addrolemember-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest) para adicionar o usuário a uma função de banco de dados de uma grande classe de recursos.  O código abaixo adiciona um usuário à função de banco de dados largerc.  Cada pedido recebe 22% da memória do sistema.

```sql
EXEC sp_addrolemember 'largerc', 'loaduser';
```

Para diminuir a classe de recurso, use [sp_droprolemember](/sql/relational-databases/system-stored-procedures/sp-droprolemember-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest).  Se 'loaduser' não for um membro ou qualquer outra classe de recursos, eles entrarão na classe de recursos smallrc padrão com uma concessão de memória de 3%.  
...
>[!NOTE]
>Considere aproveitar os recursos de gerenciamento de carga[de trabalho (isolamento da carga de trabalho,](sql-data-warehouse-workload-isolation.md) [classificação](sql-data-warehouse-workload-classification.md) e [importância)](sql-data-warehouse-workload-importance.md)para maior controle sobre sua carga de trabalho e desempenho previsível.  
>
>

Recomendamos a criação de um usuário dedicado a executar um tipo específico de consulta ou operação de carga. Dê a esse usuário uma classe de recursos permanente em vez de alterar a classe de recursos com freqüência. As classes de recursos estáticos oferecem maior controle geral sobre a carga de trabalho, por isso sugerimos usar classes de recursos estáticos antes de considerar classes de recursos dinâmicos.
...
- Use uma classe de recursos dinâmicos quando os requisitos de memória de tabela variarem muito. As cargas podem exigir mais memória do que a DWU atual ou que o nível de cDWU fornece. O dimensionamento do data warehouse adiciona mais memória às operações de carga, o que permite que as cargas sejam realizadas mais rapidamente.
...
Você pode usar o seguinte procedimento armazenado especificado para descobrir a concessão de moeda e memória por classe de recurso em uma determinada SLO e a melhor classe de recursos para operações de CCI intensivas de memória na tabela CCI não particionada em uma determinada classe de recursos:
...
2. Para ver a melhor classe de recursos para as operações de CCI com uso intensivo de memória (carga, tabela de cópia, índice de reconstrução, etc.) na tabela CCI não particionada em uma determinada classe de recursos. O procedimento armazenado usa o esquema da tabela para descobrir a concessão de memória necessária.
...
- Este procedimento armazenado depende das ofertas atuais (por exemplo, configuração de hardware, configuração de DMS), e se alguma dessas alterações, então este proc armazenado não funcionará corretamente.  
- Este procedimento armazenado depende das ofertas de limite de concorrência existentes e, se essas alterações, esse procedimento armazenado não funcionará corretamente.  
- Este procedimento armazenado depende das ofertas de classe de recursos existentes e, se essas alterações, esse procedimento armazenado não funcionará corretamente.  
...
>
>Por exemplo, no DW100c, a maior concessão de memória disponível é de 1 GB, e se o esquema de tabela for largo o suficiente para cruzar a exigência de 1 GB.
