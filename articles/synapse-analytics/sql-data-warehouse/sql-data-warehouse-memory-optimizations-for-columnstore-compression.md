...
O Dmv sys.dm_pdw_nodes_db_column_store_row_group_physical_stats ([sys.dm_db_column_store_row_group_physical_stats](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-db-column-store-row-group-physical-stats-transact-sql) contém a definição de exibição correspondente ao SQL DB) que expõe informações úteis, como número de linhas em grupos de linhas e a razão para aparar, se houve corte. 
