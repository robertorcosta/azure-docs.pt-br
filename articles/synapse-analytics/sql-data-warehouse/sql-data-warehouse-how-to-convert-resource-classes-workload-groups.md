...
Anteriormente, o mapeamento de consultas às classes de recursos era feito com [sp_addrolemember](https://docs.microsoft.com/azure/sql-data-warehouse/resource-classes-for-workload-management#change-a-users-resource-class).  Para obter a mesma funcionalidade e mapear solicitações para grupos de carga de trabalho, use a sintaxe [CREATE WORKLOAD CLASSIFIER.](https://docs.microsoft.com/sql/t-sql/statements/create-workload-classifier-transact-sql)  O uso sp_addrolemember só permitia mapear recursos para uma solicitação com base em um login.  Um classificador oferece opções adicionais além do login, tais como:
    - label
    - sessão
    - tempo O exemplo abaixo atribui `AdfLogin` consultas do login que também `factloads` têm o `wgDataLoads` [RÓTULO OPÇÃO](https://docs.microsoft.com/azure/sql-data-warehouse/sql-data-warehouse-develop-label) definido para o grupo de carga de trabalho criado acima.
...
Carga de trabalho link de isolamento tbd Como criar um link de grupo de carga de trabalho tbd