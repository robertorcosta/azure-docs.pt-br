...
1. Selecione **Blockchain** > **Azure Blockchain Workbench (versão prévia)**.
    ...
    | Local | Especifique a região em que você deseja implantar a estrutura. |
...
1. Um GUID é necessário para o manifesto. Você pode gerar um GUID usando o comando `[guid]::NewGuid()` ou `New-GUID` o cmdlet do PowerShell. Outra opção é usar um site gerador de GUID.
1. Para o aplicativo que você registrou, selecione **manifesto** na seção **gerenciar** .
1. Em seguida, atualize a seção **appRoles** do manifesto. Substituir `"appRoles": []` pelo JSON fornecido. Certifique-se de substituir o valor do campo **id** pelo GUID gerado. 
...
1. No painel de navegação esquerdo, selecione o serviço do **Azure Active Directory**. Selecione **registros de aplicativo**.
