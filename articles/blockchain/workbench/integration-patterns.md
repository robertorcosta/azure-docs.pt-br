...
description: Visão geral dos padrões inteligentes de integração de contratos no Azure Blockchain Workbench Preview.
ms.date: 11/20/2019
ms.topic: conceptual
...
Por exemplo, se a temperatura de um caminhão de entrega de medicamentos dispara a 110 graus, pode afetar a eficiência do medicamento e pode causar um problema de segurança pública se não for detectado e removida da cadeia de fornecimento. Se um motorista acelerasse seu carro a 160 km/h, as informações resultantes do sensor poderiam desencadear um cancelamento do seguro pelo seu provedor de seguros. Se um carro for um carro alugado, dados de GPS poderão indicar quando o motorista saiu da área coberta pelo seu contrato de aluguel e cobrar uma multa.
