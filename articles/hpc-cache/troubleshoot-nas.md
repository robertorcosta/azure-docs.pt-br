...
Verifique essas configurações no próprio NAS, bem como em todos os firewalls entre o sistema de armazenamento e a sub-rede de cache.
...
* Os servidores Linux geralmente adicionam ``no_root_squash`` ao caminho exportado no ``/etc/exports``.
...
O ``/ifs/accounting/payroll`` de exportação é um filho de ``/ifs/accounting``e ``/ifs/accounting`` próprio é um filho de ``/ifs``.

Se você adicionar a exportação de ``payroll`` como um destino de armazenamento de cache do HPC, o cache realmente montará ``/ifs/`` e acessará o diretório de folha de pagamento a partir daí. Portanto, o cache HPC do Azure precisa de acesso à raiz para ``/ifs`` para acessar a ``/ifs/accounting/payroll`` exportação.
...
* Se a sua VPN permitir comandos ping, você poderá testar o envio de um pacote de tamanho completo.
  ...
  * ``-s 1472``-defina o tamanho da carga como 1472 bytes. Essa é a carga de tamanho máximo para um pacote de 1500 bytes após a contabilização da sobrecarga de Ethernet.
  ...
  Se o ping falhar com 1472 bytes, talvez seja necessário configurar o MSS fixação MSS na VPN para fazer com que o sistema remoto detecte corretamente o tamanho máximo do quadro. Leia a [documentação de parâmetros de IPSec/IKE do gateway de VPN](../vpn-gateway/vpn-gateway-about-vpn-devices.md#ipsec) para saber mais.
