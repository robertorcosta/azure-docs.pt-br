---
title: Gerenciar servidor-CLI do Azure-banco de dados do Azure para MySQL
description: Saiba como gerenciar um servidor de banco de dados do Azure para MySQL do CLI do Azure.
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: how-to
...
ms.locfileid: "90933741"
---
# <a name="manage-an-azure-database-for-mysql-single-server-using-the-azure-cli"></a>Gerenciar um servidor único do banco de dados do Azure para MySQL usando o CLI do Azure
...
Se você ainda não criou um servidor, consulte este guia de [início rápido](quickstart-create-mysql-server-database-using-azure-cli.md) para criar um.

## <a name="scale-compute-and-storage"></a>Dimensionar a computação e o armazenamento
Você pode escalar verticalmente seu tipo de preço, computação e armazenamento facilmente usando o comando a seguir. Você pode ver toda a operação do servidor que pode executar [AZ MySQL Server Overview](/cli/azure/mysql/server?view=azure-cli-latest)
...
> - Não há suporte para escalar verticalmente do tipo de preço básico para uso geral ou com otimização de memória. Você pode escalar verticalmente com o  [uso de um script bash](https://techcommunity.microsoft.com/t5/azure-database-for-mysql/upgrade-from-basic-to-general-purpose-or-memory-optimized-tiers/ba-p/830404) ou [usando o MySQL Workbench](https://techcommunity.microsoft.com/t5/azure-database-support-blog/how-to-scale-up-azure-database-for-mysql-from-basic-tier-to/ba-p/369134)
...
| --- | ---| --- |
|[criar banco de BD AZ MySQL](/cli/azure/sql/db#az-mysql-db-create)|```az mysql db create -g myresourcegroup -s mydemoserver -n mydatabasename``` |Cria um banco de dados|
|[excluir banco de BD AZ MySQL](/cli/azure/sql/db#az-mysql-db-delete)|```az mysql db delete -g myresourcegroup -s mydemoserver -n mydatabasename```|Exclua seu banco de dados do servidor. Esse comando não exclui o servidor. |
|[az mysql db list](/cli/azure/sql/db#az-mysql-db-list)|```az mysql db list -g myresourcegroup -s mydemoserver```|lista todos os bancos de dados no servidor|
|[AZ MySQL DB show](/cli/azure/sql/db#az-mysql-db-show)|```az mysql db show -g myresourcegroup -s mydemoserver -n mydatabasename```|Mostra mais detalhes do banco de dados|
...
Se você quiser apenas excluir o servidor único MySQL, poderá executar o comando [AZ MySQL Server Delete](/cli/azure/mysql/server#az-mysql-server-delete) .
