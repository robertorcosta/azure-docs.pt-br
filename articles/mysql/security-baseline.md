---
title: Linha de base de segurança do Azure para banco de dados Azure para MySQL
description: Linha de base de segurança do Azure para banco de dados Azure para MySQL
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 03/23/2020
...
ms.locfileid: "81261951"
---
# <a name="azure-security-baseline-for-azure-database-for-mysql"></a>Linha de base de segurança do Azure para banco de dados Azure para MySQL

A linha de base de segurança do Azure para o MySQL contém recomendações que o ajudarão a melhorar a postura de segurança de sua implantação.
...
**Orientação**: Configure o link privado para o banco de dados Azure para MySQL com pontos finais privados. O Link Privado permite que você se conecte a vários serviços de PaaS no Azure por meio de um ponto de extremidade privado. O Azure Private Link essencialmente traz os serviços do Azure dentro da sua Rede Virtual privada (VNet). O tráfego entre sua rede virtual e a instância MySQL viaja pela rede backbone da Microsoft.

Alternativamente, você pode usar os Pontos Finais do Serviço de Rede Virtual para proteger e limitar o acesso da rede ao seu banco de dados Azure para implementações MySQL. As regras da rede virtual são um recurso de segurança de firewall que controla se o servidor do Banco de Dados do Azure para MySQL aceita comunicações que sejam enviadas de sub-redes particulares em redes virtuais.

Você também pode proteger seu banco de dados Azure para o servidor MySQL com regras de firewall. O firewall do servidor impede todo o acesso ao servidor de banco de dados até que você especifique quais computadores têm permissão. Para configurar seu firewall, você deve criar regras de firewall que especifiquem intervalos de endereços IP aceitáveis. Você pode criar regras de firewall no nível de servidor.

Como configurar o Private Link for Azure Database para MySQL:https://docs.microsoft.com/azure/mysql/howto-configure-privatelink-portal

Como criar e gerenciar os pontos finais de serviço vnet e as regras do VNet no Banco de Dados Azure para MySQL:https://docs.microsoft.com/azure/sql-database/sql-database-vnet-service-endpoint-rule-overview

Como configurar o Banco de Dados Azure para regras de firewall MySQL:https://docs.microsoft.com/azure/mysql/howto-manage-firewall-using-portal
...
**Orientação**: Use proteção avançada contra ameaças para o banco de dados Azure para MySQL. O Advanced Threat Protection detecta atividades anômalas indicando tentativas incomuns e potencialmente prejudiciais de acessar ou explorar bancos de dados.

Habilite o Padrão de Proteção DDoS nas Redes Virtuais associadas ao banco de dados Do Azure para instâncias MySQL para se proteger contra ataques DDoS. Use o Azure Security Center Integrated Threat Intelligence para negar comunicações com endereços IP da Internet mal-intencionados ou não usados.

Como configurar o Advanced Threat Protection for Azure Database for MySQL:https://docs.microsoft.com/azure/mysql/howto-database-threat-protection-portal
...
Entenda o uso da tag de serviço para o Banco de Dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/concepts-data-access-and-security-vnet#terminology-and-description
...
**Orientação**: Defina e implemente configurações de segurança padrão para configurações de rede e recursos de rede associados ao banco de dados Azure para instâncias MySQL com a Diretiva Azure. Use aliases de diretiva do Azure nos espaços de nome "Microsoft.DBforMySQL" e "Microsoft.Network" para criar políticas personalizadas para auditar ou impor a configuração de rede do banco de dados Do Azure para instâncias Do MySQL. Você também pode fazer uso de definições de diretiva incorporadas relacionadas ao networking ou ao seu banco de dados Azure para instâncias Do MySQL, tais como:
...
**Orientação**: Use tags para recursos relacionados à segurança da rede e fluxo de tráfego para o banco de dados Do Azure para instâncias MySQL para fornecer metadados e organização lógica.
...
**Orientação**: Use o Azure Activity Log para monitorar configurações de recursos de rede e detectar alterações para recursos de rede relacionados ao banco de dados Do Azure para instâncias Do MySQL. Crie alertas no Azure Monitor que serão acionados quando ocorrerem alterações nos recursos críticos da rede.
...
**Orientação**: A Microsoft mantém a fonte de tempo usada para os recursos do Azure, como o Azure Database for MySQL para carimbos de tempo nos logs.
...
**Orientação**: Habilite configurações de diagnóstico e logs de servidor e ingerir logs para agregar dados de segurança gerados pelo banco de dados Do Azure para instâncias Do MySQL. No Azure Monitor, use o Log Analytics Workspace(s) para consultar e executar análises e use contas de armazenamento do Azure para armazenamento de longo prazo/arquivamento. Alternativamente, você pode habilitar e a bordo dados para o Azure Sentinel ou um SIEM de terceiros.

Entenda os registros do servidor para o Banco de Dados Do Azure para MySQL:https://docs.microsoft.com/azure/mysql/concepts-monitoring#server-logs
...
**Orientação**: Habilite as configurações de diagnóstico no banco de dados Do Azure para instâncias MySQL para acesso a auditoria, consulta lenta e registros de métricas MySQL. Certifique-se de que você habilite especificamente o registro de auditoria MySQL. Os registros de atividades, que estão disponíveis automaticamente, incluem origem do evento, data, usuário, carimbo de data, endereços de origem, endereços de destino e outros elementos úteis. Você também pode ativar as configurações de diagnóstico do Log de atividade do Azure e enviar os logs para o mesmo espaço de trabalho do Log Analytics ou da conta de armazenamento.

Entenda os registros do servidor para o Banco de Dados Do Azure para MySQL:https://docs.microsoft.com/azure/mysql/concepts-monitoring#server-logs

Como configurar e acessar registros de consulta lenta satisfações para o Azure Database for MySQL:https://docs.microsoft.com/azure/mysql/howto-configure-server-logs-in-portal

Como configurar e acessar logs de auditoria para o Banco de Dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/howto-configure-audit-logs-portal
...
**Orientação**: No Monitor Do Azure, para que o espaço de trabalho do Log Analytics seja usado para manter seu banco de dados Azure para logs MySQL, defina o período de retenção de acordo com os regulamentos de conformidade da sua organização. Use contas de armazenamento do Azure para armazenamento de longo prazo/arquivamento.
...
**Orientação**: Analise e monitore registros do banco de dados Do Azure para instâncias Do MySQL para comportamento anômalo. Use o Log Analytics do Azure Monitor para revisar logs e executar consultas em dados de log. Alternativamente, você pode habilitar e a bordo dados para o Azure Sentinel ou um SIEM de terceiros.
...
**Orientação**: Habilite a Proteção Avançada contra Ameaças para o Banco de Dados Azure para MySQL. O Advanced Threat Protection detecta atividades anômalas indicando tentativas incomuns e potencialmente prejudiciais de acessar ou explorar bancos de dados.

Além disso, você pode habilitar logs de servidor e configurações de diagnóstico para MySQL e enviar logs para um espaço de trabalho do Log Analytics. A bordo do espaço de trabalho do Log Analytics para o Azure Sentinel, pois ele fornece uma solução de resposta automatizada de orquestração de segurança (SOAR). Isso permite que as cartilhas (soluções automatizadas) sejam criadas e usadas para corrigir problemas de segurança.

Como ativar o Advanced Threat Protection for Azure Database for MySQL (Preview):https://docs.microsoft.com/azure/mysql/howto-database-threat-protection-portal
...
**Orientação**: Não aplicável; O Banco de Dados Do Azure para MySQL não processa ou produz logs relacionados a malware.
...
**Orientação**: Não aplicável; O Banco de Dados Azure para MySQL não processa ou produz registros relacionados ao DNS.
...
**Orientação**: Mantenha um inventário das contas de usuário que tenham acesso administrativo ao plano de gerenciamento (por exemplo, portal Azure) do seu Banco de Dados Azure para MySQLinstances. Além disso, mantenha um inventário das contas administrativas que tenham acesso ao plano de dados (dentro do próprio banco de dados) do seu Banco de Dados Azure para instâncias Do MySQL. (Ao criar o servidor MySQL, você fornece credenciais para um usuário administrador. Este administrador pode ser usado para criar usuários adicionais do MySQL.)

O Azure Database for MySQL não suporta controle de acesso integrado baseado em funções, mas você pode criar funções personalizadas com base em opções específicas de provedor de recursos.
...
Entenda o Banco de Dados Do Azure para operações de provedores de recursos MySQL:https://docs.microsoft.com/azure/role-based-access-control/resource-provider-operations#microsoftdbformysql

Entenda o gerenciamento de acesso para o Banco de Dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/concepts-security#access-management
...
**Orientação**: O Azure AD não tem o conceito de senhas padrão.

Após a criação do próprio recurso Azure para o mySQL, o Azure força a criação de um usuário administrativo com uma senha forte. No entanto, uma vez criada a instância MySQL, você pode usar a primeira conta de administração do servidor criada para criar usuários adicionais e conceder acesso administrativo a eles. Ao criar essas contas, certifique-se de configurar uma senha diferente e forte para cada conta.

Como criar contas adicionais para o Azure Database for MySQL:https://docs.microsoft.com/azure/mysql/howto-create-users

Como atualizar a senha do admin:https://docs.microsoft.com/azure/mysql/howto-create-manage-server-portal#update-admin-password
...
**Orientação**: Crie procedimentos operacionais padrão em torno do uso de contas administrativas dedicadas que tenham acesso ao seu banco de dados Azure para instâncias MySQL. Use o Azure Security Center Identity e o gerenciamento de acesso para monitorar o número de contas administrativas.
...
Entenda como criar usuários de administradores no Banco de Dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/howto-create-users
...
**Orientação**: A assinatura no Banco de Dados Azure para MySQL é suportada tanto usando nome de usuário/senha configurado diretamente no banco de dados, quanto usando uma identidade Azure Active Directory (AD) e utilizando um token Azure AD para se conectar. Ao usar um token Azure AD, diferentes métodos são suportados, como um usuário azure AD, um grupo Azure AD ou um aplicativo Azure AD conectado ao banco de dados.

Separadamente, o acesso ao plano de controle para MySQL está disponível via API REST e suporta SSO. Para autenticar, defina o cabeçalho de autorização para suas solicitações em um Token Web JSON que você obtém do Azure Active Directory.

Use o Azure Active Directory para autenticação com o Banco de Dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/howto-configure-sign-in-azure-ad-authentication

Entenda o banco de dados do Azure para a API MySQL REST:https://docs.microsoft.com/rest/api/mysql/
...
**Orientação**: Use estações de trabalho de acesso privilegiado (PAWs) com MFA (Multi-Factor Authentication, autenticação multifatorial) configuradas para fazer login e configurar recursos do Azure.
...
**Orientação**: Habilite a Proteção Avançada contra Ameaças para o Banco de Dados Azure para MySQL para gerar alertas para atividades suspeitas.

Além disso, você pode usar o Azure AD Privileged Identity Management (PIM) para geração de registros e alertas quando atividades suspeitas ou inseguras ocorrem no ambiente.
...
Para entrar no Azure Database for MySQL, recomenda-se usar o Azure AD e utilizar um token Azure AD para conectar. Ao usar um token Azure AD, diferentes métodos são suportados, como um usuário azure AD, um grupo Azure AD ou um aplicativo Azure AD conectado ao banco de dados. 

As credenciais Azure AD também podem ser usadas para administração no nível do plano de gerenciamento (por exemplo, o portal Azure) para controlar as contas administrativas do MySQL.
...
**Orientação**: Revise os registros do Azure Active Directory para ajudar a descobrir contas obsoletas que podem incluir aquelas com banco de dados Azure para funções administrativas MySQL. Além disso, use o Azure Identity Access Reviews para gerenciar com eficiência as associações de grupos, o acesso a aplicativos corporativos que podem ser usados para acessar o Banco de Dados Do Azure para MySQL e atribuições de função. O acesso do usuário deve ser revisto regularmente, como a cada 90 dias, para garantir que apenas os Usuários tenham acesso contínuo.

Entenda o relatório do Azure ADhttps://docs.microsoft.com/azure/active-directory/reports-monitoring/
...
**Orientação**: Habilite as configurações de diagnóstico para o Azure Database para MySQL e Azure Active Directory, enviando todos os logs para um espaço de trabalho do Log Analytics. Configure os alertas desejados (como tentativas de autenticação falhadas) no Log Analytics.
...
**Orientação**: Não aplicável; O Customer Lockbox ainda não tem suporte para o Azure Database for MySQL.
...
**Orientação**: Use tags para ajudar no rastreamento do Banco de Dados Azure para instâncias MySQL ou recursos relacionados que armazenam ou processam informações confidenciais.
...
**Orientação**: Implementar assinaturas separadas e/ou grupos de gerenciamento para desenvolvimento, teste e produção. Use uma combinação de regras de Link Privado, Pontos finais de serviço e/ou firewall para isolar e limitar o acesso à rede ao seu banco de dados Azure para instâncias Do MySQL.
...
Como configurar o Private Link for Azure Database para MySQL:https://docs.microsoft.com/azure/mysql/concepts-data-access-security-private-link

Como criar e gerenciar os pontos finais de serviço vnet e as regras do VNet no Banco de Dados Azure para MySQL:https://docs.microsoft.com/azure/sql-database/sql-database-vnet-service-endpoint-rule-overview

Como configurar o Banco de Dados Azure para regras de firewall MySQL:https://docs.microsoft.com/azure/mysql/concepts-firewall-rules
...
**Orientação**: Ao usar as VMs do Azure para acessar o Banco de Dados Azure para instâncias MySQL, faça uso de private link, configurações de rede MySQL, grupos de segurança de rede e tags de serviço para mitigar a possibilidade de exfiltração de dados.

A Microsoft gerencia a infra-estrutura subjacente para o Azure Database for MySQL e implementou controles rigorosos para evitar a perda ou exposição de dados do cliente.

Como mitigar a exfiltração de dados para o Banco de Dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/concepts-data-access-security-private-link#data-exfiltration-prevention
...
**Orientação**: O Banco de Dados Azure para MySQL suporta conectar seu servidor MySQL a aplicativos clientes usando o Secure Sockets Layer (SSL). Impor conexões SSL entre seu servidor de banco de dados e os aplicativos cliente ajuda a proteger contra ataques de "intermediários" criptografando o fluxo de dados entre o servidor e seu aplicativo. No portal Azure, certifique-se de que "Impor conexão SSL" esteja habilitado para todo o seu Banco de Dados Azure para instâncias MySQL por padrão.

Atualmente, a versão TLS suportada para O Banco de Dados Azure para MySQL são TLS 1.0, TLS 1.1, TLS 1.2.

Como configurar criptografia em trânsito para o Banco de Dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/concepts-ssl-connection-security
...
**Orientação**: Os recursos de identificação, classificação e prevenção de perdas ainda não estão disponíveis para o Banco de Dados Azure para MySQL. Implementar solução de terceiros, se necessário para fins de conformidade.
...
**Orientação**: Use o Controle de Acesso Baseado em Função (RBAC) do Azure para controlar o acesso ao banco de dados Azure para o plano de controle MySQL (por exemplo, portal Azure). Para acesso a plano de dados (dentro do próprio banco de dados), use consultas SQL para criar usuários e configurar permissões de usuário. O RBAC não afeta as permissões do usuário no banco de dados.

Como configurar o RBAC no Azure:https://docs.microsoft.com/azure/role-based-access-control/role-assignments-portal

Como configurar o acesso do usuário com O SQL para O Banco de Dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/howto-create-users
...
**Orientação**: O banco de dados Azure para o serviço MySQL usa o módulo criptográfico validado fips 140-2 para criptografia de armazenamento de dados em repouso. Os dados, incluindo backups, são criptografados em disco, com exceção de arquivos temporários criados durante a execução de consultas. O serviço usa a criptografia AES de 256 bits incluída na criptografia de armazenamento do Azure e as chaves são gerenciadas pelo sistema. A criptografia de armazenamento está sempre ativada e não pode ser desabilitada.

A criptografia de dados com chaves gerenciadas pelo cliente para o Azure Database for MySQL permite que você traga sua própria chave (BYOK) para proteção de dados em repouso. Neste momento, você deve solicitar acesso para usar esse recurso. Para isso, entre em contato:
...
Como configurar chaves gerenciadas pelo cliente para o banco de dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/concepts-data-encryption-mysql
...
**Orientação**: Use o Monitor Azure com o Registro de Atividades do Azure para criar alertas para quando as alterações ocorrerem nas instâncias de produção do Banco de Dados Azure para MySQL e outros recursos críticos ou relacionados.
...
**Orientação**: Atualmente não disponível; O Azure Security Center ainda não suporta avaliação de vulnerabilidades para o Azure Database for MySQL.
...
**Orientação**: A Microsoft executa o gerenciamento de vulnerabilidades nos sistemas subjacentes que suportam o Azure Database for MySQL.
...
**Orientação**: Aplique tags no Banco de Dados Azure para instâncias MySQL e outros recursos relacionados que dão metadados para organizá-los logicamente em uma taxonomia.
...
**Orientação**: Use tagging, grupos de gerenciamento e assinaturas separadas, quando apropriado, para organizar e rastrear o Banco de Dados Do Azure para instâncias do MySQL e recursos relacionados. Reconcilie o inventário regularmente e garanta que os recursos não autorizados sejam excluídos da assinatura em tempo hábil.
...
**Orientação**: Defina e implemente configurações de segurança padrão para o banco de dados Do Azure para instâncias MySQL com a Política Do Azure. Use aliases de diretiva do Azure no namespace "Microsoft.DBforMySQL" para criar políticas personalizadas para auditar ou impor a configuração de rede do seu banco de dados Azure para instâncias MySQL. Você também pode fazer uso de definições de diretiva incorporadas relacionadas ao seu banco de dados Do Azure para instâncias Do MySQL, tais como:
...
**Orientação**: Se usar definições de diretiva personalizadas do Azure para o banco de dados Do Azure para instâncias Do MySQL e recursos relacionados, use os Repos do Azure para armazenar e gerenciar seu código com segurança.
...
**Orientação**: Use aliases de diretiva azure no namespace "Microsoft.DBforMySQL" para criar políticas personalizadas para alertar, auditar e impor configurações do sistema. Além disso, desenvolva um processo e um pipeline para gerenciar exceções políticas.
...
**Orientação**: Use aliases de diretiva azure no namespace "Microsoft.DBforMySQL" para criar políticas personalizadas para alertar, auditar e impor configurações do sistema. Use a política do Azure [auditoria], [negue], e [implantar se não existir] para aplicar automaticamente configurações para o seu Banco de Dados Azure para instâncias do MySQL e recursos relacionados.
...
**Orientação**: Para que máquinas virtuais do Azure ou aplicativos da Web em execução no Azure App Service sejam usados para acessar seu banco de dados Azure para instâncias MySQL, use identidade de serviço gerenciada em conjunto com o Azure Key Vault para simplificar e proteger o banco de dados Azure para gerenciamento secreto MySQL. Certifique-se de que o Key Vault Soft Delete esteja ativado.
...
**Orientação**: O banco de dados Azure para a instância MySQL suporta a autenticação do Azure Active Directory (em visualização) para acessar bancos de dados.  Ao criar o banco de dados Do Azure para a instância MySQL, você fornece credenciais para um usuário administrador. Este administrador pode ser usado para criar usuários adicionais de banco de dados.  

Para que as máquinas virtuais do Azure ou aplicativos da Web em execução no Azure App Service sejam usados para acessar seu banco de dados Azure para instâncias MySQL, use a Identidade de Serviço Gerenciado em conjunto com o Azure Key Vault para armazenar e recuperar credenciais para o Banco de Dados Azure para a instância MySQL. Certifique-se de que o Key Vault Soft Delete esteja ativado.
...
O anti-malware da Microsoft está habilitado no host subjacente que suporta serviços do Azure (por exemplo, O Banco de Dados Azure para SQL), no entanto, ele não é executado no conteúdo do cliente.
...
**Orientação**: O anti-malware da Microsoft é habilitado no host subjacente que suporta serviços do Azure (por exemplo, O Banco de Dados Azure para MySQL), no entanto, ele não é executado no conteúdo do cliente.

Pré-escaneie qualquer conteúdo que seja carregado para recursos não computados do Azure, como App Service, Data Lake Storage, Blob Storage, Azure Database for MySQL, etc. A Microsoft não pode acessar seus dados nessas instâncias.
...
O anti-malware da Microsoft está habilitado no host subjacente que suporta serviços do Azure (por exemplo, O Banco de Dados Azure para MySQL), no entanto, ele não é executado no conteúdo do cliente.
...
**Orientação**: O Banco de Dados Azure para MySQL cria automaticamente backups de servidor e os armazena em armazenamento localmente redundante ou geo-redundante, de acordo com a escolha do usuário. Os backups podem ser usados para restaurar o servidor pontualmente. Os recursos de backup e restauração são uma parte essencial de qualquer estratégia de continuidade dos negócios, pois eles protegem seus dados contra exclusão ou corrupção acidentais. 

Se usar o Azure Key Vault para armazenar credenciais para o banco de dados Do Azure para instâncias MySQL, certifique backups automatizados regulares de suas chaves. 

Entenda backups do Banco de Dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/howto-restore-server-portal 
...
Teste periodicamente a restauração do banco de dados Azure para instâncias MySQL.

Entenda backup e restauração no Banco de Dados Do Azure para MySQL:https://docs.microsoft.com/azure/mysql/concepts-backup
...
**Orientação**: O Banco de Dados Azure para MySQL recebe backups completos, diferenciais e de registro de transações. Esses backups permitem que você restaure um servidor pontualmente dentro de seu período de retenção de backup configurado. O período de retenção de backup padrão é de sete dias. Você pode, opcionalmente, configurá-lo para até 35 dias. Todos os backups são criptografados usando a criptografia AES de 256 bits. Certifique-se de que o Key Vault Soft Delete esteja ativado.
