...
A biblioteca de `Microsoft.FeatureManagement` inclui dois filtros de recurso:
...
Você registra um filtro de recurso chamando o método `AddFeatureFilter`, especificando o nome do filtro de recursos. Por exemplo, o código a seguir registra `PercentageFilter`:
...
Você pode definir essas configurações para sinalizadores de recursos definidos na configuração do Azure App. Por exemplo, siga estas etapas para usar `PercentageFilter` para habilitar o sinalizador de recurso para 50% das solicitações para um aplicativo Web:
...
1. Clique no menu de contexto ao lado da chave de filtro de recurso. Clique em **Editar parâmetros**.
    ...
    > [!div class="mx-imgBorder"]
    > ![editar parâmetros de filtro de recurso](./media/feature-flag-edit-filter-parameters.png)
