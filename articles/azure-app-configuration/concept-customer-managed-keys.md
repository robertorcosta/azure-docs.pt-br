...
2. Conceda as permissões `GET`de identidade, `WRAP`e `UNWRAP` na política de acesso do Key Vault de destino.
...
- [Criar um aplicativo .NET Framework com a configuração Azure App](quickstart-dotnet-app.md)
- [Criar um aplicativo Spring Java com configuração Azure App](quickstart-java-spring-app.md)
...
1. Crie um Azure Key Vault usando o CLI do Azure.  Observe que tanto `vault-name` quanto `resource-group-name` são fornecidos pelo usuário e devem ser exclusivos.  Usamos `contoso-vault` e `contoso-resource-group` nestes exemplos.
...
1. Crie uma chave de Key Vault. Forneça um `key-name` exclusivo para essa chave e substitua os nomes do Key Vault (`contoso-vault`) criado na etapa 1. Especifique se você prefere `RSA` ou `RSA-HSM` criptografia.
