---
title: Entenda a configuração do aplicativo Azure Configuração loja de valor-chave
description: Entenda como os dados de configuração são armazenados na configuração do aplicativo Azure.
author: lisaguthrie
ms.author: lcozzens
...
A configuração do Aplicativo Azure AD e armazena dados de configuração como pares chave-valor. Os pares de valor-chave são uma representação simples e flexível das configurações de aplicativos usadas pelos desenvolvedores.

## <a name="keys"></a>simétricas

As chaves servem como identificadores para pares de valores-chave e são usadas para armazenar e recuperar valores correspondentes. É uma prática comum organizar chaves em um namespace hierárquico usando um caractere delimitador, como `/` ou `:`. Use uma convenção mais adequada à sua aplicação. A Configuração de Aplicativo trata as chaves como um todo. Ela não analisa as chaves para descobrir como seus nomes são estruturados nem impõem regras a elas.

Aqui estão dois exemplos de nomes-chave estruturados em uma hierarquia:
...
O uso de dados de configuração dentro de frameworks de aplicativos pode ditar esquemas de nomeação específicos para valores-chave. Por exemplo, a estrutura nuvem `Environment` de primavera do Java define recursos que fornecem configurações para um aplicativo de Primavera.  Estes são parametrizados por variáveis que incluem nome e *perfil* *do aplicativo* . As chaves dos dados de configuração relacionados ao Spring Cloud normalmente começarão com esses dois elementos, separadamente por um delimitador.

As chaves armazenadas na Configuração de Aplicativo são cadeias de caracteres baseadas em Unicode que diferenciam maiúsculas de minúsculas. As chaves *app1* e *App1* são distintas em um repositório de Configuração de Aplicativos. Tenha isso em mente ao usar definições de configuração em um aplicativo, pois algumas estruturas manipulam as chaves de configuração sem diferenciar maiúsculas de minúsculas. Não recomendamos o uso de caixa para diferenciar chaves.

Você pode usar qualquer caractere unicode `*` `,`em `\`nomes-chave, exceto para , e .  Se você precisar incluir um desses caracteres `\{Reserved Character}`reservados, escape-o usando . 

Há um limite de tamanho combinado de 10 KB em um par de valor-chave. Esse limite inclui todos os caracteres na chave, seu valor e todos os atributos opcionais associados. Dentro desse limite, você pode ter vários níveis hierárquicos para chaves.
...
* É mais fácil de ler. Delimitadores em um nome de chave hierárquica funcionam como espaços em uma frase. Também fornecem quebras naturais entre palavras.
...
Valores de chave na Configuração de Aplicativo podem, opcionalmente, ter um atributo de rótulo. Os rótulos são usados para diferenciar os valores de chave com a mesma chave. Uma chave *app1* com rótulos *A* e *B* forma duas chaves separadas em um repositório de Configuração de Aplicativos. Por padrão, um valor-chave não tem rótulo. Para referenciar explicitamente um valor-chave `\0` sem um `%00`rótulo, use (URL codificado como ).
...
A configuração do aplicativo não faz a versão de valores-chave automaticamente. Use rótulos como uma maneira de criar várias versões de um valor de chave. Por exemplo, você pode inserir um número de versão do aplicativo ou uma ID de confirmação do Git em rótulos para identificar valores de chave associados a um build de software específico.
...
| `key=abc,xyz` | Combina com os **principais** nomes abc ou **xyz**. Limitado a cinco CSVs |
...
Os dados de configuração armazenados em uma loja de configuração de aplicativos são criptografados em repouso e em trânsito. As chaves não estão criptografadas em repouso. A Configuração de Aplicativo não é uma solução substituta do Azure Key Vault. Não armazene segredos do aplicativo nele.
