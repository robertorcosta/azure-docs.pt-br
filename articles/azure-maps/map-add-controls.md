...
description: Como adicionar controle de zoom, controle de pitch, controle de rotação e um seletor de estilo a um mapa no Microsoft Azure Maps.
author: philmea
ms.author: philmea
ms.date: 07/29/2019
ms.topic: conceptual
...
Este artigo mostra como adicionar controles a um mapa. Você também aprenderá como criar um mapa com todos os controles e um [seletor de estilo.](https://docs.microsoft.com/azure/azure-maps/choose-map-style)

## <a name="add-zoom-control"></a>Adicionar o controle de zoom

Um controle de zoom adiciona botões para ampliar o mapa para dentro e para fora. A amostra de código a seguir cria uma instância da classe [ZoomControl](/javascript/api/azure-maps-control/atlas.control.zoomcontrol) e adiciona-a no canto inferior direito do mapa.
...
Um controle de tom adiciona botões para inclinar o tom para mapear em relação ao horizonte. A amostra de código a seguir cria uma instância da classe [PitchControl.](/javascript/api/azure-maps-control/atlas.control.pitchcontrol) Ele adiciona o PitchControl ao canto superior direito do mapa.
...
Um controle de bússola adiciona um botão para girar o mapa. A amostra de código a seguir cria uma instância da classe [Controle de Bússola](/javascript/api/azure-maps-control/atlas.control.compasscontrol) e adiciona-a no canto inferior esquerdo do mapa.
...
Vários controles podem ser colocados em uma matriz e adicionados ao mapa de uma só vez e posicionados na mesma área do mapa para simplificar o desenvolvimento. O seguinte adiciona os controles de navegação padrão ao mapa usando esta abordagem.
...
A amostra de código a seguir adiciona os controles de zoom, bússola, pitch e seletor de estilo no canto superior direito do mapa. Observe como eles se acumulam automaticamente. A ordem dos objetos de controle no script determina a ordem em que aparecem no mapa. Para alterar a ordem dos controles no mapa, você pode alterar sua ordem na matriz.
...
O controle do seletor de estilo é definido pela classe [StyleControl.](/javascript/api/azure-maps-control/atlas.control.stylecontrol) Para obter mais informações sobre como usar o controle do seletor de estilo, consulte [escolher um estilo de mapa](choose-map-style.md).
...
Consulte as <a href='https://codepen.io/azuremaps/pen/LwBZMx/'>opções</a> de controle de<a href='https://codepen.io/azuremaps'>@azuremaps</a>navegação de caneta pelo Azure Maps ( ) no <a href='https://codepen.io'>CodePen</a>.
</iframe>

Se você quiser criar controles de navegação personalizados, `atlas.Control` crie uma classe que se estenda da classe ou crie um elemento HTML e posicione-o acima da div do mapa. Peça a este controle `setCamera` de iu a função de mapas para mover o mapa. 
