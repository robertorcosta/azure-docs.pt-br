...
|------------|-------------|-------------|
| `['at', number, array]` | objeto | Retrieves an item from an array. |
...
| `['has', string]` | Booliano | Determines if the properties of a feature have the specified property. |
| `['has', string, object]` | Booliano | Determines if the properties of the object have the specified property. |
| `['id']` | value | Gets the feature's ID if it has one. |
| `['length', string | array]` | número | Gets the length of a string or array. |
...
| Expression | Tipo de retorno | Descrição |
|------------|-------------|-------------|
| `['+', number, number, …]` | número | Calculates the sum of the specified numbers. |
| `['-', number]` | número | Subtracts 0 by the specified number. |
| `['-', number, number]` | número | Subtracts the first numbers by the second number. |
| `['*', number, number, …]` | número | Multiplies the specified numbers together. |
| `['/', number, number]` | número | Divides the first number by the second number. |
| `['%', number, number]` | número | Calculates the remainder when dividing the first number by the second number. |
| `['^', number, number]` | número | Calculates the value of the first value raised to the power of the second number. |
| `['abs', number]` | número | Calculates the absolute value of the specified number. |
| `['acos', number]` | número | Calculates the arccosine of the specified number. |
| `['asin', number]` | número | Calculates the arcsine of the specified number. |
| `['atan', number]` | número | Calculates the arctangent of the specified number. |
| `['ceil', number]` | número | Rounds the number up to the next whole integer. |
| `['cos', number]` | número | Calculates the cos of the specified number. |
| `['e']` | número | Returns the mathematical constant `e`. |
| `['floor', number]` | número | Rounds the number down to the previous whole integer. |
| `['ln', number]` | número | Calculates the natural logarithm of the specified number. |
| `['ln2']` | número | Returns the mathematical constant `ln(2)`. |
| `['log10', number]` | número | Calculates the base-ten logarithm of the specified number. |
| `['log2', number]` | número | Calculates the base-two logarithm of the specified number. |
| `['max', number, number, …]` | número | Calculates the maximum number in the specified set of numbers. |
| `['min', number, number, …]` | número | Calculates the minimum number in the specified set of numbers. |
| `['pi']` | número | Returns the mathematical constant `PI`. |
| `['round', number]` | número | Rounds the number to the nearest integer. Halfway values are rounded away from zero. For example, `['round', -1.5]` evaluates to -2. |
| `['sin', number]` | número | Calculates the sine of the specified number. |
| `['sqrt', number]` | número | Calculates the square root of the specified number. |
| `['tan', number]` | número | Calculates the tangent of the specified number. |
...
| Expression | Tipo de retorno | Descrição |
|------------|-------------|-------------|
| `['! ', boolean]` | Booliano | Logical negation. Returns `true` if the input is `false`, and `false` if the input is `true`. |
| `['!= ', value, value]` | Booliano | Returns `true` if the input values are not equal, `false` otherwise. |
| `['<', value, value]` | Booliano | Returns `true` if the first input is strictly less than the second, `false` otherwise. The arguments are required to be either both strings or both numbers. |
| `['<=', value, value]` | Booliano | Returns `true` if the first input is less than or equal to the second, `false` otherwise. The arguments are required to be either both strings or both numbers. |
| `['==', value, value]` | Booliano | Returns `true` if the input values are equal, `false` otherwise. The arguments are required to be either both strings or both numbers. |
| `['>', value, value]` | Booliano | Returns `true` if the first input is strictly greater than the second, `false` otherwise. The arguments are required to be either both strings or both numbers. |
| `['>=' value, value]` | Booliano | Returns `true` if the first input is greater than or equal to the second, `false` otherwise. The arguments are required to be either both strings or both numbers. |
| `['all', boolean, boolean, …]` | Booliano | Returns `true` if all the inputs are `true`, `false` otherwise. |
| `['any', boolean, boolean, …]` | Booliano | Returns `true` if any of the inputs are `true`, `false` otherwise. |
...
| `['to-boolean', value]` | Booliano | Converts the input value to a boolean. The result is `false` when the input is an empty string, `0`, `false`, `null`, or `NaN`; otherwise its `true`. |
| `['to-color', value]`<br/><br/>`['to-color', value1, value2…]` | cor | Converts the input value to a color. If multiple values are provided, each one is evaluated in order until the first successful conversion is obtained. If none of the inputs can be converted, the expression is an error. |
| `['to-number', value]`<br/><br/>`['to-number', value1, value2, …]` | número | Converts the input value to a number, if possible. If the input is `null` or `false`, the result is 0. If the input is `true`, the result is 1. If the input is a string, it's converted to a number using the [ToNumber](https://tc39.github.io/ecma262/#sec-tonumber-applied-to-the-string-type) string function of the ECMAScript Language Specification. If multiple values are provided, each one is evaluated in order until the first successful conversion is obtained. If none of the inputs can be converted, the expression is an error. |
...
| Expression | Tipo de retorno | Descrição |
|------------|-------------|-------------|
| `['rgb', number, number, number]` | cor | Creates a color value from *red*, *green*, and *blue* components that must range between `0` and `255`, and an alpha component of `1`. If any component is out of range, the expression is an error. |
| `['rgba', number, number, number, number]` | cor | Creates a color value from *red*, *green*, *blue* components that must range between `0` and `255`, and an alpha component within a range of `0` and `1`. If any component is out of range, the expression is an error. |
...
> [!TIP]
> The color at index 0 in an interpolation expression or the default color of a step color, defines the color of the area where there's no data and can be used to define a background color. Muitos preferem definir esse valor como transparente ou semitransparente preto. 
...
| `['var', name: string]` | qualquer | References a variable that was created using the `let` expression. |
