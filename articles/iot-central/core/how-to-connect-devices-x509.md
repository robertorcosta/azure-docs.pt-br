...
Nesta seção, você usará um certificado X. 509 para conectar um dispositivo com um certificado derivado do certificado do grupo de registro, que pode se conectar ao seu aplicativo IoT Central.
...
2. Navegue até o script gerador de certificado e instale os pacotes necessários:
...
3. Crie um certificado raiz e, em seguida, derive um certificado de dispositivo executando o script. Certifique-se de usar apenas caracteres alfanuméricos minúsculos e hifens para o nome do certificado.
...
Isso produzirá três arquivos cada para a raiz e o certificado do dispositivo
...
1. Agora, abra o aplicativo IoT Central e navegue até **Administração**  no painel esquerdo e clique em **conexão do dispositivo**. 

2. Selecione + **Criar grupo de registro**e crie um novo grupo de registro chamado _MyX509Group_ com um tipo de atestado de **certificados (X. 509)**:


3. Abra o grupo de registro que você criou e clique em **gerenciar primário**. 

4. Selecione a opção arquivo e carregue o arquivo de certificado raiz chamado _mytestrootcert_cert. pem_ que você gerou anteriormente:
...
5. Para concluir a verificação, copie o código de verificação e crie um certificado de verificação X. 509 com esse código no prompt de comando.

    ```cmd/sh
    node create_test_cert.js verification --ca mytestrootcert_cert.pem --key mytestrootcert_key.pem --nonce  {verification-code}
    ```

6. Carregue o certificado de verificação assinado _verification_cert. pem_ para concluir a verificação.
...
1. No aplicativo IoT Central do Azure, clique em **dispositivos**e crie um novo dispositivo com _MYTESTDEVICE_ como a **ID do dispositivo** do modelo de dispositivo do sensor ambiental.


2. Copie o _mytestdevice_key. pem_ e _mytestdevice_cert. pem_ para a pasta que contém o aplicativo _environmentalSensor.js_ quando você concluiu o [tutorial conectar um dispositivo (Node.js)](./tutorial-connect-device-nodejs.md).

3. Navegue até a pasta que contém o aplicativo environmentalSensor.js e execute o seguinte comando para instalar o pacote X. 509:

    ```cmd/sh
    npm install azure-iot-security-x509 --save
    ```

4. Edite o arquivo de **environmentalSensor.js** .
    - Substitua o `idScope` valor pelo **escopo de ID** que você fez uma anotação anteriormente 
    - Substituir `registrationId` valor por `mytestdevice` .

5. Edite as `require` instruções da seguinte maneira:
...
6. Edite a seção que cria o cliente da seguinte maneira:
...
7. Modifique a seção que abre a conexão da seguinte maneira:
...
8. Execute o script e verifique se o dispositivo foi provisionado com êxito.
...
Nesta seção, você usará um certificado X. 509 autoassinado para conectar dispositivos para registro individual, que são usados para registrar um único dispositivo. Os certificados autoassinados são apenas para teste.
...
1. No aplicativo IoT Central do Azure, selecione **dispositivos**e crie um novo dispositivo com a **ID do dispositivo** como _Mytestselfcertprimary_ do modelo de dispositivo sensor ambiental. Anote o **escopo da ID**

2. Abra o dispositivo que você criou e selecione **conectar**

3. Selecione **registros individuais** como o método Connect e **certificados (X. 509)** como mecanismo.

    ![Registro individual](./media/how-to-connect-devices-x509/individual-device-connect.png)


4. Selecione a opção arquivo em primário e carregue o arquivo de certificado chamado _mytestselfcertprimary_cert. pem_ que você gerou anteriormente. 

5. Selecione a opção arquivo para o certificado secundário e carregue o arquivo de certificado chamado _mytestselfcertsecondary_cert. PEM._ Em seguida, selecione **salvar**
...
1. Copie o _mytestselfcertprimary_key. pem_ e _mytestselfcertprimary_cert. pem_para a pasta que contém o aplicativo environmentalSensor.js quando você concluiu o [tutorial conectar um dispositivo (Node.js)](./tutorial-connect-device-nodejs.md).


2. Edite o arquivo de **environmentalSensor.js** da seguinte maneira e salve-o.
...
3. Execute o script e verifique se o dispositivo foi provisionado com êxito.
