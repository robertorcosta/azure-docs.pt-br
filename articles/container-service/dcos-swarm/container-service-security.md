...
description: Considerações sobre a segurança de contêineres do Docker implantado no Serviço de Contêiner do Azure e nos serviços do Azure relacionados.
author: sauryadas
...
As empresas preocupam-se profundamente com segurança e, para se protegerem contra ataques, devem armazenar e recuperar imagens de um registro privado, como o Registro de Contêiner do Azure ou Docker Trusted Registry. Além de fornecer um registro privado gerenciado, o Registro de Contêiner do Azure dá suporte à [autenticação baseada em entidade de serviço](../../container-registry/container-registry-authentication.md) através do Active Directory do Azure para fluxos de autenticação básica, incluindo o acesso baseado em função para permissões de somente leitura, gravação e proprietário.
...
* **DC/OS**: [proteção do cluster](https://docs.mesosphere.com/1.12/administering-clusters/securing-your-cluster)
...
* Para saber mais sobre segurança da plataforma do Azure, veja a [Central de Segurança do Azure](https://www.microsoft.com/trustcenter/cloudservices/azure).
