...
|**Provisionamento de autoatendimento**|Compatível<br/>A criação pode ser feita por meio de Azure Data Studio, [!INCLUDE [azure-data-cli-azdata](../../../includes/azure-data-cli-azdata.md)] , ou ferramentas nativas Kubernetess (Helm, kubectl, OC etc.) ou usando o Azure Arc habilitado kubernetes o provisionamento de GitOps.|Compatível<br/>Além das opções de criação de modo conectado indiretamente, você também pode criar por meio do portal do Azure, APIs de Azure Resource Manager, os modelos de CLI do Azure ou ARM. **Disponibilidade pendente do modo conectado diretamente**
|**Escalabilidade elástica**|Com suporte|Com suporte<br/>**Disponibilidade pendente do modo conectado diretamente**|
|**Billing**|Compatível<br/>Os dados de cobrança são exportados periodicamente e enviados para o Azure.|Compatível<br/>Os dados de cobrança são enviados de forma automática e contínua ao Azure e refletidos quase em tempo real. **Disponibilidade pendente do modo conectado diretamente**|
|**Gerenciamento de estoque**|Compatível<br/>Os dados de inventário são exportados periodicamente e enviados para o Azure.<br/><br/>Use ferramentas de cliente como Azure Data Studio, CLI de dados do Azure ou `kubectl` para exibir e gerenciar o inventário localmente.|Compatível<br/>Os dados de inventário são enviados de forma automática e contínua ao Azure e refletidos quase em tempo real. Assim, você pode gerenciar o inventário diretamente do portal do Azure. **Disponibilidade pendente do modo conectado diretamente**|
|**Atualizações automáticas e aplicação de patch**|Compatível<br/>O controlador de dados deve ter acesso direto ao MCR (registro de contêiner da Microsoft) ou as imagens de contêiner precisam ser extraídas de MCR e enviadas por push para um registro de contêiner privado e local ao qual o controlador de dados tem acesso.|Compatível<br/>**Disponibilidade pendente do modo conectado diretamente**|
|**Backup e restauração automáticos**|Compatível<br/>Backup e restauração locais automáticos.|Compatível<br/>Além de backup e restauração locais automatizados, você pode, _opcionalmente_ , enviar backups para o backup do Azure para retenção de longo prazo fora do local. **Disponibilidade pendente do modo conectado diretamente**|
|**Monitoring**|Compatível<br/>Monitoramento local usando painéis Grafana e Kibana.|Compatível<br/>Além dos painéis de monitoramento locais, _opcionalmente_ , você pode enviar dados e logs de monitoramento para Azure monitor para o monitoramento em escala de vários sites em um único local. **Disponibilidade pendente do modo conectado diretamente**|
