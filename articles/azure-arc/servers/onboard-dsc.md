...
Os recursos neste módulo são projetados para gerenciar a configuração do agente de máquina conectado do Azure. Também está incluído um script do PowerShell `AzureConnectedMachineAgent.ps1`, encontrado na pasta `AzureConnectedMachineDsc\examples`. Ele usa recursos da Comunidade para automatizar o download e a instalação e estabelecer uma conexão com o arco do Azure. Esse script executa etapas semelhantes descritas no artigo [conectar máquinas híbridas ao Azure do portal do Azure](onboard-portal.md) .
...
Os documentos de configuração (Arquivos MOF) podem ser aplicados ao computador usando o cmdlet `Start-DscConfiguration`.
...
- `SubscriptionId`: a ID da assinatura (GUID) da sua assinatura do Azure na qual você deseja que os computadores estejam.
...
1. Em um console do PowerShell, navegue até a pasta em que você salvou o arquivo de `.ps1`.
