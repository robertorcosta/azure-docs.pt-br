...
1. Execute o comando a seguir. Você deve armazenar a saída do [`New-AzADServicePrincipal`](/powershell/module/az.resources/new-azadserviceprincipal) cmdlet em uma variável ou não poderá recuperar a senha necessária em uma etapa posterior.
...
2. Para recuperar a senha armazenada na `$sp` variável, execute o seguinte comando:
...
3. Na saída, localize o valor da senha na **senha** do campo e copie-o. Encontre também o valor sob o campo **ApplicationId** e copie-o também. Salve-os para mais tarde em um local seguro. Se você esquecer ou perder sua senha de entidade de serviço, poderá redefini- [`New-AzADSpCredential`](/powershell/module/azurerm.resources/new-azurermadspcredential) la usando o cmdlet.

Os valores das propriedades a seguir são usados com parâmetros passados para `azcmagent`:

* O valor da propriedade **ApplicationId** é usado para o valor `--service-principal-id` do parâmetro
* O valor da propriedade **password** é usado para o `--service-principal-secret` parâmetro usado para conectar o agente.
...
As etapas a seguir instalam e configuram o agente de computador conectado em seus computadores híbridos usando o modelo de script, que executa etapas semelhantes descritas no artigo [conectar máquinas híbridas ao Azure do portal do Azure](onboard-portal.md) . A diferença está na etapa final em que você estabelece a conexão com o arco do Azure `azcmagent` usando o comando usando a entidade de serviço. 
...
* `subscription-id`: A ID da assinatura (GUID) da sua assinatura do Azure na qual você deseja que os computadores.
...
Você pode aprender mais sobre a `azcmagent` ferramenta de linha de comando examinando a [referência de Azcmagent](azcmagent-reference.md).
