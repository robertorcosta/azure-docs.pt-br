...
Observe que há várias colunas disponíveis nessas tabelas. Neste artigo, estamos apresentando apenas os mais relevantes para um consumo de log mais fácil.
...
As alterações de configuração são auditadas na tabela **GatewayDiagnosticLog** . Observe que pode levar alguns minutos antes que as alterações executadas sejam refletidas nos logs.
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-26-set-gateway.png" alt-text="Definir exemplo de operação de gateway.":::
...
| **S de instância \_** | a instância da função de gateway que disparou o evento. Ele pode ser GatewayTenantWorker \_ em \_ 0 ou GatewayTenantWorker \_ em 1, \_ que são os nomes das duas instâncias do gateway.|
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-16-tunnel-connected.png" alt-text="Exemplo de evento de túnel conectado.":::
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-31-bgp-route.png" alt-text="Exemplos de rota BGP.":::
...
|---        | ---               |
|**TimeGenerated** | o carimbo de data/hora de cada evento, no fuso horário UTC|
| **RemoteIP** | o endereço IP do dispositivo VPN local. Em cenários do mundo real, é útil filtrar pelo endereço IP do dispositivo local relevante; haverá mais de um |
...
|**Evento** | contém uma mensagem de diagnóstico útil para solução de problemas. Normalmente, eles começam com uma palavra-chave e referim-se às ações executadas pelo Azure gateway **\[ Send \]** indicam que um evento causado por um pacote IPsec enviado pelo gateway do Azure **\[ recebido \]** indica que um evento em consequência de um pacote recebido do **\[ local \]** do dispositivo localmente indica uma ação realizada localmente pelo gateway do Azure |
...
Os [parâmetros padrão de IPSec/IKE](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpn-devices#ipsec) lista os parâmetros IPSec com suporte no gateway do Azure com as configurações padrão


## <a name="p2sdiagnosticlog"></a><a name="P2SDiagnosticLog"></a>P2SDiagnosticLog

A última tabela disponível para diagnóstico de VPN é **P2SDiagnosticLog**. Isso rastreia a atividade de ponto a site.
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-28-p2s-log-event.png" alt-text="Exemplo de logs do P2S.":::
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-29-eap.png" alt-text="Exemplo de logs de EAP.":::
