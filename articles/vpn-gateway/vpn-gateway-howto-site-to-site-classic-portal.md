...
Este artigo mostra como usar o portal do Azure para criar uma conexão de gateway de VPN Site a Site de sua rede local para a rede virtual. As etapas deste artigo se aplicam ao modelo clássico de implantação e não se aplicam ao modelo de implantação atual, O Gerenciador de Recursos. Você também pode criar essa configuração usando uma ferramenta de implantação ou um modelo de implantação diferente, selecionando uma opção diferente na lista a seguir:
...
* O PowerShell é necessário para especificar a chave compartilhada e criar a conexão de gateway VPN. [!INCLUDE [vpn-gateway-classic-powershell](../../includes/vpn-gateway-powershell-classic-locally.md)]
...
## <a name="1-create-a-virtual-network"></a><a name="CreatVNet"></a>1. Crie uma rede virtual
...
1. A partir de um navegador, navegue até o [portal do Azure](https://portal.azure.com) e, se necessário, faça login com sua conta do Azure.
2. Clique em **+Criar um recurso*. No campo **Pesquisar no Marketplace**, digite "Rede Virtual". Localize a **Rede Virtual** a partir da lista retornada e clique para abrir a página **da Rede Virtual.**
3. clique **(mude para Clássico)** e clique **em Criar**.
4. Na página **Criar rede virtual (clássico)**, defina as configurações da VNet. Nessa página, você adiciona o primeiro espaço de endereço e um único intervalo de endereços da sub-rede. Depois de criar o VNet, você pode voltar e adicionar sub-redes adicionais e espaços de endereço.

   ![Criar página da rede virtual](./media/vpn-gateway-howto-site-to-site-classic-portal/createvnet.png "Criar página da rede virtual")
5. Verifique se a **Assinatura** é a correta. Você pode alterar as assinaturas usando o menu suspenso.
6. Clique em **Grupo de recursos** e selecione um grupo de recursos existente ou crie um novo digitando um nome. Para obter mais informações sobre grupos de recursos, visite [a visão geral do Azure Resource Manager](../azure-resource-manager/management/overview.md#resource-groups).
...
1. Na página do seu VNet, em **Configurações,** clique em **Diagrama**.
1. Na página **de conexões VPN,** clique **em Você não tem nenhuma conexão VPN existente. Clique aqui para começar**.
1. Para **o tipo Conexão,** deixe **site-to-site** selecionado.
...
Clique em **OK** para fechar a página do site local. **Não clique em OK para fechar a página Nova conexão VPN**.

## <a name="5-configure-the-gateway-subnet"></a><a name="gatewaysubnet"></a>5. Configure a sub-rede gateway
...
1. Na página **Nova Conexão VPN**, marque a caixa de seleção **Criar gateway imediatamente**. A página 'Configuração de gateway opcional' será exibida. Se você não marcar a caixa de seleção, não verá a página para configurar a sub-rede de gateway.
   ...
   ![Configuração do gateway - Sub-rede, tamanho, roteamento](./media/vpn-gateway-howto-site-to-site-classic-portal/optional.png "Configuração do gateway - Sub-rede, tamanho, roteamento")
...
3. Na página **Configuração de gateway**, clique em **Subnet - Definir as configurações necessárias** para abrir a página **Adicionar sub-rede**. Quando terminar de configurar essas configurações, clique em **OK**.

   ![Configuração do gateway - sub-rede gateway](./media/vpn-gateway-howto-site-to-site-classic-portal/subnetrequired.png "Configuração do gateway - sub-rede gateway")
...
## <a name="6-specify-the-sku-and-vpn-type"></a><a name="sku"></a>6. Especifique o tipo SKU e VPN
...
1. Selecionar o **tamanho** do gateway. Este é o gateway SKU que você usa para criar o gateway de rede virtual. Gateways VPN clássicos usam SKUs de gateway antigos (herdados). Para saber mais sobre os SKUs de gateway antigos, confira [Trabalhar com SKUs de gateway de rede virtual (SKUs antigos)](vpn-gateway-about-skus-legacy.md).
   ...
   ![Selecione o tipo SKUL e VPN](./media/vpn-gateway-howto-site-to-site-classic-portal/sku.png "Selecione o tipo SKU e VPN")
2. Selecione o **Tipo de Roteamento** para seu gateway. Isso é também conhecido como o tipo de VPN. É importante selecionar o tipo correto porque você não pode converter o gateway de um tipo para outro. Seu dispositivo VPN deve ser compatível com o tipo de roteamento selecionado. Para obter mais informações sobre o tipo de roteamento, consulte [Sobre as configurações do Gateway VPN](vpn-gateway-about-vpn-gateway-settings.md#vpntype). Você pode ver artigos sobre os tipos de VPN 'RouteBased' e 'PolicyBased'. 'Dinâmico' corresponde a 'RouteBased' e 'Estático' corresponde a 'PolicyBased'.
...
## <a name="7-configure-your-vpn-device"></a><a name="vpndevice"></a>7. Configure seu dispositivo VPN
...
## <a name="8-create-the-connection"></a><a name="CreateConnection"></a>8. Crie a conexão
...
> [!NOTE]
> No momento, essa configuração não está disponível no Portal do Azure. Use a versão de SM (Gerenciamento de Serviço) dos cmdlets do Azure PowerShell. Consulte [Antes de começar](#before) para obter informações sobre a instalação desses cmdlets.
>

### <a name="step-1-connect-to-your-azure-account"></a>Etapa 1. Conectar-se à sua conta do Azure

Você deve executar esses comandos localmente usando o módulo de gerenciamento de serviços PowerShell. 
...
2. Conecte-se à sua conta. Use o exemplo a seguir para ajudar a se conectar:
   ...
   Add-AzureAccount
   ```
3. Verificar as assinaturas da conta.

   ```powershell
   Get-AzureSubscription
   ```
4. Se você tiver mais de uma assinatura, selecione a assinatura que deseja usar.
...
Quando você cria um VNet clássico no portal (sem usar o PowerShell), o Azure adiciona o nome do grupo de recursos ao nome curto. Por exemplo, de acordo com o Azure, o nome do VNet que você criou para este exercício é "Group TestRG1 TestVNet1", e não "TestVNet1". O PowerShell requer o nome completo da rede virtual, não o nome curto que aparece no portal. O nome longo não é visível no portal. As etapas a seguir ajudam a exportar o arquivo de configuração de rede para obter os valores exatos para o nome da rede virtual. 
...
## <a name="9-verify-your-connection"></a><a name="verify"></a>9. Verifique sua conexão
