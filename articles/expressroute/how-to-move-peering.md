---
title: 'Azure ExpressRoute: Mova um peering público para o peering da Microsoft'
...
O peering público do Azure tem 1 endereço IP NAT associado a cada sessão BGP. O peering da Microsoft permite configurar suas próprias alocações de NAT, bem como usar filtros de rota para anúncios de prefixo seletivo. O Public Peering é um serviço unidirecional que usa o qual a conectividade é sempre iniciada a partir dos serviços WAN para Microsoft Azure. Os serviços do Microsoft Azure não poderão iniciar conexões a sua rede por meio desse domínio de roteamento.

Após o emparelhamento público ser habilitado, você poderá se conectar a todos os serviços do Azure. Não permitimos que você escolha seletivamente os serviços para os quais podemos anunciar rotas. Enquanto o peering da Microsoft é uma conectividade bidirecional onde a conexão pode ser iniciada a partir do serviço Microsoft Azure, juntamente com o seu WAN. Para obter mais informações sobre domínios de roteamento e peering, consulte [circuitos ExpressRoute e domínios de roteamento](expressroute-circuit-peerings.md).

## <a name="before-you-begin"></a><a name="before"></a>Antes de começar

Para se conectar ao emparelhamento da Microsoft, você precisa configurar e gerenciar o NAT. Seu provedor de conectividade pode configurar e gerenciar o NAT como um serviço gerenciado. Caso você pretenda acessar os serviços de PaaS e SaaS do Azure no emparelhamento do Microsoft, é importante dimensionar o pool IP do NAT corretamente. Para obter mais informações sobre o NAT para o ExpressRoute, consulte os [Requisitos do NAT para o emparelhamento da Microsoft](expressroute-nat.md#nat-requirements-for-microsoft-peering). Quando você se conecta à Microsoft através do Azure ExpressRoute (peering microsoft), você tem vários links para a Microsoft. Um link é sua conexão com a Internet existente e o outro é via ExpressRoute. Parte do tráfego para a Microsoft pode passar pela Internet, mas voltar por meio do ExpressRoute ou vice-versa.
...
Consulte [o roteamento assimétrico com vários caminhos](https://docs.microsoft.com/azure/expressroute/expressroute-asymmetric-routing) de rede para ressalvas de roteamento assimétrico antes de configurar o peering da Microsoft.
...
## <a name="1-create-microsoft-peering"></a><a name="create"></a>1. Crie o peering da Microsoft
...
Se a camada 3 for gerenciada por você, as seguintes informações são necessárias antes de prosseguir:
...
* **Opcional** - Customer ASN: Se você estiver anunciando prefixos que não estão registrados no número AS correspondente, você pode especificar o número AS ao qual eles estão registrados.<br>
* **Opcional** - Um hash MD5 se você optar por usar um.

Instruções detalhadas para habilitar o peering da Microsoft podem ser encontradas nos seguintes artigos:
...
## <a name="3-configure-and-attach-a-route-filter-to-the-circuit"></a><a name="routefilter"></a>3. Configure e conecte um filtro de rota ao circuito

Por padrão, os novos peerings da Microsoft não anunciam nenhum prefixo até que um filtro de rota seja conectado ao circuito. Ao criar uma regra de filtro de rota, você pode especificar a lista de comunidades de serviços para regiões Do Zure que você deseja consumir para os serviços do Azure PaaS. Isso fornece a flexibilidade para filtrar as rotas conforme sua exigência, conforme mostrado na captura de tela a seguir:
...
## <a name="4-delete-the-public-peering"></a><a name="delete"></a>4. Exclua o peering público
...
## <a name="5-view-peerings"></a><a name="view"></a>5. Ver peerings
