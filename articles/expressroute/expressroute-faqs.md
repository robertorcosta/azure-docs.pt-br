...
### <a name="how-are-vnets-advertised-on-expressroute-private-peering"></a>Como os VNets são anunciados no ExpressRoute Private Peering?

O gateway ExpressRoute anunciará os *espaços de endereço(s)* do Azure VNet, você não pode incluir/excluir no nível da sub-rede. É sempre o Espaço de Endereço susceptino de ser anunciado. Além disso, se o VNet Peering for usado e o VNet peered tiver o "Use Remote Gateway" ativado, o Espaço de endereço do VNet peered também será anunciado.

### <a name="how-many-prefixes-can-be-advertised-from-a-vnet-to-on-premises-on-expressroute-private-peering"></a>Quantos prefixos podem ser anunciados de um VNet para on-premises no ExpressRoute Private Peering?

Há um máximo de 200 prefixos anunciados em uma única conexão ExpressRoute, ou através de peering VNet usando o gateway transit. Por exemplo, se você tiver 199 espaços de endereço em um único VNet conectado a um circuito ExpressRoute, todos os 199 desses prefixos serão anunciados no local. Alternativamente, se você tiver um VNet habilitado para permitir o trânsito de gateway com 1 espaço de endereço e 150 VNets de 150 raios habilitados usando a opção "Permitir gateway remoto", o VNet implantado com o gateway anunciará 151 prefixos para o local.

### <a name="what-happens-if-i-exceed-the-prefix-limit-on-an-expressroute-connection"></a>O que acontece se eu exceder o limite de prefixo em uma conexão ExpressRoute?

A conexão entre o circuito ExpressRoute e o gateway (e vNets peered usando o gateway transit, se aplicável) irá cair. Ele restabelecerá quando o limite de prefixo não for mais excedido.  

### <a name="can-i-filter-routes-coming-from-my-on-premises-network"></a>Posso filtrar rotas que vêm da minha rede local?

A única maneira de filtrar/incluir rotas é no roteador de borda no local. Rotas definidas pelo usuário podem ser adicionadas no VNet para afetar o roteamento específico, mas isso será estático e não fará parte do anúncio do BGP.
...
O ExpressRoute suporta três domínios de [roteamento](expressroute-circuit-peerings.md) para vários tipos de serviços: peering privado, peering da Microsoft e peering público (preterido).
...
Se o circuito ExpressRoute estiver habilitado para peering do Azure Microsoft, você poderá acessar as [faixas públicas de endereçoIP](../virtual-network/virtual-network-ip-addresses-overview-arm.md#public-ip-addresses) usadas no Azure durante o circuito. O peering do Azure Microsoft fornecerá acesso aos serviços atualmente hospedados no Azure (com restrições geográficas dependendo do SKU do seu circuito). Para validar a disponibilidade de um serviço específico, você pode verificar a documentação desse serviço para ver se há uma faixa reservada publicada para esse serviço. Em seguida, consulte as faixas IP do serviço de destino e compare com as faixas listadas nas faixas ip e tags de serviço do [Azure – arquivo XML da nuvem pública](https://www.microsoft.com/download/details.aspx?id=56519). Alternativamente, você pode abrir um bilhete de apoio para o serviço em questão para esclarecimentos.

**Suportado:**

* [Escritório 365](https://aka.ms/ExpressRouteOffice365)
* Power BI - Disponível através de uma Comunidade Regional do Azure, veja [aqui](https://docs.microsoft.com/power-bi/service-admin-where-is-my-tenant-located) como descobrir a região do seu inquilino power bi.
...
* A maioria dos outros serviços do Azure também são suportados. Verifique diretamente o serviço que você deseja utilizar para confirmar o suporte.
...
Emparelhamento público foi desabilitado em novos circuitos do ExpressRoute. Os serviços do Azure já estão disponíveis no peering da Microsoft. Se você um circuito que foi criado antes de o peering público ser preterido, você pode optar por usar o peering da Microsoft ou peering público, dependendo dos serviços que você deseja.

Para obter mais informações e etapas de configuração para peering público, consulte [peering público expressRoute](about-public-peering.md).

### <a name="why-i-see-advertised-public-prefixes-status-as-validation-needed-while-configuring-microsoft-peering"></a>Por que vejo o status 'Prefixos públicos anunciados' como 'Validação necessária', enquanto configuro o peering da Microsoft?
...
### <a name="is-dynamics-365-supported-on-expressroute"></a>O Dynamics 365 é suportado no ExpressRoute?

Os ambientes Dynamics 365 e Common Data Service (CDS) estão hospedados no Azure e, portanto, os clientes se beneficiam do suporte subjacente ao ExpressRoute para recursos do Azure. Você pode se conectar aos pontos finais de serviço se o filtro do roteador incluir as regiões Do Zure em que os ambientes Dynamics 365/CDS estão hospedados.
...
### <a name="how-do-i-implement-redundancy-on-private-peering"></a>Como implementar a redundância em peering privado?

Vários circuitos ExpressRoute de diferentes locais de peering podem ser conectados à mesma rede virtual para fornecer alta disponibilidade no caso de um único circuito ficar indisponível. Em seguida, você pode [atribuir pesos mais altos](https://docs.microsoft.com/azure/expressroute/expressroute-optimize-routing#solution-assign-a-high-weight-to-local-connection) à conexão local para favorecer um circuito específico. É fortemente recomendável que os clientes configurem pelo menos dois circuitos ExpressRoute para evitar pontos únicos de falha. 
...
### <a name="how-i-do-implement-redundancy-on-microsoft-peering"></a>Como implemento redundância no peering da Microsoft?

É altamente recomendado quando os clientes estão usando o peering da Microsoft para acessar serviços públicos do Azure, como o Azure Storage ou o Azure SQL, bem como clientes que estão usando o peering da Microsoft para o Office 365 que implementam vários circuitos em diferentes peering locais para evitar pontos únicos de falha. Os clientes podem anunciar o mesmo prefixo em ambos os circuitos e usar [o AS PATH pré-pendente](https://docs.microsoft.com/azure/expressroute/expressroute-optimize-routing#solution-use-as-path-prepending) ou anunciar diferentes prefixos para determinar o caminho a partir do local.

Veja [aqui](https://docs.microsoft.com/azure/expressroute/designing-for-high-availability-with-expressroute) para projetar para alta disponibilidade.
...
### <a name="how-do-i-ensure-that-my-traffic-destined-for-azure-public-services-like-azure-storage-and-azure-sql-on-microsoft-peering-or-public-peering-is-preferred-on-the-expressroute-path"></a>Como garantir que meu tráfego destinado a serviços públicos do Azure, como o Azure Storage e o Azure SQL, no microsoft peering ou peering público seja preferido no caminho do ExpressRoute?

Você deve implementar o atributo *Preferência Local* em seu roteador para garantir que o caminho do local para o Azure seja sempre preferido em seus circuitos ExpressRoute( s).

Veja detalhes adicionais [aqui](https://docs.microsoft.com/azure/expressroute/expressroute-optimize-routing#path-selection-on-microsoft-and-public-peerings) sobre a seleção de caminhos BGP e configurações comuns do roteador. 
...
Sim. Você pode vincular uma única rede virtual com até quatro circuitos ExpressRoute nos mesmos ou diferentes locais de peering. 
...
Não aceitamos prefixos privados (RFC1918) para a sessão BGP de emparelhamento Microsoft. Aceitamos qualquer tamanho de prefixo (até /32) tanto na Microsoft quanto no peering privado.
...
ExpressRoute Local é um circuito SKU da ExpressRoute, além do Standard SKU e do Premium SKU. Uma característica importante do Local é que um circuito local em um local de peering expressRoute lhe dá acesso apenas a uma ou duas regiões do Azure dentro ou perto do mesmo metrô. Em contraste, um circuito Standard dá acesso a todas as regiões do Azure em uma área geopolítica e um circuito Premium para todas as regiões do Azure globalmente. 

### <a name="what-are-the-benefits-of-expressroute-local"></a>Quais são os benefícios do ExpressRoute Local?
Embora você precise pagar transferência de dados para o seu circuito Standard ou Premium ExpressRoute, você não paga transferência de dados de saída separadamente para o seu circuito Local ExpressRoute. Em outras palavras, o preço do ExpressRoute Local inclui taxas de transferência de dados. ExpressRoute Local é uma solução mais econômica se você tiver uma quantidade maciça de dados para transferir e você pode trazer seus dados sobre uma conexão privada para um local de peering ExpressRoute perto de suas regiões Azure desejadas. 

### <a name="what-features-are-available-and-what-are-not-on-expressroute-local"></a>Quais recursos estão disponíveis e o que não estão no ExpressRoute Local?
Comparado a um circuito Standard ExpressRoute, um circuito local tem o mesmo conjunto de recursos, exceto:
* Escopo de acesso às regiões do Azure conforme descrito acima
* ExpressRoute Global Reach não está disponível no Local

ExpressRoute Local também tem os mesmos limites de recursos (por exemplo, o número de VNets por circuito) como Padrão. 

### <a name="where-is-expressroute-local-available-and-which-azure-regions-is-each-peering-location-mapped-to"></a>Onde o ExpressRoute Local está disponível e para quais regiões do Azure cada local de peering é mapeado?
O ExpressRoute Local está disponível nos locais de peering onde uma ou duas regiões do Azure estão próximas. Não está disponível em um local onde não há região azure naquele estado ou província ou país. Consulte os mapeamentos exatos na [página Locais](expressroute-locations-providers.md).  
...
### <a name="can-my-existing-expressroute-circuits-support-connectivity-to-office-365-services"></a>Meus circuitos ExpressRoute existentes podem suportar conectividade aos serviços do Office 365?
...
Seu circuito existente continuará anunciando os prefixos do Office 365. Se você quiser adicionar anúncios de prefixos públicos do Azure sobre o mesmo peering da Microsoft, você pode criar um filtro de rota, selecionar os serviços que você precisa anunciado (incluindo os serviços do Office 365 que você precisa) e anexar o filtro ao seu peering microsoft. Para obter instruções, consulte [Configurar os filtros de rota para emparelhamento da Microsoft](how-to-routefilter-powershell.md).
