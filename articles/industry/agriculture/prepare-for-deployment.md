...
|S.Não  |Nome do Recurso  |Azure FarmBeats Component  |
|---------|---------|---------|
|1  |       Azure Cosmos DB   |  Hub de dados       |
|2  |    Percepções sobre o Aplicativo      |     Data hub/Accelerator     |
|3  |Cache Redis do Azure   |Hub de dados   |
|4  |       Azure KeyVault    |  Data hub/ Accelerator        |
|5  |    Análises de séries temporais       |     Hub de dados      |
|6 |      EventHub Namespace    |  Hub de dados       |
|7  |    Azure Data Factory V2       |     Data hub/ Accelerator      |
|8  |Conta do Batch    |Hub de dados   |
|9  |       Conta de armazenamento     |  Data hub/ Accelerator        |
|10  |    Aplicativo lógico        |     Hub de dados      |
|11  |    API connection        |     Hub de dados      |
|12|      serviço de aplicativo      |  Data hub/Accelerator       |
|13 |    Plano do serviço de aplicativo        |     Data hub/ Accelerator      |
|14 |Azure Maps account     |Acelerador    |
...
Vá para https://scihub.copernicus.eu/dhus/#/self-registration. In the registration page, provide a first name, last name, username, password, and email.
...
**Case 2**: This method is the preferred step when you don't have enough rights to create and configure an Azure AD app registration within your subscription. Request your  admin to use the [custom script](https://aka.ms/FarmBeatsAADScript), which will help IT admin automatically generate and configure the Azure AD app registration on the Azure portal. As an output to running this custom script using PowerShell environment the IT admin needs to share an Azure Active Directory Application Client ID and password secret with you. Anote esses valores.
...
4. First-time users will be prompted to select a subscription to create a storage account and Microsoft Azure Files share. Selecione **Criar armazenamento**.
...
The steps for upgrade are similar to the first-time installation. Siga estas etapas:
...
4. Escolha o nome do grupo de recursos. Type in the name again to double-check, and select Delete to remove the resource group, and all its underlying resources.
