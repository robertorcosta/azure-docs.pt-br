...
Digamos que desejamos criar um serviço que processa imagens. Para fazer isso, o serviço obtém uma imagem e a série de conversões a serem executadas na imagem. Este serviço retorna um ouvinte de comunicação (suponhamos que é uma WebAPI) que expõe uma API como `ConvertImage(Image i, IList<Conversion> conversions)`. Quando recebe uma solicitação, o serviço a armazena em uma `IReliableQueue`e retorna alguma ID ao cliente para que possa rastrear a solicitação.
...
Embora esse serviço pareça um serviço .NET típico, a diferença é que as estruturas de dados que estão sendo usadas (`IReliableQueue` e `IReliableDictionary`) são fornecidas por Service Fabric e são altamente confiáveis, disponíveis e consistentes.
