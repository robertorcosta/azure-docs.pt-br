...
### <a name="design-time-service-type-service-package-and-manifest-application-type-application-package-and-manifest"></a>Tempo de projeto: tipo de serviço, pacote de serviço e manifesto, tipo de aplicativo, pacote de aplicativo e manifesto
Um tipo de serviço é o nome/versão atribuída aos pacotes de códigos, pacotes de dados e pacotes de configuração de um serviço. Isso é definido em um arquivo ServiceManifest.xml. O tipo de serviço é composto por configurações de código e configuração de serviço executáveis, que são carregadas em tempo de execução, e dados estáticos que são consumidos pelo serviço.
...
Um [cluster Service Fabric](service-fabric-deploy-anywhere.md) é um conjunto de máquinas virtuais ou físicas conectadas à rede em que seus microserviços são implantados e gerenciados. Os clusters podem ser dimensionados para milhares de máquinas.
...
Uma diferenciação fundamental com a Service Fabric é seu forte foco na construção de serviços estatais, seja com [os modelos de programação embutidos](service-fabric-choose-framework.md) ou com serviços estatais contêineres. Os [cenários de aplicativo](service-fabric-application-scenarios.md) descrevem os cenários em que os serviços com monitoração de estado são usados.
...
Para criar serviços que são realmente em escala de nuvem, é essencial verificar se os aplicativos e serviços podem dar suporte a falhas reais. O Serviço de Análise de Falha foi desenvolvido para testar serviços criados com base no Service Fabric. Com o [Serviço de Análise de Falhas,](service-fabric-testability-overview.md)você pode induzir falhas significativas e executar cenários de teste completos contra seus aplicativos. Esses cenários e falhas praticam e validam os vários estados e transições pelos quais um serviço passa durante seu tempo de vida, tudo de maneira consistente, segura e controlada.
...
[Os cenários](service-fabric-testability-scenarios.md) são operações complexas compostas por uma ou mais ações. O Serviço de Análise de Falha fornece dois cenários internos completos:
...
A execução de clusters do Service Fabric no Azure proporciona integração a outros recursos e serviços do Azure, o que facilita e torna as operações e o gerenciamento do cluster mais confiável. Um cluster é um recurso do Azure Resource Manager, portanto você pode modelar clusters da mesma maneira que qualquer outro recurso no Azure. O Resource Manager também proporciona o gerenciamento fácil de todos os recursos usados pelo cluster como uma única unidade. Os clusters no Azure são integrados com diagnósticos do Azure e logs do Monitor Do Azure. Os tipos de nó de cluster são [conjuntos de dimensionamento de máquinas virtuais](/azure/virtual-machine-scale-sets/index) e, portanto, a funcionalidade de dimensionamento automático é interna.
...
Um cluster autônomo é um recurso que é totalmente seu. Você é responsável por corrigir o sistema operacional subjacente e iniciar atualizações de malha. Se o cluster [https://www.microsoft.com/download](https://www.microsoft.com/download)puder se conectar, você pode definir seu cluster para baixar e provisionar automaticamente o novo pacote de tempo de execução Service Fabric. Em seguida, você iniciaria a atualização. Se o cluster não [https://www.microsoft.com/download](https://www.microsoft.com/download)puder acessar, você pode baixar manualmente o novo pacote de tempo de execução de uma máquina conectada à Internet e, em seguida, iniciar a atualização. Para obter mais informações, leia [Atualizar um cluster autônomo do Service Fabric](service-fabric-cluster-upgrade-windows-server.md).
...
Os componentes do Service Fabric apresentam relatórios de integridade prontos para uso sobre todas as entidades do cluster. [Os relatórios de saúde do sistema](service-fabric-understand-and-troubleshoot-with-system-health-reports.md) proporcionam visibilidade sobre a funcionalidade do cluster e da aplicação e sinalizam problemas através da saúde. Para aplicativos e serviços, os relatórios de integridade do sistema verificam se as entidades estão implementadas e se comportando corretamente da perspectiva do runtime do Service Fabric. Os relatórios não fornecem nenhum monitoramento de saúde da lógica empresarial do serviço nem detectam processos que deixaram de responder. Para adicionar informações de integridade específicas à lógica do seu serviço, [implemente relatórios de integridade personalizados](service-fabric-report-health.md) em seus serviços.
