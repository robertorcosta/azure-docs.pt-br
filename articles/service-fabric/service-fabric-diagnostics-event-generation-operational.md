---
title: Lista de eventos de malha de serviço do Azure
...
ms.locfileid: "79258532"
---
# <a name="list-of-service-fabric-events"></a>Lista de eventos do Service Fabric 

O Service Fabric expõe um conjunto primário de eventos de cluster para informar a você sobre o status do seu cluster como [Eventos do Service Fabric](service-fabric-diagnostics-events.md). Eles são baseados em ações realizadas pelo Service Fabric nos seus nós e no seu cluster ou decisões de gerenciamento tomadas por um proprietário/operador do cluster. Esses eventos podem ser acessados configurando de várias maneiras, incluindo a configuração de [logs do Monitor Do Azure com o cluster](service-fabric-diagnostics-oms-setup.md)ou consultando o [EventStore](service-fabric-diagnostics-eventstore.md). Nos computadores Windows, esses eventos são alimentados no EventLog - então você pode ver os Eventos do Service Fabric no Visualizador de Eventos. 
...
| EventId | Nome | Categoria | Descrição |Origem (Tarefa) | Nível |
| --- | --- | ---| --- | --- | --- |
| 61701 | Dicionário confiávelAberto | Ciclo de vida | Dicionário confiável foi aberto | Dicionário distribuído | Informativo |
| 61702 | Dicionário confiávelfechado | Ciclo de vida | Dicionário confiável fechou | Dicionário distribuído | Informativo |
| 61703 | ConfiávelDictionaryCheckpointRecuperado | Ciclo de vida | Dicionário confiável recuperou seu ponto de verificação | Dicionário distribuído | Informativo |
| 61704 | ConfiávelDictionaryCheckpointFilesSent | Ciclo de vida | A réplica enviou arquivos de verificação de dicionário confiáveis | Dicionário distribuído | Informativo |
| 61705 | Arquivos de verificação de dicionário confiáveisrecebidos | Ciclo de vida | A réplica recebeu arquivos de verificação de dicionário confiáveis | Dicionário distribuído | Informativo |
| 61963 | ConfiávelQueueAberto | Ciclo de vida | Fila confiável foi aberta | Queue Distribuída | Informativo |
| 61964 | ConfiávelQueueFechado | Ciclo de vida | Fila confiável fechou | Queue Distribuída | Informativo |
| 61965 | Queueque-de-confiançarecuperado | Ciclo de vida | Fila confiável recuperou seu ponto de verificação | Queue Distribuída | Informativo |
| 61966 | ConfiávelQueueCheckpointFilesSent | Ciclo de vida | A réplica enviou arquivos de verificação de fila confiáveis | Queue Distribuída | Informativo |
| 63647 | ConfiávelQueueCheckpointFilesRecebido | Ciclo de vida | A réplica recebeu arquivos de verificação de fila confiáveis | Queue Distribuída | Informativo |
| 63648 | ConfiávelConcurrentQueueAberto | Ciclo de vida | Fila simultânea confiável foi aberta | ConfiávelConcurrentQueue | Informativo |
| 63649 | ConfiávelConcurrentQueueFechado | Ciclo de vida | Fila simultânea confiável fechou | ConfiávelConcurrentQueue | Informativo |
| 63650 | ConfiávelConcurrentQueueCheckpointRecuperado | Ciclo de vida | Fila simultânea confiável recuperou seu ponto de verificação | ConfiávelConcurrentQueue | Informativo |
| 61687 | Erro de TStore | Falha | A coleção confiável recebeu um erro inesperado | TStore | Erro |
| 63831 | PrimaryFullCopyIniciado | Ciclo de vida | A réplica primária iniciou uma cópia completa | Replicador Treplicator | Informativo |
| 63832 | PrincipalParcialCópiaIniciada | Ciclo de vida | A réplica primária iniciou uma cópia parcial | Replicador Treplicator | Informativo |
| 16831 | BuildIdlereplicainiciado | Ciclo de vida | Réplica primária começou a construir réplica ociosa | Replicação | Informativo |
| 16832 | BuildIdlereplicaconcluída | Ciclo de vida | Réplica primária completou a construção de réplica ociosa | Replicação | Informativo |
| 16833 | BuildIdleReplicaFailed | Ciclo de vida | A réplica primária falhou na construção de réplica ociosa | Replicação | Aviso |
| 16834 | Replicação primáriaQueueFull | Integridade | A fila de replicação da réplica primária está cheia | Replicação | Aviso |
| 16835 | PrimaryReplicationQueueWarning | Integridade | A fila de replicação da réplica primária está quase cheia | Replicação | Aviso |
| 16836 | Queue de replicação primáriaAvisomitigado | Integridade | A fila de replicação da réplica primária está bem | Replicação | Informativo |
| 16837 | Replicação secundáriaQueueFull | Integridade | A fila de replicação da réplica secundária está cheia | Replicação | Aviso |
| 16838 | Advertência de replicação secundária | Integridade | A fila de replicação da réplica secundária está quase cheia | Replicação | Aviso |
| 16839 | Queue de replicação secundáriaAvisomitigado | Integridade | A fila de replicação da réplica secundária está bem | Replicação | Informativo |
| 16840 | PrimaryFaultedSlowSecondary | Integridade | A réplica primária falhou uma réplica secundária lenta | Replicação | Aviso |
| 16841 | ReplicadorCom falta | Integridade | Réplica tem falha | Replicação | Aviso |
