...
`InstanceCount` para o serviço indique o número desejado de instâncias que precisam estar em execução. Quando qualquer (ou todas) das instâncias falhar, Service Fabric responderá criando automaticamente as instâncias de substituição em outros nós. Service Fabric continuará criando substituições até que o serviço volte à contagem de instâncias desejada.
Outro exemplo, se o serviço sem estado tiver `InstanceCount` de-1, o que significa que uma instância deve ser executada em cada um dos nós no cluster. Se algumas dessas instâncias tiverem falhado, Service Fabric detectará que o serviço não está em seu estado desejado e tentará criar as instâncias nos nós em que estão ausentes.

#### <a name="stateful-services"></a>Serviços com estado
Há dois tipos de serviços com estado:
1. Com estado persistente
2. Com estado sem persistência (o estado é armazenado na memória)

A recuperação de falha de serviço com estado depende do tipo de serviço com estado e de quantas réplicas o serviço tinha e de quantas falharam.

O que é quorum?
Em um serviço com estado, os dados de entrada são replicados entre as réplicas (primária e ActiveSecondary). Se a maioria das réplicas receber os dados, os dados serão considerados quorum confirmados (para 5 réplicas, 3 será _Quorum_). Isso significa que, em qualquer momento, é garantido que haverá pelo menos o quorum de réplicas com os dados mais recentes. Se as réplicas falharem (digamos que 2 de 5 réplicas falharam), podemos usar o valor de quorum para calcular se pudermos recuperar (desde que as 3 das cinco réplicas restantes ainda estejam ativas), é garantido que pelo menos uma réplica terá dados completos).

O que é perda de quorum?
Quando o quorum de réplicas falha, a partição é declarada como está no estado de perda de quorum. Digamos que uma partição tenha 5 réplicas, o que significa que pelo menos 3 tem a garantia de ter dados completos. Se o quorum (3 de 5) de réplicas falhar, Service Fabric não poderá determinar se as réplicas restantes (2 de saída 5) têm dados suficientes para restaurar a partição.

Determinar se um desastre ocorreu para um serviço com estado e gerenciá-lo é um processo de três estágios:
...
2. Determinar se a perda de quorum é permanente ou não
   - Na maioria das vezes, as falhas são transitórias. Os processos são reiniciados, os nós são reiniciados, as VMs são relançadas e as partições de rede se recuperam. No entanto, algumas vezes as falhas são permanentes. 
     - Para serviços sem estado persistente, a falha de um quorum ou mais de réplicas causa _imediatamente_ uma perda de quorum permanente. Quando Service Fabric detectar perda de quorum em um serviço não persistente com estado, ele passará imediatamente para a etapa 3 declarando (potencial) perda de dados. Prosseguir com a perda de dados faz sentido porque Service Fabric sabe que não há nenhum ponto em esperar que as réplicas voltem, porque mesmo se elas estivessem sendo recuperadas, devido à natureza não persistente do serviço, os dados foram perdidos.
     - Para serviços persistentes com estado, uma falha de um quorum ou mais de réplicas faz Service Fabric aguardar que as réplicas voltem e restaurem o quorum. Isso resulta em uma interrupção de serviço para qualquer _gravação_ na partição afetada (ou "conjunto de réplicas") do serviço. No entanto, leituras ainda podem ser possíveis, com garantias de consistência reduzidas. A quantidade de tempo padrão que o Service Fabric aguarda o quorum ser restaurado é infinita, uma vez que prosseguir é um evento de (potencial) perda de dados e traz consigo outros riscos.
...
As réplicas podem estar inativas intermitentemente devido a uma falha transitória. Aguarde algum tempo enquanto Service Fabric tentará trazê-las. Se as réplicas estiverem inativas por mais tempo que a duração esperada, siga as etapas de solução de problemas abaixo.
- As réplicas podem estar falhando. Verifique os relatórios de integridade no nível da réplica e os logs do aplicativo. Colete despejos de memória e execute a ação necessária para recuperar.
- O processo de réplica pode ter se tornado sem resposta. Inspecione os logs de aplicativo para verificar isso. Colete o despejo do processo e, em seguida, encerre o processo sem resposta. Service Fabric criará o processo de substituição e tentará retornar a réplica.
- Os nós que hospedam as réplicas podem estar inativos. Reinicie a VM subjacente para trazer os nós para cima.

Pode haver situações em que não é possível recuperar réplicas por exemplo, as unidades podem ter falhado ou os computadores não estão respondendo fisicamente. Nesses casos, Service Fabric precisa ser instruído a não esperar pela recuperação de réplica.
Não use esses métodos se a possível perda de dados não for aceitável para colocar o serviço online nesse caso, todos os esforços devem ser feitos para a recuperação de computadores físicos.

As etapas a seguir podem resultar em perda de data. Certifique-se antes de seguir:
   
> [!NOTE]
> _Nunca_ é seguro usar esses métodos além de uma maneira direcionada contra partições específicas. 
>

- Use `Repair-ServiceFabricPartition -PartitionId` ou `System.Fabric.FabricClient.ClusterManagementClient.RecoverPartitionAsync(Guid partitionId)` API. Essa API permite especificar a ID da partição a ser recuperada do QuorumLoss com potencial perda de dados.
- Se o cluster encontrar falhas frequentes fazendo com que os serviços entrem no estado de perda de quorum e a possível _perda de dados for aceitável_, a especificação de um valor [QuorumLossWaitDuration](https://docs.microsoft.com/powershell/module/servicefabric/update-servicefabricservice?view=azureservicefabricps) apropriado poderá ajudar a recuperação automática do serviço. Service Fabric aguardará o QuorumLossWaitDuration fornecido (o padrão é infinito) antes de executar a recuperação. Esse método _não é recomendado_ porque isso pode causar perdas de dados inesperadas.
