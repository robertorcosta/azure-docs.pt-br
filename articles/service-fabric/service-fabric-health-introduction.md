---
title: Monitoramento de saúde em Malha de Serviços
...
As entidades de integridade espelham as entidades do Service Fabric. (Por exemplo, **a entidade de aplicativo de saúde** corresponde a uma instância de aplicativo implantada no cluster, enquanto a entidade do nó de **saúde** corresponde a um nó de malha de malha de serviço.) A hierarquia em saúde capta as interações das entidades do sistema, e é a base para a avaliação avançada da saúde. Você pode aprender sobre os principais conceitos do Service Fabric em [Visão geral técnica do Service Fabric](service-fabric-technical-overview.md). Para saber mais sobre aplicativos, confira [Modelo de aplicativo do Service Fabric](service-fabric-application-model.md).
...
* **Ok, eu**não sei o que fazer A entidade está íntegra. Não há nenhum problema conhecido reportado em seus filhos (quando aplicável).
...
* [ConsidereWarningasError](https://docs.microsoft.com/dotnet/api/system.fabric.health.clusterhealthpolicy.considerwarningaserror). Especifica se os relatórios de integridade Aviso devem ser tratados como erros durante a avaliação de integridade. Padrão: falso.
...
* **FonteId**. Uma cadeia de caracteres que identifica exclusivamente o relator do evento de integridade.
