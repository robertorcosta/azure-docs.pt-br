---
title: Visão geral do monitoramento e diagnóstico do azure Service Fabric
...
Os diagnósticos fornecidos estão na forma de um conjunto abrangente de eventos prontos para uso. Esses [eventos de Malha de Serviço](service-fabric-diagnostics-events.md) ilustram ações feitas pela plataforma em diferentes entidades, como Nodes, Aplicativos, Serviços, Partições etc. No último cenário acima, se um nó fosse para baixo, `NodeDown` a plataforma emitisse um evento e você poderia ser notificado imediatamente pela sua ferramenta de monitoramento de escolha. Outros exemplos comuns incluem `ApplicationUpgradeRollbackStarted` ou `PartitionReconfigured` durante um failover. **Os mesmos eventos estão disponíveis nos clusters Windows e Linux.**

Os eventos são enviados através de canais padrão no Windows e no Linux e podem ser lidos por qualquer ferramenta de monitoramento que os suporte. A solução do Monitor Azure é o Azure Monitor logs. Sinta-se livre para ler mais sobre nossa [integração de logs do Azure Monitor,](service-fabric-diagnostics-event-analysis-oms.md) que inclui um painel operacional personalizado para o seu cluster e algumas consultas de exemplo das quais você pode criar alertas. Mais conceitos de monitoramento de cluster estão disponíveis em [evento de nível de plataforma e geração de log](service-fabric-diagnostics-event-generation-infra.md).
...
Service Fabric também fornece um conjunto de contadores de desempenho para os modelos de programação de Serviços confiáveis e atores. Se você estiver usando um desses modelos, esses contadores de desempenho poderão fornecer informações para garantir que seus atores estejam girando corretamente, ou que suas solicitações de serviço confiáveis sejam tratadas com rapidez suficiente. Para obter mais informações, consulte [Monitoramento para comunicação remota do Reliable Service](service-fabric-reliable-serviceremoting-diagnostics.md#performance-counters) e [Monitoramento de desempenho para Reliable Actors](service-fabric-reliable-actors-diagnostics.md#performance-counters). 

A solução do Azure Monitor para recolhê-los é o Azure Monitor logs, assim como o monitoramento de nível da plataforma. Você deve usar o [agente Log Analytics](service-fabric-diagnostics-oms-agent.md) para coletar os contadores de desempenho apropriados e visualizá-los nos registros do Monitor Do Azure.
...
* Monitoramento de cluster [com registros do Monitor de Diagnósticos](service-fabric-diagnostics-event-aggregation-wad.md) e do Monitor do [Azure](service-fabric-diagnostics-oms-setup.md)
* Monitoramento de infra-estrutura com [registros do Monitor Do Azure](service-fabric-diagnostics-oms-agent.md)
...
Embora as duas soluções que recomendamos, os [logs do Azure Monitor](service-fabric-diagnostics-event-analysis-oms.md) e [o Application Insights](service-fabric-diagnostics-event-analysis-appinsights.md) tenham construído em integração com o Service Fabric, muitos eventos são escritos através de provedores de ETW e são extensíveis com outras soluções de registro. Você também deve examinar o [Elastic Stack](https://www.elastic.co/products) (especialmente se estiver considerando a execução de um cluster em um ambiente offline), [Dynatrace](https://www.dynatrace.com/), ou qualquer outra plataforma de sua preferência. Temos uma lista de parceiros integrados disponíveis [aqui](service-fabric-diagnostics-partners.md).
...
* Configure a integração de logs do Monitor do Azure com o Service Fabric no [Configure logs do Monitor Azure para um cluster](service-fabric-diagnostics-oms-setup.md)
