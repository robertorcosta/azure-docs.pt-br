...
Neste artigo, fornecemos recomendações para integrar uma implantação da AVS (solução do Azure VMware) em uma [arquitetura de Hub e spoke](/azure/architecture/reference-architectures/hybrid-networking/shared-services) existente ou nova no Azure. 
...
As conexões do ExpressRoute permitem que o tráfego flua entre o local, a AVS e a malha de rede do Azure. O AVS usa o [ExpressRoute alcance global](../expressroute/expressroute-global-reach.md) para implementar essa conectividade.
...
Você pode encontrar mais detalhes sobre a rede AVS e conceitos de interconectividade na [documentação do produto AVS](./concepts-networking.md).
