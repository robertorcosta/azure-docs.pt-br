...
Neste artigo, vamos percorrer os procedimentos para fazer backup de VMs (máquinas virtuais) VMware em execução na solução VMware do Azure usando Servidor de Backup do Azure. Antes de começar, verifique se você definiu minuciosamente a [configuração do servidor de backup do Microsoft Azure para a solução VMware do Azure](set-up-mabs-for-avs.md).

Em seguida, percorremos todos os procedimentos necessários para:
...
## <a name="add-the-provisioning-ip-address-for-azure-vmware-solution-esxi-hosts-on-azure-backup-server"></a>Adicione o endereço IP de provisionamento para hosts ESXi da solução Azure VMware no Servidor de Backup do Azure

Durante a versão prévia, a solução Azure VMware não resolve o host ESX da máquina virtual implantada na rede virtual. Você precisará executar etapas adicionais para adicionar a entrada do arquivo de host na máquina virtual Servidor de Backup do Azure.

### <a name="identify-the-ip-address-for-esxi-hosts"></a>Identificar o endereço IP para hosts ESXi

1. Abra o navegador e entre nas URLs do vCenter. 

   > [!TIP]
   > Você pode encontrar as URLs em [conectar-se ao vCenter local de sua nuvem privada](tutorial-access-private-cloud.md#connect-to-the-local-vcenter-of-your-private-cloud).

1. No cliente vSphere, selecione o cluster para o qual você planeja habilitar o backup.

   :::image type="content" source="media/avs-backup/vsphere-client-select-host.png" alt-text="Cliente VSphere – selecionar host":::

1. Selecione **Configurar**  >  **rede**  >  **adaptadores VMkernel**. Na lista de dispositivos, identifique o adaptador de rede que tem a função de **provisionamento** habilitada. Anote o **endereço IP** e o nome do host ESXi.

   :::image type="content" source="media/avs-backup/vmkernel-adapters-provisioning-enabled.png" alt-text="Adaptadores VMKernel – provisionamento de dispositivos habilitados":::

1. Repita a etapa anterior para cada host ESXi em cada cluster para o qual você planeja habilitar o backup.

### <a name="update-the-host-file-on-azure-backup-server"></a>Atualizar o arquivo de host no Servidor de Backup do Azure

1. Abra o Bloco de Notas como um administrador.

1. Selecione **arquivo**  >  **abrir**e procure c:\Windows\System32\Drivers\etc\hosts.

1. Adicione a entrada para cada host ESXi junto com o endereço IP que você identificou na seção anterior.
...
1. Na caixa de diálogo **gerenciar credenciais** , selecione **Adicionar**.
   ...
   ![Caixa de diálogo Gerenciar Credenciais do Servidor de Backup do Azure](../backup/media/backup-azure-backup-server-vmware/mabs-manage-credentials-dialog.png)
...
1. Selecione **Adicionar** para adicionar a nova credencial.
   ...
   ![Caixa de diálogo Gerenciar Credenciais do Servidor de Backup do Azure](../backup/media/backup-azure-backup-server-vmware/new-list-of-mabs-creds.png)
...
1. Na página **concluir** , examine as configurações e selecione **fechar**.
   ...
   Você deve ver o servidor do vCenter listado **em servidor de produção** com o tipo de **servidor VMware** e o **status do agente** como **OK**. Se você vir o **status do agente** como **desconhecido**, selecione **Atualizar**.
...
1. Especifique por quanto tempo deseja manter os dados de backup em disco.
   ...
   :::image type="content" source="media/avs-backup/new-protection-group-specify-short-term-goals.png" alt-text="Especifique seus objetivos de curto prazo para a proteção baseada em disco":::
...
1. Na página **examinar armazenamento em disco alocação** , examine o espaço em disco fornecido para os backups de VM.
   ...
   - **Espaço em disco:** Quantidade recomendada de espaço em disco para o grupo de proteção. Se você quiser modificar essa configuração, aloque o espaço total que seja ligeiramente maior do que o valor estimado por cada fonte de dados.
   ...
   :::image type="content" source="media/avs-backup/review-disk-allocation.png" alt-text="Examinar o espaço em disco alocado no pool de armazenamento":::
...
1. Na página **especificar agendamento de backup online** , indique com que frequência você deseja fazer backup de dados do armazenamento local no Azure e, em seguida, selecione **Avançar**. 
...
1. Na página **especificar política de retenção online** , indique por quanto tempo deseja manter os pontos de recuperação criados a partir dos backups diários, semanais, mensais ou anuais para o Azure e, em seguida, selecione **Avançar**.
...
- Na guia **alertas** do painel **monitoramento** , você pode monitorar erros, avisos e informações gerais para um grupo de proteção, para um computador protegido específico ou por severidade de mensagem. Você pode exibir alertas ativos e inativos e configurar notificações por email.
- Na guia **trabalhos** do painel **monitoramento** , você pode exibir os trabalhos iniciados por servidor de backup do Azure para uma fonte de dados protegida específica ou um grupo de proteção. Você pode seguir o progresso do trabalho ou verificar os recursos consumidos pelos trabalhos.
...
:::image type="content" source="media/avs-backup/monitor-backup-jobs-in-mabs.png" alt-text="Monitorar o status dos trabalhos de backup no Servidor de Backup do Azure":::
...
1. Usando o painel **Procurar**, procure ou filtre para localizar a VM que você deseja recuperar. Depois de selecionar uma VM ou pasta, o painel **pontos de recuperação** exibe os pontos de recuperação disponíveis.

   ![Pontos de recuperação disponíveis](../backup/media/restore-azure-backup-server-vmware/recovery-points.png)

1. No painel **pontos de recuperação para** , use os menus de calendário e suspenso para selecionar uma data em que um ponto de recuperação foi tirado. Datas do calendário em negrito têm pontos de recuperação disponíveis. Como alternativa, você pode clicar com o botão direito do mouse na VM e selecionar **Mostrar todos os pontos de recuperação** e, em seguida, selecionar o ponto de recuperação na lista.
...
1. Antes de recuperar de um ponto de recuperação online, verifique se o local de preparo contém espaço livre suficiente para abrigar o tamanho completo descompactado da VM que você deseja recuperar. O local de preparo pode ser exibido ou alterado executando o **Assistente para definir configurações de assinatura**.
   ...
   :::image type="content" source="media/avs-backup/mabs-recovery-folder-settings.png" alt-text="Servidor de Backup do Azure configurações da pasta de recuperação":::
...
1. Na página **Selecionar tipo de recuperação** , escolha se deseja recuperar na instância original ou em um novo local e, em seguida, selecione **Avançar**.
...
1. Na tela **especificar opções de recuperação** , siga um destes procedimentos:
...
1. Na tela **especificar destino** , selecione **procurar** para localizar um local de rede para seus arquivos ou pastas. Servidor de Backup do Azure cria uma pasta na qual todos os itens recuperados são copiados. O nome da pasta tem o prefixo MABS_day-mês-ano. Quando você seleciona um local para os arquivos ou a pasta recuperados, são fornecidos os detalhes desse local, como **destino**, **caminho de destino**e **espaço disponível**.
...
> [Guia de solução de problemas para Servidor de Backup do Azure](../backup/backup-azure-mabs-troubleshoot.md)
