---
title: Criptografia de dados com chave gerenciada pelo cliente - Banco de dados Azure para PostgreSQL - Servidor único
description: O banco de dados Do Zure para criptografia de dados de servidor único do PostgreSQL com uma chave gerenciada pelo cliente permite que você traga sua própria chave (BYOK) para proteção de dados em repouso. Também permite que as organizações implementem a separação de deveres na gestão de chaves e dados.
author: kummanish
ms.author: manishku
...
> [!NOTE]
> Esse recurso está disponível em todas as regiões do Azure, onde o Banco de Dados Azure para servidor PostgreSQL Único suporta níveis de preços "Propósito Geral" e "Otimização da Memória".

## <a name="benefits"></a>Benefícios

A criptografia de dados para o banco de dados Azure para servidor PostgreSQL Single fornece os seguintes benefícios:
...
Para que um servidor PostgreSQL use chaves gerenciadas pelo cliente armazenadas no Key Vault para criptografia do DEK, um administrador do Key Vault fornece os seguintes direitos de acesso ao servidor:
...
## <a name="requirements-for-configuring-data-encryption-for-azure-database-for-postgresql-single-server"></a>Requisitos para configurar a criptografia de dados para o Banco de Dados Azure para servidor único PostgreSQL

A seguir, os requisitos para configurar o Key Vault:

* O key vault e o azure Database para servidor PostgreSQL Single devem pertencer ao mesmo inquilino do Azure Active Directory (Azure AD). As interações entre o Cofre de Chaves e o servidor não são suportadas. A movimentação de recursos depois requer que você reconfigure a criptografia de dados.
...
* Conceda ao Banco de Dados Do Azure o acesso do servidor PostgreSQL ao cofre principal com as permissões get, wrapKey e unwrapKey usando sua identidade gerenciada exclusiva. No portal Azure, a identidade única é criada automaticamente quando a criptografia de dados é ativada no servidor PostgreSQL Single. Consulte [a criptografia de dados para o banco de dados Do Zure para servidor PostgreSQL Único usando o portal Azure](howto-data-encryption-portal.md) para obter instruções detalhadas e passo a passo quando estiver usando o portal Azure.
...
* Certifique-se de que o Key Vault e o Azure Database para servidor PostgreSQL Single residam na mesma região, para garantir um acesso mais rápido às operações de envoltório e desembrulho do DEK.
...
* Inicie o processo de criação de réplicas de restauração ou leitura do master Azure Database para servidor PostgreSQL Single.
...
Saiba como configurar a [criptografia de dados com uma chave gerenciada pelo cliente para o banco de dados Do Azure para o servidor PostgreSQL Single usando o portal Azure](howto-data-encryption-portal.md).
