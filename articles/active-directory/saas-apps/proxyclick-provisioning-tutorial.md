...
6. Entre no console do [administrador do Proxyclick](https://app.proxyclick.com/login//?destination=%2Fdefault). Navegue até **configurações** > **integrações** > **procurar Marketplace**.
    ...
    A interface do usuário de configuração das configurações apropriadas agora deve aparecer em **configurações** > **integrações**. Selecione **configurações** no **Azure AD (provisionamento de usuário)** .
