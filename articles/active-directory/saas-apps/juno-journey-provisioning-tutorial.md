---
title: 'Tutorial: Configure juno journey para provisionamento automático do usuário com o Azure Active Directory | Microsoft Docs'
description: Aprenda a provisionar e desprovisionar automaticamente contas de usuários do Azure AD para juno journey.
...
ms.locfileid: "81641843"
---
# <a name="tutorial-configure-juno-journey-for-automatic-user-provisioning"></a>Tutorial: Configure Juno Journey para provisionamento automático do usuário

Este tutorial descreve as etapas que você precisa executar tanto no Juno Journey quanto no Azure Active Directory (Azure AD) para configurar o provisionamento automático do usuário. Quando configurado, o Azure AD fornece automaticamente e desprovisiona usuários e grupos para [a Juno Journey](https://www.junojourney.com/) usando o serviço de provisionamento Ad do Azure. Para detalhes importantes sobre o que esse serviço faz, como funciona e as perguntas frequentes, consulte [Automatizar o provisionamento e desprovisionamento de usuários para aplicativos SaaS com o Azure Active Directory](../manage-apps/user-provisioning.md). 


## <a name="capabilities-supported"></a>Recursos suportados
> [!div class="checklist"]
> * Criar usuários na Juno Journey
> * Remova os usuários na Juno Journey quando eles não precisarem mais de acesso
> * Mantenha os atributos do usuário sincronizados entre o Azure AD e o Juno Journey
> * [Único login em](https://docs.microsoft.com/azure/active-directory/saas-apps/juno-journey-tutorial) Juno Journey (recomendado)
...
*  Um [inquilino da Juno Journey.](https://www.junojourney.com/getstartedwithjuno)
*  Uma conta de usuário em Juno Journey com permissões de administração.
...
3. Determine quais dados [a mapear entre o Azure AD e o Juno Journey](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes). 

## <a name="step-2-configure-juno-journey-to-support-provisioning-with-azure-ad"></a>Etapa 2. Configure juno journey para suportar provisionamento com a Azure AD

1. Para **o Contact Secret Token** e O URL do **inquilino** entre em contato com a equipe de suporte da Juno Journey em support@the-juno.com. Esse valor será inserido nos campos **De Token Secreto** e **URL do Inquilino,** respectivamente, na guia Provisionamento do aplicativo Juno Journey no portal Azure. 

## <a name="step-3-add-juno-journey-from-the-azure-ad-application-gallery"></a>Etapa 3. Adicionar Juno Journey da galeria de aplicativos Azure AD

Adicione Juno Journey da galeria de aplicativos Azure AD para começar a gerenciar o provisionamento para a Juno Journey. Se você já configurou juno journey para SSO, você pode usar o mesmo aplicativo. No entanto, é recomendável que você crie um aplicativo separado ao testar a integração inicialmente. Saiba mais sobre como adicionar um aplicativo na galeria [aqui](https://docs.microsoft.com/azure/active-directory/manage-apps/add-gallery-app). 
...
* Ao atribuir usuários e grupos ao Juno Journey, você deve selecionar uma função diferente **do Default Access**. Os usuários com a função Default Access são excluídos do provisionamento e serão marcados como não efetivamente habilitados nos registros de provisionamento. Se a única função disponível no aplicativo for a função de acesso padrão, você pode [atualizar o manifesto do aplicativo](https://docs.microsoft.com/azure/active-directory/develop/howto-add-app-roles-in-azure-ad-apps) para adicionar funções adicionais. 
...
## <a name="step-5-configure-automatic-user-provisioning-to-juno-journey"></a>Etapa 5. Configure o provisionamento automático do usuário para a Juno Journey 
...
### <a name="to-configure-automatic-user-provisioning-for-juno-journey-in-azure-ad"></a>Para configurar o provisionamento automático do usuário para Juno Journey no Azure AD:
...
2. Na lista de aplicativos, escolha **Juno Journey**.
    ...
    ![O link Juno Journey na lista de aplicativos](common/all-applications.png)
...
5. Na seção **Credenciais de Admin,** insira o valor de URL do inquilino recuperado anteriormente na **URL do inquilino**. Insira o valor do token secreto recuperado anteriormente no **Secret Token**. Clique **em Conexão de teste** para garantir que o Azure AD possa se conectar ao Juno Journey. Se a conexão falhar, certifique-se de que sua conta juno journey tenha permissões de administração e tente novamente.
...
8. Na seção **Mapeamentos,** selecione **Sincronizar usuários do diretório ativo do Azure para a Jornada juno**.

9. Revise os atributos do usuário sincronizados do Azure AD para o Juno Journey na seção **Mapeamento de atributos.** Os atributos selecionados como **propriedades de correspondência** são usados para corresponder às contas de usuário no Juno Journey para operações de atualização. Se você optar por alterar o [atributo de destino correspondente,](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)você precisará garantir que a API Juno Journey suporte usuários filtrantes com base nesse atributo. Selecione o botão **Salvar** para confirmar as alterações.
...
11. Para habilitar o serviço de provisionamento Azure AD para a Juno Journey, altere o **Status de Provisionamento** para **Ativado** na seção **Configurações.**

    ![Status do provisionamento ativado](common/provisioning-toggle-on.png)

12. Defina os usuários e/ou grupos que você gostaria de prover para a Juno Journey escolhendo os valores desejados no **Escopo** na seção **Configurações.**
