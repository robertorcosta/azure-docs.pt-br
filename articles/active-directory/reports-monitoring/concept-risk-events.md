...
> [!NOTE]
> Se o nome de usuário e a senha de logon não corresponderem, a entrada falhará e a detecção de risco não ocorrerá. A entrada de detecções de risco de local não familiares só é disparada em logons com êxito.
...
Locais desconhecidos podem fornecer uma forte indicação de que um invasor é capaz de usar uma identidade roubada. Falsos positivos podem ocorrer quando um usuário está viajando, experimentando um novo dispositivo ou usando uma nova VPN. Como resultado desses falsos positivos, o nível de risco para esse tipo de evento é **médio**.
