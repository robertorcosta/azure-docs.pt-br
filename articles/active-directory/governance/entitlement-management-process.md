...
| Aprovação pendente | If the policy for an access package requires approval, a request moves to pending approval. |
| Expirado | If no approvers approve a request within the approval request timeout, the request expires. To try again, the user will have to resubmit their request. |
| Denied | Approver denies a request. |
| Aprovado | Approver approves a request. |
| Fornecimento | User has **not** been assigned access to all the resources in the access package. If this is an external user, the user may not have accessed the resource directory yet. They also may not have accepted the consent prompt. |
| Entregue | User has been assigned access to all the resources in the access package. |
...
| # | Assunto do email | When sent | Sent to |
| --- | --- | --- | --- |
