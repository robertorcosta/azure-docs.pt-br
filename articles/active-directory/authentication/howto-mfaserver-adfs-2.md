...
4. Para detectar o nome de usuário, a senha e as variáveis de domínio automaticamente, insira `https://sso.contoso.com/adfs/ls`a URL de logon (como) na caixa de diálogo Configurar automaticamente site baseado em formulário e clique em **OK**.
...
7. Na caixa de diálogo Adicionar site baseado em formulário, digite a URL para a página de logon AD FS no campo URL de envio ( `https://sso.contoso.com/adfs/ls`como) e insira um nome de aplicativo (opcional). O nome do aplicativo aparece nos relatórios da Autenticação Multifator do Azure e pode ser exibido nas mensagens de autenticação por SMS ou Aplicativo Móvel.
...
17. Clique na caixa **habilitar autenticação IIS** na parte superior da tela.
...
3. Clique em **Adicionar**.
4. Na caixa de diálogo Adicionar URL base, insira a URL para o site AD FS em que a autenticação HTTP é executada `https://sso.domain.com/adfs/ls/auth/integrated`(como) no campo URL base. Em seguida, insira um nome de aplicativo (opcional). O nome do aplicativo aparece nos relatórios da Autenticação Multifator do Azure e pode ser exibido nas mensagens de autenticação por SMS ou Aplicativo Móvel.
...
10. Clique na caixa **habilitar autenticação IIS** na parte superior da tela.
