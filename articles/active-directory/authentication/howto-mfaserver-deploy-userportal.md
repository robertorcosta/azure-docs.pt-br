---
title: Portal de usuários para Azure MFA Server - Azure Active Directory
...
![Página de login do Portal do Usuário do Servidor MFA](./media/howto-mfaserver-deploy-userportal/portal.png)
...
4. Vincule um Certificado TLS/SSL ao site no IIS.

Se você tiver dúvidas sobre a configuração de um Certificado TLS/SSL em um servidor IIS, consulte o artigo [Como configurar o SSL no IIS](https://docs.microsoft.com/iis/manage/configuring-security/how-to-set-up-ssl-on-iis).

O SDK do Serviço Web deve ser protegido com um certificado TLS/SSL. Um certificado autoassinado é suficiente para essa finalidade. Importe o certificado para a loja "Trusted Root Certification Authorities" da conta do Computador Local no servidor web do Portal do Usuário para que ele confie nesse certificado ao dar início à conexão TLS.
...
Os seguintes pré-requisitos são necessários para instalar o portal do usuário no **mesmo servidor** do Servidor de Autenticação Multifatorial do Azure:
...
* Proteja o SDK do Serviço Web de Autenticação Multifato do Azure com um certificado TLS/SSL.
...
3. Vincular um certificado TLS/SSL ao site no IIS

   > [!NOTE]
   > Este Certificado TLS/SSL é geralmente um certificado TLS/SSL assinado publicamente.

4. Abra um navegador web de qualquer computador e navegue até a `https://mfa.contoso.com/MultiFactorAuth`URL onde o portal do usuário foi instalado (Exemplo: ). Certifique-se de que nenhum aviso de certificado ou erro seja exibido.
...
Se o servidor onde o Azure Multi-Factor Authentication Server está sendo executado não estiver voltado para a Internet, você deve instalar o portal do usuário em um **servidor separado e voltado para a Internet**.
...
* Proteja o portal do usuário com um certificado TLS/SSL.
* Proteja o SDK do Serviço Web de Autenticação Multifato do Azure com um certificado TLS/SSL.
* Certifique-se de que o portal do usuário possa se conectar ao SDK de autenticação multifatorial do Azure sobre TLS/SSL.
...
3. Vincule um Certificado TLS/SSL ao site no IIS.
...
5. Edite o arquivo Web.Config no Bloco de Notas
    ...
    * Encontre o **https://www.contoso.com/MultiFactorAuthWebServiceSdk/PfWsSdk.asmx** valor e altere essa URL de espaço reservado para a URL SDK do Serviço Web que instalamos na etapa 2.
    * Salve o arquivo Web.Config e feche o Bloco de Notas.

6. Abra um navegador web de qualquer computador e navegue até a `https://mfa.contoso.com/MultiFactorAuth`URL onde o portal do usuário foi instalado (Exemplo: ). Certifique-se de que nenhum aviso de certificado ou erro seja exibido.
...
3. Defina quem deve ser administradores na guia **Administradores.** Você pode criar permissões administrativas granulares usando as caixas de seleção e as gotas nas caixas Adicionar/Editar.
...
> [!IMPORTANT]
> A partir de março de 2019, as opções de chamadas telefônicas não estarão disponíveis para usuários do MFA Server em inquilinos Ad gratuitos/de teste. As mensagens SMS não são impactadas por essa mudança. A chamada telefônica continuará disponível para os usuários em inquilinos ad pagos do Azure. Essa alteração só afeta os inquilinos azure AD gratuitos/trial.
...
![Gerencie sua conta do MFA Server usando o portal do usuário](./media/howto-mfaserver-deploy-userportal/portalsettings.png)

### <a name="self-service-user-enrollment"></a>Registro de usuário de autoatendimento

Se você quiser que seus usuários façam login e se inscrevam, você deve selecionar o **Permitir que os usuários façam login** e permitir opções de inscrição do **usuário** na guia Configurações. Lembre-se de que as configurações selecionadas afetam a experiência de login do usuário.
...
Se o usuário for solicitado a usar um PIN quando autenticar, a página também lhe pedirá para criar um PIN. Depois de digitar seu número de telefone e PIN (se aplicável), o usuário clica no botão **Telefonar para Mim para Autenticar**. A Autenticação Multifatorial do Azure realiza uma verificação de chamada telefônica para o número de telefone principal do usuário. O usuário deve responder à chamada telefônica, inserir o PIN (se aplicável) e pressionar # para passar para a próxima etapa do processo de auto-registro.

Se o usuário selecionar o método de verificação de Mensagem de Texto ou tiver sido pré-configurado para usar esse método, a página solicitará ao usuário seu número de telefone celular. Se o usuário for solicitado a usar um PIN quando autenticar, a página também lhe pedirá para inserir um PIN.  Depois de digitar seu número de telefone e PIN (se aplicável), o usuário clica no botão **Enviar uma Mensagem de Texto para Autenticar**. A Autenticação Multifatorial do Azure realiza uma verificação de SMS no celular do usuário. O usuário recebe a mensagem de texto com uma OTP (senha de uso único) e responde à mensagem com esta OTP e seu PIN (se aplicável).

![Verificação do portal do usuário usando SMS](./media/howto-mfaserver-deploy-userportal/text.png)
...
- [Implantar o serviço web do aplicativo móvel do servidor de autenticação multifatorial do Azure](howto-mfaserver-deploy-mobileapp.md)
