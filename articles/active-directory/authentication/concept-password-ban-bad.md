...
Embora a lista global de proibidos da Microsoft seja pequena em comparação a algumas listas em massa de terceiros, seus efeitos de segurança são amplificados pelo fato de que ele é originado da telemetria de segurança do mundo real em ataques de irrigação de senha reais, além do fato de que o algoritmo de validação de senha da Microsoft usa técnicas de correspondência de fuzzing inteligente. O resultado final é que ele detectará com eficiência e bloqueará a utilização de milhões de senhas fracas mais comuns em sua empresa. Os clientes que optarem por adicionar termos específicos da organização à lista de senhas excluídas personalizada também se beneficiarão do mesmo algoritmo.
...
| Usuários | Proteção por senha do AD do Azure com a lista de senhas banidas global | Proteção por senha do AD do Azure com a lista de senhas banidas personalizado|
| --- | --- | --- |
