...
6. Select whether you are defining the location using IP ranges or Countries/Regions
   1. If using IP Ranges
      1. Decide whether to mark the location as Trusted. Conectar de um local confiável diminui o risco de entrada do usuário. Only mark this location as trusted if you know the IP ranges entered are established and credible in your organization.
...
A mobile app like the Microsoft Authenticator app generates a new OATH verification code every 30 seconds. O usuário digita o código de verificação na interface de entrada. The mobile app option can be used whether or not the phone has a data or cellular signal.
...
A finalidade dessa configuração é determinar o que fazer quando um usuário não estiver inscrito na MFA. The effects of changing this setting are listed in the table below.

| Configurações | User MFA Status | Effects |
| --- | --- | --- |
