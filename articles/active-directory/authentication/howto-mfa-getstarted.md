---
title: Considerações de implantação para a autenticação multifator do Azure
description: Saiba mais sobre as considerações e a estratégia de implantação para a implementação bem-sucedida da autenticação multifator do Azure
...
Este guia de implantação mostra como planejar e testar uma distribuição da autenticação multifator do Azure.

Para ver rapidamente a autenticação multifator do Azure em ação e, em seguida, voltar para entender as considerações de implantação adicionais:
...
Execute este PowerShell em uma janela do ISE ou salve como `.PS1` um arquivo para ser executado localmente.
...
| Chave definida como falsa ou verdadeira | Registrado | Deve autenticar com MFA |
