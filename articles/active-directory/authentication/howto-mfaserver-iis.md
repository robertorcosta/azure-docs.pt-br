---
title: Autenticação IIS e servidor MFA do Azure-Azure Active Directory
...
4. Para detectar o nome de usuário, senha e variáveis de domínio automaticamente, insira a `https://localhost/contoso/auth/login.aspx`URL de logon (como) na caixa de diálogo Configurar automaticamente site baseado em formulário e clique em **OK**.
...
10. Marque a caixa **exigir correspondência do usuário da autenticação multifator do Azure** se todos os usuários tiverem sido ou forem importados para o servidor e estiverem sujeitos à autenticação multifator. Se um número significativo de usuários ainda não tiver sido importado no Servidor e/ou for isentado da autenticação multifator, deixe a caixa desmarcada.
...
3. Clique em **Adicionar**.
4. Na caixa de diálogo Adicionar URL base, insira a URL para o site em que a autenticação HTTP é executada <http://localhost/owa>(como) e forneça um nome de aplicativo (opcional). O nome do aplicativo aparece nos relatórios da Autenticação Multifator do Azure e pode ser exibido nas mensagens de autenticação por SMS ou Aplicativo Móvel.
...
7. Marque a caixa **cache de cookies** , se desejar.
8. Clique em **OK**.
...
1. Se estiver executando no IIS 6, clique na guia **ISAPI** . Selecione o site em que o aplicativo Web está sendo executado (por exemplo, site padrão) para habilitar o plug-in de filtro ISAPI da autenticação multifator do Azure para esse site.
...
3. Clique na caixa **habilitar autenticação IIS** na parte superior da tela. A Autenticação Multifator do Azure agora está protegendo o aplicativo IIS selecionado. Verifique se os usuários foram importados para o servidor.
...
3. Quando a caixa de diálogo Adicionar IPs confiáveis for exibida, selecione o botão de opção **IP único**, **intervalo de IP**ou **sub-rede** .
4. Insira o endereço IP, o intervalo de endereços IP ou a sub-rede que deve ser permitida. Se for inserir uma sub-rede, selecione a Máscara de Rede apropriada e clique em **OK**.
