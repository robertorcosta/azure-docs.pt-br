...
MSAL.js usa um iframe oculto para executar a aquisição e a renovação de tokens silenciosas como parte das `acquireTokenSilent` chamadas. As solicitações de token silenciosas dependem do iframe que tem acesso à sessão de usuário autenticado representada pelos cookies definidos pelo Azure AD. Com o ITP 2,0 impedindo o acesso a esses cookies, MSAL.js falha ao adquirir e renovar silenciosamente tokens e isso resulta em `acquireTokenSilent` falhas.
