...
| `wids` | Matriz de GUIDs [RoleTemplateID](../roles/permissions-reference.md#all-roles) | Denota as funções de todo o locatário atribuídas a esse usuário, da seção de funções presentes nas [funções internas do Azure ad](../roles/permissions-reference.md#all-roles).  Esta declaração é configurada por aplicativo, por meio da propriedade `groupMembershipClaims` do [manifesto do aplicativo](reference-app-manifest.md).  É necessário configurá-la como "All" or "DirectoryRole".  Pode não estar presente em tokens obtidos por meio do fluxo implícito devido a problemas de comprimento do token. |
...
| `oid` | Cadeia de caracteres, um GUID | O identificador imutável para a "entidade de segurança" da solicitação-o usuário ou a entidade de serviço cuja identidade foi verificada.  Em tokens de ID e aplicativos + tokens de usuário, essa é a ID de objeto do usuário.  Em tokens somente de aplicativo, essa é a ID de objeto da entidade de serviço de chamada. Também pode ser usada para realizar verificações de autorização com segurança e como uma chave em tabelas de banco de dados. Essa ID identifica exclusivamente a entidade de segurança entre aplicativos-dois aplicativos diferentes que se conectam ao mesmo usuário receberão o mesmo valor na `oid` declaração. Portanto, `oid` pode ser usada ao fazer consultas nos serviços online da Microsoft, como o Microsoft Graph. O Microsoft Graph retornará essa ID como a propriedade `id` para uma determinada [conta de usuário](/graph/api/resources/user). Como o `oid` permite que vários aplicativos correlacionem entidades, o `profile` escopo é necessário para receber essa declaração para os usuários. Observe que, se um único usuário existir em vários locatários, o usuário conterá uma ID de objeto diferentes em cada locatário - são consideradas contas diferentes, mesmo que o usuário faça logon em cada conta com as mesmas credenciais. |
