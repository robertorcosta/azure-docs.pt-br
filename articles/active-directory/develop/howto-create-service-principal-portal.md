---
title: Crie um aplicativo Azure AD & o principal de serviço no portal
titleSuffix: Microsoft identity platform
description: Crie um novo aplicativo do Azure Active Directory & o principal de serviço para gerenciar o acesso aos recursos com controle de acesso baseado em função no Azure Resource Manager.
...
1. Nomeie o aplicativo. Selecione um tipo de conta suportada, que determina quem pode usar o aplicativo. Em **Redirecionar URI,** selecione **Web** para o tipo de aplicativo que deseja criar. Digite o URI para onde o token de acesso é enviado. Não é possível criar as credenciais para um [Aplicativo nativo](../manage-apps/application-proxy-configure-native-client-application.md). Não é possível usar esse tipo para um aplicativo automatizado. Depois de definir os valores, selecione **Registrar**.

   ![Digite um nome para sua aplicação](./media/howto-create-service-principal-portal/create-app.png)
...
1. No portal Azure, selecione o nível de escopo ao que deseja atribuir o aplicativo. Por exemplo, para atribuir uma função no escopo da assinatura, procurar e selecionar **Assinaturas**ou selecionar **Assinaturas** na **página** inicial.
...
1. Selecione a função que deseja atribuir ao aplicativo. Por exemplo, para permitir que o aplicativo execute ações como **rinicializar,** **iniciar** e **interromper** instâncias, selecione a **função Contribuinte.**  Leia mais sobre as [funções disponíveis](../../role-based-access-control/built-in-roles.md) Por padrão, os aplicativos Azure AD não são exibidos nas opções disponíveis. Para localizar seu aplicativo, pesquise o nome e selecione-o.

   ![Selecione a função a atribuir ao aplicativo](./media/howto-create-service-principal-portal/select-role.png)
...
1. Copie o ID do Diretório (inquilino) e armazene-o no código do aplicativo.
...
Os aplicativos Daemon podem usar duas formas de credenciais para autenticar com o Azure AD: certificados e segredos de aplicativos.  Recomendamos o uso de um certificado, mas você também pode criar um novo segredo de aplicativo.

### <a name="upload-a-certificate"></a>Carregar um certificado

Você pode usar um certificado existente se tiver um.  Opcionalmente, você pode criar um certificado auto-assinado apenas para *fins de teste*. Abra o PowerShell e execute [o New-SelfSignedCertificate](/powershell/module/pkiclient/new-selfsignedcertificate) com os seguintes parâmetros para criar um certificado auto-assinado na loja de certificados do usuário em seu computador: 

```powershell
$cert=New-SelfSignedCertificate -Subject "CN=DaemonConsoleCert" -CertStoreLocation "Cert:\CurrentUser\My"  -KeyExportPolicy Exportable -KeySpec Signature
```

Exporte este certificado para um arquivo usando o snap-in Do Certificado de [Usuário Gerenciar](/dotnet/framework/wcf/feature-details/how-to-view-certificates-with-the-mmc-snap-in) acessível no Painel de Controle do Windows.
...
1. Selecione **o certificado Upload** e selecione o certificado (um certificado existente ou o certificado auto-assinado que você exportou).

    ![Selecione o certificado Upload e selecione o que você deseja adicionar](./media/howto-create-service-principal-portal/upload-cert.png)

1. Selecione **Adicionar**.

Depois de registrar o certificado com seu aplicativo no portal de registro de aplicativos, você precisa habilitar o código do aplicativo do cliente para usar o certificado.

### <a name="create-a-new-application-secret"></a>Crie um novo segredo de aplicativo

Se você optar por não usar um certificado, você pode criar um novo segredo de aplicativo.
...
1. Forneça uma descrição do segredo e uma duração. Ao terminar, selecione **Adicionar**.
   ...
   Depois de salvar o segredo do cliente, o valor do segredo do cliente é exibido. Copie este valor porque você não será capaz de recuperar a chave mais tarde. Você fornecerá o valor-chave com o ID do aplicativo para fazer login como o aplicativo. Armazene o valor da chave onde seu aplicativo possa recuperá-lo.
...
Tenha em mente que você pode precisar configurar permissões adicionais nos recursos que seu aplicativo precisa acessar. Por exemplo, você também deve [atualizar as políticas de acesso de um cofre de chaves](/azure/key-vault/key-vault-secure-your-key-vault#data-plane-and-access-policies) para dar ao seu aplicativo acesso a chaves, segredos ou certificados.  

1. No [portal Azure,](https://portal.azure.com)navegue até o cofre principal e selecione **políticas de acesso**.  
1. Selecione **Adicionar política de acesso**e selecione as permissões de tecla, secreta e certificado que deseja conceder ao seu aplicativo.  Selecione o principal de serviço que você criou anteriormente.
1. Selecione **Adicionar** para adicionar a política de acesso e, em seguida, **salvar** para comprometer suas alterações.
    ![Adicionar política de acesso](./media/howto-create-service-principal-portal/add-access-policy.png)

## <a name="required-permissions"></a>Permissões necessárias

Você deve ter permissões suficientes para registrar um aplicativo com seu inquilino Azure AD e atribuir ao aplicativo uma função em sua assinatura do Azure.
...
1. Anote sua função. Se você tiver a função **Usuário**, garanta que não administradores possam registrar aplicativos.
   ...
   ![Encontre seu papel. Se você é um Usuário, certifique-se de que os não-admins podem registrar aplicativos](./media/howto-create-service-principal-portal/view-user-info.png)
...
Em sua assinatura do Azure, sua conta deve ter `Microsoft.Authorization/*/Write` acesso para atribuir uma função a um aplicativo de Anúncios. Esta ação deve ser concedida pela função [Proprietário](../../role-based-access-control/built-in-roles.md#owner) ou pela função [Administrador de Acesso do Usuário](../../role-based-access-control/built-in-roles.md#user-access-administrator). Se a sua conta for atribuída à função **contribuinte,** você não terá permissão adequada. Você receberá um erro ao tentar atribuir uma função ao diretor de serviço.
...
1. Selecione **Minhas permissões**. Em seguida, selecione **Clique aqui para exibir o acesso completo detalhes para essa assinatura**.

   ![Selecione a assinatura que deseja criar o principal do serviço em](./media/howto-create-service-principal-portal/view-details.png)

1. Selecione **Exibir** **em Funções de função** para exibir suas funções atribuídas e determine se você tem permissões adequadas para atribuir uma função a um aplicativo de Anúncio. Caso contrário, peça ao administrador da assinatura para adicioná-lo à função Administrador de Acesso do Usuário. Na imagem a seguir, o usuário é atribuído à função Proprietário, o que significa que o usuário tem permissões adequadas.

   ![Este exemplo mostra que o usuário é atribuído à função Proprietário](./media/howto-create-service-principal-portal/view-user-role.png)
