...
Quando seu aplicativo estiver aberto em várias guias e você entrar primeiro no usuário em uma guia, o usuário também será conectado nas outras guias sem ser solicitado. O MSAL. js armazena em cache o token de ID para o usuário `localStorage` no navegador e o conectará ao aplicativo nas outras guias abertas.

Por padrão, o MSAL. js `sessionStorage` usa, que não permite que a sessão seja compartilhada entre as guias. Para obter o SSO entre as guias, certifique-se `cacheLocation` de definir o no MSAL `localStorage` . js como mostrado abaixo.
...
> [!Note]
> O SID pode ser usado somente com solicitações de autenticação silenciosa `acquireTokenSilent` feitas por chamada em MSAL. js.
...
Se você não tiver a declaração de Sid configurada ou precisar ignorar o prompt de seleção de conta nas chamadas de autenticação interativas, `login_hint` poderá fazer isso fornecendo um nos parâmetros de `domain_hint` solicitação `extraQueryParameters` e, opcionalmente, como nos métodos`loginPopup`interativos do MSAL. js (, `loginRedirect` `acquireTokenPopup` e `acquireTokenRedirect`). Por exemplo:
...
* **domain_hint** só precisa ser passada ao usar a autoridade/Common. A dica de domínio é determinada pela ID de locatário (TID).  Se a `tid` declaração no token de ID for `9188040d-6c67-4c5b-b112-36a304b66dad` de consumidores. Caso contrário, é organizações.
...
Passe o `sid` se disponível (ou `login_hint` e, opcionalmente `domain_hint`) como parâmetros de solicitação para a chamada `acquireTokenSilent` MSAL. js da seguinte maneira:
...
Para aproveitar o comportamento de SSO (logon único) ao atualizar do ADAL. js, você precisará garantir que as bibliotecas estejam usando `localStorage` para os tokens de cache. Defina `cacheLocation` como `localStorage` na configuração MSAL. js e Adal. js na inicialização da seguinte maneira:
