...
Broker support is enabled on a per-PublicClientApplication basis. Isso está desabilitado por padrão. Use the `WithBroker()` parameter (set to true by default) when you create the PublicClientApplication through the PublicClientApplicationBuilder.
