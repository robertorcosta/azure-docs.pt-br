---
title: Personalizar declarações de token SAML de aplicativo do Azure AD
...
Primeiro, o Azure AD verifica se o tipo de usuário do `All guests`Brenda é. Como isso é verdadeiro, o Azure AD atribui a origem para a Declaração `user.extensionattribute1`. Em segundo lugar, o Azure AD verifica se o tipo de `AAD guests`usuário do Brenda é, pois isso também é verdadeiro e o Azure ad atribui a origem para `user.mail`a declaração. Por fim, a declaração é emitida com `user.email` o valor para Brenda.
