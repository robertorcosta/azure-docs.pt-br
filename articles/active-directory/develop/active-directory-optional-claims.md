---
title: Fornecer declarações opcionais para aplicativos do Azure AD
...
| `xms_pl`                   | Idioma preferido do usuário  | JWT ||O idioma preferido do usuário, se definido. Originado de seu locatário inicial, em cenários de acesso de convidado. Formatado LL-CC ("en-US"). |
| `xms_tpl`                  | Idioma preferido do locatário| JWT | | O idioma preferido do locatário do recurso, se definido. Format LL ("en"). |
...
| `nickname`    | Apelido                        | Um nome adicional para o usuário. O apelido é separado de First ou Last Name. Requer o `profile` escopo.|
| `family_name` | Sobrenome                       | Fornece o sobrenome, sobrenome ou nome de família do usuário, conforme definido no objeto de usuário. <br>"family_name":"Barros" | Com suporte no MSA e no Azure AD. Requer o `profile` escopo.   |
| `given_name`  | Nome                      | Fornece o primeiro ou "dado" nome do usuário, conforme definido no objeto de usuário.<br>"given_name": "Davi"                   | Com suporte no MSA e no Azure AD.  Requer o `profile` escopo. |
| `upn`         | Nome UPN | Um identificador para o usuário que pode ser usado com o parâmetro username_hint.  Não é um identificador durável para o usuário e não deve ser usado para dados de chave. | Ver [propriedades adicionais](#additional-properties-of-optional-claims) abaixo para a configuração da declaração. Requer o `profile` escopo.|
...
2. Quando terminar, selecione **salvar**. Agora, as declarações opcionais especificadas serão incluídas nos tokens para seu aplicativo.
...
Além do conjunto de declarações opcionais padrão, você também pode configurar tokens para incluir extensões. Para obter mais informações, consulte [a documentação de Microsoft Graph extensãoproperty](https://docs.microsoft.com/graph/api/resources/extensionproperty?view=graph-rest-1.0).

O esquema e as extensões abertas não têm suporte de declarações opcionais, somente as extensões de diretório do estilo AAD-Graph. Esse recurso é útil para anexar informações adicionais do usuário que o aplicativo pode usar; por exemplo, um identificador adicional ou uma opção de configuração importante que o usuário configurou. Consulte a parte inferior desta página para obter um exemplo.

> [!NOTE]
> As extensões de esquema de diretório são um recurso somente do Azure AD. Se o manifesto do aplicativo solicitar uma extensão personalizada e um usuário do MSA fizer logon em seu aplicativo, essas extensões não serão retornadas.

### <a name="directory-extension-formatting"></a>Formatação de extensão de diretório

Ao configurar declarações opcionais de extensão de diretório usando o manifesto do aplicativo, use o nome completo da extensão (no `extension_<appid>_<attributename>`formato:). O `<appid>` deve corresponder à ID do aplicativo que está solicitando a declaração.
...
1. Selecione os tipos de grupo a serem retornados (**todos os grupos, grupo**de **segurança**ou **DirectoryRole**). A opção **todos os grupos** inclui o grupo de **segurança**, **DirectoryRole**e **DistributionList**
...
- Você pode usar a interface do usuário de **configuração de token** (Veja o exemplo abaixo)
...
1. Selecione **Adicionar declaração opcional**, selecione o tipo de token de **ID** , selecione **UPN** na lista de declarações e, em seguida, selecione **Adicionar**.

1. Selecione **Adicionar declaração opcional**, selecione o tipo de token de **acesso** , selecione **auth_time** na lista de declarações e, em seguida, selecione **Adicionar**.

1. Na tela Visão geral da configuração do token, selecione o ícone de lápis ao lado de **UPN**, selecione a alternância **autenticada externamente** e, em seguida, selecione **salvar**.

1. Selecione **Adicionar declaração opcional**, selecione o tipo de token **SAML** , selecione **Extn. SkypeID** na lista de declarações (aplicável somente se você tiver criado um objeto de usuário do Azure AD chamado SkypeID) e, em seguida, selecione **Adicionar**.
...
1. Quando tiver terminado de atualizar o manifesto, selecione **salvar** para salvar o manifesto.
