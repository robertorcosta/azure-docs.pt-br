...
O RBAC do Azure AD permite que a empresa restrinja o número de operações que um indivíduo pode fazer quando eles têm acesso ao portal do Azure. Ao usar o RBAC para controlar o acesso ao portal, os administradores de TI podem delegar o acesso usando as seguintes abordagens de gerenciamento de acesso:

* **Atribuição de função baseada em grupo**: você pode atribuir acesso a grupos do Azure AD que podem ser sincronizados do Active Directory local. Isso permite aproveitar os investimentos existentes que sua organização já fez em ferramentas e processos para gerenciar os grupos. Também é possível usar o recurso de gerenciamento de grupos delegado do Azure AD Premium.
