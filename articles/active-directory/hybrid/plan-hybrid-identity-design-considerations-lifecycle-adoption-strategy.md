...
O Azure AD RBAC permite que a empresa restrinja o número de operações que um indivíduo pode fazer quando tiver acesso ao portal Azure. Ao usar o RBAC para controlar o acesso ao portal, os administradores de TI podem delegar o acesso usando as seguintes abordagens de gerenciamento de acesso:

* **Atribuição de função baseada em grupo**: Você pode atribuir acesso a grupos AD do Azure que podem ser sincronizados a partir do diretório ativo local. Isso permite aproveitar os investimentos existentes que sua organização já fez em ferramentas e processos para gerenciar os grupos. Também é possível usar o recurso de gerenciamento de grupos delegado do Azure AD Premium.
...
* Os administradores podem fornecer acesso condicional com base no recurso do aplicativo, identidade do dispositivo e usuário, localização da rede e autenticação multifatorial.
