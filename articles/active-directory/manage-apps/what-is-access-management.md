...
### <a name="requiring-user-assignment-for-an-app"></a>Exigindo atribuição de usuário para um aplicativo

Com certos tipos de aplicativos, você tem a opção de [exigir que os usuários sejam designados para o aplicativo.](assign-user-or-group-access-portal.md#configure-an-application-to-require-user-assignment) Ao fazer isso, você impede que todos façam login, exceto os usuários que você atribui explicitamente ao aplicativo. Os seguintes tipos de aplicativos suportam essa opção:
...
* Aplicativos construídos na plataforma de aplicativos Azure AD que usam OAuth 2.0 / OpenID Connect Authentication após um usuário ou administrador ter consentido com esse aplicativo. Certos aplicativos corporativos oferecem controle adicional sobre quem pode entrar.

Quando a atribuição do usuário não é *necessária,* os usuários não atribuídos não vêem o aplicativo em seu painel de acesso Meus Aplicativos, mas ainda podem entrar no próprio aplicativo (também conhecido como logon iniciado por SP) ou podem usar a URL de acesso ao **usuário** na página **Propriedades** do aplicativo (também conhecida como logon iniciado pelo IDP).

Para alguns aplicativos, a opção de exigir a atribuição do usuário não está disponível nas propriedades do aplicativo. Nesses casos, você pode usar o PowerShell para definir a propriedade appRoleAssignmentRequired no principal do serviço.
...
O Azure AD oferece [várias maneiras personalizáveis de implantar aplicativos](end-user-experiences.md) para usuários finais em sua organização:

* Painel de acesso a Azure AD My Apps
* Iniciador de aplicativos do Office 365
* Inscrição direta para aplicativos federados (service-pr)
* Links profundos a aplicativos federados, baseado em senha, ou existentes

Você pode determinar se os usuários atribuídos a um aplicativo corporativo podem vê-lo no painel de acesso e no launcher de aplicativos do Office 365.

## <a name="example-complex-application-assignment-with-azure-ad"></a>Exemplo: Atribuição de aplicativo complexa com o Azure AD
...
## <a name="access-to-microsoft-applications"></a>Acesso aos aplicativos da Microsoft
...
- Para aplicativos que a Microsoft ou terceiros publica livremente para qualquer pessoa usar, os usuários podem ter acesso através [do consentimento do usuário](configure-user-consent.md). Isso significa que eles entram no aplicativo com sua conta Azure AD Work ou School e permitem que ele tenha acesso a algum conjunto limitado de dados em sua conta.
- Para aplicativos que a Microsoft ou terceiros publica livremente para qualquer pessoa usar, os usuários também podem ter acesso através do [consentimento do administrador](manage-consent-requests.md). Isso significa que um administrador determinou que o aplicativo pode ser usado por todos na organização, portanto, entram no aplicativo com uma conta de Administrador Global e concedem acesso a todos na organização.

Algumas aplicações combinam esses métodos. Por exemplo, certos aplicativos da Microsoft fazem parte de uma assinatura do Office 365, mas ainda requerem consentimento.

Os usuários podem acessar os aplicativos do Office 365 através de seus portais office 365. Você também pode mostrar ou ocultar aplicativos do Office 365 no painel de acesso Meus Aplicativos com a visibilidade do [Office 365 alternando](hide-application-from-user-portal.md) nas **configurações**de usuário do seu diretório. 

Assim como acontece com os aplicativos corporativos, você pode [atribuir usuários](assign-user-or-group-access-portal.md) a determinados aplicativos da Microsoft através do portal Azure ou, se a opção de portal não estiver disponível, usando o PowerShell.
