...
1. O **serviço de provisionamento do Azure ad** inicia um [ciclo inicial](user-provisioning.md) no sistema de origem e no sistema de destino. 

1. O **serviço de provisionamento do Azure ad** consulta o sistema de origem para todos os usuários e grupos alterados desde o ciclo inicial e envia por push as alterações em [ciclos incrementais](user-provisioning.md).
...
1. O **serviço de provisionamento do Azure ad** consulta o sistema de origem para todos os usuários e grupos atualizados desde o ciclo inicial e executa quaisquer [ciclos incrementais](user-provisioning.md).
...
Use o portal do AD do Azure para exibir e gerenciar todos os aplicativos que dão suporte ao provisionamento. Consulte [localizando seus aplicativos no portal](configure-automatic-user-provisioning-portal.md).
...
1. Use o suporte de provisionamento de usuário genérico [BYOA scim](use-scim-to-provision-users-and-groups.md) para o aplicativo. Esse é um requisito para o Azure AD provisionar usuários para o aplicativo sem um conector de provisionamento previamente integrado.

1. Se o aplicativo for capaz de utilizar o conector BYOA SCIM, consulte o [tutorial de integração do BYOA scim](use-scim-to-provision-users-and-groups.md) para configurar o conector BYOA scim para o aplicativo.

Para obter mais informações, consulte [quais aplicativos e sistemas posso usar com o provisionamento automático de usuário do Azure ad?](user-provisioning.md)

### <a name="collect-information-to-authorize-application-access"></a>Coletar informações para autorizar o acesso ao aplicativo

A configuração do provisionamento automático de usuário é um processo por aplicativo. Para cada aplicativo, você precisa fornecer [credenciais de administrador](configure-automatic-user-provisioning-portal.md) para se conectar ao ponto de extremidade de gerenciamento de usuário do sistema de destino.
...
* Use [filtros de escopo](define-conditional-rules-for-provisioning-user-accounts.md) para definir regras baseadas em atributo que determinam quais usuários são provisionados para um aplicativo.

* Em seguida, use as [atribuições de usuário e grupo](assign-user-or-group-access-portal.md) conforme necessário para filtragem adicional.

### <a name="define-user-and-group-attribute-mapping"></a>Definir mapeamento de atributo de usuário e grupo

Para implementar o provisionamento automático de usuário, você precisa definir os atributos de usuário e grupo que são necessários para o aplicativo. Há um conjunto pré-configurado de atributos e [mapeamentos de atributo](configure-automatic-user-provisioning-portal.md) entre os objetos de usuário do Azure AD e os objetos de usuário de cada aplicativo SaaS. Nem todos os aplicativos SaaS habilitam atributos de grupo.

O Azure AD dá suporte ao mapeamento direto de atributo para atributo, fornecendo valores constantes ou [gravando expressões para mapeamentos de atributo](functions-for-customizing-application-data.md). Essa flexibilidade lhe dá um controle fino do que será populado no atributo do sistema de destino. Você pode usar [Microsoft Graph API](export-import-provisioning-configuration.md) e o explorador do Graph para exportar os mapeamentos e o esquema de atributo de provisionamento de usuário para um arquivo JSON e importá-los de volta para o Azure AD.

Para obter mais informações, consulte [Personalizando o atributo de provisionamento de usuário-mapeamentos para aplicativos SaaS no Azure Active Directory](customize-application-attributes.md).
...
1. Examine o [relatório de resumo](check-status-user-account-provisioning.md) de provisionamento e [Provisione os logs](check-status-user-account-provisioning.md#provisioning-logs-preview) para determinar quais operações incorretas ocorreram nos usuários e/ou grupos afetados.
...
Use o [portal do Azure](https://portal.azure.com/) para gerenciar o provisionamento e desprovisionamento de conta de usuário automático para aplicativos que dão suporte a ele. Siga as etapas em [como fazer configurar o provisionamento automático para um aplicativo?](user-provisioning.md)
...
Após um [ciclo inicial](user-provisioning.md)bem-sucedido, o serviço de provisionamento do Azure ad executará atualizações incrementais indefinidamente, em intervalos específicos de cada aplicativo, até que um dos seguintes eventos ocorra:
...
Para entender quanto tempo os ciclos de provisionamento levam e monitoram o progresso do trabalho de provisionamento, você pode [verificar o status do provisionamento do usuário](application-provisioning-when-will-provisioning-finish-specific-user.md).

### <a name="gain-insights-from-reports"></a>Obter informações de relatórios

O Azure AD pode fornecer [informações adicionais](application-provisioning-when-will-provisioning-finish-specific-user.md) sobre o uso de provisionamento de usuário da sua organização e a integridade operacional por meio de relatórios e logs de auditoria.

Os administradores devem verificar o relatório de Resumo de provisionamento para monitorar a integridade operacional do trabalho de provisionamento. Todas as atividades executadas pelo serviço de provisionamento são registradas nos logs de auditoria do Azure AD. Consulte [tutorial: relatórios sobre o provisionamento automático de conta de usuário](check-status-user-account-provisioning.md).
...
* [Problema ao configurar o provisionamento de usuário para um aplicativo da galeria do Azure AD](application-provisioning-config-problem.md)
...
* [O provisionamento de usuário para um aplicativo da galeria do Azure AD está demorando horas ou mais](application-provisioning-when-will-provisioning-finish.md)
...
* [Nenhum usuário está sendo provisionado para um aplicativo da galeria do Azure AD](application-provisioning-config-problem-no-users-provisioned.md)

* [Um conjunto incorreto de usuários está sendo provisionado para um aplicativo da galeria do Azure AD](application-provisioning-config-problem-wrong-users-provisioned.md)
...
* [Gravando expressões para mapeamentos de atributo no Azure Active Directory](functions-for-customizing-application-data.md)
