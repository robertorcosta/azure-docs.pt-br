...
|---------|---------|---------|
|Pelo menos uma regra não migrada foi detectada para AdditionalAuthentication.       | Aprovação/aviso          | A terceira parte confiável no AD FS está usando um provedor de MFA local. Para mudar para o Azure AD, recomendamos que você mova para a integração do Azure MFA ou de controles personalizados com o provedor de MFA de terceiros.  [Saiba mais sobre o Azure MFA](https://docs.microsoft.com/azure/active-directory/authentication/concept-mfa-howitworks).        |
|A terceira parte confiável tem AdditionalWSFedEndpoint definido como true.       | Aprovado/Reprovado          | A terceira parte confiável no AD FS permite vários pontos de extremidade de asserção de WS-alimentados. O Azure AD dá suporte apenas a (1) uma delas hoje. Se você tiver um cenário em que isso está bloqueando a migração, [informe-nos](https://feedback.azure.com/forums/169401-azure-active-directory/suggestions/38695621-allow-multiple-ws-fed-assertion-endpoints).     |
|A terceira parte confiável definiu AllowedAuthenticationClassReferences.       | Aprovado/Reprovado          | Essa é uma configuração no AD FS que permite especificar se o aplicativo está configurado para permitir apenas determinados tipos de autenticação. O Azure AD não dá suporte a isso hoje. Se você tiver um cenário em que isso está bloqueando a migração, [informe-nos](https://feedback.azure.com/forums/169401-azure-active-directory/suggestions/38695672-allow-in-azure-ad-to-specify-certain-authentication).          |
|AlwaysRequireAuthentication está configurado.      | Aprovado/Reprovado          | Se o aplicativo está configurado para ignorar cookies de SSO e ' sempre solicitar autenticação '. Não tem suporte do Azure AD hoje.          |
|A terceira parte confiável tem AutoUpdateEnabled definido como true       | Aprovação/aviso          | Essa é uma configuração no AD FS que permite especificar se AD FS está configurado para atualizar automaticamente o aplicativo com base nas alterações nos metadados da Federação. O Azure AD não dá suporte a isso hoje, mas não deve bloquear a migração do aplicativo para o Azure AD.           |
|A terceira parte confiável tem vários ClaimsProviders habilitados       | Aprovado/Reprovado          | A terceira parte confiável está configurada para declarações de origem de um provedor de declaração diferente de Active Directory.  Não há suporte para isso no Azure AD. Se você tiver um cenário em que isso está bloqueando a migração, [informe-nos](https://feedback.azure.com/forums/169401-azure-active-directory/suggestions/38695717-allow-to-source-user-attributes-from-external-dire).          |
|DelegationAuthorization é válido      | Aprovado/Reprovado          | O aplicativo tem regras de autorização de delegação personalizadas definidas. O Azure AD não dá suporte a isso hoje. Se você tiver um cenário em que isso está bloqueando a migração, [informe-nos](https://feedback.azure.com/forums/169401-azure-active-directory/suggestions/38695747-allow-to-define-delegation-authorization-rules).          |
|A terceira parte confiável tem ADFSRPImpersonationAuthorizationRules       | Aprovação/aviso          | O aplicativo tem regras de autorização de representação personalizadas definidas. O Azure AD não dá suporte a isso hoje, mas não deve bloquear a migração do aplicativo para o Azure AD.          |
|Pelo menos uma regra não migrada foi detectada para IssuanceAuthorization.       | Aprovação/aviso          | O aplicativo tem regras de autorização de emissão personalizadas definidas em AD FS. O Azure AD dá suporte a essa funcionalidade com acesso condicional do Azure AD. [Saiba mais sobre o acesso condicional](https://docs.microsoft.com/azure/active-directory/conditional-access/overview). Além disso, no Azure AD, você pode restringir o acesso ao aplicativo por usuário ou grupos atribuídos ao aplicativo. [Saiba mais sobre como atribuir usuários e grupos para acessar aplicativos](https://docs.microsoft.com/azure/active-directory/manage-apps/methods-for-assigning-users-and-groups). Consulte a tabela de [testes de regra de declaração](#check-the-results-of-claim-rule-tests)abaixo para obter informações sobre as regras de IssuanceAuthorization brutas configuradas no AD FS.           |
|Pelo menos uma regra não migrada foi detectada para IssuanceTransform.       | Aprovação/aviso          | O aplicativo tem regras de transformação de emissão personalizadas definidas em AD FS. O Azure AD dá suporte à personalização das declarações emitidas no token. Para saber mais, confira [Personalizar declarações emitidas no token SAML para aplicativos empresariais](https://docs.microsoft.com/azure/active-directory/develop/active-directory-saml-claims-customization).  Consulte a tabela de [testes de regra de declaração](#check-the-results-of-claim-rule-tests)abaixo para obter informações sobre as regras de IssuanceTransformation brutas configuradas no AD FS.          |
|A terceira parte confiável tem MonitoringEnabled definido como true.       | Aprovação/aviso          | Essa é uma configuração no AD FS que permite especificar se AD FS está configurado para monitorar os metadados da Federação para este aplicativo. Isso não tem suporte no Azure AD, mas não deve bloquear a migração do aplicativo para o Azure AD.          |
|A terceira parte confiável tem NotBeforeSkew configurado.      | Aprovação/aviso          | AD FS permite uma distorção de tempo com base nas horas de não antes e NotOnOrAfter no token SAML. O Azure AD não dá suporte a isso hoje, mas não deve bloquear a migração do aplicativo para o Azure AD.          |
|A terceira parte confiável tem RequestMFAFromClaimsProviders definido como true.       | Aprovação/aviso          | Essa é uma configuração no AD FS que permite especificar se o aplicativo está embutido em código para outro provedor de declaração e requer MFA. Para mudar para o Azure AD, recomendamos que você mova para a integração do Azure MFA ou de controles personalizados com um provedor de MFA de terceiros.  [Saiba mais sobre o Azure MFA](https://docs.microsoft.com/azure/active-directory/authentication/concept-mfa-howitworks).          |
|A terceira parte confiável tem SignedSamlRequestsRequired definido como true       | Aprovado/Reprovado          | O aplicativo é configurado no AD FS para verificar a assinatura na solicitação SAML. Essa é uma configuração opcional e não deve bloquear sua migração. Azure Active Directory usa as URLs de resposta para validar o provedor de serviços.  Se você tiver um cenário em que isso está bloqueando a migração, [informe-nos](https://feedback.azure.com/forums/169401-azure-active-directory/suggestions/13394589-saml-signature).          |
|TokenLifetimeCheckResult        | Aprovado/Reprovado         | O aplicativo está configurado para um tempo de vida de token personalizado. AD FS padrão é de 1 hora. O Azure AD dá suporte a essa funcionalidade usando o acesso condicional. Para saber mais, confira [Configurar o gerenciamento de sessão de autenticação com acesso condicional](https://docs.microsoft.com/azure/active-directory/conditional-access/howto-conditional-access-session-lifetime).          |
|A terceira parte confiável está definida para criptografar declarações. Isso é suportado pelo Azure AD       | Aprovado          | Com o Azure AD, você pode criptografar o token enviar para o aplicativo. Para saber mais, confira [Configurar a criptografia de token SAML do Azure ad](https://docs.microsoft.com/azure/active-directory/manage-apps/howto-saml-token-encryption).          |
|A terceira parte confiável está definida para criptografar NameID no token SAML.      | Aprovado/Reprovado          | O aplicativo está configurado para criptografar a declaração NameID no token SAML. Com o Azure AD, você pode criptografar todo o token enviado ao aplicativo. Ainda não há suporte para criptografia de declarações específicas. Para saber mais, confira [Configurar a criptografia de token SAML do Azure ad](https://docs.microsoft.com/azure/active-directory/manage-apps/howto-saml-token-encryption).         |
