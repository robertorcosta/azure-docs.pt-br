...
Agora, é preciso criar um objeto `AudioConfig` que aponte para seu dispositivo de entrada. Insira esse código no método `startIntentRecognizeAsyncButton.addEventListener()`, logo abaixo da configuração de Fala.
