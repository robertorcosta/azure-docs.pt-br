...
## <a name="build-and-run-the-application-to-create-a-new-conversation"></a>Crie e execute o aplicativo para criar uma conversa
...
1. Após a mensagem `Started transcribing` ser exibida, você poderá começar a conversar. Você verá as transcrições sendo exibidas à medida que você falar
    - Se você compartilhar o código da conversa com as outras pessoas e elas ingressarem na conversa, você verá as transcrições delas também.

1. Depois que você terminar de conversar, pressione <kbd>Ctrl+C</kbd> em seu teclado para parar a captura de áudio.

    > [!NOTE]
    > Você pode ver uma mensagem do Visual Studio sobre uma exceção semelhante a: `Exception thrown at 0x76EB90BF (KernelBase.dll) in helloworld.exe: 0x40010005: Control-C.` Você pode ignorar isso com segurança.
    > <br/> <br/>
    > Pressione <kbd>F5</kbd> para continuar.

## <a name="build-and-run-the-application-to-join-an-existing-conversation"></a>Criar e executar o aplicativo para ingressar em uma conversa existente

1. Copie e cole a seguinte função em seu **helloworld.cpp** antes da função `int main()`:
...
2. Substitua `StartNewConversation();` em sua função `int main()` por:
...
4. Volte para o Visual Studio e substitua `YourConversationId` em sua função `int main()` pela ID da conversa da etapa anterior.

5. Na barra de menus, selecione **Compilar** > **Compilar solução** para compilar o aplicativo. O código deve compilar sem erros.

6. Escolha **Depurar** > **Iniciar depuração** (ou pressione <kbd>F5</kbd>) para iniciar o aplicativo **Olá Mundo**.

7. Após a mensagem `Started transcribing` ser exibida, você poderá começar a conversar. Você verá as transcrições sendo exibidas à medida que você fala.
    - Se você voltar para o seu navegador, deverá ver suas transcrições sendo exibidas lá à medida que você conversar também.

8.  Depois que você terminar de conversar, pressione <kbd>Ctrl+C</kbd> para parar a captura de áudio e encerrar a conversa.
