...
1. Abra **Program.cs** e substitua todo o conteúdo nele pelo seguinte código:
...
1. Após a mensagem `Started transcribing` ser exibida, você poderá começar a conversar. Você verá as transcrições sendo exibidas à medida que você fala.
...
1. Depois que você terminar de conversar, pressione <kbd>Ctrl+C</kbd> para parar a captura de áudio e encerrar a conversa.
...
2. Substitua `CreateConversationAsync();` em sua função `public static async Task Main(string[] args)` por:
...
4. Volte para o Visual Studio e substitua a cadeia de caracteres `YourConversationId` pela ID de conversa que você criou na etapa anterior.

5. Na barra de menus, selecione **Compilar** > **Compilar solução** para compilar o aplicativo. Agora, o código deve compilar sem erros.

6. Escolha **Depurar** > **Iniciar depuração** (ou pressione **F5**) para iniciar o aplicativo **Olá Mundo**.
