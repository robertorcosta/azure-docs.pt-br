...
A tabela acima mostra as pontuações que são esperadas na maioria das Bases de conhecimento. No entanto, como cada KB é diferente, e tem diferentes tipos de palavras, intenções e objetivos, recomendamos que você teste e escolha o limiar que melhor funciona para você. Por padrão, o limite é definido como 0, para que todas as respostas possíveis sejam devolvidas. O limite recomendado que deve funcionar para a maioria dos KBs é **de 50**.
...
## <a name="set-threshold"></a>Definir o limiar

Defina a pontuação limite como uma propriedade do [corpo GerarResposta API JSON](../how-to/metadata-generateanswer-usage.md#generateanswer-request-configuration). Isso significa que você defini-lo para cada chamada para Gerar Resposta.

A partir da estrutura bot, defina a pontuação como parte do objeto de opções com [C#](../how-to/metadata-generateanswer-usage.md?#use-qna-maker-with-a-bot-in-c) ou [Node.js](../how-to/metadata-generateanswer-usage.md?#use-qna-maker-with-a-bot-in-nodejs).
...
A pontuação de confiança de resposta pode alterar pouco entre o teste e a versão publicada da base de dados de conhecimento, mesmo se o conteúdo for o mesmo. Isso porque o conteúdo do teste e a base de conhecimento publicada estão localizados em diferentes índices de Pesquisa Cognitiva do Azure.

O índice de teste contém todos os pares de QnA de suas bases de conhecimento. Ao consultar o índice de teste, a consulta se aplica a todo o índice e, em seguida, os resultados ficam restritos à partição para essa base de conhecimento específica. Se os resultados da consulta de teste estiverem afetando negativamente sua capacidade de validar a base de conhecimento, você poderá:
* organize sua base de conhecimento usando um dos seguintes:
    * 1 recurso restrito a 1 KB: restringir seu único recurso de QnA (e o índice de teste de pesquisa cognitiva do Azure resultante) a uma única base de conhecimento.
    * 2 recursos - 1 para teste, 1 para produção: ter dois recursos do QnA Maker, utilizando um para testes (com seus próprios índices de teste e produção) e um para produto (também com seus próprios índices de teste e produção)
* e, sempre, use os mesmos parâmetros, como **[top](../how-to/improve-knowledge-base.md#use-the-top-property-in-the-generateanswer-request-to-get-several-matching-answers)** ao consultar tanto sua base de conhecimento de teste quanto de produção
