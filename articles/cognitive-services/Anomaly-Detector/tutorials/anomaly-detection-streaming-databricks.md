...
> * Este tutorial não pode ser concluído com uma assinatura (`F0`) da Camada gratuita da API do Detector de Anomalias nem do Azure Databricks.

Crie uma [assinatura do Azure](https://azure.microsoft.com/free/cognitive-services) caso não tenha uma.

## <a name="prerequisites"></a>Pré-requisitos

- Um [namespace dos Hubs de Eventos do Azure](../../../event-hubs/event-hubs-create.md) e um hub de eventos.
...
3. Na página **Novo Cluster**, forneça os valores para criar um cluster.
   ...
   * Neste artigo, crie um cluster com o runtime **5.2**. NÃO selecione o runtime **5.3**.
...
1. Em um navegador da Web, vá para [Gerenciamento de Aplicativo do Twitter](https://apps.twitter.com/) e selecione **Criar Novo Aplicativo**.
    ...
    ![Criar um aplicativo do Twitter](../media/tutorials/databricks-create-twitter-app.png "Criar um aplicativo do Twitter")
...
3. No Azure Marketplace, selecione **IA + Machine Learning** > **Ver tudo** > **Serviços Cognitivos – Mais** > **Detector de Anomalias**. Se preferir, use [este link](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesAnomalyDetector) para acessar a caixa de diálogo **Criar** diretamente.
    ...
    ![Criar um recurso do Detector de Anomalias](../media/tutorials/databricks-cognitive-services-anomaly-detector.png "Criar um recurso do Detector de Anomalias")
...
1. No workspace do Azure Databricks, selecione **Workspace** no painel esquerdo. Na lista suspensa **Workspace**, selecione **Criar** e selecione **Bloco de Anotações**.
    ...
    ![Criar notebook no Databricks](../media/tutorials/databricks-create-notebook.png "Criar notebook no Databricks")
...
Em seguida, obtenha o resultado agregado da saída para o Delta. Como a detecção de anomalias exige uma janela de histórico mais longa, estamos usando o Delta para manter os dados do histórico no ponto em que você deseja fazer a detecção.
Substitua o "[espaço reservado: nome da tabela]" por um nome qualificado da tabela do Delta a ser criado (por exemplo, "tweets"). Substitua "[espaço reservado: nome da pasta para pontos de verificação]" por um valor de cadeia de caracteres que seja exclusivo sempre que você executar esse código (por exemplo, "etl-from-eventhub-20190605").
Para saber mais sobre o Delta Lake no Azure Databricks, veja o [Guia do Delta Lake](/databricks/delta/)
...
Agora os dados agregados de série temporal são ingeridos continuamente no Delta. Em seguida, você poderá agendar um trabalho por hora para detectar as anomalias do último ponto.
Substitua o "[espaço reservado: nome da tabela]" pelo mesmo nome da tabela do Delta selecionado acima.
...
![Interromper um cluster Databricks](../media/tutorials/terminate-databricks-cluster.png "Interromper um cluster Databricks")
