...
* Importe um aplicativo LUIS de um `.lu` arquivo `.json` ou que já contenha intenções, declarações e entidades.
...
1. Escolha a cultura do aplicativo e, em seguida, selecione **concluído**. A descrição e o recurso de previsão são opcionais neste ponto. Você pode definir, a qualquer momento, na seção **gerenciar** do Portal.
    ...
    Depois que o aplicativo é criado, o portal do LUIS mostra a lista de **tentativas** com `None` a intenção já criada para você. Agora você tem um aplicativo vazio. 
...
* Exportar logs de ponto de extremidade `.csv`, como um, para análise offline
* Excluir aplicativo
