...
A versão ativa do LUIS é a versão que recebe alterações no modelo. No portal [LUIS,](luis-reference-regions.md) se você quiser fazer alterações em uma versão que não é a versão ativa, você precisa primeiro definir essa versão como ativa.

## <a name="authoring"></a><a name="authoring"></a>Criação

A autoria é a capacidade de criar, gerenciar e implantar um [aplicativo LUIS,](#luis-app)seja usando o portal [LUIS](luis-reference-regions.md) ou as [APIs de autoria.](https://go.microsoft.com/fwlink/?linkid=2092087)
...
O teste em lote é a capacidade de validar o modelo de um aplicativo LUIS atual com um conjunto de testes consistente e conhecido de expressões do usuário. O teste em lote é definido em um [arquivo formatado JSON](luis-concept-batch-test.md#batch-file-format).
...
Um colaborador/colaborador, não é o [proprietário](#owner) do aplicativo, mas tem as mesmas permissões para adicionar, editar e excluir as intenções, entidades, enunciados.
...
Um descritor é um [recurso](#features) aplicado a um modelo na hora do treinamento, incluindo [listas de frases](#phrase-list) e [entidades](#entity). 
...
## <a name="endpoint"></a><a name="endpoint"></a>Ponto de extremidade
...
Rotular, ou marcar, é o processo de associar uma palavra ou frase na expressão de uma [intenção](#utterance) com uma [entidade](#entity) (tipo de dados).

## <a name="luis-app"></a><a name="luis-app"></a>Aplicativo LUIS

Um aplicativo LUIS é uma coleção de modelos de linguagem para processamento de linguagem natural, incluindo [intenções,](#intent) [entidades](#entity)e [expressões rotuladas.](#utterance)
...
Uma [lista de frases](luis-concept-feature.md) inclui um grupo de valores (palavras ou frases) que pertencem à mesma classe e devem ser tratados da mesma forma (por exemplo, nomes de cidades ou produtos). Uma lista intercambiável é tratada como sinônimos.
...
## <a name="speech-priming"></a><a name="speech-priming"></a>Preparação da fala
...
## <a name="train"></a><a name="train"></a>Trem
...
## <a name="utterance"></a><a name="utterance"></a>Declaração
