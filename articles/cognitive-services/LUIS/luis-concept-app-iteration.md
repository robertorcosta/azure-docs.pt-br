...
Uma versão pode ser importada no nível do aplicativo. Essa versão se torna a versão ativa e usa a ID da versão `versionId` na Propriedade do arquivo do aplicativo. Você também pode importar para um aplicativo existente, no nível de versão. A nova versão torna-se a versão ativa. 
