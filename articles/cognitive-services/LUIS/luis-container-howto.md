...
ms.locfileid: "79219924"
---
# <a name="install-and-run-luis-docker-containers"></a>Instalar e executar os contêineres de docker LUIS
 
O contêiner "Compreensão de Idiomas) (LUIS) carrega seu modelo de compreensão de idiomas treinado ou publicado. Como um [aplicativo LUIS,](https://www.luis.ai)o contêiner docker fornece acesso às previsões de consulta dos pontos finais da API do contêiner. Você pode coletar registros de consulta do contêiner e enviá-los de volta para o aplicativo Language Understanding para melhorar a precisão de previsão do aplicativo.

O vídeo a seguir demonstra como usar esse contêiner.

[![Demonstração de contêineres para serviços cognitivos](./media/luis-container-how-to/luis-containers-demo-video-still.png)](https://aka.ms/luis-container-demo)
...
|Recurso do `Cognitive Services` Azure e arquivo [de aplicativo empacotado luis](luis-how-to-start-new-app.md) |Para usar o contêiner, você precisará ter:<br><br>* Um recurso _azure de serviços cognitivos_ e a chave de faturamento associada do uri ponto final de faturamento. Ambos os valores estão disponíveis nas páginas Visão Geral e Chaves para o recurso e são necessários para iniciar o contêiner. <br>* Um aplicativo publicado ou treinado empacotado como uma entrada montada para o contêiner com sua ID do aplicativo associado. Você pode obter o arquivo embalado no portal LUIS ou nas APIs de Autoria. Se você estiver recebendo o aplicativo luis embalado das [APIs de autoria,](#authoring-apis-for-package-file)você também precisará de sua _Chave de Autoria_.<br><br>Esses requisitos são usados para transmitir argumentos de linha de comando para as seguintes variáveis:<br><br>**{AUTHORING_KEY}**: Esta chave é usada para obter o aplicativo embalado do serviço LUIS na nuvem e carregar os logs de consulta de volta para a nuvem. O formato é `xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`.<br><br>**{APP_ID}**: Este ID é usado para selecionar o Aplicativo. O formato é `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`.<br><br>**{API_KEY}**: Esta tecla é usada para ligar o recipiente. Você pode encontrar a chave do ponto de extremidade em dois locais. O primeiro é o portal Azure dentro da lista de chaves do recurso cognitive _services._ A chave do ponto de extremidade também está disponível no portal do LUIS na página Configurações de chaves e de ponto de extremidade. Não use a chave de inicialização.<br><br>**{ENDPOINT_URI}**: O ponto final conforme fornecido na página Visão Geral.<br><br>A [chave de criação e a chave do ponto de extremidade](luis-boundaries.md#key-limits) têm finalidades diferentes. Não use uma no lugar da outra. |

[!INCLUDE [Gathering required container parameters](../containers/includes/container-gathering-required-parameters.md)]

### <a name="authoring-apis-for-package-file"></a>Autor de APIs para arquivo de pacote

Autor de APIs para aplicativos embalados:
...
* [API de pacote não publicado e treinado](https://westus.dev.cognitive.microsoft.com/docs/services/5890b47c39e2bb17b84a55ff/operations/apps-packagetrainedapplicationasgzip)
...
Use [`docker pull`](https://docs.docker.com/engine/reference/commandline/pull/) o comando para baixar `mcr.microsoft.com/azure-cognitive-services/luis` uma imagem de contêiner do repositório:
...
1. Mova o arquivo de pacote para o diretório de **entrada** exigido no [computador host](#the-host-computer). Não renomeie, altere, sobreescreva ou descompacte o arquivo do pacote LUIS.
...
O diretório de montagem de entrada pode conter os modelos **Production**, **Staging**e **Versioned** do aplicativo simultaneamente. Todos os pacotes são montados.

|Tipo de pacote|API do ponto de extremidade de consulta|Disponibilidade de consulta|Formato do nome de arquivo do pacote|
|--|--|--|--|
|Versionado|GET, POST|Somente contêiner|`{APP_ID}_v{APP_VERSION}.gz`|
|Staging|GET, POST|Azure e contêiner|`{APP_ID}_STAGING.gz`|
|Produção|GET, POST|Azure e contêiner|`{APP_ID}_PRODUCTION.gz`|

> [!IMPORTANT]
> Não renomeie, altere, sobreescreva ou descompacte os arquivos do pacote LUIS.
...
|--|--|
|Instância de recurso do Azure _Cognitive Services_|As regiões com suporte incluem<br><br>Oeste dos EUA (`westus`)<br>Europa Ocidental (`westeurope`)<br>Leste da Austrália (`australiaeast`)|
...
### <a name="export-versioned-apps-package-from-luis-portal"></a>Exportar pacote do aplicativo versionado do portal LUIS

O pacote do aplicativo versionado está disponível na página da lista **Versões.**
...
Para baixar o pacote publicado, consulte a documentação da [API aqui][download-published-package]. Se baixado com sucesso, a resposta é um arquivo de pacote LUIS. Salve o arquivo no local de armazenamento especificado para a montagem de entrada do contêiner. 

### <a name="export-versioned-apps-package-from-api"></a>Exportar pacote do aplicativo versionado da API
...
| Espaço reservado | Valor |
|-------------|-------|
| **{APP_ID}** | O ID de aplicação do aplicativo LUIS treinado. |
| **{APP_VERSION}** | A versão de aplicação do aplicativo LUIS treinado. |
| **{AUTHORING_KEY}** | A chave de criação da conta do LUIS para o aplicativo LUIS publicado.<br/>Você pode obter sua chave de criação na página **Configurações do Usuário** no portal do LUIS. |
| **{AZURE_REGION}** | A região do Azure apropriada:<br/><br/>`westus` – Oeste dos EUA<br/>`westeurope` – Europa Ocidental<br/>`australiaeast` – Leste da Austrália |

Para baixar o pacote em versão, consulte a documentação da [API aqui][download-versioned-package]. Se baixado com sucesso, a resposta é um arquivo de pacote LUIS. Salve o arquivo no local de armazenamento especificado para a montagem de entrada do contêiner. 
...
* Este exemplo usa o `C:` diretório fora da unidade para evitar quaisquer conflitos de permissão no Windows. Se você precisar usar um diretório específico como o diretório de entrada, talvez seja necessário conceder ao Docker permissão de serviço. 
* Não altere a ordem dos argumentos a menos que esteja familiarizado com os recipientes docker.
...
* Economiza logs de contêiner e LUIS para montagem de saída na *saída C:\,* localizado no host do contêiner
...
> O valor do ApiKey é a **chave** da página Recursos do **Azure** `Cognitive Services` no portal LUIS e também está disponível na página chaves de recursos do Azure.  

[!INCLUDE [Running multiple containers on the same host](../../../includes/cognitive-services-containers-run-multiple-same-host.md)]

## <a name="endpoint-apis-supported-by-the-container"></a>APIs de ponto final suportadas pelo contêiner

Ambas as versões V2 e [V3](luis-migration-api-v3.md) da API estão disponíveis com o contêiner. 

## <a name="query-the-containers-prediction-endpoint"></a>Consultar o ponto de extremidade de previsão do contêiner

O contêiner fornece APIs de ponto de extremidade de previsão de consulta com base em REST. Os pontos finais para aplicativos publicados (encenação ou produção) têm uma rota _diferente_ dos pontos finais para aplicativos com versões.
...
|Versionado|GET, POST|`/luis/v3.0/apps/{appId}/versions/{versionId}/predict?`|`query={query}`<br>[`&verbose`]<br>[`&log`]<br>[`&show-all-intents`]|
...
|`verbose`|booleano|Um valor booleano indicando se deve retornar todos os metadados para os modelos previstos. O padrão é false.|
...
|`show-all-intents`|booleano|Um valor booleano indicando se deve retornar todas as intenções ou apenas a intenção de pontuação superior. O padrão é false.|
...
|Versionado|GET, POST|`/luis/v2.0/apps/{appId}/versions/{versionId}?`|`q={q}`<br>[`&timezoneOffset`]<br>[`&verbose`]<br>[`&log`]|
...
Para fazer consultas ao ambiente `production` **Staging,** substitua-a na rota por `staging`:

`http://localhost:5000/luis/v3.0/apps/{APP_ID}/slots/staging/predict`

Para consultar um modelo com versão, use a seguinte API:
...
Se uma montagem de saída for especificada para o contêiner LUIS, os `{INSTANCE_ID}` arquivos de registro de consulta de aplicativo serão salvos no diretório de saída, onde está o ID do contêiner. O log de consulta do aplicativo contém a consulta, a resposta e os carimbos de data/hora para cada consulta de previsão enviada para o contêiner de LUIS. 
...
O contêiner LUIS envia informações de cobrança para o Azure, usando um recurso _de Serviços Cognitivos_ em sua conta do Azure. 
...
* Revisão [Configure contêineres](luis-container-configuration.md) para configurações.
* Consulte [as limitações do contêiner LUIS](luis-container-limitations.md) para obter restrições de capacidade conhecidas.
