...
O objeto `imageInfo` deve incluir o campo `url` ou `imageInsightsToken` mas não ambos. Defina o campo `url` como a URL de uma imagem acessível pela Internet. O tamanho máximo de imagem compatível é 1 MB.
...
| NOME | Valor | Digite | obrigatórios |
| --- | --- | --- | --- |
...
Se houver insights disponíveis para a imagem, a resposta conterá uma ou mais `tags` que contêm os insights. O campo `image` contém o token de informações para a imagem de entrada:
...
Se a imagem contiver um texto reconhecido pelo serviço, uma das marcas conterá um insight de TextResults (ação). A `displayName` do Insight contém o texto reconhecido:
