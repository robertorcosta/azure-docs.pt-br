...
| [MetricsAdvisorAdministrationClient](https://azuresdkdocs.blob.core.windows.net/$web/python/azure-ai-metricsadvisor/latest/azure.ai.metricsadvisor.html?highlight=metricsadvisoradministrationclient#azure.ai.metricsadvisor.MetricsAdvisorAdministrationClient)| **Permite a você:** <br> – Gerenciar feeds de dados <br> – Criar, configurar, recuperar, listar e excluir configurações de detecção de anomalias <br> – Criar, configurar, recuperar, listar e excluir configurações de alerta de anomalias <br> – Gerenciar ganchos  | |
...
| [Métrica](https://azuresdkdocs.blob.core.windows.net/$web/python/azure-ai-metricsadvisor/latest/azure.ai.metricsadvisor.models.html?highlight=metric#azure.ai.metricsadvisor.models.Metric) | Uma `Metric` é uma medida quantificável usada para monitorar e avaliar o status de um processo empresarial específico. Pode ser uma combinação de vários valores de série temporal divididos em dimensões. Por exemplo, uma métrica de integridade da Web pode conter dimensões para contagem de usuários e mercado en-us. |
...
Em um novo método, crie instruções de importação como o exemplo abaixo. Substitua `sql_server_connection_string` pela sua cadeia de conexão do SQL Server e substitua `query` por uma consulta que retorne seus dados em um só carimbo de data/hora. Você também precisará ajustar os valores de `metric` e `dimension` com base nos seus dados personalizados.
...
Em um novo método, crie instruções de importação como o exemplo abaixo. Substitua `metric_id` pela ID da métrica que deseja configurar. Crie um cliente com as chaves e o ponto de extremidade e use `client.create_metric_anomaly_detection_configuration` para criar uma configuração de detecção. As condições de limite especificam os parâmetros da detecção de anomalias.
...
Em um novo método, crie instruções de importação como o exemplo abaixo. Substitua `anomaly_detection_configuration_id` pela ID da configuração de detecção de anomalias e substitua `hook_id` pelo gancho criado anteriormente. Crie um cliente com as chaves e o ponto de extremidade e use `client.create_anomaly_alert_configuration()` para criar uma configuração de alerta. `metric_alert_configurations` é uma lista de objetos `MetricAlertConfiguration` que especificam as condições e o escopo de cada configuração.
...
Em um novo método, crie uma instrução de importação como o exemplo abaixo. Substitua `alert_id` pela ID do alerta e substitua `alert_config_id` pela ID de configuração do alerta. Crie um cliente com as chaves e o ponto de extremidade e use `client.list_anomalies_for_alert()` para listar uma configuração de alerta. 
