...
Para obter insights sobre uma imagem, capture o token [imageInsightsToken](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#image-imageinsightstoken) da imagem na resposta.
...
Se você tiver a URL para uma imagem da qual deseja obter insights, use o parâmetro de consulta [imgUrl](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#imgurl) em vez do parâmetro [insightsToken](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#insightstoken) para especificar a imagem. Ou se tem o arquivo de imagem, envie o binário da imagem no corpo de uma solicitação POST. Se você usar uma solicitação POST, o cabeçalho `Content-Type` precisará ser definido como `multipart/data-form`. Com uma dessas opções, o tamanho da imagem não pode exceder 1 MB.  
...
Para solicitar todos os insights de uma imagem, defina o parâmetro de consulta [modules](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#modulesrequested) como `All`. Para obter as pesquisas relacionadas, a solicitação precisa incluir a cadeia de caracteres de consulta do usuário. Este exemplo mostra o uso do [insightsToken](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#insightstoken) para especificar a imagem.  
...
O objeto de nível superior é um objeto [ImageInsightsResponse](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#imageinsightsresponse), em vez de um objeto [Images](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#images).  
...
O recurso de reconhecimento de entidade identifica as entidades em uma imagem, atualmente, somente pessoas. Para identificar as entidades em uma imagem, defina o parâmetro de consulta [modules](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#modulesrequested) como `RecognizedEntities`.  
...
O exemplo a seguir mostra como especificar a imagem usando o parâmetro [imgUrl](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#imgurl). Lembre-se de codificar os parâmetros de consulta em URL.  
...
-   A esquerda é mapeada para [cal](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#cal)  
-   A parte superior é mapeada para [cat](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#cat)  
-   A direita é mapeada para [car](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#car)  
-   A parte inferior é mapeada para [cab](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#cab)  

## <a name="finding-visually-similar-images"></a>Encontrando imagens visualmente semelhantes  

Para encontrar imagens visualmente semelhantes à imagem original, defina o parâmetro de consulta [modules](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#modulesrequested) como SimilarImages.  
...
Para localizar imagens que contêm produtos que são visualmente semelhantes aos produtos encontrados na imagem original, defina o parâmetro de consulta [modules](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference#modulesrequested) como SimilarProducts.  
...
Para obter uma lista dos comerciantes que oferecem o produto online (confira o campo [offerCount](/rest/api/cognitiveservices-bingsearch/bing-images-api-v7-reference)), chame a API novamente e defina `modules` como ShoppingSources. Em seguida, defina o parâmetro de consulta `insightsToken` com o token encontrado na imagem de resumo de produto.  
