...
ms.locfileid: "84725793"
---
# <a name="create-custom-sdks-for-azure-digital-twins-with-autorest"></a>Criar SDKs personalizados para o gêmeos digital do Azure com o autorest
...
* O [arquivo Swagger (Azure digital gêmeos openapi)](https://github.com/Azure/azure-rest-api-specs/blob/master/specification/digitaltwins/resource-manager/Microsoft.DigitalTwins/preview/2020-03-01-preview/digitaltwins.json)
...
1. Copie o arquivo Swagger do Azure digital gêmeos em um diretório de trabalho.
2. Em um prompt de comando, alterne para esse diretório de trabalho.
3. Execute o comando executar como a seguir.

```cmd/sh
autorest --input-file=adtApiSwagger.json --csharp --output-folder=ADTApi --add-credentials --azure-arm --namespace=ADTApi
```

Como resultado, você verá uma nova pasta chamada *ADTApi* em seu diretório de trabalho. Os arquivos do SDK gerados terão o namespace *ADTApi*, que você continuará a usar por meio do restante dos exemplos.
...
Você pode incluir os arquivos gerados pelo autorest diretamente em uma solução .NET. No entanto, como você provavelmente precisará do SDK do Azure digital gêmeos em vários projetos separados (seus aplicativos cliente, Azure Functions aplicativos etc.), recomendamos que você crie um projeto separado (uma biblioteca de classes do .NET) a partir dos arquivos gerados. Você pode então incluir esse projeto de biblioteca de classes em outras soluções como uma referência de projeto.

Esta seção fornece instruções sobre como criar o SDK como uma biblioteca de classes, que é seu próprio projeto e pode ser incluído em outros projetos. Aqui estão as etapas para fazer isso:

1. Criar uma nova solução do Visual Studio para uma biblioteca de classes
2. Use o nome "ADTApi" como o nome do projeto
