...
1. Na sua máquina de desenvolvimento, crie uma pasta vazia chamada **iot-java-twin-getstarted**.

2. Na pasta **iot-java-twin-getstarted,** crie um projeto Maven chamado **add-tags-query** usando o seguinte comando no seu prompt de comando:
...
4. Usando um editor de texto, abra o arquivo **pom.xml** na pasta **add-tags-query** e adicione a seguinte dependência ao nó **dependências.** Essa dependência permite que você use o pacote **iot-service-client** em seu aplicativo para se comunicar com seu hub IoT:
...
5. Adicione o seguinte nó **buid** após o nó **dependencies**. Esta configuração instrui a Maven a usar java 1.8 para construir o aplicativo.
...
9. Adicione as seguintes variáveis no nível da classe à classe **App** . Substitua-a pela `{youriothubconnectionstring}` seqüência de conexão de hub IoT que você copiou em [Obter a seqüência de conexão de hub IoT](#get-the-iot-hub-connection-string).
...
11. Substitua o código no método **principal** pelo seguinte código para criar os objetos **DeviceTwin** e **DeviceTwinDevice.** O objeto **DeviceTwin** lida com a comunicação com seu hub IoT. O objeto **DeviceTwinDevice** representa o dispositivo gêmeo com suas propriedades e marcas:
...
15. Salvar e fechar o **arquivo add-tags-query\src\main\java\com\mycompany\app\App.java**
...
1. Na pasta **iot-java-twin-getstarted,** crie um projeto Maven chamado **dispositivo simulado** usando o seguinte comando no seu prompt de comando:
...
3. Usando um editor de texto, abra o arquivo **pom.xml** na pasta **do dispositivo simulado** e adicione as seguintes dependências ao nó **dependências.** Essa dependência permite que você use o pacote **iot-device-client** em seu aplicativo para se comunicar com seu hub IoT.
...
9. Adicione as seguintes variáveis no nível da classe à classe **App** . Substitua pela `{yourdeviceconnectionstring}` seqüência de conexão do dispositivo que você copiou [em Registrar um novo dispositivo no hub IoT](#register-a-new-device-in-the-iot-hub).
...
13. Adicione o seguinte código ao final do método **principal.** A espera da tecla **Enter** permite tempo para o IoT Hub informar o status das operações duplas do dispositivo.
...
15. Salvar e fechar o **dispositivo simulado\src\main\java\com\mycompany\app\App\java.**

16. Compile o aplicativo **simulated-device** e corrija os erros. No prompt de comando, navegue até a pasta **do dispositivo simulado** e execute o seguinte comando:
...
2. Em um prompt de comando na pasta **de dispositivo simulado,** execute o seguinte comando para adicionar a propriedade **de conectividadeType** relatada ao dispositivo gêmeo:
...
3. Em um prompt de comando na pasta **add-tags-query,** execute o seguinte comando para executar o aplicativo de serviço **add-tags-query** uma segunda vez:
    ...
    Agora que seu dispositivo enviou a propriedade **connectivityType** para o IoT Hub, a segunda consulta retorna seu dispositivo.
...
* Controle dispositivos interativamente (como ligar um ventilador de um aplicativo controlado pelo usuário) com o tutorial [Use direct methods.](quickstart-control-device-java.md)
