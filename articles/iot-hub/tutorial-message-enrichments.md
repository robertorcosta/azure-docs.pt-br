...
1. Faça logon no portal do Azure. Clique em **+ criar um recurso**. Isso abre a caixa de pesquisa. Procure **implantação de modelo**. No painel de resultados, selecione **implantação de modelo (implantar usando o modelo personalizado)** .
...
Clique duas vezes no arquivo de solução (IoT_SimulatedDevice.sln) para abrir o código no Visual Studio, em seguida, abra o Program.cs. Substitua o nome do Hub IoT pelo `{your hub name}`do marcador. O formato do nome de host do Hub IoT é **{Your Hub Name}. Azure-Devices.net**. Para este tutorial, o nome de host do hub é **ContosoTestHubMsgEn.Azure-Devices.net**. Em seguida, substitua a chave do dispositivo que você salvou anteriormente ao executar o script para criar os recursos para o marcador `{your device key}`.
...
Para remover o grupo de recursos, use o comando [excluir grupo az](https://docs.microsoft.com/cli/azure/group?view=azure-cli-latest#az-group-delete). `$resourceGroup` foi definido como **ContosoResourcesMsgEn** de volta no início deste tutorial.
