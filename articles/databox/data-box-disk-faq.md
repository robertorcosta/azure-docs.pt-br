...
a. O conjunto de ferramentas disponível com o Data Box Disk contém três ferramentas:
 - **Ferramenta de desbloqueio de disco da caixa de dados**: Use esta ferramenta para desbloquear os discos criptografados que são enviados da Microsoft. Ao desbloquear os discos usando a ferramenta, você precisa fornecer uma chave de acesso disponível no pedido do Data Box Disk no portal do Azure. 
 - **Ferramenta de validação de disco da caixa de dados**: Use esta ferramenta para validar os nomes de tamanho, formato e blob de acordo com as convenções de nomeação do Azure. Ela também gera somas de verificação para os dados copiados que, em seguida, são usados para verificar os dados carregados no Azure.
 - **Ferramenta Data Box Disk Split Copy**: Use esta ferramenta quando estiver usando vários discos e tenha um grande conjunto de dados que precisa ser dividido e copiado em todos os discos. Essa ferramenta já está disponível para Windows. Esta ferramenta não é compatível com discos gerenciados. Esta ferramenta também faz a validação enquanto copia os dados. Portanto, você pode ignorar a etapa de validação ao usá-la.
...
a. Os arquivos do Azure são suportados com o Data Box Disk, mas não funcionarão bem com o Azure File Sync. Os metadados não são retidos se os dados do arquivo forem usados com o Azure File Sync.
