...
| **Nome do campo** | **Descrição** | **Partner Center equivalent** |
| --- | --- | --- |
...
| **PartnerName** | Name of the partner Azure Active Directory tenant. | Nome do parceiro. |
...
| servicePeriodEndDate | End date for the period when the service usage was rated for charges. The prices for Azure services are determined based on the rating period. | N/D |
...
| meterRegion | Identifica o local do datacenter para determinados serviços que são cobrados com base no local do datacenter. | The regional location of a data center for services, where applicable and populated. |
| ID da assinatura | Unique Microsoft generated identifier for the Azure subscription. | N/D |
| subscriptionName | Nome da assinatura do Azure. | N/D |
| Prazo | Exibe o prazo de validade da oferta. For example, reserved instances show 12 months of a yearly term of the reserved instance. For one-time purchases or recurring purchases, the term displays one month for SaaS, Azure Marketplace, and support. Not applicable for Azure consumption. | N/D |
...
| frequência | Payment frequency for a reserved instance. | N/D |
...
| Location | Normalized location of the resource. | N/D |
...
| Quantidade | Measured quantity purchased or consumed. The amount of the meter used during the billing period. | Number of units. Ensure it matches the information in your billing system during reconciliation. |
| unitOfMeasure | Identifica a unidade em que o serviço é cobrado. For example, GB and hours. | Identifica a unidade em que o serviço é cobrado. For example, GB, hours, and 10,000s. |
...
| serviceInfo1 | Um campo herdado que captura os metadados específicos do serviço opcional. | Internal Azure service metadata. |
| serviceInfo2 | Um campo herdado que captura os metadados específicos do serviço opcional. | Service information. Por exemplo, um tipo de imagem para uma máquina virtual e o nome do ISP para o ExpressRoute.|
| additionalInfo | Metadados específicos ao serviço. Por exemplo, um tipo de imagem para uma máquina virtual. | Any additional information not covered in other columns. The service-specific metadata. Por exemplo, um tipo de imagem para uma máquina virtual.|
| marcas | Tag that you assign to the meter. Use marcas para agrupar registros de cobrança. Por exemplo, você pode usar marcas para distribuir os custos entre os departamentos que usam o medidor. | Tags added by the customer.|
...
Then, call the asynchronous operation property value. Por exemplo:
