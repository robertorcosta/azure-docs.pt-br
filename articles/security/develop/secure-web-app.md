...
O aplicativo de exemplo usa a criptografia TLS/SSL de ponta a ponta para dados em trânsito que fluem para dentro e para fora da rede. O gateway é configurado com um certificado autoassinado.
> [!IMPORTANT]
...
- [CLI do Azure](/cli/azure/install-azure-cli) em seu computador de desenvolvimento.
...
3. Há arquivos na pasta scripts que são específicos para a plataforma que você está usando (Windows ou Linux). Como o CLI do Azure já foi instalado, entre na conta do Azure no prompt de comando executando este comando CLI do Azure:
...
Para obter mais informações sobre como configurar o TLS e a verificação de AC (autoridade de certificação) para PostgreSQL, consulte [Configurar a conectividade TLS no banco de dados do Azure para PostgreSQL](/azure/postgresql/concepts-ssl-connection-security).
...
2. [Baixe e instale o OpenSSL em seu computador](/azure/postgresql/concepts-ssl-connection-security).
...
Depois que os segredos estiverem em Key Vault e o aplicativo tiver acesso aos segredos e ao banco de dados, o serviço de aplicativo poderá ser acessado por meio da URL\/do aplicativo do gateway (https:/GATEWAY_HASH. cloudapp. net), que você pode obter em sua folha.
...
Para localizar e corrigir dependências de aplicativo, você pode usar a [verificação de dependência do OWASP](https://owasp.org/www-project-dependency-check/).
