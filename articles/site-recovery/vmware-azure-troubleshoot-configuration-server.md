...
1. Abra o arquivo C:\ProgramData\ASR\home\svsystems\var\configurator_register_host_static_info.log. (A pasta ProgramData pode ser uma pasta oculta. Se você não ver a pasta ProgramData, no Explorador de arquivos, na guia **Exibir,** na seção **Mostrar/ocultar,** selecione a caixa **de seleção de itens ocultos.)** Falhas podem ser causadas por múltiplos problemas.
...
4. Se a cadeia de caracteres **Reason=>NULL** não for encontrada, no computador de origem, abra o arquivo C:\ProgramData\ASRSetupLogs\UploadedLogs\ASRUnifiedAgentInstaller.log. (A pasta ProgramData pode ser uma pasta oculta. Se você não ver a pasta ProgramData, no Explorador de arquivos, na guia **Exibir,** na seção **Mostrar/ocultar,** selecione a caixa **de seleção de itens ocultos.)** Falhas podem ser causadas por múltiplos problemas. 

5. Procure a **solicitação de postagem de string: (7) - Não foi capaz de se conectar ao servidor**. Se a cadeia de caracteres for encontrada,
...
6. Se a solicitação de postagem de **string: (7) - Não foi possível conectar-se ao servidor** não for encontrado, no mesmo arquivo de log, procure a solicitação de **string: (60) - O certificado peer não pode ser autenticado com certificados CA dados**. Esse erro pode ocorrer porque o certificado do servidor de configuração expirou ou o computador de origem não oferece suporte a TLS 1.0 ou a protocolos SSL mais recentes. Tal erro também pode ocorrer se um firewall bloquear a comunicação SSL entre o computador de origem e o servidor de configuração. Se a cadeia de caracteres for encontrada, 
...
## <a name="installation-failure-failed-to-load-accounts"></a>Falha de instalação: Falha na carga de contas
...
## <a name="acs50008-saml-token-is-invalid"></a>ACS50008: Token SAML é inválido
...
## <a name="failure-to-activate-windows-license-from-server-standard-evaluation-to-server-standard"></a>Falha em ativar a licença do Windows da AVALIAÇÃO padrão do servidor para o padrão do servidor

1. Como parte da implantação do servidor de configuração através do OVF, é usada uma licença de avaliação, que é válida por 180 dias. É preciso ativar esta licença antes que ela expire. Caso outra coisa, isso pode resultar em desligamento frequente do servidor de configuração e, portanto, causar obstáculos às atividades de replicação.
...
- Verifique se o sinalizador **Usuário pode criar aplicativo** está definido como *true* no AAD. Para obter mais informações, consulte [Como: Use o portal para criar um aplicativo azure AD e um principal de serviço que possa acessar recursos](../active-directory/develop/howto-create-service-principal-portal.md#required-permissions).
