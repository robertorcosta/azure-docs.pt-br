...
Depois que as VMs tiverem sido criadas após o failover no Azure, os NSGs poderão ser usados para limitar o tráfego de rede à rede virtual e às VMs. O Site Recovery não cria NSGs como parte da operação de failover. É recomendável criar os NSGs do Azure necessários antes de iniciar o failover. Em seguida, você pode associar os NSGs a falhas em VMs automaticamente durante o failover, usando scripts de automação com os poderosos [planos de recuperação](site-recovery-create-recovery-plans.md)do Site Recovery .
...
O Site Recovery não cria ou replica NSGs como parte da operação de failover. É recomendável criar os NSGs necessários na região de destino do Azure antes de iniciar o failover. Em seguida, você pode associar os NSGs a falhas em VMs automaticamente durante o failover, usando scripts de automação com os poderosos [planos de recuperação](site-recovery-create-recovery-plans.md)do Site Recovery .
