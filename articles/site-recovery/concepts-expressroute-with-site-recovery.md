---
title: Sobre o uso do ExpressRoute com a recuperação do site do Azure
...
Um circuito ExpressRoute tem vários domínios de roteamento associados a ele. Saiba mais sobre os domínios de roteamento do ExpressRoute[aqui](../expressroute/expressroute-circuit-peerings.md#peeringcompare).
...
A Recuperação do Site replica dados em uma conta do Azure Storage ou em uma réplica de Disco Gerenciado na região do Azure de destino em um ponto final público. Para usar o tráfego de replicação do ExpressRoute for Site Recovery, você pode utilizar [o peering da Microsoft](../expressroute/expressroute-circuit-peerings.md#microsoftpeering) ou um [peering público](../expressroute/about-public-peering.md) existente (preterido para novas criações). Emparelhamento da Microsoft é o domínio de roteamento recomendado para replicação. Observe que a replicação não é suportada por peering privado.
...
No caso de você usar proxy no local e desejar usar o ExpressRoute para tráfego de replicação, você precisa configurar a lista de desvio proxy no Servidor de Configuração e servidores de processo. Siga as etapas abaixo:

- Baixe a ferramenta PsExec [daqui](https://aka.ms/PsExec) para acessar o contexto do usuário do sistema.
...
Isso garantirá que apenas o tráfego de replicação flua através do ExpressRoute enquanto a comunicação pode passar por proxy.
