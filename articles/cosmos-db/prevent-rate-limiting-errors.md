...
```
O comando a seguir **desabilitará** SSR para todas as coleções em sua conta de banco de dados. Pode levar até 15 min para que essa alteração entre em vigor.
```bash
az cosmosdb update --name accountname --resource-group resourcegroupname --capabilities EnableMongo DisableRateLimitingResponses
```
