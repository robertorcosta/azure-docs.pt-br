...
Na versão de visualização pública, há suporte para o Link do Synapse apenas para a API de SQL (Core) do Azure Cosmos DB. O suporte para a API do Cosmos DB para o Mongo DB e API do Cassandra está atualmente em visualização restrita.  Para solicitar acesso à visualização restrita, entre em contato com a [equipe do Azure Cosmos DB](mailto:cosmosdbsynapselink@microsoft.com).
