...
* **Throughput provisionado**: O rendimento provisionado (também chamado de throughput reservado) garante alto desempenho em qualquer escala. Especifique a taxa de transferência (RU/s) de que você precisa e o Azure Cosmos DB dedicará os recursos necessários para garantir a taxa de transferência configurada. Você será cobrado por hora pela taxa de transferência máxima provisionada para determinada hora.
...
* **Armazenamento consumido**: Você é cobrado uma taxa fixa para a quantidade total de armazenamento (GBs) consumida por dados e os índices por uma determinada hora.
