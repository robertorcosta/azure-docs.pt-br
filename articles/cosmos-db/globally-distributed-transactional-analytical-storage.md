...
|Retenção (tempo de vida ou TTL)   |  Orientado por política, configurado no contêiner Cosmos do Azure usando `DefaultTimeToLive` a propriedade.       |   Orientado por política, configurado no contêiner Cosmos do Azure usando `ColumnStoreTimeToLive` a propriedade.      |
