---
title: Configure a diretiva TLS usando o PowerShell
...
ms.locfileid: "81312684"
---
# <a name="configure-tls-policy-versions-and-cipher-suites-on-application-gateway"></a>Configure versões de política TLS e suítes de cifras no Gateway de aplicativos

Saiba como configurar versões de política TLS/SSL e suítes de cifra no Application Gateway. Você pode selecionar a partir de uma lista de políticas predefinidas que contêm diferentes configurações de versões de diretiva TLS e suítes de cifrahabilitadas. Você também tem a capacidade de definir uma [política TLS personalizada](#configure-a-custom-tls-policy) com base em seus requisitos.

[!INCLUDE [updated-for-az](../../includes/updated-for-az.md)]

## <a name="get-available-tls-options"></a>Obter opções tls disponíveis
...
## <a name="configure-a-custom-tls-policy"></a>Configure uma política TLS personalizada

Ao configurar uma diretiva TLS personalizada, você passa os seguintes parâmetros: PolicyType, MinProtocolVersion, CipherSuite e ApplicationGateway. Se tentar passar outros parâmetros, você receberá um erro ao criar ou atualizar o Gateway de Aplicativo. 
...
> [!IMPORTANT]
> TLS_RSA_WITH_AES_256_CBC_SHA256 devem ser selecionadas ao configurar uma diretiva TLS personalizada. O gateway de aplicativo usa esse conjunto de cifras para gerenciamento de back-end. Você pode usá-lo em combinação com qualquer outra suíte, mas este deve ser selecionado também. 
...
## <a name="create-an-application-gateway-with-a-pre-defined-tls-policy"></a>Crie um gateway de aplicativo com uma política TLS pré-definida

Ao configurar uma diretiva TLS predefinida, você passa os seguintes parâmetros: PolicyType, PolicyName e ApplicationGateway. Se tentar passar outros parâmetros, você receberá um erro ao criar ou atualizar o Gateway de Aplicativo.

O exemplo a seguir cria um novo gateway de aplicativo com uma política TLS pré-definida.
...
## <a name="update-an-existing-application-gateway-with-a-pre-defined-tls-policy"></a>Atualize um gateway de aplicativo existente com uma política TLS pré-definida

Para definir uma política TLS personalizada, passe os seguintes parâmetros: **PolicyType,** **MinProtocolVersion,** **CipherSuite**e **ApplicationGateway**. Para definir uma diretiva TLS predefinida, passe os seguintes parâmetros: **PolicyType,** **PolicyName**e **ApplicationGateway**. Se tentar passar outros parâmetros, você receberá um erro ao criar ou atualizar o Gateway de Aplicativo.
