...
* `az` -CLI do Azure: [instruções de instalação](/cli/azure/install-azure-cli?view=azure-cli-latest)
...
1. Criar entidade de serviço do AD ([Leia mais sobre o RBAC](../role-based-access-control/overview.md)):
    ```azurecli
...
1. Crie o arquivo de parâmetro que será usado na implantação do modelo de Azure Resource Manager mais tarde.
    ...
    EOF
    ```
    Para implantar um cluster habilitado para **RBAC** , defina o `aksEnableRBAC` campo como `true`
