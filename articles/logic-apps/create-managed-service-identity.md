...
1. No menu do aplicativo lógico, em **Configurações**, selecione **Identidade**. Selecione > **atribuído ao sistema** **em** > **salvar**. Quando o Azure solicitar que você confirme, selecione **Sim**.
...
1. Forneça informações sobre sua identidade gerenciada e, em seguida, selecione **criar**, por exemplo:
   ...
   |----------|----------|-------|-------------|
   | **Nome do recurso** | Sim | <> de *nome de identidade atribuído pelo usuário* | O nome para fornecer sua identidade atribuída pelo usuário. Este exemplo usa "fabrikam-User-Assigned-Identity". |
   ...
   | **Localidade** | Sim | <*Azure-region*> | A região do Azure onde armazenar informações sobre o recurso. Este exemplo usa "Leste dos EUA". |
   |||||
...
1. Na portal do Azure, localize e abra seu aplicativo lógico no designer de aplicativo lógico.

1. No menu do aplicativo lógico, em **configurações**, selecione **identidade**e, em seguida, selecione **usuário atribuído** > **Adicionar**.
...
Este exemplo mostra uma definição de recurso de aplicativo lógico para uma solicitação HTTP PUT e inclui um objeto de `identity` não parametrizado. A resposta para a solicitação PUT e a operação GET subsequente também têm esse `identity` objeto:
...
| Propriedade (JSON) | Valor | DESCRIÇÃO |
|-----------------|-------|-------------|
| `principalId` | <*principal-ID*> | O GUID (identificador global exclusivo) para a identidade gerenciada atribuída pelo usuário no locatário do Azure AD |
| `clientId` | <*client-ID*> | Um GUID (identificador global exclusivo) para a nova identidade do seu aplicativo lógico que é usado para chamadas durante o tempo de execução |
||||

Se o modelo também incluir a definição de recurso de identidade gerenciada, você poderá parametrizar o objeto `identity`. Este exemplo mostra como o objeto `userAssignedIdentities` filho faz referência a uma variável `userAssignedIdentity` que você define na seção `variables` do seu modelo. Essa variável faz referência à ID de recurso para sua identidade atribuída pelo usuário.
...
1. No menu do aplicativo lógico, em **configurações**, selecione **identidade**e siga as etapas para sua identidade:
   ...
   * Selecione > **atribuído ao sistema** **em** > **salvar**. Quando o Azure solicitar que você confirme, selecione **Sim**.
...
Se você criou a identidade gerenciada do aplicativo lógico usando um modelo de Azure Resource Manager, defina a propriedade filho `type` do objeto `identity` como `None`. Para a identidade gerenciada pelo sistema, essa ação também exclui a ID da entidade de segurança do Azure AD.
