...
1. No menu do seu espaço de trabalho, selecione **Resumo do espaço de trabalho** > gerenciamento de **aplicativos lógicos**.
...
* `clientTrackingId`: se não for fornecido, o Azure gerará essa ID automaticamente e correlacionará eventos em uma execução do aplicativo lógico, incluindo os fluxos de trabalho aninhados que são chamados no aplicativo lógico. Você pode especificar manualmente essa ID em um gatilho passando um cabeçalho de `x-ms-client-tracking-id` com seu valor de ID personalizado na solicitação de gatilho. Use um gatilho de solicitação, gatilho HTTP ou gatilho de webhook.

* `trackedProperties`: para controlar entradas ou saídas em dados de diagnóstico, você pode adicionar uma seção `trackedProperties` a uma ação usando o designer de aplicativo lógico ou diretamente na definição de JSON do aplicativo lógico. As propriedades acompanhadas só podem acompanhar entradas e saídas de uma única ação, mas você pode usar as propriedades `correlation` dos eventos para correlacionar entre as ações em uma execução. Para rastrear mais de uma propriedade, uma ou mais propriedades, adicione a seção `trackedProperties` e as propriedades que você deseja para a definição de ação.
...
Este exemplo mostra como o evento de `ActionCompleted` inclui os atributos `clientTrackingId` e `trackedProperties`:
