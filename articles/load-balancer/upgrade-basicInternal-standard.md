...
Baixe o script de migração do [Galeria do PowerShell](https://www.powershellgallery.com/packages/AzureILBUpgrade/1.0).
...
* Se você não tiver os módulos AZ do Azure instalados ou não se lembrar de desinstalar os módulos AZ do Azure, a melhor opção é usar a opção `Install-Script` para executar o script.
* Se você precisar manter os módulos AZ do Azure, sua melhor aposta é baixar o script e executá-lo diretamente.

Para determinar se você tem os módulos AZ do Azure instalados, execute `Get-InstalledModule -Name az`. Se você não vir nenhum módulo AZ instalado, poderá usar o método `Install-Script`.
...
Primeiro, verifique se o script criou com êxito um novo Load Balancer interno padrão com a configuração exata migrada do seu Load Balancer interno básico. Você pode verificar isso no portal do Azure.

Lembre-se de enviar uma pequena quantidade de tráfego por meio do Standard Load Balancer como um teste manual.
  
Aqui estão alguns cenários de como você adiciona VMs a pools de back-end das Load Balancer internas padrão recém-criadas que podem ser configuradas e nossas recomendações para cada uma delas:

* **Mover VMs existentes de pools de back-end de Load Balancer básicas internos antigos para pools de back-end de Load Balancer internos padrão recém-criados**.
...
* **Criação de novas VMs para adicionar aos pools de back-end do Load balancer interno padrão criado recentemente**.
