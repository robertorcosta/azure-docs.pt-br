...
3. Criar uma regra de saída no balanceador de carga para conexão de saída
...
* O script só dá suporte à atualização de Load Balancer pública. Para a atualização de Load Balancer básica interna, crie um Load Balancer interno padrão se a conectividade de saída não for desejada e crie um Load Balancer padrão interno e um Load Balancer público padrão se a conectividade de saída for necessária.
