...
1. Após os prompts, forneça as seguintes informações:
    ...
    | ------ | ----- | ----------- |
    | Selecione uma linguagem de programação para o seu projeto de aplicativo de função. | PowerShell | Crie um projeto local do PowerShell Functions. |
...
### <a name="configure-function-app-to-use-powershell-7"></a>Configurar o aplicativo de funções para usar o PowerShell 7

Abra o arquivo *local.settings.json* e confirme se uma configuração chamada `FUNCTIONS_WORKER_RUNTIME_VERSION` está definida como `~7`. Se ela estiver ausente ou definida com outro valor, atualize o conteúdo do arquivo.
...
1. Após os prompts, forneça as seguintes informações:
    ...
    | Prompt | Valor | Descrição |
    | ------ | ----- | ----------- |
    | Selecione um modelo para a função | Orquestrador das Durable Functions (versão prévia) | Crie uma orquestração das Durable Functions |
    | Forneça um nome de função | HelloOrchestrator | Nome da função durável |

Você adicionou um orquestrador para coordenar as funções de atividade. Abra *HelloOrchestrator/run.ps1* para ver a função de orquestrador. Cada chamada ao cmdlet `Invoke-ActivityFunction` invoca uma função de atividade chamada `Hello`.
...
1. Após os prompts, forneça as seguintes informações:
    ...
    | Prompt | Valor | Descrição |
    | ------ | ----- | ----------- |
    | Selecione um modelo para a função | Atividade das Durable Functions (versão prévia) | Crie uma função de atividade |
    | Forneça um nome de função | Olá | Nome da função de atividade |

Você adicionou a função de atividade `Hello`, que é invocada pelo orquestrador. Abra *Hello/run.ps1* para ver que ele está usando um nome como entrada e retornando uma saudação. Uma função de atividade é onde você executará ações como fazer uma chamada de banco de dados ou executar um cálculo.
...
1. Após os prompts, forneça as seguintes informações:
    ...
    | Prompt | Valor | Descrição |
    | ------ | ----- | ----------- |
    | Selecione um modelo para a função | Iniciador de HTTP das Durable Functions (versão prévia) | Crie uma função de iniciador de HTTP |
    | Forneça um nome de função | HttpStart | Nome da função de atividade |
    | Nível de autorização | Anônima | Para fins de demonstração, permita que a função seja chamada sem autenticação |

Você adicionou uma função disparada por HTTP que inicia uma orquestração. Abra *HttpStart/run.ps1* para ver que ele usa o cmdlet `Start-NewOrchestration` para iniciar uma orquestração. Em seguida, ela usa o cmdlet `New-OrchestrationCheckStatusResponse` para retornar uma resposta HTTP contendo URLs que podem ser usadas para monitorar e gerenciar a nova orquestração.
...
As Ferramentas Principais do Azure Functions permitem executar um projeto do Azure Functions no seu computador de desenvolvimento local. Você precisará instalar essas ferramentas na primeira vez que iniciar um aplicativo de funções no Visual Studio Code.

1. Para testar a função, defina um ponto de interrupção no código da função de atividade `Hello` (*Hello/run.ps1*). Pressione F5 ou selecione `Debug: Start Debugging` na paleta de comandos para iniciar o projeto do aplicativo de funções. A saída do Core Tools é exibida no painel **Terminal**.
...
1. Forneça as seguintes informações nos prompts:
    ...
    + **Selecione um runtime**: Escolha a versão do PowerShell em que está sendo feita a execução localmente. É possível usar o comando `pwsh -version` para verificar sua versão.

        > [!NOTE]
        > A extensão do VS Code para Azure Functions pode ainda não dar suporte ao PowerShell 7. Se o PowerShell 7 não estiver disponível como opção, selecione o PowerShell 6.x por enquanto e [atualize a versão manualmente](#update-function-app-ps7) depois que o aplicativo de funções for criado.
...
1. <a name="update-function-app-ps7"></a>Se não for possível selecionar o *PowerShell 7* antes de criar o aplicativo de funções, clique em F1 (ou CTRL/Cmd + SHIFT + P) para abrir a paleta de comandos. Na paleta de comandos, pesquise e selecione `Azure Functions: Upload Local Settings...`. Siga os avisos para selecionar o aplicativo de funções criado. Se precisar substituir as configurações existentes, selecione *Não para todas*.
...
Você usou o Visual Studio Code para criar e publicar um aplicativo de funções duráveis no PowerShell.
