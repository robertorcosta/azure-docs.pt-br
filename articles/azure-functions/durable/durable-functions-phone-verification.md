...
* `E4_SmsPhoneVerification`: Uma [função orquestradora](durable-functions-bindings.md#orchestration-trigger) que executa o processo de verificação do telefone, incluindo o gerenciamento de tempoouts e repetições.
* `E4_SendSmsChallenge`: Uma [função de atividade](durable-functions-bindings.md#activity-trigger) que envia um código via mensagem de texto.

### <a name="e4_smsphoneverification-orchestrator-function"></a>função orquestradora de E4_SmsPhoneVerification
...
> [!NOTE]
> Pode não ser óbvio a princípio, mas essa função de orquestrador é totalmente determinística. É determinista porque `CurrentUtcDateTime` a propriedade é usada para calcular o tempo de validade do temporizador, e retorna o mesmo valor em cada replay neste ponto no código orquestrador. Esse comportamento é importante para `winner` garantir que os `Task.WhenAny`mesmos resultados de cada chamada repetida para .
...
> [!NOTE]
> Pode não ser óbvio a princípio, mas essa função de orquestrador é totalmente determinística. É determinista porque `currentUtcDateTime` a propriedade é usada para calcular o tempo de validade do temporizador, e retorna o mesmo valor em cada replay neste ponto no código orquestrador. Esse comportamento é importante para `winner` garantir que os `context.df.Task.any`mesmos resultados de cada chamada repetida para .
...
O usuário recebe uma mensagem SMS com um código de 4 dígitos. Eles têm 90 segundos para enviar esse mesmo código de quatro dígitos de volta à instância da função orquestradora para concluir o processo de verificação. Se enviar o código errado, ele terá mais três tentativas para acertar (na mesma janela de 90 segundos).
...
A função **E4_SendSmsChallenge** usa a vinculação Twilio para enviar a mensagem SMS com o código de quatro dígitos para o usuário final.
...
> [!NOTE]
> Você precisará instalar `Microsoft.Azure.WebJobs.Extensions.Twilio` o pacote Nuget para executar o código de amostra.
...
Para responder com o código, você pode usar `{eventName}` `SmsChallengeResponse` [ `RaiseEventAsync` (.NET) ou `raiseEvent` (JavaScript)](durable-functions-instance-management.md) dentro de outra função ou invocar o **webhook sendEventUrl** HTTP POST referenciado na resposta 202 acima, substituindo pelo nome do evento:
...
Esta amostra demonstrou algumas das capacidades avançadas das `WaitForExternalEvent` Funções Duráveis, notadamente e `CreateTimer` APIs. Você viu como elas podem ser combinadas com `Task.WaitAny` para implementar um sistema de tempo limite confiável, que geralmente é útil para interagir com pessoas reais. Você pode aprender mais sobre como usar Funções Duráveis lendo uma série de artigos que oferecem uma cobertura aprofundada de tópicos específicos.
