...
Se sua função não for uma função assíncrona, **você deverá chamar** `context.done` para informar ao tempo de execução que sua função está concluída. Os tempos de execução se está ausente.
