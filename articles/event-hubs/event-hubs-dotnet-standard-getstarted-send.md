---
title: Enviar e receber eventos do Azure Event Hubs usando .NET (antigo)
description: Este artigo fornece um passo a passo para criar um aplicativo .NET Core que envia/recebe eventos para/a partir do Azure Event Hubs usando o antigo pacote Microsoft.Azure.EventHubs.
services: event-hubs
...
> [!WARNING]
> Este quickstart usa o antigo pacote **Microsoft.Azure.EventHubs.** Para obter um início rápido que use a biblioteca **Azure.Messaging.EventHubs** mais recente, consulte [Enviar e receber eventos usando a biblioteca Azure.Messaging.EventHubs](get-started-dotnet-standard-send-v2.md). Para mover seu aplicativo de usar a biblioteca antiga para uma nova, consulte o [Guia para migrar do Microsoft.Azure.EventHubs para o Azure.Messaging.EventHubs](https://github.com/Azure/azure-sdk-for-net/blob/master/sdk/eventhub/Azure.Messaging.EventHubs/migration-guide-from-v4.md).
...
- **Criar um namespace de Hubs de Eventos e um hub de eventos**. A primeira etapa é usar o [portal do Azure](https://portal.azure.com) para criar um namespace do tipo Hubs de eventos e obter as credenciais de gerenciamento das quais que seu aplicativo precisa para se comunicar com o hub de eventos. Para criar um namespace e um hub de eventos, siga o procedimento [nesse artigo](event-hubs-create.md). Em seguida, obtenha a **seqüência de conexões para o namespace do hub de eventos** seguindo as instruções do artigo: Obter string de [conexão](event-hubs-get-connection-string.md#get-connection-string-from-the-portal). Você usa a cadeia de conexão posteriormente no início rápido.
...
Inicie o Visual Studio. No menu **Arquivo,** clique em **Novo**e clique em **Projeto**. Crie um aplicativo de console do .NET Core.
...
Adicione [`Microsoft.Azure.EventHubs`](https://www.nuget.org/packages/Microsoft.Azure.EventHubs/) o pacote NuGet da biblioteca .NET Core ao seu projeto seguindo estas etapas: 
...
> [!NOTE]
> Você pode baixar este quickstart como uma amostra `EventHubConnectionString` `EventHubName`do `StorageAccountName` `StorageAccountKey` [GitHub](https://github.com/Azure/azure-event-hubs/tree/master/samples/DotNet/Microsoft.Azure.EventHubs/SampleEphReceiver), substituir e, , e `StorageContainerName` strings com os valores do seu hub de eventos, e executá-lo. Como alternativa, é possível seguir as etapas deste tutorial para criar sua própria solução.
