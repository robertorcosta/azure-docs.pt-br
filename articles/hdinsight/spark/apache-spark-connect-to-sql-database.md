...
Aprenda a conectar um cluster Apache Spark no Azure HDInsight com um Banco de Dados SQL do Azure. Em seguida, leia, escreva e transmita dados para o banco de dados SQL. As instruções deste artigo usam um Caderno Jupyter para executar os trechos do código Scala. No entanto, você pode criar um aplicativo autônomo em Scala ou Python e fazer as mesmas tarefas.
...
Comece criando um Jititer Notebook associado ao cluster Spark. Você usa esse notebook para executar os snippets de código usados neste artigo.
...
1. Um novo notebook é aberto com um nome padrão, **Untitled**. Clique no nome do notebook e insira o nome de sua escolha.
...
1. Agora você pode fazer operações no dataframe, como obter o esquema de dados:

    ```scala
    sqlTableDF.printSchema
    ```

    Você vê uma saída semelhante à seguinte imagem:

    ![saída esquema](./media/apache-spark-connect-to-sql-database/read-from-sql-schema-output.png "saída esquema")

1. Você também pode fazer operações como, recuperar as 10 principais linhas.
...
1. A saída mostra o esquema de **HVAC.csv**. O `hvactable` tem o mesmo esquema também. A saída lista as colunas na tabela.
    ...
    !['hdinsight Apache Spark esquema tabela'](./media/apache-spark-connect-to-sql-database/hdinsight-schema-table.png "Esquema da tabela")
