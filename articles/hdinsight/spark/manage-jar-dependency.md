...
O exemplo a seguir mostra um arquivo `pom.xml` que foi atualizado para sombrear um pacote usando o plug-in Maven-Shad-plugin.  A seção XML `<relocation>…</relocation>` move classes do pacote `com.google.guava` para o pacote `com.google.shaded.guava` movendo as entradas de arquivo JAR correspondentes e reescrevendo o código de bytes afetado.

Depois de alterar `pom.xml`, você pode executar `mvn package` para criar o uber-jar sombreado.
...
O SBT também é uma ferramenta de compilação para escala e Java. SBT não tem um plug-in de sombreador como Maven-Shad-plugin. Você pode modificar `build.sbt` arquivo para sombrear pacotes. 

Por exemplo, para sombrear `com.google.guava`, você pode adicionar o comando abaixo ao arquivo de `build.sbt`:
...
Em seguida, você pode executar `sbt clean` e `sbt assembly` para criar o arquivo JAR sombreado. 
