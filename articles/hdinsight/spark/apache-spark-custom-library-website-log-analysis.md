...
Uma vez que seus dados sejam salvos como uma tabela Apache Hive, na próxima seção nos conectaremos à tabela Colmeia usando ferramentas de BI como Power BI e Tableau.
...
1. Crie um novo bloco de anotações. Selecione **Novo**e, em seguida, **PySpark**.
    ...
    ![Crie um novo notebook Apache Jupyter](./media/apache-spark-custom-library-website-log-analysis/hdinsight-create-jupyter-notebook.png "Criar um novo bloco de anotações do Jupyter")

1. Um novo bloco de anotações é criado e aberto com o nome Untitled.pynb. Selecione o nome do notebook na parte superior e digite um nome amigável.
...
1. Como você criou um notebook usando o kernel PySpark, você não precisa criar nenhum contexto explicitamente. Os contextos do Spark e do Hive serão criados automaticamente para você ao executar a primeira célula do código. Você pode começar importando os tipos que são obrigatórios para este cenário. Cole o seguinte trecho em uma célula vazia e, em seguida, **pressione Shift + Enter**.
...
1. Crie um RDD usando os dados de log de exemplo já disponíveis no cluster. Você pode acessar os dados na conta de `\HdiSamples\HdiSamples\WebsiteLogSampleData\SampleLog\909f2b.log`armazenamento padrão associada ao cluster em . Execute o seguinte código:
...
1. Na saída acima, as primeiras linhas incluem as informações de cabeçalho, e cada linha restante coincide com o esquema descrito naquele cabeçalho. Analisar esses logs pode ser complicado. Então, usamos uma biblioteca personalizada do Python (**iislogparser.py**) que torna a análise desses logs muito mais fácil. Por padrão, esta biblioteca está incluída com `/HdiSamples/HdiSamples/WebsiteLogSampleData/iislogparser.py`o cluster Spark no HDInsight em .

    No entanto, esta biblioteca `PYTHONPATH` não está no, então não podemos `import iislogparser`usá-la usando uma declaração de importação como . Para usar essa biblioteca, devemos distribuí-la para todos os nós de trabalho. Execute o snippet de código a seguir.
...
1. A classe `LogLine`, por sua vez, tem alguns métodos úteis, como `is_error()`, que retorna se uma entrada de log tem um código de erro. Use esta classe para calcular o número de erros nas linhas de log extraídas e, em seguida, registre todos os erros em um arquivo diferente.
    ...
    A saída `There are 30 errors and 646 log entries`deve ser detonada.
...
1. Você pode então executar a seguinte consulta SQL para obter todos os registros da tabela **AverageTime** .
   ...
   ![hdinsight jupyter sql query saída](./media/apache-spark-custom-library-website-log-analysis/hdinsight-jupyter-sql-qyery-output.png "Saída da consulta SQL")
...
1. Agora você pode usar Matplotlib, uma biblioteca usada para construir a visualização de dados para criar um gráfico. Como o gráfico deve ser criado da estrutura de dados **averagetime** mantida localmente, o snippet de código deve começar com a mágica `%%local`. Isso garante que o código seja executado localmente no servidor do Jupyter.
   ...
   ![apache faísca web log gráfico de análise](./media/apache-spark-custom-library-website-log-analysis/hdinsight-apache-spark-web-log-analysis-plot.png "Saída de Matplotlib")

1. Depois de concluir a execução do aplicativo, você deve encerrar o bloco de anotações para liberar os recursos. Para fazer isso, no menu **Arquivo** do notebook, selecione **Fechar e Interromper**. Esta ação vai desligar e fechar o caderno.

## <a name="next-steps"></a>Próximas etapas

Explorar os seguintes artigos:
