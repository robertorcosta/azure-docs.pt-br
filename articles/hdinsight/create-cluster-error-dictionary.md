...
"O local da ação de script não pode\<ser acessado URI: URL\>de ação de script"
...
"A \<versão\> \<do esquema do metastore\> META_STORE_TYPE METASTORE_MAJOR_VERSION \<no\> banco de dados database_name é incompatível com a versão \<do cluster CLUSTER_VERSION\>"
...
"As regras de segurança no grupo de segurança de\<rede/subscriptions/\>SubscriptionId/ResourceGroups/<nome\> do grupo de recursos default/Providers/Microsoft\<. Network/networkSecurityGroups\> /grupo de segurança\<de rede\>nome\<configurado com a\> sub-rede/subscriptions/SubscriptionId/resourceGroups/nome do grupo de recursos\<RG-westeurope-vnet-TomTom-default/Providers\</Microsoft\> . Network/virtualNetworks/nome da rede virtual\>/Subnets/nome da sub-rede não permite conectividade de entrada e/ou saída necessária Para obter mais informações, visite [planejar uma rede virtual para o Azure HDInsight](https://docs.microsoft.com/azure/hdinsight/hdinsight-plan-virtual-network-deployment)ou contate o suporte. "
