...
2. Insira a seguinte consulta de Hive:
    ...
    * `CREATE TABLE IF NOT EXISTS`: cria uma tabela, caso ela ainda não exista. Como a palavra-chave `EXTERNAL` não é usada, essa instrução cria uma tabela interna. As tabelas internas são armazenadas no data warehouse do Hive e gerenciadas por ele.
...
* [Usar MapReduce no Apache Hadoop em HDInsight](hdinsight-use-mapreduce.md)
