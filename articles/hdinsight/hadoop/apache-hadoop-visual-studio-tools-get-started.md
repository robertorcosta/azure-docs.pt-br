...
Aprenda a usar as ferramentas microsoft azure data lake e stream analytics para o Visual Studio (também chamado de Data Lake Tools) para se conectar aos [clusters Apache Hadoop no Azure HDInsight](apache-hadoop-introduction.md) e enviar consultas hive.  
...
Para obter mais informações sobre como se conectar a um cluster Apache Storm, consulte [Develop C# topologias for Apache Storm usando as ferramentas Data Lake para Visual Studio](../storm/apache-storm-develop-csharp-visual-studio-topology.md).
...
- Para visual studio 2017 ou Visual Studio 2019:
    ...
    Para as instalações existentes do Visual Studio, vá até a barra de menus do IDE e selecione **Ferramentas** > **Obter Ferramentas e Recursos** para abrir o Visual Studio Installer. Na guia **Cargas de Trabalho,** selecione pelo menos a carga de trabalho **de desenvolvimento do Azure** (na **Web & Cloud)** ou a carga de trabalho de **armazenamento e processamento de dados** (em **Outras ferramentas ).**
...
6. Expanda um cluster do HDInsight. O cluster contém nós para **bancos de dados hive,** uma conta de armazenamento padrão, quaisquer contas adicionais de armazenamento vinculado e **registro de serviço hadoop**. Você pode expandir mais as entidades.
...
Para tirar dúvidas e/ou fornecer feedback do Visual Studio:
...
2. Digite uma **Url de conexão** no *formulário https\://\<nome de cluster&nbsp;>.azurehdinsight.net*. O **Nome do Cluster** preenche automaticamente com a parte de nome do cluster da sua URL quando você vai para outro campo. Em seguida, digite um **nome de usuário** e **senha**e selecione **Next**.
...
3. Escolha o modo de execução:
    ...
    * **Batch**  
        ...
        Na primeira lista baixa, escolha **'Lote'** e, em seguida, **selecione 'Enviar'(ou** selecione o ícone de saque ao lado de **Enviar** e escolher **Avançado**).
...
Mesmo que Tez seja especificado como o motor de execução, o gráfico de trabalho pode não aparecer se nenhum aplicativo Tez for lançado.  Essa situação pode ocorrer porque o trabalho não contém instruções dml, ou porque as declarações do DML podem retornar sem iniciar um aplicativo Tez. Por exemplo, `SELECT * FROM table1` não será lançado o aplicativo Tez.
