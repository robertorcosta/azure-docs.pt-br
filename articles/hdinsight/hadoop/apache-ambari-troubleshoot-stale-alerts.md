...
O valor padrão de `alert_grace_period` é 5 segundos. Você pode definir essa configuração em/etc/ambari-Agent/conf/ambari-Agent.ini. Para hosts nos quais os alertas obsoletos ocorrem em intervalos regulares, tente aumentar o valor para 10. Em seguida, reinicie o agente Ambari.
...
* Conecte-se com [@AzureSupport](https://twitter.com/azuresupport) no Twitter. Essa é a conta de Microsoft Azure oficial para melhorar a experiência do cliente. Ele conecta a Comunidade do Azure aos recursos certos: respostas, suporte e especialistas.
