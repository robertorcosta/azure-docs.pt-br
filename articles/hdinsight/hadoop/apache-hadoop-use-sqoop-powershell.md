...
Este exemplo exporta dados da tabela Hive `hivesampletable` para a `mobiledata` tabela no banco de dados SQL. Defina os valores para as variáveis abaixo e execute o comando.
...
Se você receber a mensagem de erro `The specified blob does not exist.`, tente novamente após alguns minutos.

## <a name="sqoop-import"></a>Importação do Sqoop

De SQL Server para o armazenamento do Azure. Este exemplo importa dados da `mobiledata` tabela no banco de dados SQL para o `wasb:///tutorials/usesqoop/importeddata` diretório no HDInsight. Os campos nos dados que são separados por um caractere de tabulação, e as linhas serão encerradas por um caractere de nova linha. Este exemplo pressupõe que você concluiu o exemplo anterior.
...
Este é um exemplo robusto que exporta dados `/tutorials/usesqoop/data/sample.log` do da conta de armazenamento padrão e, em seguida, importa-os `log4jlogs` para uma tabela chamada em um banco de dados SQL Server. Este exemplo não depende dos exemplos anteriores.

O seguinte script do PowerShell processa previamente o arquivo de origem e, em seguida, exporta-o para um `log4jlogs`banco de dados SQL do Azure para a tabela. Substitua `CLUSTERNAME`, `CLUSTERPASSWORD`e `SQLPASSWORD` pelos valores que você usou do pré-requisito.
