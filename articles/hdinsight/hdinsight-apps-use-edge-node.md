...
description: Como adicionar um nó de borda vazio a um cluster HDInsight. Usado como um cliente e, em seguida, teste ou hospede seus aplicativos do HDInsight.
...
Saiba como adicionar um nó de borda vazio a um cluster HDInsight. Um nó de borda vazio é uma máquina virtual Linux com as mesmas ferramentas de cliente instaladas e configuradas dos nós principais. Mas sem serviços de [Apache Hadoop](./hadoop/apache-hadoop-introduction.md) em execução. Você pode usar o nó de borda para acessar o cluster, testar e hospedar seus aplicativos clientes.

Você pode adicionar um nó de borda vazio a um cluster HDInsight existente ou a um novo cluster quando ele é criado. A adição de um nó de borda vazio é feita usando o modelo Azure Resource Manager.  O exemplo a seguir demonstra como isso é feito usando um modelo:
...
Como mostrado no exemplo, você pode opcionalmente chamar uma [ação de script](hdinsight-hadoop-customize-cluster-linux.md) para fazer uma configuração adicional. Como instalar o [Apache matiz](hdinsight-hadoop-hue-linux.md) no nó de borda. O script de ação de script deve ser publicamente acessível na Web.  Por exemplo, se o script for armazenado no armazenamento do Azure, use contêineres públicos ou BLOBs públicos.
...
>
> Se você estiver usando uma tecnologia Apache, poderá encontrar assistência por meio dos sites do projeto do Apache [https://apache.org](https://apache.org), como o site do [Apache Hadoop](https://hadoop.apache.org/) .
...
Nesta seção, você pode usar um modelo do Resource Manager para adicionar um nó de borda a um cluster HDInsight existente.  O modelo do Resource Manager pode ser encontrado no [GitHub](https://azure.microsoft.com/resources/templates/101-hdinsight-linux-add-edge-node/). O modelo do Resource Manager chama uma ação de script https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/101-hdinsight-linux-add-edge-node/scripts/EmptyNodeSetup.shlocalizada em. O script não realiza nenhuma ação.  É demonstrar a chamada de ação de script de um modelo do Resource Manager.

1. Selecione a imagem a seguir para entrar no Azure e abra o modelo Azure Resource Manager no portal do Azure.
