...
description: Configurar backup e replicação para Apache HBase e Apache Phoenix no Azure HDInsight
author: ashishthaps
ms.author: ashishth
ms.reviewer: jasonh
...
* No Azure Data Lake `hbase` Storage, a pasta reside o caminho raiz especificado ao provisionar um cluster. Normalmente, esse caminho raiz tem uma pasta `clusters`, com uma subpasta nomeada de acordo com o seu cluster do HDInsight:
...
No cluster HDInsight de origem, use o [utilitário Export](https://hbase.apache.org/book.html#export) (incluído no HBase) para exportar dados de uma tabela de origem para o armazenamento conectado padrão. Em seguida, você pode copiar a pasta exportada para o local de armazenamento de destino e executar o [utilitário Importação](https://hbase.apache.org/book.html#import) no cluster HDInsight de destino.

Para exportar dados da tabela, primeiro SSH no nó principal do `hbase` cluster HDInsight de origem e, em seguida, execute o seguinte comando:

    hbase org.apache.hadoop.hbase.mapreduce.Export "<tableName>" "/<path>/<to>/<export>"

O diretório de exportação ainda não deve existir. O nome da mesa é sensível a maiúsculas e minúsculas.

Para importar dados da tabela, SSH no nó principal do cluster `hbase` HDInsight de destino e, em seguida, execute o seguinte comando:
...
Quando ambos os clusters do HDInsight estão na mesma rede virtual, conforme descrito anteriormente, a resolução de nomes de host internos é automática. Para usar o CopyTable para clusters HDInsight em duas redes virtuais separadas conectadas por um Gateway VPN, você precisará fornecer os endereços IP host dos nós do Zookeeper no quórum.
...
Os instantâneos fornecem um backup completo de uma tabela no momento do comando `snapshot`. Os snapshots não fornecem a capacidade de executar instantâneos incrementais por janelas de tempo, nem de especificar subconjuntos de colunas que as famílias incluem no snapshot.

## <a name="replication"></a>Replicação

[A replicação do HBase](https://hbase.apache.org/book.html#_cluster_replication) empurra automaticamente as transações de um cluster de origem para um cluster de destino, usando um mecanismo assíncrono com sobrecarga mínima no cluster de origem. No HDInsight, você pode configurar a replicação entre os clusters, onde:
...
* [Trabalhando com o Utilitário de Importação e Exportação HBase](https://blogs.msdn.microsoft.com/data_otaku/2016/12/21/working-with-the-hbase-import-and-export-utility/)
