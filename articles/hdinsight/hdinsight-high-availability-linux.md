...
Para obter os nomes de host para diferentes tipos de nó em seu cluster, use a [API REST do amAmbari](hdinsight-hadoop-manage-ambari-rest-api.md#get-the-fqdn-of-cluster-nodes).
...
Esse comando retorna um valor semelhante ao seguinte, que contém a URL interna a ser usada com o `oozie` comando:
...
| `ResourceManager`Interface do usuário da Web                   | Esse alerta no nível do host será disparado se a `ResourceManager` interface do usuário da Web estiver inacessível.                                                                                                             |
