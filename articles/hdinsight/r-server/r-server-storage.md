...
Os serviços ML no HDInsight podem usar diferentes soluções de armazenamento para persistir dados, códigos ou objetos que contenham resultados da análise. Essas soluções incluem as seguintes opções:
...
Você também tem a opção de acessar várias contas de armazenamento ou contêineres do Azure com o cluster HDInsight. O armazenamento de arquivos Do Zure é uma opção conveniente de armazenamento de dados para uso no nó de borda que permite que você monte um compartilhamento de arquivos de armazenamento Azure para, por exemplo, o sistema de arquivos Linux. Mas os compartilhamentos de arquivos do Azure podem ser montados e usados por qualquer sistema que tenha um sistema operacional com suporte, como Windows ou Linux.

Quando você cria um cluster Apache Hadoop no HDInsight, você especifica uma conta **de armazenamento Azure** ou **armazenamento no lago de dados.** Um contêiner de armazenamento específico dessa conta mantém o sistema de arquivos do cluster que você cria (por exemplo, o Sistema de Arquivos Distribuído Hadoop). Para obter mais informações e diretrizes, consulte:
...
Também é possível adicionar acesso de cluster a uma ou mais contas de armazenamento do Data Lake após a criação de clusters. Abra a entrada do portal do Azure para um Data Lake Storage e vá para **Data Explorer > Acesso > Adicionar**.
...
Depois de ter dado acesso ao Data Lake Storage Gen1, você pode usar o armazenamento no cluster ML Services no HDInsight da maneira que você faria uma conta de armazenamento Azure secundária. A única diferença é que o prefixo **wasbs://** muda para **adl://** da seguinte forma:
...
Há também uma opção conveniente de armazenamento de dados para uso no nó de borda chamado [Azure Files](https://azure.microsoft.com/services/storage/files/). Esse recurso permite montar um compartilhamento de arquivos do Armazenamento do Azure para o sistema de arquivos do Linux. Essa opção pode ser útil para armazenar arquivos de dados, scripts R e objetos de resultado que podem ser necessários posteriormente, especialmente quando fizer sentido usar o sistema de arquivos nativo no nó de borda em vez do HDFS.
...
- [Como usar o armazenamento de arquivos do Azure com Linux](../../storage/files/storage-how-to-use-files-linux.md)
