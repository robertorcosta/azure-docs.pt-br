...
| Name       |Porta                        |Protocolo|Fonte           |Destino|Ação|
|------------|----------------------------|--------|-----------------|-----------|------|
...
O **tls 1,2 é imposto em conexões de saída**: em Janeiro 2020 Microsoft imposto TLS 1,2 para tráfego dentro de serviço em todos os serviços do Azure. Para a instância gerenciada do banco de dados SQL do Azure, isso resultou no TLS 1,2 sendo aplicado em conexões de saída usadas para replicação e conexões de servidor vinculado a SQL Server. Se você estiver usando versões do SQL Server mais antigas, 2016 com Instância Gerenciada, verifique se [as atualizações específicas do TLS 1,2](https://support.microsoft.com/help/3135244/tls-1-2-support-for-microsoft-sql-server) foram aplicadas.

No momento, não há suporte para os seguintes recursos de rede virtual com o Instância Gerenciada:
- **Emparelhamento da Microsoft**: habilitar [o emparelhamento da Microsoft](../expressroute/expressroute-faqs.md#microsoft-peering) em circuitos de rota expressa emparelhados direta ou transitivamente com a rede virtual em que instância gerenciada residem afetar o fluxo de tráfego entre os componentes instância gerenciada dentro de rede virtual e serviços que dependem de causar problemas de disponibilidade. Espera-se que Instância Gerenciada implantações na rede virtual com o emparelhamento da Microsoft já esteja habilitado.
- **Emparelhamento de rede virtual global**: a conectividade de [emparelhamento de rede virtual](../virtual-network/virtual-network-peering-overview.md) entre regiões do Azure não funciona para instância gerenciada devido a [restrições de balanceador de carga documentadas](../virtual-network/virtual-networks-faq.md#what-are-the-constraints-related-to-global-vnet-peering-and-load-balancers).
- **AzurePlatformDNS**: usar a [marca de serviço](../virtual-network/service-tags-overview.md) AzurePlatformDNS para bloquear a resolução de DNS da plataforma renderizaria instância gerenciada indisponível. Embora Instância Gerenciada dê suporte ao DNS definido pelo cliente para a resolução DNS dentro do mecanismo, há uma dependência no DNS da plataforma para operações de plataforma.
- **Gateway NAT**: usar [NAT de rede virtual](../virtual-network/nat-overview.md) para controlar a conectividade de saída com endereço IP público específico renderizaria instância gerenciada indisponível. O serviço de Instância Gerenciada atualmente está limitado ao uso do balanceador de carga básico que não fornece a coexistência de fluxos de entrada e saída com NAT de rede virtual.
