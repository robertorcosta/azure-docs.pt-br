---
title: Configurar a replicação transacional entre duas instâncias gerenciadas e o SQL Server
...
> - Configurar um SQL Server como um assinante. 

![Replicação entre um publicador da MI do SQL, um distribuidor da MI do SQL e um assinante do SQL Server](media/sql-database-managed-instance-failover-group-tutorial/sqlmi-to-sql-replication.png)

Este tutorial destina-se a um público-alvo experiente e pressupõe que o usuário esteja familiarizado com a implantação e a conexão com ambas as instâncias gerenciadas e as VMs do SQL Server no Azure. Portanto, algumas etapas deste tutorial são ignoradas. 

Para saber mais, confira os artigos [Visão geral da instância gerenciada do Banco de Dados SQL do Azure](sql-database-managed-instance-index.yml), [Funcionalidades](sql-database-managed-instance.md) e [Replicação transacional do SQL](sql-database-managed-instance-transactional-replication.md).
...
- Um assinante do SQL Server, local ou uma VM do Azure. Este tutorial usa uma VM do Azure.  
- [SSMS (SQL Server Management Studio) 18.0 ou posterior](/sql/ssms/download-sql-server-management-studio-ssms).
- A última versão do [Azure PowerShell](/powershell/azure/install-az-ps?view=azps-1.7.0).
...
Para obter mais informações sobre como criar uma instância gerenciada, confira [Criar uma instância gerenciada no portal](sql-database-managed-instance-get-started.md)
...
Para obter mais informações sobre como implantar uma VM do SQL Server no Azure, confira [Início Rápido: Criar uma VM do SQL Server](../virtual-machines/windows/sql/quickstart-sql-vm-create-portal.md).
...
## <a name="5---create-private-dns-zone"></a>5 – Criar uma zona DNS privado

Uma zona DNS privado permite o roteamento DNS entre as instâncias gerenciadas e o SQL Server. 

### <a name="create-private-dns-zone"></a>Criar uma zona DNS privado
...
1. Forneça um nome arbitrário para a zona DNS, como `repldns.com`. 
   ...
   ![Criar uma zona DNS privado](media/sql-database-managed-instance-configure-replication-tutorial/create-private-dns-zone.png)

1. Selecione **Examinar + criar**. Examine os parâmetros da zona DNS privado e, em seguida, selecione **Criar** para criar o recurso. 

### <a name="create-a-record"></a>Criar um registro A
...
1. Forneça o nome da VM do SQL Server, bem como o endereço IP interno privado. 
   ...
   ![Configurar o registro A](media/sql-database-managed-instance-configure-replication-tutorial/configure-a-record.png)
...
1. Marque a caixa ao lado de **Habilitar o registro automático**. 
   ...
   ![Criar um link da VNET](media/sql-database-managed-instance-configure-replication-tutorial/configure-vnet-link.png)
...
## <a name="6---create-azure-storage-account"></a>6 – Criar uma conta de armazenamento do Azure
...
Crie um banco de dados na MI do publicador. Para fazer isso, siga estas etapas:

1. Inicie o SSMS (SQL Server Management Studio) no SQL Server. 
...
1. Na página **Segurança do Agente**, selecione **Configurações de Segurança...** . Forneça as credenciais de logon do SQL Server a serem usadas para o Agente de Instantâneo e para se conectar ao publicador. Selecione **OK** para fechar a página **Segurança do Agente de Instantâneo**. Selecione **Avançar**. 
   ...
   ![Configurar a segurança do Agente de Instantâneo](media/sql-database-managed-instance-configure-replication-tutorial/snapshot-agent-security.png)
...
O uso de uma barra "/" em vez de uma barra invertida no caminho do arquivo para o compartilhamento de arquivo pode causar esse erro. Não há problemas nisto: `\\replstorage.file.core.windows.net\replshare` Isso pode causar um erro do sistema operacional 55: `'\\replstorage.file.core.windows.net/replshare'`
