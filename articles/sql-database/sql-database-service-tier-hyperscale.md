...
1. Crie um servidor de banco de dados SQL do Azure na região de destino se você ainda não tiver um servidor apropriado.  Esse servidor deve pertencer à mesma assinatura que o servidor original (origem).
...
| Problema | Descrição |
| :---- | :--------- |
| O painel gerenciar backups de um servidor lógico não mostra bancos de dados de hiperescala, que serão filtrados da exibição  | O hiperscale tem um método separado para gerenciar backups e, como tal, a retenção de longo prazo e as configurações de retenção de backup point-in-time não se aplicam/são invalidadas. Da mesma forma, os bancos de dados da Hiperescala não aparecem no painel Gerenciar Backup. |
| Restauração em um momento determinado | Você pode restaurar um banco de dados de hiperescala em um banco de dados que não seja de hiperescala, dentro do período de retenção do banco de dados não hiperescala. Não é possível restaurar um banco de dados que não seja de hiperescala em um banco de dados de hiperescala.|
