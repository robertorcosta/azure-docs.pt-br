...
Você recebe mensagens de erro quando a conexão com o banco de dados SQL do Azure falha. Os problemas de conexão podem ser causados pela reconfiguração de banco de dados do SQL Azure, as configurações do firewall, tempo limite de conexão ou informações de logon incorreto. Além disso, se o limite máximo de alguns recursos do banco de dados SQL do Azure for atingido, você não poderá se conectar ao banco de dados SQL do Azure.
...
### <a name="error-10053-a-transport-level-error-has-occurred-when-receiving-results-from-the-server"></a>Erro 10053: ocorreu um erro de nível de transporte ao receber os resultados do servidor.
...
Normalmente, o administrador de serviços pode usar as etapas a seguir para adicionar o logon:

1. Faça logon no servidor usando SQL Server Management Studio (SSMS).
2. Verifique se o nome de logon está desabilitado usando a seguinte consulta SQL:
...
4. Se o nome de usuário de logon do SQL não existir, crie-o usando o SSMS:
   ...
   1. Clique duas vezes em **Segurança** para expandir. 
   2. Clique com o botão direito em **Logons** e escolha **Novo logon**. 
   3. No script gerado com espaços reservados, você pode editar e executar a seguinte consulta SQL:
...
9. No script gerado com espaços reservados, você pode editar e executar a seguinte consulta SQL: 
...
Para obter mais informações, consulte [Gerenciando bancos de dados e logons no Banco de Dados SQL do Azure](https://docs.microsoft.com/azure/sql-database/sql-database-manage-logins).
...
3. Se o tamanho do banco de dados já tiver passado do tamanho máximo com suporte para sua edição, você poderá executar uma das seguintes etapas:
   ...
   2. Particione ou exclua dados, descarte índices ou consulte a documentação para conhecer as possíveis resoluções. 
...
### <a name="error-40549-session-is-terminated-because-you-have-a-long-running-transaction"></a>Erro 40549: A sessão foi encerrada porque você tem uma transação de longa duração.
...
### <a name="error-40552-the-session-has-been-terminated-because-of-excessive-transaction-log-space-usage"></a>Erro 40552: A sessão foi encerrada devido a uso excessivo do espaço de log de transação.
