...
description: Saiba como armazenar e restaurar backups automatizados para um banco de dados em pool ou individual do Banco de Dados SQL do Azure (por até dez anos) usando o Portal do Azure e o PowerShell
...
No Banco de Dados SQL do Azure, é possível configurar um banco de dados individual ou em pool com uma [política de retenção de backup de longo prazo](sql-database-long-term-retention.md) (LTR) para reter automaticamente backups do banco de dados em contêineres de armazenamento de Blobs do Azure separados por até dez anos. Em seguida, você pode recuperar um banco de dados usando esses backups pelo Portal do Azure ou o PowerShell.

> [!IMPORTANT]
> Para a instância gerenciada do Banco de Dados do SQL do Azure, confira [Suporte LTR para instância gerenciada](sql-database-long-term-retention.md#managed-instance-support).
...
> [!IMPORTANT]
> Quando você habilita uma política de retenção de backup de longo prazo, pode levar até sete dias para o primeiro backup ficar visível e disponível para restauração. Para obter detalhes sobre o ritmo de backup de LTR, confira [Retenção de backup de longo prazo](sql-database-long-term-retention.md).
