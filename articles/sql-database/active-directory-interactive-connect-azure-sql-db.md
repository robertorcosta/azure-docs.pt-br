...
A partir do .NET Framework versão 4.7.2, o [`SqlAuthenticationMethod`](https://docs.microsoft.com/dotnet/api/system.data.sqlclient.sqlauthenticationmethod) de enumeração tem um novo valor: `ActiveDirectoryInteractive`. Em um programa C# cliente, o valor de enumeração direciona o sistema para usar o modo interativo Azure Active Directory (AD do Azure) que dá suporte à autenticação multifator para se conectar a um banco de dados SQL do Azure. O usuário que executa o programa verá as seguintes caixas de diálogo:
...
O exemplo C# depende do namespace [`System.Data.SqlClient`](https://docs.microsoft.com/dotnet/api/system.data.sqlclient). De interesse especial para a autenticação multifator é o `SqlAuthenticationMethod`de enumeração, que tem os seguintes valores:
...
| RedirectUri | new Uri("https://mywebserver.com/") | **Azure Active Directory** > **registros de aplicativo** > **Pesquisar por nome** >  *[seu-app-Registration]*  > **configurações** > **RedirectURIs**<br /><br />Para este artigo, qualquer valor válido é adequado para RedirectUri, pois ele não é usado aqui. |
| &nbsp; | &nbsp; | &nbsp; |
