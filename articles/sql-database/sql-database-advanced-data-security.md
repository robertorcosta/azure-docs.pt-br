...
Para substituir as configurações da ADS para determinado banco de dados, marque a caixa de seleção **Habilitar Segurança de Dados Avançada no nível do banco de dados**. Use essa opção somente se você tiver um requisito específico para receber alertas de proteção avançada de ameaças separados ou resultados de avaliação de vulnerabilidades para o banco de dados individual, no lugar ou além dos alertas e resultados recebidos para todos os bancos de dados no servidor de banco de dados ou instância gerenciada.
...
- Saiba mais sobre [avaliação de vulnerabilidades](sql-vulnerability-assessment.md) 
