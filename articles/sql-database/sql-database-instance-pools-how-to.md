---
title: Guia de instruções dos pools de instâncias (versão prévia)
description: Este artigo descreve como criar e gerenciar pools de Instâncias do Banco de Dados SQL do Azure (versão prévia).
...
ms.locfileid: "83773034"
---
# <a name="azure-sql-database-instance-pools-preview-how-to-guide"></a>Guia de instruções do Banco de Dados SQL do Azure (versão prévia)

Este artigo fornece detalhes sobre como criar e gerenciar os [pools de instâncias](sql-database-instance-pools.md).
...
|Atualizar uso do recurso de instância gerenciada|Sim |Sim|
|Verificar o uso e as propriedades de instância gerenciada|Sim|Sim|
|Excluir instância gerenciada do pool.|Sim|Sim|
|Criar um banco de dados na instância gerenciada colocada no pool|Sim|Sim|
|Excluir um banco de dados em uma instância gerenciada|Sim|Sim|

[Comandos do PowerShell](https://docs.microsoft.com/powershell/module/az.sql/) disponíveis

|Cmdlet |Descrição |
|:---|:---|
|[New-AzSqlInstancePool](/powershell/module/az.sql/new-azsqlinstancepool/) | Cria um pool de instância do Banco de Dados SQL do Azure. |
|[Get-AzSqlInstancePool](/powershell/module/az.sql/get-azsqlinstancepool/) | Retorna informações sobre a instância do SQL do Azure. |
|[Set-AzSqlInstancePool](/powershell/module/az.sql/set-azsqlinstancepool/) | Define propriedades para um pool de instância do Banco de Dados SQL do Azure. |
|[Remove-AzSqlInstancePool](/powershell/module/az.sql/remove-azsqlinstancepool/) | Remove um pool de instância do Banco de Dados SQL do Azure. |
|[Get-AzSqlInstancePoolUsage](/powershell/module/az.sql/get-azsqlinstancepoolusage/) | Retorna informações sobre a instância do pool do SQL do Azure. |
...
Para operações relacionadas a instâncias dentro de pools e instâncias únicas, use os [comandos de instância gerenciada padrão](sql-database-managed-instance-create-manage.md#powershell-create-and-manage-managed-instances), mas o *nome do pool de instâncias* propriedade deve ser populada ao usar esses comandos para uma instância em um pool.
...
1. Implantação de pool de instância única. Essa é uma operação de execução prolongada, em que a duração é a mesma que a implantação de um [única instância criada em uma sub-rede vazia](sql-database-managed-instance.md#managed-instance-management-operations).
...
2. [Criar um pool de instância](#create-an-instance-pool).
...
- [Determinar o tamanho da sub-rede da VNet para o banco de dados da instância gerenciada do Banco de Dados SQL do Azure](sql-database-managed-instance-determine-size-vnet-subnet.md).
...
## <a name="scale-a-managed-instance-inside-a-pool"></a>Escalar uma instância gerenciada dentro de um pool 
...
## <a name="connect-to-a-managed-instance-inside-a-pool"></a>Conecte-se a uma instância gerenciada dentro de um pool
...
### <a name="enable-the-public-endpoint-for-the-instance"></a>Habilite o ponto de extremidade público para a instância
...
Para obter mais detalhes, confira [Permitir tráfego de ponto de extremidade público no grupo de segurança de rede](sql-database-managed-instance-public-endpoint-configure.md#allow-public-endpoint-traffic-on-the-network-security-group).
