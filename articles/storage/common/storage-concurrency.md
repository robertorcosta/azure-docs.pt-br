...
description: Aprenda a gerenciar a concorrência para os serviços Blob, Fila, Tabela e Arquivo.
services: storage
author: tamram
ms.service: storage
ms.devlang: dotnet
...
2. Simultaneidade pessimista: um aplicativo procurando realizar uma atualização bloqueará um objeto, impedindo que outros usuários atualizem os dados até que o bloqueio seja liberado. Por exemplo, em um cenário de replicação de dados mestre/subordinado, onde apenas o mestre executará atualizações, o mestre normalmente manterá um bloqueio exclusivo por um longo período de tempo nos dados para garantir que ninguém mais possa atualizá-lo.
...
## <a name="azure-storage-simplifies-cloud-development"></a>O Azure Storage simplifica o desenvolvimento da nuvem
...
## <a name="managing-concurrency-in-blob-storage"></a>Gerenciamento de simultuou no armazenamento Blob

Você pode optar por usar modelos de concorrência otimistas ou pessimistas para gerenciar o acesso a blobs e contêineres no serviço Blob. Se você não especificar explicitamente uma estratégia, o padrão será último a gravar vence.  
...
O Azure Storage também inclui suporte para cabeçalhos condicionais adicionais, como **If-Modified-Since,** **If-Unmodified-Since** e **If-None-Match,** bem como combinações. Para obter mais informações, consulte [Especificar cabeçalhos condicionais para operações de serviço blob](https://msdn.microsoft.com/library/azure/dd179371.aspx).  
...
O snippet de C# a seguir mostra um exemplo de aquisição de uma concessão exclusiva por 30 segundos em um blob, atualizando o conteúdo do blob e liberando a concessão. Se já houver uma locação válida na bolha quando você tentar adquirir um novo contrato de locação, o serviço Blob retorna um resultado de status "HTTP (409) Conflict". O snippet de código a seguir usa um objeto **AccessCondition** para encapsular as informações da concessão quando ela faz uma solicitação para atualizar o blob no serviço de Armazenamento.  Você pode baixar o exemplo completo aqui: [Gerenciando a Simultaneidade usando o Armazenamento do Azure](https://code.msdn.microsoft.com/Managing-Concurrency-using-56018114).
...
Se você tentar realizar uma operação de gravação em um blob sob concessão sem enviar a ID de concessão, a solicitação falhará com um erro 412. Observe que se a concessão expirar antes de chamar o método **UploadText**, mas você ainda utilizar a ID de concessão, a solicitação também falhará com um erro **412**. Para obter mais informações sobre o gerenciamento de prazos de validade de locação e iDs de locação, consulte a documentação [Lease Blob](https://msdn.microsoft.com/library/azure/ee691972.aspx) REST.  
...
* [Blob de Locação](https://msdn.microsoft.com/library/azure/ee691972.aspx)

## <a name="managing-concurrency-in-table-storage"></a>Gerenciamento de simultuos no armazenamento de tabelas

O serviço Table usa verificações de concorrência otimistas como o comportamento padrão quando você está trabalhando com entidades, ao contrário do serviço Blob, onde você deve explicitamente optar por realizar verificações de concorrência otimistas. A outra diferença entre os serviços de tabela e Blob é que você só pode gerenciar o comportamento de simultâneos de entidades, enquanto que com o serviço Blob você pode gerenciar a concorrência de contêineres e blobs.  
...
## <a name="managing-concurrency-in-azure-files"></a>Gerenciamento de simultuos em arquivos Azure
