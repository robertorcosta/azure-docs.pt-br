...
description: Faça operações no armazenamento de filas do Azure com o PowerShell. Com o armazenamento de filas do Azure, você pode armazenar grandes números de mensagens que podem ser acessadas por HTTP/HTTPS.
...
As operações que afetam as mensagens reais na fila usam a biblioteca de cliente de armazenamento do .NET como exposta no PowerShell. Para adicionar uma mensagem a uma fila, crie uma nova instância do objeto de mensagem, a classe [Microsoft. Azure. Storage. Queue. CloudQueueMessage](/java/api/com.microsoft.azure.storage.queue.cloudqueuemessage) . Em seguida, chame o método [AddMessage](/java/api/com.microsoft.azure.storage.queue.cloudqueue.addmessage) . Um CloudQueueMessage pode ser criado por meio de uma cadeia de caracteres (em formato UTF-8) ou de uma matriz de bytes.
