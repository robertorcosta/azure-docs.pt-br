...
As extensões de armazenamento do Azure têm suporte para operações em dados de fila. As operações que você pode chamar dependem das permissões concedidas à entidade de segurança do Azure AD com a qual você entra no CLI do Azure. As permissões para filas de armazenamento do Azure são atribuídas por meio do RBAC do Azure. Por exemplo, se você tiver atribuído a função de **leitor de dados de fila de armazenamento** , poderá executar comandos de script que lêem dados de uma fila. Se você tiver atribuído a função **colaborador de dados da fila de armazenamento** , poderá executar comandos de script que lêem, gravam ou excluem uma fila ou os dados que eles contêm.
