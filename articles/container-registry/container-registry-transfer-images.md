...
* **[PipelineRun](#create-pipelinerun-for-export-with-resource-manager)** -o recurso usado para invocar um recurso ExportPipeline ou ImportPipeline.  
  ...
  * No momento, um máximo de **10 artefatos** pode ser transferido com cada PipelineRun.
...
* Os exemplos de pipeline criam identidades gerenciadas atribuídas pelo sistema para acessar os segredos do cofre de chaves. ExportPipelines e ImportPipelines também oferecem suporte a identidades atribuídas pelo usuário. Nesse caso, você deve configurar os cofres de chaves com políticas de acesso para as identidades. 
...
Execute o [grupo de implantação AZ Create][az-deployment-group-create] para criar o recurso. O exemplo a seguir nomeia o *exportPipeline*de implantação.
...
Execute o [grupo de implantação AZ Create][az-deployment-group-create] para criar o recurso.
...
Para excluir um recurso de pipeline, exclua sua implantação do Resource Manager usando o comando [AZ Deployment Group Delete][az-deployment-group-delete] . Os exemplos a seguir excluem os recursos de pipeline criados neste artigo:
...
* **Problemas de transferência de artefatos**
  * Nem todos os artefatos, ou nenhum, são transferidos. Confirme a ortografia dos artefatos na execução de exportação e o nome do blob nas execuções de exportação e importação. Confirme que você está transferindo um máximo de 10 artefatos.
