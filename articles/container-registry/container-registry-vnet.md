...
* To use the Azure CLI steps in this article, Azure CLI version 2.0.58 or later is required. Se você precisa instalar ou fazer upgrade, veja [Instalar a CLI do Azure][azure-cli].
...
* If you want to restrict registry access using a virtual network in a different Azure subscription, you need to register the resource provider for Azure Container Registry in that subscription. Por exemplo:
...
A criação da VM demora alguns minutos. Quando o comando for concluído, observe o `publicIpAddress` exibido pela CLI do Azure. Use this address to make SSH connections to the VM, and optionally for later setup of firewall rules.
...
Siga as etapas em [Instalar a CLI do Azure com apt](/cli/azure/install-azure-cli-apt?view=azure-cli-latest) para instalar a CLI do Azure em sua máquina virtual do Ubuntu. For this article, ensure that you install version 2.0.58 or later.
...
1. In the search box at the top of the [Azure portal][azure-portal], enter *virtual networks*. Quando **Redes virtuais** aparecer nos resultados da pesquisa, selecione essa opção.
...
1. Select **Add existing virtual network**, and select the virtual network and subnet you configured with a service endpoint. Selecione **Adicionar**.
1. Clique em **Salvar**.
...
For each rule that is configured, run the [az acr network-rule remove][az-acr-network-rule-remove] command to remove it. Por exemplo:
