...
- **Dimensionar conforme necessário** -aumente o número de instâncias em um pool de agentes para tarefas de computação intensiva ou dimensione para zero. A cobrança é baseada na alocação.
...
Esse recurso está disponível na camada de serviço do registro de contêiner **Premium** . Para obter informações sobre limites e camadas de serviço do registro, consulte [SKUs do registro de contêiner do Azure][acr-tiers].
...
- Os pools de agente de tarefa atualmente dão suporte a nós do Linux. No momento, não há suporte para nós do Windows.
- Os pools de agente de tarefa estão disponíveis em versão prévia nas seguintes regiões: oeste dos EUA 2, Sul EUA Central, leste dos EUA 2 e leste dos EUA.
- Para cada registro, a cota padrão total de vCPU (núcleo) para todos os pools de agentes é 16. Abra uma [solicitação de suporte][open-support-ticket] para alocação adicional.

## <a name="prerequisites"></a>Pré-requisitos

* Para usar as etapas de CLI do Azure neste artigo, CLI do Azure versão 2.3.1 ou posterior é necessária. Se você precisa instalar ou atualizar, consulte [Instalar a CLI do Azure][azure-cli]. Ou execute em [Azure cloud Shell](../cloud-shell/quickstart.md).
...
A criação de um pool de agentes e de outras operações de gerenciamento de pool levará vários minutos para ser concluída.
...
> [!NOTE]
> Se suas tarefas exigirem recursos adicionais da Internet pública, adicione as regras correspondentes. Por exemplo, essas regras são necessárias se você executar uma tarefa de Build do Docker que precisa extrair as imagens base do Hub do Docker ou restaurar um pacote NuGet.
...
### <a name="quick-task-run"></a>Execução de tarefa rápida

Enfileirar uma execução rápida no pool de agentes usando o comando [AZ ACR Build][az-acr-build] e passar `--agent-pool` o parâmetro:
...
Por exemplo, crie uma tarefa agendada no pool de agentes com [AZ ACR tarefa Create][az-acr-task-create], passando `--agent-pool` o parâmetro.
