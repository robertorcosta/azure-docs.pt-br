...
Este artigo pressupõe que você já criou um registro de contêiner do Azure privado. Você também precisa ter um cluster kubernetes em execução e acessível por meio da ferramenta de linha de comando `kubectl`.
...
Crie um segredo de pull de imagem com o seguinte comando de `kubectl`:
...
No exemplo anterior, `your-awesome-app:v1` é o nome da imagem a ser extraído do registro de contêiner do Azure e `acr-secret` é o nome do segredo de pull que você criou para acessar o registro. Quando você implantar o Pod, o kubernetes extrairá automaticamente a imagem do registro, se ela ainda não estiver presente no cluster.
