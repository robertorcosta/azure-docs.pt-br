...
- Crie um ambiente do Time Series Insights. For more information, see [How to get started with Time Series Insights](./time-series-insights-get-started.md).
...
1. To start, open the [Time Series Insights explorer](https://insights.timeseries.azure.com/) in your web browser. On the left side of the window, select an environment. Todos os ambientes aos quais você tem acesso são listados em ordem alfabética.
...
1. Em seguida, você verá um gráfico que mostra uma contagem de todos os eventos durante o período de tempo selecionado. Aqui, você tem uma série de controles:
    ...
    - **Terms Editor panel**: The term space is where you query your environment. It’s found on the left side of the screen:
      ...
      - **Predicate**: Use the predicate to quickly filter your events by using the set of operands listed in the following table. If you conduct a search by selecting or clicking, the predicate automatically updates based on that search. Os tipos de operando com suporte incluem:
         ...
         |**HAS**     | **Cadeia de caracteres**        |  Only constant string literals are allowed at the right side. Empty string and **NULL** aren't allowed.       |
...
1. Use the **heatmap** to quickly spot unique or anomalous data series in a given query. Apenas um termo de pesquisa pode ser visualizado como um mapa de calor.
...
1. When you explore events by selecting or right-clicking, the **EVENTS** panel is made available. Aqui, você pode ver todos os seus eventos brutos e exportá-los como arquivos JSON ou CSV. Time Series Insights stores all raw data.
...
1. Select the **STATS** tab after you explore events to expose patterns and column stats.
    ...
    - **Patterns**: This feature proactively surfaces the most statistically significant patterns in a selected data region. You don't have to look at thousands of events to understand what patterns require the most time and energy. With Time Series Insights, you can jump directly into these statistically significant patterns to continue conducting an analysis. Esse recurso também é útil para investigações post-mortem em dados históricos.
