...
* `Location`define uma hierarquia com pai `states` e filho `cities`. Cada `location` um pode ter `states`vários, que, por sua vez `cities`, podem ter vários.
* `ManufactureDate`define uma hierarquia com pai `year` e filho `month`. Cada `ManufactureDate` um pode ter `years`vários, que, por sua vez `months`, podem ter vários.
...
Considere um exemplo em que **H1** a hierarquia `building`H1 `floor`tem, `room` e como parte de sua definição de **instanceFieldNames** :
...
A série **temporal ID1** e **ID4** são exibidos como parte da **hierarquia H1** no [Gerenciador de Azure Time Series insights](time-series-insights-update-explorer.md) porque eles possuem parâmetros de *compilação*, *piso*e *sala* totalmente definidos e corretamente ordenados.
