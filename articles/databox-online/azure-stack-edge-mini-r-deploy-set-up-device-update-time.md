---
title: Tutorial para configurar o dispositivo, a atualização e o tempo para o dispositivo Azure Stack Edge Mini R no portal do Azure
description: O tutorial para implantar o Azure Stack Edge Mini R orienta você a definir configurações de dispositivo, atualização e tempo para seu dispositivo físico.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
Este tutorial descreve como definir configurações relacionadas ao dispositivo para seu dispositivo Azure Stack Edge Mini R com uma GPU integrada. Você pode configurar o nome do dispositivo, o servidor de atualização e o servidor de horário por meio da IU da Web local.
...
Antes de definir as configurações relacionadas ao dispositivo em seu dispositivo Azure Stack Edge Mini R com GPU, verifique se:
...
* Para o dispositivo físico:
    ...
    - Você configurou a rede e habilitou e configurou a rede de computação em seu dispositivo, conforme detalhado no [Tutorial: Configurar a rede para o Azure Stack Edge Mini R com GPU](azure-stack-edge-mini-r-deploy-configure-network-compute-web-proxy.md).
...
1. Na página **Dispositivo**, execute as seguintes etapas:
    ...
    3. Para validar e aplicar as configurações do dispositivo, selecione **Aplicar**.
        ...
        ![Página "Dispositivo" da IU da Web local 1](./media/azure-stack-edge-mini-r-deploy-set-up-device-update-time/set-up-device-1.png)

        Se você tiver alterado o nome do dispositivo e o domínio DNS, os certificados autoassinados gerados que existiam no dispositivo não funcionarão. 

        ![Página "Dispositivo" da IU da Web local 2](./media/azure-stack-edge-mini-r-deploy-set-up-device-update-time/set-up-device-2.png)

        Você precisa escolher uma das seguintes opções ao configurar certificados: 
    ...
    4. Quando o nome do dispositivo e o domínio DNS são alterados, o ponto de extremidade SMB é criado.  
        ...
        ![Página "Dispositivo" da IU da Web local 3](./media/azure-stack-edge-mini-r-deploy-set-up-device-update-time/set-up-device-3.png)
...
1. Na página **Atualizar**, você pode configurar a localização da qual baixar as atualizações para seu dispositivo.  
    ...
    - Você pode obter as atualizações diretamente do **servidor do Microsoft Update**.
        ...
        ![Página "Servidor de Atualização" da IU da Web local](./media/azure-stack-edge-mini-r-deploy-set-up-device-update-time/update-server-1.png)
...
1. Na página **Hora**, você pode selecionar o fuso horário e os servidores NTP primário e secundário para seu dispositivo.  
    ...
    4. Para validar e aplicar as configurações de hora configuradas, selecione **Aplicar**.
        ...
        ![Página "Hora" da IU da Web local](./media/azure-stack-edge-mini-r-deploy-set-up-device-update-time/time-settings-1.png)
...
Para saber como configurar certificados para seu dispositivo Azure Stack Edge Mini R, confira:
