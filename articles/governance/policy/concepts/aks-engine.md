...
1. Criar uma atribuição de função para a entidade de serviço de cluster
   ...
   - Atribua a atribuição de função ' gravador de dados de informações de política (versão prévia) ' à ID do aplicativo principal de serviço de cluster (valor _aadClientID_ da etapa anterior) com CLI do Azure. Substitua `<subscriptionId>` por sua ID de assinatura `<aks engine cluster resource group>` e pelo grupo de recursos no qual o cluster kubernetes AKs do mecanismo de gerenciamento automático está.
...
- Instalar no cluster existente com gráficos do Helm
  ...
  1. Instale o complemento com um gráfico do Helm. Substitua `<subscriptionId>` por sua ID de assinatura `<aks engine cluster resource group>` e pelo grupo de recursos no qual o cluster kubernetes AKs do mecanismo de gerenciamento automático está.
...
O Pod do Gatekeeper, o _gatekeeper-Controller-Manager-0_, geralmente está `gatekeeper-system` no `kube-system` namespace ou, mas pode estar em um namespace diferente, dependendo de como ele é implantado.

Para exibir os logs do Gatekeeper, `kubectl`use:
