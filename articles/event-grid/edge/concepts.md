...
description: Conceitos em Event Grid on IoT Edge.
author: VidyaKukke
manager: rajarv
ms.author: vkukke
...
Para as propriedades incluídas em um evento, consulte [o esquema de eventos azure Event Grid](event-schemas.md).
...
Consulte a [documentação da API REST](api.md) sobre como gerenciar assinaturas no Event Grid.

## <a name="event-handlers"></a>Manipuladores de eventos

Sob a perspectiva de uma Grade de Eventos, um manipulador de eventos é o local em que o evento é enviado. O manipulador toma mais medidas para processar o evento. Grade de eventos do Azure dá suporte a vários tipos de manipulador. Você pode usar um serviço Azure suportado ou seu próprio gancho web como manipulador. Dependendo do tipo de manipulador, a Grade de Eventos segue diferentes mecanismos para assegurar a entrega do evento. Se o manipulador de eventos de destino for um gancho web HTTP, `200 – OK`o evento será repetido quando o manipulador retornar um código de status de . Para o Edge Hub, se o evento for entregue sem exceção, ele será considerado bem sucedido.
...
Ao usar um tópico personalizado, os eventos sempre devem ser publicados em uma matriz. Para cenários de baixo throughput, a matriz terá apenas um valor. Para casos de uso de alto volume, recomendamos que você lote vários eventos juntos por publicação para obter maior eficiência. Lotes podem ter até 1 MB. Cada evento ainda não deve ser maior que 1 MB (visualização).