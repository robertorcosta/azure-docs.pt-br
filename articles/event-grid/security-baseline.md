...
Você também pode fazer uso de definições de política internas relacionadas à grade de eventos do Azure, como:-os domínios da grade de eventos do Azure devem usar links privados – os tópicos da grade de eventos do Azure devem usar linksAzure particulares
...
**Diretrizes**: o gerenciamento de acesso aos recursos da grade de eventos é controlado por meio do Azure Active Directory (AD). O Azure AD não tem o conceito de senhas padrão.
...
**Orientação**: não aplicável; O serviço de grade de eventos não usa a autenticação multifator
...
**Diretrizes**: Não aplicável. A grade de eventos não usa o Azure AD para autenticar clientes de publicação de eventos; Ele dá suporte à autenticação por meio de chaves SAS.
...
Use o Azure Active Directory (AD) Privileged Identity Management (PIM) para a geração de logs e alertas quando uma atividade suspeita ou não segura ocorrer no ambiente.
