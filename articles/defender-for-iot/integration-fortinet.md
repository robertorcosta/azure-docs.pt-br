...
## <a name="the-defender-for-iot-platform"></a>A plataforma defender para IoT
...
:::image type="content" source="media/integration-fortinet/firewall.png" alt-text="Exibição da exibição de janela do firewall do FortiGate.":::
...
1. Salve a chave de API.

    :::image type="content" source="media/integration-fortinet/api-key.png" alt-text="A descrição do telefone celular gera automaticamente a nova chave de API":::

## <a name="block-suspected-traffic-using-the-fortigate-firewall"></a>Bloquear o tráfego suspeito usando o Firewall do FortiGate

1. No painel esquerdo, selecione **encaminhamento**.

    [:::image type="content" source="media/integration-fortinet/forwarding-view.png" alt-text="A opção de janela de encaminhamento em um sensor.":::](media/integration-fortinet/forwarding-view.png#lightbox)

1. Selecione **criar regras de encaminhamento** e definir parâmetros de regra.

    :::image type="content" source="media/integration-fortinet/new-rule.png" alt-text="Captura de tela da janela criar regra de encaminhamento":::
    ...
    | **Protocolos** | Por padrão, todos os protocolos são selecionados.<br><br>Para selecionar um protocolo específico, selecione **específico** e selecione o protocolo para o qual essa regra é aplicada. |
    | **Motores** | Por padrão, todos os mecanismos de segurança estão envolvidos.<br><br>Para selecionar um mecanismo de segurança específico para o qual essa regra é aplicada, selecione **específico** e selecione o mecanismo. |
    | **Notificações do sistema** | Encaminhar sensor *online e status offline* . Essa opção só estará disponível se você tiver feito logon no console de gerenciamento local. |

1. Para instruir o defender para IoT a enviar regras de firewall para o FortiGate, selecione **ação** e, em seguida, selecione **Enviar para FortiGate**.

    :::image type="content" source="media/integration-fortinet/fortigate.png" alt-text="Criar janela de regra de encaminhamento e selecione Enviar para FortiGate":::

1. Para configurar a regra de encaminhamento de FortiGate:

    :::image type="content" source="media/integration-fortinet/configure.png" alt-text="Configurar a janela criar regra de encaminhamento":::
...
1. No painel **ações** , defina os seguintes parâmetros:
    ...
    |Configurar| Configure as seguintes opções para permitir o bloqueio das fontes suspeitas pelo firewall do FortiGate: <br /><br />**Bloquear códigos de função inválidos**: violações de protocolo-valor de campo inválido violando a especificação do protocolo ICS (exploração em potencial)<br /><br />**Bloquear atualizações de firmware/programação de PLC não autorizadas**: alterações de PLC não autorizadas<br /><br />**Bloquear interrupção não autorizada de PLC**: parada de PLC (tempo de inatividade)<br /><br />**Bloquear alertas relacionados a malware**: bloqueio de tentativas de malware industrial (Triton, NotPetya, etc.). Você pode selecionar a opção de **bloqueio automático**. Nesse caso, o bloqueio é executado automaticamente e imediatamente.<br /><br />*Bloquear a verificação não autorizada*: verificação não autorizada (reconhecimento em potencial)
