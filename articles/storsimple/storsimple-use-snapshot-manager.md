...
#### <a name="menu-access"></a>Acesso ao menu
Para visualizar o menu **Ação**, faça o seguinte:

* Clique com o botão direito do mouse em um item nos painéis **Escopo** ou **Resultados**.
* Selecione um item nos painéis **Escopo** ou **Resultados** e, em seguida, clique em **Ação** na barra do menu. 

Por exemplo, se você selecionar o nó superior no painel **Escopo** e depois clicar com o botão direito ou clicar em **Ação** na barra de menus, o seguinte menu será exibido.

![Menu Ação do StorSimple Snapshot Manager](./media/storsimple-use-snapshot-manager/HCS_SSM_Action_menu.png)

O painel **Ações** (à direita do console) contém a mesma lista de ações que o menu **Ação**. Além disso, o painel **Ações** contém as opções do menu **Exibir**, o que permite a criação de uma visualização personalizada do painel **Resultados**.
...
* A coluna **Ação** relaciona ações que podem ser realizadas em nós e resultados. 
* A coluna **Navegação** explica como exibir o menu **Ação** apropriado de modo que se possa selecionar a ação. Algumas ações aparecem em vários menus **Ação**. Para essas ações, selecione uma opção de **Navegação** na lista com marcadores. 
* A coluna **Descrição** descreve como utilizar cada ação do menu **Ação** ou do painel Ações e explica o que ela faz.

> [!NOTE]
> O painel **Ações** e os menus **Ação** contêm opções adicionais, como **Exibir**, **Nova Janela a partir Daqui**, **Atualizar**, **Exportar Lista** e **Ajuda**. Essas opções estão disponíveis como parte do MMC e não são específicas ao StorSimple Snapshot Manager. A tabela inclui descrições dessas opções.
> 
> 
