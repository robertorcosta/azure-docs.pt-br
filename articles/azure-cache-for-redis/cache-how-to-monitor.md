...
1. Na página **Cache Azure para Redis,** o título **Monitoramento,** **selecione Diagnóstico**.
2. Selecione **+ Adicione a configuração de diagnóstico**.
...
5. Sob a **métrica**de título da tabela, caixa de seleção ao lado dos itens de linha que você deseja armazenar, como **AllMetrics**. Especifique uma política **de retenção (dias).** A retenção máxima de dias que você pode especificar é **de 365 dias**. No entanto, se você quiser reter os dados de métricas para sempre, defina **Retenção (dias)** para **0**.
6. Clique em **Salvar**.
...
>[!NOTE]
>Além de arquivar suas métricas de cache para armazenamento, você também pode [transmiti-las para um hub de eventos ou enviá-las para logs do Monitor do Azure](../azure-monitor/platform/rest-api-walkthrough.md#retrieve-metric-values).
>
...
| Métrica | Descrição |
| --- | --- |
| Acertos do Cache |O número de pesquisas de chave com êxito durante o intervalo de relatório especificado. Este número `keyspace_hits` mapeia para a partir do comando Redis [INFO.](https://redis.io/commands/info) |
| Latência do cache (versão prévia) | A latência do cache calculada com base na latência de entre nós do cache. Essa métrica é medida em microssegundos e `Avg` `Min`tem `Max`três dimensões: , e , que representam a latência média, mínima e máxima do cache, respectivamente, durante o intervalo de relatórios especificado. |
| Erros de Cache |O número de pesquisas de chave com falha durante o intervalo de relatório especificado. Este número `keyspace_misses` mapeia para a partir do comando Redis INFO. Erros de cache não significam necessariamente que há um problema com o cache. Por exemplo, ao se usar o padrão de programação cache-aside, um aplicativo procura um item no cache primeiro. Se o item não estiver lá (erro de cache), o item será recuperado do banco de dados e adicionado ao cache na próxima vez. Erros de cache são o comportamento normal para o padrão de programação cache-aside. Se o número de erros de cache for maior do que o esperado, examine a lógica do aplicativo que popula e lê do cache. Se os itens estão sendo despejados do cache devido à pressão da memória, então pode haver algumas `Used Memory` `Evicted Keys`falhas de cache, mas uma métrica melhor para monitorar a pressão de memória seria ou . |
| Cache Lido |A quantidade de dados lidos do cache, em MB/s, durante o intervalo de relatório especificado. Esse valor é derivado das placas de adaptador de rede que dão suporte à máquina virtual que hospeda o cache e não é específico do Redis. **Esse valor corresponde à largura de banda da rede usada por este cache. Se você quiser configurar alertas para os limites de largura de `Cache Read` banda da rede do lado do servidor, crie-o usando este contador. Consulte [esta tabela](cache-faq.md#cache-performance) para obter os limites de largura de banda observados para vários níveis e tamanhos de preços de cache.** |
...
| Clientes conectados |O número de conexões de cliente com o cache durante o intervalo de relatório especificado. Este número `connected_clients` mapeia para a partir do comando Redis INFO. Uma vez que o [limite de conexão](cache-configure.md#default-redis-server-configuration) seja atingido, as tentativas de conexão subseqüentes ao cache falharão. Mesmo que não existam aplicativos de clientes ativos, ainda pode haver alguns casos de clientes conectados devido a processos internos e conexões. |
...
| Errors | Falhas específicas e problemas de desempenho que o cache pode estar enfrentando durante um intervalo de relatório especificado. Essa métrica tem oito dimensões representando diferentes tipos de erros, mas poderia ter mais adicionadas no futuro. Os tipos de erros representados agora são os seguintes: <br/><ul><li>**Failover** – quando um cache falha (subordinado promove ao mestre)</li><li>**Perda de dados** – quando há perda de dados no cache</li><li>**UnresponsiveClients** – quando os clientes não estão lendo dados do servidor com rapidez suficiente</li><li>**AOF** – quando há um problema relacionado à persistência do AOF</li><li>**RDB** – quando há um problema relacionado à persistência do RDB</li><li>**Importação** – quando há um problema relacionado à Importação do RDB</li><li>**Exportação** – quando há um problema relacionado à Exportação do RDB</li></ul> |
| Chaves removidas |O número de itens removidos do cache durante o intervalo de relatório especificado devido ao limite de `maxmemory` . Este número `evicted_keys` mapeia para a partir do comando Redis INFO. |
...
| Carga do Servidor Redis |O percentual de ciclos em que o servidor Redis está ocupado processando, em vez de ocioso esperando por mensagens. Se este contador atingir 100, significa que o servidor Redis atingiu um teto de desempenho e a CPU não pode processar o trabalho mais rápido. Se você estiver vendo alta carga do Servidor Redis, então você verá exceções de tempo incerto no cliente. Neste caso, você deve considerar a escalação ou particionamento de seus dados em vários caches. |
...
| Total de chaves  | O número máximo de chaves no cache durante o período de relatório anterior. Este número `keyspace` mapeia para a partir do comando Redis INFO. Devido a uma limitação do sistema subjacente de métricas, para os caches com clustering habilitado, o Total de Chaves retorna o número máximo de chaves do fragmento que tinha o número máximo de chaves durante o intervalo do relatório.  |
| Total de Operações |O número total de comandos processados pelo servidor de cache durante o intervalo de relatório especificado. Esse valor é mapeado para `total_commands_processed` do comando INFO do Redis. Quando o Cache do Azure for Redis for usado exclusivamente `Cache Misses` `Gets`para `Sets`pub/sub, `Total Operations` não haverá métricas para, `Cache Hits`ou , mas haverá métricas que refletem o uso de cache para operações de pub/sub. |
...
| Porcentagem de memória utilizada | A % da memória total que está sendo usada durante o intervalo de relatório especificado.  Este valor faz `used_memory` referência ao valor do comando Redis INFO para calcular a porcentagem. |
...
Para saber mais sobre como Logs de atividade, consulte [Visão geral dos logs de atividade do Azure](../azure-monitor/platform/platform-logs-overview.md).
