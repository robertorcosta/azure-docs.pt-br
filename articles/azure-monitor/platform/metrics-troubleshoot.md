---
title: Solução de problemas Gráficos métricos do Azure Monitor
description: Soluciona os problemas com a criação, personalização ou interpretação de gráficos métricos
author: vgorbenko
...
Use este artigo se você encontrar problemas com a criação, personalização ou interpretação de gráficos no explorador de métricas do Azure. Se você é novo em métricas, aprenda a [começar com o explorador de métricas](metrics-getting-started.md) e recursos [avançados do explorador de métricas](metrics-charts.md). Você também pode ver exemplos dos [gráficos](metric-chart-samples.md) métricos configurados.

## <a name="cant-find-your-resource-to-select-it"></a>Não consigo encontrar seu recurso para selecioná-lo
...
Às vezes, os gráficos podem não mostrar dados depois de selecionar recursos e métricas corretos. Esse comportamento pode ser causado por várias das seguintes razões:
...
**Solução:** Certifique-se de que você tem permissões suficientes para o recurso a partir do qual você está explorando métricas.
...
**Solução:** Altere o tempo do gráfico para uma gama mais ampla. Você pode começar a partir de "Últimos 30 dias" usando uma granularidade de tempo maior (ou contando com a opção "Granularidade automática do tempo").
...
**Solução:** Se você vir um gráfico em branco ou seu gráfico exibir apenas parte dos dados métricos, verifique se a diferença entre datas de início e término no seletor de horas não excede o intervalo de 30 dias.
...
### <a name="you-are-looking-at-a-guest-os-metric-but-didnt-enable-azure-diagnostic-extension"></a>Você está olhando para uma métrica do Sistema Operacional convidado, mas não habilitou a extensão de diagnóstico do Azure
...
**Solução:** Se o Azure Diagnostics Extension estiver habilitado, mas você ainda não puder ver suas métricas, siga as [etapas descritas no guia de solução de problemas do Azure Diagnostics Extension](diagnostics-extension-troubleshooting.md#metric-data-doesnt-appear-in-the-azure-portal). Veja também as etapas de solução de problemas para [Não É possível escolher o namespace e as métricas do sistema](metrics-troubleshoot.md#cannot-pick-guest-os-namespace-and-metrics) operacional convidado

## <a name="error-retrieving-data-message-on-dashboard"></a>Mensagem "Erro de recuperação de dados" no painel
...
**Solução:** Atualize o azulejo com falha escolhendo uma métrica alternativa para o seu gráfico no painel. Você pode [revisar uma lista das métricas disponíveis para os serviços do Azure](metrics-supported.md).

## <a name="chart-shows-dashed-line"></a>Gráfico mostra linha tracejada

Os gráficos de métricas do Azure usam o estilo de linha tracejada para indicar que há um valor ausente (também conhecido como "valor nulo") entre dois pontos de dados de grão de tempo conhecidos. Por exemplo, se no seletor de tempo você escolheu granularidade de tempo de "1 minuto", mas a métrica foi relatada às 07:26, 07:27, 07:29 e 07:30 (nota um minuto de diferença entre o segundo e o terceiro pontos de dados), então uma linha tracejada se conectará 07:27 e 07:29 e uma linha sólida se conectará todos os outros pontos de dados. A linha tracejada cai para zero quando a métrica usa **contagem** **e** soma agregação. Para as agregações **avg,** **min** ou **max,** a linha tracejada conecta dois pontos de dados conhecidos mais próximos. Além disso, quando os dados estão ausentes no lado mais à direita ou à esquerda do gráfico, a linha tracejada se expande na direção do ponto de dados ausente.
  ![imagem de métrica](./media/metrics-troubleshoot/missing-data-point-line-chart.png)

**Solução:** Esse comportamento é por design. Ele é útil para identificar pontos de dados ausentes. O gráfico de linha é uma escolha superior para visualizar tendências de métricas de alta densidade, mas pode ser difícil de interpretar para as métricas com valores esparsos, especialmente quando a corelação de valores com o grão temporal é importante. A linha tracejada facilita a leitura desses gráficos, mas se seu gráfico ainda não estiver claro, considere a possibilidade de visualizar suas métricas em outro tipo de gráfico. Por exemplo, um gráfico de gráfico disperso para a mesma métrica mostra claramente cada grão de tempo apenas visualizando ![um ponto quando há um valor e pulando o ponto de dados completamente quando o valor está faltando: imagem métrica](./media/metrics-troubleshoot/missing-data-point-scatter-chart.png)
...
## <a name="cannot-pick-guest-os-namespace-and-metrics"></a>Não é possível escolher o namespace e as métricas do Guest OS

Máquinas virtuais e conjuntos de escala de máquinas virtuais têm duas categorias de métricas: métricas **de Host de máquina virtual** coletadas pelo ambiente de hospedagem do Azure e métricas guest OS **(clássicas)** que são coletadas pelo [agente de monitoramento](agents-overview.md) em execução em suas máquinas virtuais. O agente de monitoramento é instalado habilitando a [Extensão de Diagnóstico do Azure](diagnostics-extension-overview.md).
...
**Solução:** Se você não ver o namespace **do Guest OS (clássico)** e as métricas no explorador de métricas:
...
1. Certifique-se de que o provedor de recursos **Microsoft.Insights** esteja [registrado para sua assinatura](metrics-troubleshoot.md#microsoftinsights-resource-provider-isnt-registered-for-your-subscription).
...
* [Conheça os recursos avançados do Metric Explorer](metrics-charts.md)
