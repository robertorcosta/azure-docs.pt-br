...
- Algumas [regras de alerta clássicas que não podem ser migradas automaticamente](../alerts/alerts-understand-migration.md#manually-migrating-classic-alerts-to-newer-alerts) e exigem que a ação manual dos usuários continuem a ser executadas.

> [!IMPORTANT]
> O monitor de Microsoft Azure foi distribuído na [ferramenta de fases para migrar voluntariamente](../alerts/alerts-using-migration-tool.md) suas regras de alerta clássicas para a nova plataforma em breve. E executá-lo por força para todas as regras de alerta clássicas que ainda existem e podem ser migradas. Os clientes precisam garantir que a automação que consome conteúdo de regra de alerta clássica seja adaptada para lidar com o novo conteúdo de [Métricas e Alertas Unificados no Application Insights](#unified-metrics-and-alerts-in-application-insights) ou [Métricas e Alertas Unificados para outros recursos do Azure](#unified-metrics-and-alerts-for-other-azure-resources) após a migração das regras de alerta clássicas. Para obter mais informações, consulte [preparar para a migração da regra de alerta clássica](../alerts/alerts-prepare-migration.md)
...
* Saiba mais sobre os novos [alertas do Azure](../platform/alerts-overview.md).
