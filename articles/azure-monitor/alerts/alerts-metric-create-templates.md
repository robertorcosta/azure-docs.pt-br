---
title: Criar um alerta de métrica mais recente com o modelo de Azure Resource Manager
...
Este artigo mostra como você pode usar um [o modelo do Azure Resource Manager](../../azure-resource-manager/templates/template-syntax.md) para configurar [alertas de métricas novos](./alerts-metric-near-real-time.md) no Azure Monitor. Os modelos do Resource Manager permitem que você configure programaticamente os alertas de maneira consistente e reproduzível em seus ambientes. Alertas de métrica novos estão disponíveis atualmente [neste conjunto de tipos de recurso](./alerts-metric-near-real-time.md#metrics-and-dimensions-supported).

> [!IMPORTANT]
> Modelo de recursos para criar alertas de métrica para o tipo de recurso: O espaço de trabalho do Azure Log Analytics (ou seja `Microsoft.OperationalInsights/workspaces`), requer etapas adicionais. Para obter detalhes, consulte o artigo sobre [Alerta de Métrica para Logs - Modelo de Recursos](./alerts-metric-logs.md#resource-template-for-metric-alerts-for-logs).
...
Salve o json abaixo como simplestaticmetricalert.json para usar neste passo a passo.
...
Uma explicação do esquema e das propriedades para uma regra de alerta [está disponível aqui](/rest/api/monitor/metricalerts/createorupdate).
...
Salve o json abaixo como simplestaticmetricalert.parameters.json e modifique-o conforme necessário.
...
Salve o json abaixo como simpledynamicmetricalert.parameters.json e modifique-o conforme necessário.
...
Salve e modifique o json abaixo como advancedstaticmetricalert.parameters.json para usar neste passo a passo.
...
Uma regra de alerta pode monitorar várias séries temporais de métrica por vez, o que resulta em menos regras de alerta para gerenciar.
...
Salve o JSON abaixo como multidimensionalstaticmetricalert.jsno para fins deste passo a passos.
...
Salve e modifique o JSON abaixo como multidimensionalstaticmetricalert.parameters.jsno para fins deste passo a passos.
...
Salve e modifique o json abaixo como advanceddynamicmetricalert.parameters.json para usar neste passo a passo.
...
Salve e modifique o JSON abaixo como customstaticmetricalert.parameters.jsno para fins deste passo a passos.
...
> [!NOTE]
>
> Em uma regra de alerta de métrica que monitora vários recursos, as seguintes limitações se aplicam:
> - O escopo da regra de alerta deve conter pelo menos um recurso do tipo de recurso selecionado.
> - A regra de alerta pode conter apenas uma condição.
...
Salve e modifique o JSON abaixo como all-vms-in-resource-group-static.parameters.json para usar neste passo a passo.
...
Você pode criar o alerta de métrica estática usando o arquivo de parâmetros e modelo usando o PowerShell ou a CLI do Azure em seu diretório de trabalho atual.
...
Esse modelo criará uma regra de alerta de métrica de limites dinâmicos que monitora o percentual de CPU para todas as máquinas virtuais (em uma região do Azure) em um ou mais grupos de recursos.

Salve o json abaixo como all-vms-in-resource-group-dynamic.json para usar neste passo a passo.
...
Você pode usar o modelo acima com o arquivo de parâmetro abaixo.
Salve e modifique o JSON abaixo como all-vms-in-resource-group-dynamic.parameters.json para usar neste passo a passo.
...
Esse modelo criará uma regra de alerta de métrica de limite estático que monitora o percentual de CPU para todas as máquinas virtuais (em uma região do Azure) em uma assinatura.

Salve o JSON abaixo como all-vms-in-subscription-static.json para usar neste passo a passo.
...
Você pode usar o modelo acima com o arquivo de parâmetro abaixo.
Salve e modifique o JSON abaixo como all-vms-in-subscription-static.parameters.json para usar neste passo a passo.
...
Esse modelo criará uma regra de alerta de métrica de limites dinâmicos que monitora o percentual de CPU para todas as máquinas virtuais (em uma região do Azure) em uma assinatura.

Salve o JSON abaixo como all-vms-in-subscription-dynamic.json para usar neste passo a passo.
...
Você pode usar o modelo acima com o arquivo de parâmetro abaixo.
Salve e modifique o JSON abaixo como all-vms-in-subscription-dynamic.parameters.json para usar neste passo a passo.
...
Esse modelo criará uma regra de alerta de métrica de limite estático que monitora o percentual de CPU para de máquinas virtuais (em uma região do Azure) em uma assinatura.

Salve o JSON abaixo como list-of-vms-static.json para usar neste passo a passo.
...
Você pode usar o modelo acima com o arquivo de parâmetro abaixo.
Salve e modifique o JSON abaixo como list-of-vms-static.parameters.json para usar neste passo a passo.
...
Esse modelo criará uma regra de alerta de métrica de limites dynamicos que monitora o percentual de CPU para uma lista de máquinas virtuais (em uma região do Azure) em uma assinatura.

Salve o JSON abaixo como list-of-vms-dynamic.json para usar neste passo a passo.
...
Você pode usar o modelo acima com o arquivo de parâmetro abaixo.
Salve e modifique o JSON abaixo como list-of-vms-dynamic.parameters.json para usar neste passo a passo.
...
Os alertas de teste de disponibilidade do mesmo tipo de recurso que os alertas de métrica (Microsoft.Insights/metricAlerts). O modelo de Azure Resource Manager de exemplo a seguir pode ser usado para configurar um teste de disponibilidade simples e um alerta associado.

Salve o JSON abaixo como availabilityalert.jsno para fins deste passo a passos.
...
Salve o JSON abaixo como availabilityalert.parameters.jse modifique-o conforme necessário.
