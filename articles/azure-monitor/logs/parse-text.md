...
Ao analisar os dados em tempo de coleção, você configura [Campos Personalizados](../logs/custom-fields.md) que criam propriedades na tabela. As consultas não precisam incluir qualquer lógica de análise e apenas usam essas propriedades como qualquer outro campo na tabela.
...
Para obter detalhes sobre a análise de dados conforme eles são coletados, confira [Criar campos personalizados no Azure Monitor](../logs/custom-fields.md). Isso cria propriedades personalizadas na tabela que podem ser usadas pelas consultas como qualquer outra propriedade.
...
Você pode ter várias consultas que executam a mesma análise de uma tabela específica. Nesse caso, [crie uma função](../logs/functions.md) que retorna os dados analisados em vez de replicar a lógica de análise em cada consulta. Em seguida, você pode usar o alias da função no lugar da tabela original em outras consultas.
