...
Este artigo descreve os detalhes de integração e como usar o Mapa do Serviço. Os pré-requisitos da solução são os seguintes:

* Um espaço de trabalho Log Analytics em uma [região com suporte](vminsights-configure-workspace.md#supported-regions).

* O [agente de log Analytics](vminsights-enable-overview.md#agents) instalado no computador Windows ou no servidor Linux conectado ao mesmo espaço de trabalho com o qual você habilitou a solução.
...
1. Habilite a solução de Mapa do Serviço do [Azure Marketplace](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.ServiceMapOMS?tab=Overview) ou usando o processo descrito em [Adicionar soluções de monitoramento do Galeria de soluções](../insights/solutions.md).
1. [Instale o Dependency Agent no Windows](../vm/vminsights-enable-hybrid.md#install-the-dependency-agent-on-windows) ou [Instale o Dependency Agent no Linux](../vm/vminsights-enable-hybrid.md#install-the-dependency-agent-on-linux) em cada computador em que você deseja obter dados. O Agente de Dependência pode monitorar conexões a vizinhos imediatos, portanto, talvez não seja necessário ter um agente em cada computador.
...
Para ver os dados de desempenho, talvez seja necessário [habilitar os contadores de desempenho apropriados do Log Analytics](../agents/data-sources-performance-counters.md).  Os contadores que você desejará habilitar:
