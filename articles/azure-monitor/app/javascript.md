...
| disableFetchTracking | verdadeiro | Se for true, as solicitações de busca não serão coletadas. O padrão é true |
...
| disableDataLossAnalysis | verdadeiro | Se for falso, os buffers do remetente da telemetria interna serão verificados na inicialização para os itens ainda não enviados. |
...
| enableSessionStorageBuffer | verdadeiro | Padrão verdadeiro. Se for true, o buffer com todas as telemetrias não enviadas será armazenado no armazenamento de sessão. O buffer é restaurado no carregamento da página |
...
| isBeaconApiDisabled | verdadeiro | Se for false, o SDK enviará toda a telemetria usando a [API de Beacon](https://www.w3.org/TR/beacon) |
