...
---
# <a name="data-collection-retention-and-storage-in-application-insights"></a>Coleta, retenção e armazenamento de dados em Insights de Aplicativos
...
* Você pode escrever código que envie a telemetria personalizada adicional para ajudá-lo com o uso de monitoramento e diagnóstico. (Essa extensibilidade é uma grande característica do Application Insights.) Seria possível, por engano, escrever este código para que ele inclua dados pessoais e outros confidenciais. Se o seu aplicativo funcionar com esses dados, você deve aplicar um processo de revisão completa a todo o código que você escreve.
...
* [Exceções](../../azure-monitor/app/asp-net-exceptions.md) e travamentos - `build id` **despejos de pilha**, , tipo de CPU. 
...
Os pontos de dados brutos (ou seja, itens que você pode consultar no Analytics e inspecionar na Pesquisa) são mantidos por até 730 dias. Você pode selecionar uma duração de [retenção](https://docs.microsoft.com/azure/azure-monitor/app/pricing#change-the-data-retention-period) de 30, 60, 90, 120, 180, 270, 365, 550 ou 730 dias. Se você precisar manter os dados por mais de 730 dias, você pode usar [a Exportação Contínua](../../azure-monitor/app/export-telemetry.md) para copiá-los a uma conta de armazenamento durante a ingestão de dados. 

Os dados mantidos por mais de 90 dias incorrerão em taxas adicionais. Saiba mais sobre os preços do Application Insights na página de preços do [Azure Monitor](https://azure.microsoft.com/pricing/details/monitor/).
...
* Quando você cria um novo recurso do Application Insights, você pode selecionar o local. Saiba mais sobre a disponibilidade do Application Insights por região [aqui.](https://azure.microsoft.com/global-infrastructure/services/?products=all)

#### <a name="does-that-mean-my-app-has-to-be-hosted-in-the-usa-europe-or-southeast-asia"></a>Isso significa que meu aplicativo tem que ser hospedado nos EUA, Europa ou Sudeste Asiático?
...
Os dados são armazenados em servidores do Microsoft Azure. Para contas no portal Azure, as restrições de contas são descritas no [documento Azure Security, Privacy e Compliance](https://go.microsoft.com/fwlink/?linkid=392408).
...
Sim, usamos https para enviar dados para o portal de quase todos os SDKs, incluindo servidores web, dispositivos e páginas da Web HTTPS. 
...
Por padrão, `ServerTelemetryChannel` usa a pasta de dados do aplicativo local do usuário atual `%localAppData%\Microsoft\ApplicationInsights` ou a pasta temporária `%TMP%`. (Veja [a implementação](https://github.com/Microsoft/ApplicationInsights-dotnet/blob/91e9c91fcea979b1eec4e31ba8e0fc683bf86802/src/ServerTelemetryChannel/Implementation/ApplicationFolderProvider.cs#L54-L84) aqui.) Em um ambiente Linux, o armazenamento local será desativado a menos que uma pasta de armazenamento seja especificada.
...
(Para obter mais informações, consulte [AspNetCore Custom Configuration](https://github.com/Microsoft/ApplicationInsights-aspnetcore/wiki/Custom-Configuration).)
...
[O armazenamento de sessão HTML5](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage) é usado para persistir dados. Dois buffers separados `AI_buffer` `AI_sent_buffer`são usados: e . A telemetria que está loteada e `AI_buffer`esperando para ser enviada é armazenada em . A telemetria que acabou `AI_sent_buffer` de ser enviada é colocada até que o servidor de ingestão responda que foi recebido com sucesso. Quando a telemetria é recebida com sucesso, ela é removida de todos os buffers. Em falhas transitórias (por exemplo, um usuário perde `AI_buffer` a conectividade da rede), a telemetria permanece até que seja recebida com sucesso ou o servidor de ingestão responda que a telemetria é inválida (esquema ruim ou muito antigo, por exemplo).

Os buffers de telemetria [`enableSessionStorageBuffer`](https://github.com/microsoft/ApplicationInsights-JS/blob/17ef50442f73fd02a758fbd74134933d92607ecf/legacy/JavaScript/JavaScriptSDK.Interfaces/IConfig.ts#L31) `false`podem ser desativados definindo para . Quando o armazenamento de sessão é desativado, um array local é usado como armazenamento persistente. Como o JavaScript SDK é executado em um dispositivo cliente, o usuário tem acesso a esse local de armazenamento através das ferramentas de desenvolvedor do navegador.

### <a name="opencensus-python"></a>OpenCensus Python

Por padrão, o OpenCensus Python SDK usa a pasta `%username%/.opencensus/.azure/`de usuário atual . As permissões para acessar essa pasta são restritas ao usuário atual e aos Administradores. (Veja [a implementação](https://github.com/census-instrumentation/opencensus-python/blob/master/contrib/opencensus-ext-azure/opencensus/ext/azure/common/storage.py) aqui.) A pasta com seus dados persistindo será nomeada após o arquivo Python que gerou a telemetria.

Você pode alterar a localização do seu `storage_path` arquivo de armazenamento passando o parâmetro no construtor do exportador que você está usando.
...
|.NET | Configuração com suporte varia de acordo com a versão. | Para obter informações detalhadas sobre configuração para .NET 4.7 e versões anteriores, consulte [estas instruções](https://docs.microsoft.com/dotnet/framework/network-programming/tls#support-for-tls-12).  |
|Monitor de status | Configuração com suporte exigida | O Monitor de Status conta com a [configuração](https://docs.microsoft.com/windows-server/security/tls/tls-registry-settings) + do SISTEMA[OPERACIONAL .NET para](https://docs.microsoft.com/dotnet/framework/network-programming/tls#support-for-tls-12) suportar o TLS 1.2.
|Node.js |  Configuração com suporte no v10.5.0 pode ser necessária. | Use a [documentação oficial do Node.js TLS/SSL](https://nodejs.org/api/tls.html) para qualquer configuração específica do aplicativo. |
...
Para executar um teste preliminar para ver se o sistema Linux pode se comunicar via TLS 1.2., abra o terminal e execute:
...
Os SDKs variam entre diferentes plataformas, e há vários componentes que você pode instalar. (Consulte o [Application Insights - visão geral][start].) Cada componente envia dados diferentes.
...
| DeviceContext |`Id`, IP, Locale, modelo de dispositivo, rede, tipo de rede, nome OEM, resolução da tela, Instância de função, Nome da função, tipo de dispositivo |
...
| Dependências |Digite (SQL, HTTP, ...), string de conexão ou URI, sincronização/assincronia, duração, sucesso, declaração SQL (com monitor de status) |
| **Exceções** |Digite, **mensagem,** pilhas de chamadas, arquivo de origem, número de linha,`thread id` |
| Falhas |`Process id`, `parent process id`, `crash thread id`; patch de `id`aplicação, construção;  tipo de exceção, endereço, razão; símbolos e registros ofuscados, endereços binários de início e fim, nome binário e caminho, tipo de cpu |
...
> [!NOTE]
> O IP do cliente é usado para inferir a localização geográfica mas, por padrão, os dados do IP não são mais armazenados e todos os zeros são gravados no campo associado. Para compreender mais sobre tratamento de dados pessoais, recomendamos este [artigo](../../azure-monitor/platform/personal-data-mgmt.md#application-data). Se você precisar armazenar dados de endereço IP, nosso [artigo de coleta de endereços IP](https://docs.microsoft.com/azure/azure-monitor/app/ip-collection) irá levá-lo através de suas opções.

## <a name="credits"></a>Credits
Este produto inclui dados GeoLite2 criados pela [https://www.maxmind.com](https://www.maxmind.com)MaxMind, disponíveis a partir de .
