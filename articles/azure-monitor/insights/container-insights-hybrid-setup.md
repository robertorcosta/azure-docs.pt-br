...
* O agente em contêineres requer `cAdvisor secure port: 10250` de Kubelet ou `unsecure port :10255` ser aberto em todos os nós no cluster para coletar métricas de desempenho. Recomendamos que você configure `secure port: 10250` no cAdvisor do Kubelet se ele ainda não estiver configurado.
...
* Validar `cAdvisor secure port:10250` ou `unsecure port: 10255` é aberto em todos os nós no cluster.
